...
**Status:** **OPEN — A4s** (2026-09-24). **Read [m8_state.md](m8_state.md) first.** Everest **CLOSED** (`f72b4276`). M8.0 persist **repeated** on `36d3b559` (UUID `a0ad99ac-…`, two `login:` after Force Off; `grubcfg=no` because the menu is on ext4). Earlier marker close `928d6224` (also M8.1 `RAYNU-V-M8-TLS-OK`) used a filesystem that was wiped. Phase 0 (never ISO after `EFI PART`, `setup-disk` withheld) stays. M8.2 host-ready. M8.3 standing SPA / console keys firmware-in-tree, iron A4s OPEN on the live shell. M8.4 host-ready. A3 SKU DONE. Known-good and do-not-F11 tables live in `m8_state.md`, not here.
