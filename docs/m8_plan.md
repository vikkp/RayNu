...
**M8.0 persist-first attach.** Choice remains **file-backed nested / durable LUN on iron / leftover DRAM as fallback**. Attach prefers persist (`attach_disk_keep` / `attach_lun` when the media looks installed). Nested `M8_PERSIST_IMG` backs QEMU RAM (distro OVMF ignores nvdimm/pc-dimm), default off. **NVMe I/O** (`MODE=lun`) Identify + Read/Write backs virtio when Identify succeeds; empty NVMe is not 1 GiB-zeroed. **USB BOT** (`MODE=usb`) is TCG-proven. **`MODE=lunkeep` TCG-proven:** plant GPT+ESP+ext4 at NVMe LUN offset 0, kill HV, second boot `keep=1`. NVMe I/O is post-EBS. `MODE=usbkeep` is harness+host (TCG residual). Iron marker is `RAYNU-V-M8-DISK-PERSIST-OK`. Host marker is `RAYNU-V-M8-DISK-PERSIST-HOST-OK`. Keep ADR-004 exclusive ownership. Do not claim persist from nested QEMU alone. Do not copy 1 GiB through the Cruzer ESP. Do not format the PERC. Keep [`v0.1.0-everest-closed`](https://github.com/vikkp/RayNu/releases/tag/v0.1.0-everest-closed) as the flash rollback until iron COM2 prints the persist marker.

**Next:** `MODE=full` on nested KVM (`raynuvsrv1`) for Alpine kill/restart. DurableLun mapper + **NVMe Identify + Read/Write** (`MODE=lun`) and USB BOT (`MODE=usb`) are TCG-proven. `MODE=lunkeep` plant GPT at NVMe LUN offset 0 → `keep=1` after HV kill is TCG-proven (not nested-OK; not iron). Then iron NVMe (or USB) Force Off. Do not F11 this prototype. Do not start M8.1 TLS until that iron COM2 exists (design-only overlap is allowed).
