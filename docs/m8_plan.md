...
**Status: host-ready** (firmware SPA is still host serial log; iron SPA keyboard / VNC open)
...
**Honesty:** [`ConsoleMode::HostReady`](../mgmt/console.rs) injects keys into guest COM1 RBR (`RAYNU-V-M8-CONSOLE-HOST-OK`). Firmware HTTP still serves `GET /logs/serial` (HV UART — not a guest console). SPA has no keyboard field (16 KiB budget). Host/CI never print `RAYNU-V-M8-CONSOLE-OK`. Nested QEMU ≠ R640. **not VNC**.
...
**Next:** **M8.1 TLS iron.** Freestanding TLS 1.2 is in-tree (`Tls12Listen`, `RAYNU-V-M8-TLS12-HOST-OK`). rustls/ring stay out of `uefi-bin`. Host rustls (`RAYNU-V-M8-TLS-HOST-OK`) and wrap (`RAYNU-V-M8-TLS-FW-HOST-OK`) remain. M8.2 host-ready (`RAYNU-V-M8-AUTH-HOST-OK`) is in-tree; firmware still accepts `raynu-v-bringup` when no ESP `auth.token`. M8.3 host-ready (`RAYNU-V-M8-CONSOLE-HOST-OK`) is in-tree; firmware SPA is still host serial log (**not VNC**). M8.4 host-ready (`RAYNU-V-M8-ISO-UPLOAD-HOST-OK`) is in-tree; **ESP-staged stays valid**. A3 SKU card is **DONE** ([`docs/sku.md`](sku.md)). CURL NOW on this EFI is `https://`. Flashed `4af78b43` is still `http://`. Iron close is `curl --cacert` after `BOOT-OK`. Persist honesty is wired for the next Force Off flash. Nested Alpine `MODE=full` is **closed** on `raynuvsrv1` (`ce3d8a09`). QEMU NVMe/USB ≠ R640. Sit at `localhost:~#` until F11 of this TLS EFI. Do not `setup-disk`. Do not flash Toshiba `/dev/sdc`.
