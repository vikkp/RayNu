...
3. **Iron (last):** same Phase B loop as Everest (`HOST-NIC-HTTP-OK` → SPA Start → install → F7 `DISK-BOOT-OK`), then **Force Off / reboot RayNu-V**. COM2 shows the installed disk again, prototype `build: sha=`, and `RAYNU-V-M8-DISK-PERSIST-OK`. One-time F11 the UDisk; Ubuntu-on-PERC stays the standing boot order. Do not format the PERC. DurableLun: PCI census picks NVMe (class `01:08`) then USB ≥ 1 GiB; **refuses** PERC / MegaRAID and the ESP Cruzer. **NVMe Identify + Read/Write** backs virtio when Identify succeeds (`MODE=lun` QEMU smoke). USB mass-storage I/O is still residual. Do not F11 this prototype.

**Not this gate:** TLS, VNC, Windows, Proven Core. Nested Alpine kill/restart and iron Force Off remain. DurableLun **NVMe I/O** is host-proven (`MODE=lun` QEMU Identify); USB I/O is still residual. If a persist prototype misbehaves, re-flash [`v0.1.0-everest-closed`](https://github.com/vikkp/RayNu/releases/tag/v0.1.0-everest-closed) (see [Iron rollback](#iron-rollback-m80-known-good)). HDA overall stays **99%** until iron COM2 exists.
