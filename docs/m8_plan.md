...
**Next:** iron DurableLun 1–3 on the real R640 — [runbook](runbooks/m8_persist_iron.md). PCI pick (NVMe `01:08` else USB ≥ 1 GiB; refuse PERC and the ESP Cruzer) → post-EBS `durable LUN nvme|usb I/O ready` (Intel PCH scratchpad ≤ 64; USBLEGSUP OS owned; never `USBCMD.INTE`; COM2 `xhci cap caplen=… scratch=…/64`) → Phase B `virtio-blk … (durable LUN nvme|usb)`, not leftover DRAM. Nested Alpine `MODE=full` is **closed** on `raynuvsrv1` (`ce3d8a09`). QEMU NVMe/USB ≠ R640. Iron Cap `err=1` then `scratch=34/64`; Enum HS U0; Slot DW1 `3473a0b9` `cmpl=0x40` was MaxSlots (Address Device never posted; later CCS starved). Recover EFI `568e8696`: Toshiba 298 GiB named + leftover skipped; peek/guest READ IOERR (ISO virtio stole USB BOT). This EFI isolates ISO from LUN + TUR/512e. Do not `setup-disk` on leftover. Do not F11 until COM2 shows I/O ready + virtio on the LUN **and** Alpine mounts `vdb`. Do not start M8.1 TLS until iron COM2 persist exists (design-only overlap is allowed).
