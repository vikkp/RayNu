...
**Status:** **OPEN — M8 recovery Phase 0/1** (2026-09-22). **Read [m8_state.md](m8_state.md) first.** Everest **CLOSED** (`f72b4276`). M8.0 persist **CLOSED on evidence** and marker printed (`928d6224`, also M8.1 `RAYNU-V-M8-TLS-OK`). Since then seven prototypes did not reach the installed `login:` (Toshiba USB BOT `cmpl=0xff` after idle; `08202468` / `15e3d665` fell through to the installer ISO). This branch: Phase 0 makes a probe miss safe and visible (never ISO after `EFI PART`, `setup-disk` withheld on a durable LUN, 8 s USB deadline + heartbeat, live hold); Phase 1 makes the USB fix evidence-driven (`xhci timeout` dump, ESP `usbsoak.txt` bench). M8.2 host-ready. M8.3 standing SPA / console keys firmware-in-tree, iron OPEN. M8.4 host-ready. A3 SKU DONE. Known-good and do-not-F11 tables live in `m8_state.md`, not here.
