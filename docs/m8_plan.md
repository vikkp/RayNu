...
**Next:** iron DurableLun 1–3 on the real R640 — [runbook](runbooks/m8_persist_iron.md). PCI pick (NVMe `01:08` else USB ≥ 1 GiB; refuse PERC and the ESP Cruzer) → post-EBS `durable LUN nvme|usb I/O ready` (Intel PCH scratchpad ≤ 16; USBLEGSUP OS owned; never `USBCMD.INTE`) → Phase B `virtio-blk … (durable LUN nvme|usb)`, not leftover DRAM. Nested Alpine `MODE=full` is **closed** on `raynuvsrv1` (`ce3d8a09`). QEMU NVMe/USB ≠ R640. Do not F11 until COM2 shows I/O ready + virtio on the LUN. Do not start M8.1 TLS until iron COM2 persist exists (design-only overlap is allowed).
