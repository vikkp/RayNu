...
**Status:** **OPEN — A4s** (2026-09-25). **Read [m8_state.md](m8_state.md) first.** Everest **CLOSED** (`f72b4276`). M8.0 persist **repeated** (`fa6ce771` fourth `login:`, UUID `a0ad99ac-…`; `grubcfg=no` because the menu is on ext4). The chassis is off: that EFI painted the SPA and the dashboard showed Power State OFF. Paced SOL RX was not sufficient. This EFI holds listen 30 s after one HTTP exchange. Do not open Firefox on `fa6ce771`. Earlier marker close `928d6224` (also M8.1 `RAYNU-V-M8-TLS-OK`) used a filesystem that was wiped. Phase 0 (never ISO after `EFI PART`, `setup-disk` withheld) stays. M8.2 host-ready. M8.3 standing SPA / console keys firmware-in-tree, iron A4s OPEN. M8.4 host-ready. A3 SKU DONE. Known-good and do-not-F11 tables live in `m8_state.md`, not here.
