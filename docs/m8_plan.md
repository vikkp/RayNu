...
**Status:** **OPEN — A4s** (2026-09-25). **Read [m8_state.md](m8_state.md) first.** Everest **CLOSED** (`f72b4276`). M8.0 persist **repeated** (`3e9ce45e` fifth `login:`, UUID `a0ad99ac-…`; `grubcfg=no` because the menu is on ext4). The chassis is off after seq 22543 `SYS1003` and seq 22544 `SYS1001` with no `RAC1195`. This EFI keeps one TLS session and serves later polls on it. `3f80abd0` (no re-listen) is a probe; do not flash it. Do not open Firefox on `3e9ce45e`. Earlier marker close `928d6224` (also M8.1 `RAYNU-V-M8-TLS-OK`) used a filesystem that was wiped. Phase 0 (never ISO after `EFI PART`, `setup-disk` withheld) stays. M8.2 host-ready. M8.3 standing SPA / console keys firmware-in-tree, iron A4s OPEN. M8.4 host-ready. A3 SKU DONE. Known-good and do-not-F11 tables live in `m8_state.md`, not here.
