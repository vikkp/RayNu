...
3. **Iron (last):** same Phase B loop as Everest (`HOST-NIC-HTTP-OK` → SPA Start → install → F7 `DISK-BOOT-OK`), then **Force Off / reboot RayNu-V**. COM2 shows the installed disk again, prototype `build: sha=`, and `RAYNU-V-M8-DISK-PERSIST-OK`. One-time F11 the UDisk; Ubuntu-on-PERC stays the standing boot order. Do not format the PERC. DurableLun mapper: PCI census picks NVMe (class `01:08`) or a USB data LUN ≥ 1 GiB; **refuses** PERC / MegaRAID and the ESP Cruzer that holds alpine-extended. Post-EBS NVMe/USB I/O is still residual — leftover DRAM remains the attach until that I/O exists. Do not F11 this prototype.

**Not this gate:** TLS, VNC, Windows, Proven Core. Nested Alpine kill/restart and iron Force Off remain. DurableLun **PCI census** (USB/NVMe pick, PERC refused) is host-proven; post-EBS NVMe/USB I/O is still residual. If a persist prototype misbehaves, re-flash [`v0.1.0-everest-closed`](https://github.com/vikkp/RayNu/releases/tag/v0.1.0-everest-closed) (see [Iron rollback](#iron-rollback-m80-known-good)). HDA overall stays **99%** until iron COM2 exists.
...
**Next:** `MODE=full` on nested KVM (`raynuvsrv1`) for Alpine kill/restart. DurableLun mapper (PCI census, PERC refused) is host-proven; post-EBS NVMe/USB I/O still residual. Then iron durable LUN I/O + Force Off. Do not F11 this prototype. Do not start M8.1 TLS until that iron COM2 exists (design-only overlap is allowed).
