...
3. **Iron (last):** same Phase B loop as Everest (`HOST-NIC-HTTP-OK` → SPA Start → install → F7 `DISK-BOOT-OK`), then **Force Off / reboot RayNu-V**. COM2 shows the installed disk again, prototype `build: sha=`, and `RAYNU-V-M8-DISK-PERSIST-OK`. One-time F11 the UDisk; Ubuntu-on-PERC stays the standing boot order. Do not format the PERC. DurableLun mapper: PCI census picks NVMe (class `01:08`) then USB ≥ 1 GiB; **refuses** PERC / MegaRAID and the ESP Cruzer (2–8 GiB window). **NVMe Identify + Read/Write** backs virtio when Identify succeeds (`MODE=lun`). **USB BOT/xHCI I/O** after ExitBootServices backs virtio when NVMe is absent (`MODE=usb` QEMU `qemu-xhci` + `usb-storage`). Intel PCH scratchpad ≤ 64 pages; USBLEGSUP OS owned; never `USBCMD.INTE`. COM2 always prints `xhci cap caplen=… scratch=…/64` on USB attempt. QEMU USB ≠ R640 xHCI. Iron `ac3b92cd` Cap `err=1` (16-page budget). Cap EFI `67db8a68`: `scratch=34/64`. Enum EFI `c6bdd671` / `53d1f7f7` (2026-09-14): HS U0 `sc=0xe03` then Address Device `cmd=3` `cmpl=0x11` on p10/p11/p14 (Slot Context DW1 Number of Ports was the port number). Slot DW1 EFI `3473a0b9`: Parameter Error gone; `cmpl=0x40` was MaxSlots after Address Device never posted (p11/p14 Enable Slot starved). Leftover DRAM Everest loop; Toshiba unused. Next EFI: timeout `0xff` + CRCR abort between CCS ports + vid/did + UAS detect. Do not `setup-disk` on leftover. Do not F11 until COM2 shows `I/O ready` + `virtio-blk … (durable LUN …)`. Runbook: [m8_persist_iron.md](runbooks/m8_persist_iron.md).

**Not this gate:** TLS, VNC, Windows, Proven Core. Nested Alpine kill/restart is **closed on QEMU** (`RAYNU-V-M8-DISK-PERSIST-NESTED-OK`). Iron Force Off remains. DurableLun **NVMe I/O** is host-proven (`MODE=lun` QEMU Identify); **USB BOT** is TCG-proven (`MODE=usb` `usb I/O ready`). DurableLun `keep=1` after HV kill is **`MODE=lunkeep`/`usbkeep` TCG-proven** (not nested-OK; not iron). Iron Force Off is last. If a persist prototype misbehaves, re-flash [`v0.1.0-everest-closed`](https://github.com/vikkp/RayNu/releases/tag/v0.1.0-everest-closed) (see [Iron rollback](#iron-rollback-m80-known-good)). HDA overall stays **99%** until iron COM2 exists.
...
**Next:** iron DurableLun 1–3 on the real R640 — [runbook](runbooks/m8_persist_iron.md). PCI pick (NVMe `01:08` else USB ≥ 1 GiB; refuse PERC and the ESP Cruzer) → post-EBS `durable LUN nvme|usb I/O ready` (Intel PCH scratchpad ≤ 64; USBLEGSUP OS owned; never `USBCMD.INTE`; COM2 `xhci cap caplen=… scratch=…/64`) → Phase B `virtio-blk … (durable LUN nvme|usb)`, not leftover DRAM. Nested Alpine `MODE=full` is **closed** on `raynuvsrv1` (`ce3d8a09`). QEMU NVMe/USB ≠ R640. Iron Cap `err=1` then `scratch=34/64`; Enum HS U0; Slot DW1 `3473a0b9` `cmpl=0x40` was MaxSlots (Address Device never posted; later CCS starved). Do not `setup-disk` on leftover. Do not F11 until COM2 shows I/O ready + virtio on the LUN. Do not start M8.1 TLS until iron COM2 persist exists (design-only overlap is allowed).
