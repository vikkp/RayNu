//! Verus-verified ghost model for [ADR] EPT exclusive ownership (M3.17–M3.18 / M4.6–M4.9).
//!
...
//! M4.7 claims [ADR] L3 for N-guest 4K map/unmap exclusivity
...
//! map/unmap exclusivity (no `admit`) → `ISOLON-M5-LPAGE-VERIFY-OK`.
...
//! bring-up mock) → `ISOLON-M5-NUMA-OK`.
//! M5.9 couples the ghost frame pool with concrete EPT refine (`alloc_ept_refines`)
//! and scoped precise-identity GPA==HPA correspondence → `ISOLON-M5-ALLOC-REFINE-OK`.
//! M6.0 discharges EPT-violation handling under exclusivity
//! (`theorem_ept_violation_preserves_exclusive`) → `ISOLON-M6-EPTVIO-OK`.
//! M6.1 deepens HW PTE bit-decode for 2M identity leaves
//! (`hw_2m_identity_leaf_ok` / `theorem_hw_2m_leaf_refines_identity`) →
//! `ISOLON-M6-HWPTE-OK` (full multi-level walk → polish).
//! M6.2 discharges NUMA affinity under map/unmap
//! (`theorem_numa_map_unmap_affinity` / `guest_frames_on_node`) →
//! `ISOLON-M6-NUMA-L3-OK`.
//! M6.3 discharges live-migration page transfer exclusivity
//! (`theorem_page_transfer_preserves_exclusive`) → `ISOLON-M6-MIGRATE-XFER-OK`.
//!
//! Markers:
//! - M3.16 link: `ISOLON-M3-L3-LINK-OK`
//! - M3.17 true L3: `ISOLON-M3-L3-VERIFY-OK` (via tools/verus-verify-smoke.sh)
//! - M3.18 refine: `ISOLON-M3-L3-REFINE-OK` (via tools/verus-refine-smoke.sh)
//! - M4.6 N-guest spec: `ISOLON-M4-NGUEST-SPEC-OK` (via tools/verus-nguest-spec-smoke.sh)
//! - M4.7 N-guest L3: `ISOLON-M4-NGUEST-VERIFY-OK` (via tools/verus-nguest-verify-smoke.sh)
//! - M4.8 large-page spec: `ISOLON-M4-LPAGE-OK` (via tools/verus-lpage-spec-smoke.sh)
//! - M4.9 N-guest refine: `ISOLON-M4-REFINE-OK` (via tools/verus-nguest-refine-smoke.sh)
//! - M5.7 large-page L3: `ISOLON-M5-LPAGE-VERIFY-OK` (via tools/verus-lpage-verify-smoke.sh)
//! - M5.8 NUMA ghost spec: `ISOLON-M5-NUMA-OK` (via tools/verus-numa-smoke.sh)
//! - M5.9 alloc↔EPT refine: `ISOLON-M5-ALLOC-REFINE-OK` (via tools/verus-alloc-refine-smoke.sh)
//! - M6.0 EPT-violation: `ISOLON-M6-EPTVIO-OK` (via tools/verus-eptvio-smoke.sh)
//! - M6.1 HW PTE: `ISOLON-M6-HWPTE-OK` (via tools/verus-hwpte-smoke.sh)
//! - M6.2 NUMA affinity L3: `ISOLON-M6-NUMA-L3-OK` (via tools/verus-numa-l3-smoke.sh)
//! - M6.3 migrate page transfer: `ISOLON-M6-MIGRATE-XFER-OK` (via tools/verus-migrate-xfer-smoke.sh)
...
/// Ghost EPT ownership registry ([ADR] / `ept_spec.rs`).
...
/// [ADR] exclusive ownership over the ghost maps.
...
/// can assert the N-guest post. [ADR] L3 claim → `ISOLON-M4-NGUEST-VERIFY-OK`.
...
/// preserves exclusive ownership (concrete ≥2-guest post for [ADR]).
...
// ---------------------------------------------------------------------------
// M4.8 — large-page (2M/1G) ghost *spec* ([ADR])
...
// ---------------------------------------------------------------------------
// M6.0 — EPT-violation handling preserves exclusive ownership ([ADR])
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// M6.3 — Live migration page transfer preserves exclusive ownership ([ADR])
...
/// M6.3 headline: page transfer preserves exclusivity ([ADR] live-migrate row).
...
pub const M3_L3_LINK_OK_MARKER: &str = "ISOLON-M3-L3-LINK-OK";
pub const M3_L3_VERIFY_OK_MARKER: &str = "ISOLON-M3-L3-VERIFY-OK";
pub const M3_L3_REFINE_OK_MARKER: &str = "ISOLON-M3-L3-REFINE-OK";
/// M4.6: N-guest ghost map/unmap is in the model (spec OK).
pub const M4_NGUEST_SPEC_OK_MARKER: &str = "ISOLON-M4-NGUEST-SPEC-OK";
/// M4.7: [ADR] L3 for N-guest 4K map/unmap exclusivity (no `admit`).
pub const M4_NGUEST_VERIFY_OK_MARKER: &str = "ISOLON-M4-NGUEST-VERIFY-OK";
/// M4.8: large-page (2M/1G) ghost *spec* (L3 discharge → M5).
pub const M4_LPAGE_OK_MARKER: &str = "ISOLON-M4-LPAGE-OK";
/// M4.9: N-guest ghost↔exec refine (no `admit`).
pub const M4_REFINE_OK_MARKER: &str = "ISOLON-M4-REFINE-OK";
/// M5.7: large-page (2M/1G) L3 map/unmap exclusivity (no `admit`).
pub const M5_LPAGE_VERIFY_OK_MARKER: &str = "ISOLON-M5-LPAGE-VERIFY-OK";
/// M5.8: NUMA domains in ghost *spec* (SRAT/SLIT; affinity L3 → M6).
pub const M5_NUMA_OK_MARKER: &str = "ISOLON-M5-NUMA-OK";
/// M5.9: allocator↔EPT coupled refine + scoped identity correspondence.
pub const M5_ALLOC_REFINE_OK_MARKER: &str = "ISOLON-M5-ALLOC-REFINE-OK";
/// M6.0: EPT-violation handling preserves exclusive ownership (no `admit`).
pub const M6_EPTVIO_OK_MARKER: &str = "ISOLON-M6-EPTVIO-OK";
/// M6.1: Hardware EPT PTE bit-decode for 2M identity leaves (no `admit`).
pub const M6_HWPTE_OK_MARKER: &str = "ISOLON-M6-HWPTE-OK";
/// M6.2: NUMA affinity L3 under map/unmap (no `admit`).
pub const M6_NUMA_L3_OK_MARKER: &str = "ISOLON-M6-NUMA-L3-OK";
/// M6.3: live migration page transfer preserves exclusivity (no `admit`).
pub const M6_MIGRATE_XFER_OK_MARKER: &str = "ISOLON-M6-MIGRATE-XFER-OK";
