...
- **Mount Everest residual (not paper L3 claims):** **E3b** durable mgmt on a host-owned NIC ([ADR-013](../adr/ADR-013.md) Accepted; firmware SNP hung then RSOD after EBS; WARN-only idle later closed with no RSOD; Tcp4 is a platform limit), remaining E4 polish (TLS/console), real distro installer later, live Redfish, R640 soak. E5 stamp persist closed on iron (`BOOTED-FROM-DISK`). LBA stamps ≠ guest rootfs. PRE-EBS SNP remains the bring-up mgmt UI.
