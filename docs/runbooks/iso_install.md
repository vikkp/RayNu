...
`2ae4544` on `cursor/e5-pm1-sci-a623` (CI 49/49). Pin `--run 33345731636`.
...
git log -1 --oneline   # want 2ae4544; pin run 33345731636 (firmware LAPIC timer expiry).
# HEAD after that may add IOAPIC I/O over PIT / firmware virtual-wire GSI 14;
# do not retarget --run until that CI is green.
lsusb | grep -i 0781:5151
./tools/flashcruzer.sh --no-git --wait --require-head --run 33345731636 \
