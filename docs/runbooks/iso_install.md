...
`5c0f7a2` on `cursor/e5-pm1-sci-a623` (CI green). Pin `--run 33347766697`. Do **not** F11 `2ae4544` / `--run 33345731636` (LAPIC expiry without I/O-over-PIT). flash 5c0f7a2.
product ISO fw_cfg bootorder virtio-iso scsi@3 first (empty scsi@2 second; iso=0 stays CD then disk). F11 HEAD after this SHA's CI; if `5c0f7a2` was already F11'd, paste that COM2 first.
...
git log -1 --oneline   # want 5c0f7a2; pin run 33347766697 (IOAPIC I/O over PIT).
lsusb | grep -i 0781:5151
./tools/flashcruzer.sh --no-git --wait --require-head --run 33347766697 \
