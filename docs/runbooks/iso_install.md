...
`--refat-cruzer` (do **not** pass it again). Flash HEAD after sixty-fifth-slice
CI is green (iron 512 MiB product-ISO pool so 256 MiB disk fits, disk before
scratch, CS.base+RIP MMIO fetch; EAX fallback + flash-RIP still in this EFI).
