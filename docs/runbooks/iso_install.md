...
`--refat-cruzer` (do **not** pass it again). Flash HEAD after sixty-sixth-slice
CI is green (MMIO skip-len from fetched bytes when VMCS `insn_len` is 0; iron
512 MiB product-ISO pool so 256 MiB disk fits, disk before scratch, CS.base+RIP
MMIO fetch; EAX fallback + flash-RIP still in this EFI).
