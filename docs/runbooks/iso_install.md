...
with `BOOTLOADER=grub USE_EFI=1 setup-disk -m sys -s 0 /dev/vda` after `modprobe virtio_pci; modprobe virtio_blk; modprobe sr_mod; mdev -s; sleep 1; mkdir -p /media/cdrom; mount /dev/vdb /media/cdrom || mount /dev/sr0 /media/cdrom; echo /media/cdrom/apks > /etc/apk/repositories; apk update` (and `grub` if `bootloader?`
