...
Length 143→250 so GRUB sees `initrd` / `}` and `alpine_dev=vdb` (a 143-byte read truncated
...
`timeout=10` → `timeout=0` then `set timeout=1` → `set timeout=0`; linux-line NUL-pad grow also bumps ISO9660 + Joliet `grub.cfg` Data Length 143→250 so GRUB sees `initrd`/`}` and `alpine_dev=vdb` (do not leave Data Length at 143); `gfxterm` / `efi_gop` / `efi_uga` / `all_video` / `terminal_output console` → `serial` when present;
