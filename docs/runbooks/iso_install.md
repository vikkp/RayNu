...
with `BOOTLOADER=grub USE_EFI=1 setup-disk -m sys -s 0 /dev/vda` after `mkdir -p /media/cdrom; mount /dev/vdb /media/cdrom` (and `grub` if `bootloader?`
appears, `/dev/vda` if `Which disk`, `n` if `No disks available` then `(y/n)`, or `y` if `[y/N]` / `(y/n)` erase confirm; not ISO-INSTALL-OK), the ISO cmdline is patched to
