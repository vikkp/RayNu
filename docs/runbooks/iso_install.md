...
`b824789` on `cursor/e5-pm1-sci-a623` (CI green). Pin `--run 33387614559`. Do **not** F11 `d61dc7e` / `--run 33349142609` (ConnectAll IdeBus CpuSleep). Do **not** F11 `5c0f7a2` / `--run 33347766697` (ATAPI-first bootorder). flash b824789.
product ISO fw_cfg bootorder virtio-iso scsi@3 first; flash b824789; do not F11 d61dc7e; flash d61dc7e; do not F11 5c0f7a2 (empty scsi@2 second; iso=0 stays CD then disk). Iron COM2 after F11 of `d61dc7e` is ConnectAll Started PIIX IDE (`pci_ide=1`, HLT `rip=0x7f0680d0` `ataio=0`, inj climbing, no virtio-iso IN). product ISO hides PIIX IDE. `8336a06` CI run `33387083800` failed nested-KVM kill-init after GTIMER2 (iso=0 still CDROM-OK BOTH-OK). Do **not** F11 `d61dc7e` / `8336a06` / `6c53fb0`. Flash hide-IDE `b824789` pin `--run 33387614559` after the live `d61dc7e` boot ends.
...
git log -1 --oneline   # want b824789; pin run 33387614559 (hide PIIX IDE).
lsusb | grep -i 0781:5151
./tools/flashcruzer.sh --no-git --wait --require-head --run 33387614559 \
