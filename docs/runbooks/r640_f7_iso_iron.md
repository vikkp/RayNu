...
| Probe pin (this flash) | After the apk `virtio stall dump`: two `virtio stall dump ring disk/iso qsize=128 mem_used= … last_st=0x0 … xlate_fail=0` lines, three `virtio stall probe rip= cs= cr3= rflags=` samples (~0.25 s apart), then `virtio stall dump sysrq w` → Linux `sysrq: Show Blocked State`, `sysrq m` → `Show Memory`, `sysrq t` → `Show State` (every task with a stack; long), `sysrq l` at ~40 s. **Capture the whole SOL log**; the `apk` / `busybox` / `init` stacks are the deliverable. Let it run ≥ 90 s after the dump. |
