...
| apk console output | `Installing packages to root filesystem... (1/25) Installing alpine-baselayout-data` … progress `%` lines **streaming**, not one 16-byte chunk per stray IRQ 4. With the THRE-level fix the virtio `n=` should keep climbing past 1373 and the stall dump should not fire again. |
...
w`/`m`/`t`/`l` with `apk` in `n_tty_write`, and apk's text advancing 16
bytes per SysRq, you flashed `f229d14` / run `34354322953`. That was the
probe; it answered. **Do not wait.** Re-flash from this THRE-level pin.
Do not F11 `34354322953` again expecting `ISO-INSTALL-OK`.
