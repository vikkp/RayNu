...
| Countdown survives (this pin) | **no** `RayNu-F stop exit-cap` during the menu; if RayNu-F stops it prints `wall_ms=` (wall clock, not exits, is the bound now; cap 180 s) |
| Iron residual marker | `RAYNU-V-RAYNU-F-DISK-BOOT-OK` (nested-proven; **this flash** is its first iron chance past the menu) |
