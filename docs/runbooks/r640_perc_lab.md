...
- **M8.7 fwstate** (`mgmt/megaraid.rs`) loads `outbound_msg_0` on the one `1000:0016` and refuses the 400 GB VD as a target. It does not ring a doorbell. The current flash is that EFI. Paste COM2 `boot: perc fwstate`.
