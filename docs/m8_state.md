...
PERC H740P Mini = MegaRAID SAS (`0000:18:00.0`, `1000:0016`). As of 2026-09-26 Disk Group 0 is RAID-6 with **UBUNTU0** (boot, ~400 GB, Ubuntu 26.04) and **RAYNU-SPARE** (~2.9 TB, empty, not boot). Map: [`runbooks/r640_perc_lab.md`](runbooks/r640_perc_lab.md). **M8.7** is the mailbox milestone ([`m8_plan.md`](m8_plan.md)). The host slice packs the LD-list DCMD and a one-block READ(16), and the size fence refuses UBUNTU0. It does not map BAR0. `durable_lun` still skips the whole PERC. Do not format either VD. Do not open the H840 in slot 1. Host/CI never print `RAYNU-V-M8-PERC-LUN-OK`. perc stays 15. An NVMe DurableLun remains a host-proven alternative path; this lab’s chosen Bar B disk is the spare VD.
...
Ubuntu 26.04 is up on **UBUNTU0**. **RAYNU-SPARE** (~2.9 TB) is empty. Read [`runbooks/r640_perc_lab.md`](runbooks/r640_perc_lab.md) before any disk, flash, or iDRAC storage step. Do not format either VD. Do not open the H840. A6 lived once and Power off host matched iDRAC Power State Off; that shutdown is history, and the chassis was powered on again for the reinstall. Do not power on a persist EFI just to finish the COM2 sentence. Do not curl. Do not `setup-disk`. A5 (ESP `auth.token`) is parked. **M8.7 host** packs mailbox frames and does not ring a doorbell. The next iron step is M8.7 fwstate, and only when a flash is asked for. perc stays 15.
