...
| Summary | **Firefox refresh: drain TX then abort(), never close().** First GET showed RayNu-V chrome; reload Unable to connect — FIN_WAIT held the one slot. A4s not closed until reload still answers after Alpine login. Sit at `localhost:~#`. |
...
| Gates touched | drain then abort() not close(). `./tools/sync-loihda-site.sh --check`. No MegaRAID. Sit at `localhost:~#`. |
...
| Date | Slice | A% | B% | Note |
|------|-------|----:|---:|------|
| 2026-09-19 | m8-spa-safari-idle | 78 | 18 | **Firefox refresh Unable to connect: drain then abort(), not close().** Lived first GET SPA; reload SYN failed — FIN_WAIT held the one slot. A4s not closed. Console 58 held. Bar A 78 held. |
