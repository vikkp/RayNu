...
| Summary | **Safari millicert: ignore TLS 1.3 dummy CCS; idle 3 s→15 s; TLS fail re-listens.** curl already pulled SPA HTML on `.140`. Safari Can't Connect is Mac Local Network/Private Relay or dummy CCS `ST_FAIL`. A4s not closed until a lived browser after Alpine login. Sit at `localhost:~#`. |
| Everest impact | none — HDA months 0.0 / 99% held |
| LOI impact | Bar A **78% held** / Bar B **18% held** / overall **63% held** / months A **0.75 held**. Console piece **58% held**. Auth **38% held**. A4s not closed. |
| Gates touched | dummy CCS ignore + rustls default TLS 1.3+1.2 SPA. `./tools/sync-loihda-site.sh --check`. No MegaRAID. Sit at `localhost:~#`. |
...
| Date | Slice | A% | B% | Note |
|------|-------|----:|---:|------|
| 2026-09-19 | m8-spa-safari-idle | 78 | 18 | **Safari millicert dummy CCS + 15 s idle.** Ignore RFC 8446 D.4 CCS after ClientHello so Safari/Chrome is not `ST_FAIL`. curl SPA on `.140` already worked. A4s not closed. Console 58 held. Bar A 78 held. |
