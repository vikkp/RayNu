...
| Summary | **Firefox nssFailure2: drain TX then FIN before reclaim.** Issuer warning proved millicert; abort() RST truncated the SPA. Dummy CCS + 15 s idle held. A4s not closed until a lived browser after Alpine login. Sit at `localhost:~#`. |
...
| Gates touched | drain TX before reclaim. `./tools/sync-loihda-site.sh --check`. No MegaRAID. Sit at `localhost:~#`. |
...
| Date | Slice | A% | B% | Note |
|------|-------|----:|---:|------|
| 2026-09-19 | m8-spa-safari-idle | 78 | 18 | **Firefox nssFailure2: drain TX then FIN before reclaim.** Issuer warning proved millicert; abort() RST truncated SPA. Dummy CCS + 15 s idle held. A4s not closed. Console 58 held. Bar A 78 held. |
