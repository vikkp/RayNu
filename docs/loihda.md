...
| **Bar A — dedicated-box** | **74%** | One PowerEdge we own or they dedicate. A1 Everest, A3 SKU, A4 TLS **DONE**. A2 persist **closed on evidence** (`4af78b43`, `928d6224` `RAYNU-V-M8-DISK-PERSIST-OK`) but **not reliable**. `e5cca2e0` printed `RAYNU-V-USBSOAK-DONE` (239/239, idle through 120 s) and the peek still shows the installed 8 GiB slice. That is the USB bench, not a repeated `login:`. A4s / A6 / A5 wait on a repeatable installed `login:`. |
...
**Why 85, not 97.** Since `928d6224` the installed `login:` has not repeated. `e5cca2e0` is the first boot since then whose USB reads survived idle: 239/239, gaps through 120 s, peek `installed=1` on the 8 GiB slice, and `RAYNU-V-USBSOAK-DONE`. No guest ran, so the product path is still unproven on this EFI. Score goes back up only on a **repeated** installed `login:` from the Toshiba after Force Off.
...
| **NOW** | Bar A persist **reliability** | `e5cca2e0`: `RAYNU-V-USBSOAK-DONE`, 239/239, Toshiba `installed=1`, no guest | **Boot 2.** Same EFI, remove `usbsoak.txt`, F11. Want `image=DISK-BOOTX64` and `[vda] 16777216` through `login:`. See [m8_state.md](m8_state.md) |
...
2026-10  ████░░░░  Bar A: A4 TLS iron DONE; `e5cca2e0` soak 239/239 → boot 2 installed login → A4s
...
| Summary | **`e5cca2e0` lived: USB soak passed.** `RAYNU-V-USBSOAK-DONE`, gaps 0/5/30/120 all `fail=0`, total 239/239. Toshiba `0480:a004`, peek `installed=1 guest=8589934592`. No guest. Boot 2 is the same EFI with `usbsoak.txt` removed. Scores held. |
...
NOW:           e5cca2e0 RAYNU-V-USBSOAK-DONE 239/239 · Force Off · remove usbsoak.txt (leave raynuf.txt) · F11 the same stick, no reflash · want image=DISK-BOOTX64 and [vda] 16777216 through login: · ISO-BOOTX64 means Force Off · do not curl Start · do not setup-disk · docs/m8_state.md
