...
**Iron NOW (not a score bump).** Setcfgretry COM2 (`ef7e93ec`) lived USB `ISO-INSTALL-OK` → F7 `DISK-BOOT-OK` / `root=UUID=6d549fd4-…` on 298 GiB `vda`, then hours of `usb rw ok wr=1` = **ext4lazyinit**, then Force Off peek `gpt_err=2` `installed=0` virtio `keep=0`. Native Dell `EFI stub: Loaded initrd` proved media; HV peek was a false negative. That 298 GiB image is **disposable**. This EFI (guest8g): virtio USB **8 GiB**; leftover 298 GiB GPT `fit=0` → `keep=0`; CSW tag + `xhci peekretry`; **SPA Start is intended** (SETUP wipes UUID `6d549fd4`). Do **not** SPA Start setcfgretry `ef7e93ec`. Keep Toshiba. Never flash `/dev/sdc`. Keep writequeue / okquiet / setcfgretry / CSW queue / nopretry / addrretry.
...
| **NOW** | Bar A A2 | USB/NVMe Force Off on COM2 | **NEXT** (guest8g EFI: 8 GiB virtio + leftover 298 GiB GPT `fit=0` → `keep=0`; **SPA Start intended**; CSW tag + peek retry; do not SPA Start setcfgretry `ef7e93ec`; A2 still open) |
