...
| Summary | **F7 nested reboot-to-disk proven (`fe4785a`, raynuvsrv1).** Alpine sys install → F7 relaunch → whole-disk Vendor path → `RAYNU-V-RAYNU-F-DISK-BOOT-OK` → second `Linux version` with `root=UUID=698a922a-3a7d-45ea-9da3-2b11403af82a` → second shell + `cat /proc/cmdline`. Harness: `nested reboot-to-disk reached a second Linux boot`. Nested QEMU ≠ R640. E5 closes only on iron `ISO-INSTALL-OK`. |
