...
| P0-64 | **M8** operator product hardening | — | IN PROGRESS | Everest closed | [ADR-018](adr/ADR-018.md) / [m8_plan.md](m8_plan.md) / [m8_persist_nested.md](runbooks/m8_persist_nested.md) / [m8_persist_iron.md](runbooks/m8_persist_iron.md). **M8.0 nested Alpine `MODE=full` CLOSED** on `raynuvsrv1` (`ce3d8a09`). Iron 1–3 host-ready (census / PCH scratchpad ≤64 / leftover named). Iron Cap `err=1` then `scratch=34/64`. Enum HS U0. Slot DW1 `3473a0b9` Parameter Error gone; `cmpl=0x40` was MaxSlots after Address Device never posted (command ring busy; p11/p14 starved). This EFI: timeout `0xff`, CRCR abort, vid/did, UAS detect. Nested QEMU ≠ R640. Iron COM2 `I/O ready` + `RAYNU-V-M8-DISK-PERSIST-OK` open. Then TLS, auth, console UI, ISO upload, UEFI catalog, Windows later. |
...
| Field | Value |
|-------|-------|
| Commit | m8-xhci-enum-recover |
| Summary | **Iron leftover, not persist.** Slot DW1 EFI `3473a0b9`: HS U0 then `cmpl=0x40` on p10 Address Device and p11/p14 Enable Slot. `0x40` is MaxSlots, not CC 64. Address Device never posted; command ring stayed busy; later CCS ports never got a fair try. Toshiba unused. This EFI: `cmpl=0xff` timeout, CRCR abort between ports, vid/did, UAS detect. Do not `setup-disk` on leftover. Months **0.0 held**. Overall **99 held**. |
| Everest impact | months **0.0 held**; overall **99 held**; ETA 2026-09 held. Not 100%. No persist gate. |
| Gates touched | `mgmt/xhci.rs` host tests + [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. |
...
| H10 | Leftover-DRAM disk dies on **HV** reboot | MED | **M8.0** first gate ([m8_plan.md](m8_plan.md)): persist-first attach; nested File RAM + `MODE=keep`; DurableLun NVMe/USB I/O; **`MODE=lunkeep`/`usbkeep` TCG keep=1**; leftover/File persist RAM **3584M**; **`MODE=full` nested-OK CLOSED on `raynuvsrv1` `ce3d8a09`**. **Iron 1–3 host-ready** ([m8_persist_iron.md](runbooks/m8_persist_iron.md)). Iron Cap `err=1` then `scratch=34/64`; Enum HS U0; Slot DW1 `3473a0b9` `cmpl=0x40` = MaxSlots (Address Device never posted; later CCS starved; leftover DRAM Everest loop). Do not Force Off that guest expecting persist. Do not `setup-disk` on leftover. Nested QEMU ≠ R640. Iron `RAYNU-V-M8-DISK-PERSIST-OK` still open. Guest F7 persist already closed (ADR-017). |
...
Next move:     **M8.0 iron** COM2 `I/O ready` + virtio on LUN, then Force Off / `RAYNU-V-M8-DISK-PERSIST-OK`. Iron `ac3b92cd` leftover DRAM Everest loop is **not** persist. Nested-OK closed on `raynuvsrv1` `ce3d8a09`.
