...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; capoverlap COM2 Toshiba named then CONFIG_EP `cmd=5 cmpl=0`; leftover cmd event; this EFI match Command TRB Pointer; iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (nested-OK closed; Toshiba named then CONFIG_EP leftover; Force Off open)
...
| Summary | **Iron leftover, not persist.** Capoverlap COM2 named Toshiba p11 then CONFIG_EP `cmd=5 cmpl=0` `bot=?`. Leftover cmd event, not "device missing". This EFI: match Command TRB Pointer. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | `xhci.rs` Command TRB Pointer match; [m8_persist_iron.md](runbooks/m8_persist_iron.md); [loihda.md](loihda.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
