...
| Gates touched | RayNu-F F0 DONE, F1 DONE (host), F2a DONE (host: PE32+ loader + test app + launch plan, `RAYNU-V-RAYNU-F-LOADER-OK`), F2b DONE on nested VT-x `raynuvsrv1` (serial `RAYNU-V-RAYNU-F-CONOUT-OK`, exits=2; first guest execution of our own tables; not iron). F3 memory services + owned timer tick next. Stage 46 OVMF forcing family closed (ADR-016). Not Everest E5. |
| Months Δ | 0.5→0.5 |
