...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; Port `96024edc` leftover Everest ≠ persist; SET_CONFIG held then READ CBW `cmpl=0`; slot+DCI match; iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (nested-OK closed; Port `96024edc` leftover Everest ≠ persist; READ CBW `cmpl=0`; Force Off open)
...
| Field | Value |
|-------|-------|
| Commit | m8-usb-bot-read-cbw |
| Summary | **Iron leftover, not persist.** Port COM2 still `sha=96024edc` (SET_CONFIG-first not flashed). Second boot: SET_CONFIG held; `usb I/O fail err=8 bot=cbw scsi=read cmpl=0` after CAPACITY. Leftover Everest `ISO-INSTALL-OK` → F7 `DISK-BOOT-OK` → `root=UUID=45213e85-…` → `login:`. This EFI: Transfer Event slot+DCI match; IN bounce; no Reset EP on CBW fail; settle after CAPACITY. Months **0.0 held**. Overall **99 held**. |
| Everest impact | months **0.0 held**; overall **99 held**; ETA 2026-09 held. Not 100%. No persist gate. |
| Gates touched | `xhci.rs` `consume_transfer` + `bounce_in` + `BOT_SETTLE_SPINS`; `usb_bot.rs` `usb_bot_recover_after_fail`; [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. |
