...
| Guest UEFI firmware blob | PARTIAL (RayNu-F: Alpine sys install completed to disk, nested; F7 reset→disk GRUB then rescue) | ADR-016/017: nested `499492c` alpine-extended `setup-disk -m sys` → `Installation is complete. Please reboot.`; nested `3492ebc` F7 relaunch reached installed GRUB then `disk `,gpt2' not found` / `grub rescue>` (HD node on HANDLE_DISK); this slice: Hardware/Vendor whole-disk path so GRUB names `hd0` (host-wired; nested second Linux boot unclaimed); not distro installer / not Everest E5 |
...
| M+4 | 2026-09 | TLS/console + distro installer | remaining Everest | **ETA** (nested F7 GRUB rescue `gpt2`; whole-disk path host-wired) |
...
| Summary | **F7 HANDLE_DISK whole-disk Vendor path (host).** Nested `3492ebc` F7 relaunch staged disk GRUB then `disk `,gpt2' not found` / `grub rescue>` because a Media/HardDrive node on whole-disk BlockIo made GRUB skip the only disk. This slice installs Hardware/Vendor (`GUID_RAYNU_F_DISK`) on `HANDLE_DISK`. Nested second Linux boot is **not** claimed. E5 closes only on iron `ISO-INSTALL-OK`. |
