...
| M+4 | 2026-09 | TLS/console + distro installer | remaining Everest | **ETA** (iron `c2cd099` / `34172103709` apk `n=1345` after mount ok; 3:1 after usbdelay; `ISO-INSTALL-OK` open) |
...
| Summary | **Iron COM2 (UDisk, `c2cd099` / run `34172103709`):** `Mounting boot media: ok.` then apk freeze at virtio `n=1345`. Stopping yield after `media: ok` did not move apk. This pin keeps 1:1 during usbdelay, then 3:1 PIC 11:PIT (UART or 30s TSC). `pic_pending_irq` honors `IRQ11_YIELD_PIT` after mount (`c2cd099` leftover `!apk_media_mounted()` would have made 3:1 a no-op). Do not F11 `34172103709`. Host-tested; not ISO-INSTALL-OK. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Iron apk overlay still open; do not drop months. |
| Gates touched | none. Host `cargo test --no-default-features -- --test-threads=1`. 1:1 yield before mount; 3:1 after. |
| Months Δ | 0.5→0.5 |
...
| 2026-09-08 | pit-during-apk | 0.5 | 95 | Iron UDisk `c2cd099` / run `34172103709`: Mounting boot media ok then apk n=1345; stop-yield after media:ok did not move apk; 1:1 then 3:1 PIC 11:PIT (honor yield after mount; leftover !apk_media_mounted would no-op 3:1); do not F11 34172103709; not ISO-INSTALL-OK; iron P0-14 stays 2b795a0 |
