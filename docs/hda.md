...
| Guest UEFI firmware blob | PARTIAL (RayNu-F nested F7 reboot-to-disk; iron E5 open) | Iron UDisk `63c7e05` / run `34074349118`: Linux 6.12.13-lts on RayNu-F printed `linux PIT once after DRIVER_OK`, mounted boot media (~31s of jiffies), then froze at `Installing packages` last virtio MMIO `n=1345` because HLT after DRIVER_OK kept UART first (cpuidle/`sleep`). This pin arms PIT-once on HLT + UART LSR until `login:`. Do not F11 `34074349118`. Prior: `46fd345` / `34069352671` stalled at n=1281 with no PIT-once; `55a3602` reached `EBS-OK` then cap=1 killed Linux WRMSR; `088ab25` sat in OVMF CpuSleep. Nested F7 reboot-to-disk remains `fe4785a` (not iron / not Everest E5). Operator: [r640_f7_iso_iron.md](runbooks/r640_f7_iso_iron.md). |
...
| M+4 | 2026-09 | TLS/console + distro installer | remaining Everest | **ETA** (iron `63c7e05` / `34074349118` froze at apk `n=1345`; HLT PIT until login pin; `ISO-INSTALL-OK` open) |
...
| Summary | **Iron COM2 (UDisk, `63c7e05` / run `34074349118`): Linux 6.12.13-lts on RayNu-F** printed `linux PIT once after DRIVER_OK`, mounted boot media with advancing jiffies, then froze at `Installing packages` (last virtio MMIO `n=1345`). HLT after DRIVER_OK kept UART first so cpuidle/`sleep` never saw jiffies. Fix: PIT-once on HLT + UART LSR until getty `login:`; UART first after login for auto-answer. QEMU boot gate: OVMF ALIVE/PAST-SEC/DXE/VIRTIO/BOTH informational when RayNu-F CONOUT-OK (Intel nested-KVM cap=1 bypass). Do not F11 `34074349118`. Host-tested; not ISO-INSTALL-OK. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Iron reached apk overlay then a self-inflicted HLT jiffies freeze. Nested-only / CI waiver must not drop months. |
| Gates touched | none. Host tests `product_iso_linux_hlt_pit_during_apk_until_login_then_uart` + `apk_overlay_needs_pit_until_login` + OVMF RayNu-F waiver phrase. |
| Months Δ | 0.5→0.5 |
...
| 2026-09-07 | pit-during-apk | 0.5 | 95 | Iron UDisk `63c7e05` / run `34074349118`: PIT-once printed, boot media mounted, then froze at Alpine `Installing packages` last virtio `n=1345` because HLT after DRIVER_OK kept UART first; PIT-once on HLT + UART LSR until login:; QEMU OVMF ALIVE/DXE informational when RayNu-F ran; do not F11 34074349118; not ISO-INSTALL-OK; iron P0-14 stays 2b795a0 |
