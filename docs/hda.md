...
| Summary | **Iron leftover skipped, not persist.** Stopwalk COM2 Toshiba 298 GiB ready + leftover skip + Alpine login; `vda last_st=0x1` / `sfdisk` I/O error. This commit: one native LBA per USB BOT + REQUEST SENSE after CSW fail; stall dump hold while USB live; nowait `usb rw`. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | `usb_bot.rs` nlb=1 guest chunk + REQUEST SENSE after CSW fail; [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
