...
| Gates touched | RayNu-F F0 DONE, F1 DONE (host), F2a DONE (host: PE32+ loader + test app + launch plan, `RAYNU-V-RAYNU-F-LOADER-OK`), F2b IN PROGRESS (launch wired behind ESP `raynuf.txt`, CI QEMU stages it; unproven until serial `RAYNU-V-RAYNU-F-CONOUT-OK`). Stage 46 OVMF forcing family closed (ADR-016). Not Everest E5. |
| Months Δ | 0.5→0.5 |
