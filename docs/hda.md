...
last_commit: b5ea06915ab2aa87c154aa10cddfb2cfd876c218
last_commit_short: b5ea069
updated_by: cursor
...
| **Confidence** | medium | E2+E3+E5 on COM2; PCI-restore EFI `ec08c00f` still `cand bmsr=7949` / skip-listen; PRE-EBS BMSR peek next; HTTP-OK open |
...
| **Post-EBS durable HTTP (E3b)** | MISSING | PCI-restore EFI `ec08c00f` still `cand bmsr=7949` / skip-listen. PRE-EBS BMSR peek. HTTP-OK not claimed |
...
| P0-12 | **M7.8 / E3b** Host-owned mgmt NIC (ADR-013) | C | 1.0 | P0-4 | PCI-restore `ec08c00f` still `cand bmsr=7949`; PRE-EBS BMSR peek; HTTP-OK **open** |
...
|-------|-------|
| Commit | PENDING |
| Summary | PCI-restore EFI `ec08c00f` still `cand bmsr=7949` / skip-listen. PRE-EBS BMSR peek. HTTP-OK not claimed |
| Everest impact | months 1.5 held; ETA 2026-10; overall 88 held; E3b open (host GPHY vs APE) |
| Gates touched | `peek_bcm5720_bmsr_pre_ebs`; `HOST-NIC-HTTP-OK` not claimed |
| Months Δ | 1.5 held |
...
| H4 | Firmware SNP unusable after EBS | HIGH | PCI-restore EFI `ec08c00f` still `cand bmsr=7949` / skip-listen. PRE-EBS BMSR peek; E3b = HTTP after `BOOT-OK` |
...
|------|--------|-------:|----------:|------|
| 2026-08-19 | PENDING | 1.5 | 88 | PCI-restore EFI `ec08c00f` still `cand bmsr=7949` / skip-listen; PRE-EBS BMSR peek; HTTP-OK not claimed |
...
Now:           E2+E3+E5 stamps CLOSED; Phase 0 CLOSED; Phase D PCI restore closed; E3b OPEN
Months left:   1.5  (ETA ~ 2026-10)
Next move:     flash PRE-EBS BMSR peek EFI; expect COM2 pre-EBS cand; do not curl unless link=up
