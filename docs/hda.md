...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; overlap COM2 INQUIRY lived then `scsi=capacity` leftover DRAM; this EFI overlaps CAPACITY; iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (nested-OK closed; overlap INQUIRY lived, CAPACITY leftover; Force Off open)
...
| Summary | **Iron leftover, not persist.** Overlap COM2: `xhci firstcbw overlap` then CAPACITY CBW `cmpl=0xff` leftover 1 GiB. INQUIRY overlap lived. This EFI: overlap CAPACITY + READ IN. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | `xhci.rs` / `usb_bot.rs` CAPACITY overlap; [m8_persist_iron.md](runbooks/m8_persist_iron.md); [loihda.md](loihda.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
