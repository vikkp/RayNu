...
| Summary | **Iron COM2 (UDisk, `0a9b552` / run `34141401594`):** leftover GSI 2 was a no-op on RayNu-F; apk still froze at virtio `n=1345` after paced PIT. Shared vda+vdb INTx reassert + drain without notify + no unpaced virtio-MMIO PIT after DRIVER_OK. Do not F11 `34141401594`. Host-tested; not ISO-INSTALL-OK. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Iron apk overlay still open; do not drop months. |
| Gates touched | none. Host test: sibling ISR read must not drop shared PIC 11. |
| Months Δ | 0.5→0.5 |
