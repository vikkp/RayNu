...
| M+4 | 2026-09 | TLS/console + distro installer | remaining Everest | **ETA** (iron `e717fb4` / `34220740109` DATA_SEGS=128 still apk `n=1345`; virtio used idx; `ISO-INSTALL-OK` open) |
...
| Summary | **Iron COM2 (UDisk, `e717fb4` / run `34220740109`):** DATA_SEGS=128 still froze at apk `n=1345` after `linux PIC IRQ11 yield until mount`. Do not wait. This pin does not skip avail when used.idx GPA write fails, drains on ISR ACK, dumps stall state, and handles GET_ID. Do not F11 `34220740109`. Host-tested; not ISO-INSTALL-OK. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Iron apk overlay still open; do not drop months. |
| Gates touched | none. Host `cargo test --no-default-features -- --test-threads=1`. virtio used idx. |
| Months Δ | 0.5→0.5 |
