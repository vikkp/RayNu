...
| **Confidence** | medium | E2+E3+E5 on COM2; CORECLK-sans-BMCR EFI `1404f055` still `cand bmsr=7949`; PCI restore + `phy_setup=post`; HTTP-OK open |
...
| **Post-EBS durable HTTP (E3b)** | MISSING | CORECLK-sans-BMCR EFI `1404f055` still `cand bmsr=7949` / `link=timeout`. PCI restore + `phy_setup=post`. HTTP-OK not claimed |
...
| P0-12 | **M7.8 / E3b** Host-owned mgmt NIC (ADR-013) | C | 1.0 | P0-4 | CORECLK-sans-BMCR `1404f055` still `cand bmsr=7949`; PCI restore + `phy_setup=post`; HTTP-OK **open** |
...
|-------|-------|
| Commit | f0961f1 |
| Summary | CORECLK-sans-BMCR EFI `1404f055` still `cand bmsr=7949`. PCI restore + `phy_setup=post`. HTTP-OK not claimed |
| Everest impact | months 1.5 held; ETA 2026-10; overall 88 held; E3b open (PCI restore around chip reset) |
| Gates touched | `pci_cfg_save_dword_count`; skip listen without LSTATUS; `HOST-NIC-HTTP-OK` not claimed |
| Months Δ | 1.5 held |
...
| H4 | Firmware SNP unusable after EBS | HIGH | CORECLK-sans-BMCR EFI `1404f055` still `cand bmsr=7949` / `link=timeout`. PCI restore + `phy_setup=post`; E3b = HTTP after `BOOT-OK` |
...
|------|--------|-------:|----------:|------|
| 2026-08-19 | PENDING | 1.5 | 88 | CORECLK-sans-BMCR EFI `1404f055` still `cand bmsr=7949`; PCI restore + `phy_setup=post`; HTTP-OK not claimed |
...
Now:           E2+E3+E5 stamps CLOSED; Phase 0 CLOSED; Phase D PCI restore + phy_setup=post; E3b OPEN
Months left:   1.5  (ETA ~ 2026-10)
Next move:     flash PCI-restore EFI; expect COM2 pci-restore=64 / phy_setup=post; curl only if link=up
