...
| Guest UEFI firmware blob | PARTIAL (RayNu-F runs GRUB, nested) | ADR-016 RayNu-F: Alpine GRUB `BOOTX64.EFI` executed on our own UEFI tables on nested `raynuvsrv1` (`7ee3a3b`), died in `mm_init` on two RayNu-F bugs (LoadedImage at launch, memory map/pool) fixed in F6-prep; OVMF forcing family closed; not distro installer / not Everest E5 |
...
| Summary | **GRUB ran on RayNu-F (nested `raynuvsrv1`, `7ee3a3b`):** El Torito EFI section → FAT ESP `lba=77` → `\EFI\BOOT\BOOTX64.EFI` 724 992 B staged at `0xbb1000` (1851 relocs) → `VMLAUNCH image=ISO-BOOTX64` → GRUB printed its own `Could not malloc` / `Aborted. Press any key to exit.` through **our** `ConOut` and polled **our** `ReadKeyStroke`. Both faults it named were ours and are fixed in **F6-prep** (host-proven, awaiting nested rerun): `LoadedImage` was only published by `LoadImage`, not on the direct-stage path (`OpenProtocol → NOT_FOUND` ×2); the memory map advertised unmanaged slab slack as conventional and the pool was 20 MiB while GRUB `mm_init` `AllocateAddress`-walks every conventional run for a 32 MiB heap (`AllocatePool OUT_OF_RESOURCES`, `AllocateAddress NOT_FOUND`). `PagePool` now spans slab pool + pre-mapped report-RAM from 32 MiB (cap 256 MiB); memory-map honesty rule: every conventional byte allocatable at its own address. Failed service lines carry `a1..a4` + GUID. Not ISO-INSTALL-OK. Iron P0-14 stays 2b795a0. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. A real distro bootloader executing on our firmware is the strongest E5 evidence since Stage 45, but E5 closes only on iron `ISO-INSTALL-OK`. |
| Gates touched | RayNu-F F3 CLOSED (nested). F4 media published + `LocateHandle(BlockIo)` walked on nested; guest `BLOCKIO-OK` awaited. F5c launch PROVEN (GRUB running, `mm_init` complete). F6-prep NESTED-PROVEN; CPUID exit handled next. F6 iron `ISO-INSTALL-OK` remains the only E5 close. |
| Months Δ | 0.5→0.5 |
