...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; `6ba076cc` leftover Everest ≠ persist; SET_CONFIG + CAPACITY then READ CBW `cmpl=0xff`; SNP DHCP fail stuck Phase B; native BCM5720 DHCP + CBW timeout recover; iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (nested-OK closed; `6ba076cc` leftover Everest ≠ persist; READ CBW `cmpl=0xff` + SNP DHCP stuck; native DHCP; Force Off open)
...
| Field | Value |
|-------|-------|
| Commit | m8-nic-dhcp-nolease |
| Summary | **Iron leftover, not persist.** COM2 `6ba076cc` leftover Everest (`keep=0` `vda` 1.07 GiB). p11 Toshiba SET_CONFIG + CAPACITY then READ CBW timeout `cmpl=0xff`. SNP DHCP failed → analog/coexist skip → Phase B idle `SPA Start needs HTTP`. This EFI: native BCM5720 DHCP without SNP lease; analog always post-EBS; CBW timeout recovers pipes. Months **0.0 held**. Overall **99 held**. |
| Everest impact | months **0.0 held**; overall **99 held**; ETA 2026-09 held. Not 100%. No persist gate. |
| Gates touched | `host_nic_listen.rs` `dhcp_bcm5720`; `mgmt_lease` `prefer_mac`/`load_usable`; `usb_bot.rs` timeout recover; [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. |
