...
| **Confidence** | medium | E2+E3+E5 on COM2; Phase D station `:3a`; PHY-reset EFI still `bmsr=7949` / `lpa=0000`; inherit SNP PHY or APD-off next; HTTP-OK open |
...
| **Post-EBS durable HTTP (E3b)** | MISSING | Station `:3a` on iron; PHY-reset EFI still `bmsr=7949` / `lpa=0000` after `CORECLK_RESET`. Inherit SNP copper when pre-reset `LSTATUS` is set; else `tg3_phy_toggle_apd(false)`. HTTP-OK not claimed |
...
| P0-12 | **M7.8 / E3b** Host-owned mgmt NIC (ADR-013) | C | 1.0 | P0-4 | Station `:3a`; PHY-reset EFI `bmsr=7949`; inherit SNP PHY or APD-off; HTTP-OK **open** |
...
| H4 | Firmware SNP unusable after EBS | HIGH | Station `:3a` programmed; PHY-reset EFI still `bmsr=7949`. Inherit SNP PHY or APD-off; E3b = HTTP after `BOOT-OK` |
...
Now:           E2+E3+E5 stamps CLOSED; Phase 0 CLOSED; Phase D inherit SNP PHY / APD-off; E3b OPEN
Months left:   1.5  (ETA ~ 2026-10)
Next move:     flash inherit-PHY EFI; expect COM2 pre-reset bmsr= then inherit SNP PHY or apd=off
