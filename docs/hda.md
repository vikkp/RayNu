...
| Summary | **Iron leftover, not persist.** Ep0-eval COM2: Toshiba + eval + BOT parse + SET_CONFIG then CAPACITY CBW `err=8 cmpl=0` leftover 1 GiB. Enum lived. This EFI: GET_MAX_LUN + required INQUIRY + longer settle + recover_enum keeps `cmpl`. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | `xhci.rs` GET_MAX_LUN; `usb_bot.rs` required INQUIRY; [m8_persist_iron.md](runbooks/m8_persist_iron.md); [loihda.md](loihda.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
