...
|--------|------:|-------------------|
| **Overall product readiness** | **94%** | -1 (iron apk overlay blocked since 2026-09-07; ten stimulus pins, no progress; probe pin replaces guessing) |
| **Months to Mount Everest** | **0.75** | +0.25 (E5 iron stall unexplained after 10 flashes; honest timer/IPI model likely needed after the probe) |
| **ETA month** | **2026-10** | slipped one month |
| **Confidence** | medium | E2+E3+E3b+Phase F+P0-14 stamps on COM2; E5 iron root cause named by SysRq `t` on `f229d14`: `apk` asleep in `n_tty_write` (lost 16550 THRE interrupt under SOL back-pressure); fix pin not yet on iron |
...
| **Deploy Linux ISO** | ~96% | Nested RayNu-F alpine-extended install + reboot-to-disk (`fe4785a`); iron apk stall explained (console TX deadlock, not virtio/timer); THRE-level fix pin awaits iron |
...
| M+4 | 2026-10 | TLS/console + distro installer | remaining Everest | **ETA** (iron `f229d14` / `34354322953` probe: `apk` in `n_tty_write`, lost 16550 THRE; THRE-level fix pin next; `ISO-INSTALL-OK` open) |
...
| Summary | **Iron COM2 (UDisk, `f229d14` / run `34354322953`) — probe answered.** Two `virtio stall dump ring` lines (`mem_used` matched, `last_st=0x0`, `xlate_fail=0`: completions did land), three RIP samples (`cpu_idle_poll`, `cs=0x10`), then SysRq `w` (nothing blocked in D), `m` (133 MiB free, no pressure), `t`: **`apk` pid 1043 `state:S` in `__schedule → schedule_timeout → wait_woken → n_tty_write → file_tty_write → vfs_writev` with `RDI=1` (stdout)** and `init` in `do_wait`; `l`: idle. Every SysRq IRQ 4 also flushed exactly 16 bytes of apk's progress text (`ckages to root f`, `ilesystem: (1/25`, …) — one 8250 `tx_loadsz` per interrupt. Root cause: our 16550 cleared the THRE latch on IIR read, then LSR (paced to SOL / shared diag ring) read THRE=0, so `serial8250_handle_irq` skipped `tx_chars` and no THRE interrupt was ever re-raised; the 4 KiB xmit buffer filled and `apk` slept forever on the console, issuing no more block I/O. Ten stimulus pins could never have fixed this. **This pin:** THRE is a gated level (`ier.ETBEI && tx_empty()`), not cleared by IIR read, re-asserted per resume, ended by Linux `__stop_tx`; rate bounded by SOL drain. Prior pin `09b5842`: | PIT paced lived (no soft lockup; `PIT paced n=1375` heartbeat for minutes), apk never issued another `0x300`; rings `last==used==avail==live`, `isr=0`, and the `2b3cc1e` watchdog showed `swapper/0` idle — apk sleeps on something that is neither our completion nor a tick. **Ten-flash review:** `6cfabee`…`09b5842` were all stimuli (PIT once/hold/paced, INTx pulse/reassert, IRQ11 yield); three tested hypotheses the previous run had already excluded. This pin adds no stimulus. It is a probe: `virtio stall dump ring` (in-guest `used.idx`, last used elem, status byte, GPA-miss counter), `virtio stall probe` (RIP/CS/CR3 samples, then SysRq `w`/`m`/`t`/`l` via a COM1 BREAK + `sysrq_always_enabled` on the patched cmdline). Also on record: `NMI testsuite local IPI: TIMEOUT` = xAPIC ICR self-IPI not emulated (Pin C candidate). Heuristic PIT/INTx pins frozen until the probe names the wait. Do not F11 `34302053558`. Host-tested; UEFI build compiles; not ISO-INSTALL-OK. |
| Everest impact | months 0.75 held; overall 94 held; ETA 2026-10 held. Root cause is named and the fix is a 16550 correctness change, but months only drop on iron evidence (`Installing packages` completing). Timer/ICR model (Pin C) no longer implicated in this stall; remains a known gap (`local IPI: TIMEOUT`). |
| Gates touched | none. Host `cargo test --no-default-features -- --test-threads=1`; `cargo build --release --features uefi-bin --target x86_64-unknown-uefi`. UART THRE level until stop_tx (+ probe phrases from `f229d14`). |
| Months Δ | 0.75→0.75 |
