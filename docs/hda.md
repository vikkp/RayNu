...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; `cbd9bf47` leftover Everest ≠ persist; p11 GET_DESC `cmd=4 cmpl=0xff`; STATUS TRB + `DESC_SPINS`; iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (nested-OK closed; `cbd9bf47` leftover Everest ≠ persist; GET_DESC `cmd=4` timeout; Force Off open)
...
| Field | Value |
|-------|-------|
| Commit | m8-usb-ep0-status |
| Summary | **Iron leftover, not persist.** COM2 `cbd9bf47` leftover Everest (`keep=0` `vda` 1.07 GiB `root=UUID=35bfa02f-…`). p10 Address Device timeout; p11 Toshiba device desc then config GET_DESC `cmd=4 cmpl=0xff` (`bot=?`). This EFI: wait GET_DESC STATUS TRB pointer; `DESC_SPINS`; no EP0 reset on Transfer timeout; bulk TRB ptr match. Months **0.0 held**. Overall **99 held**. |
| Everest impact | months **0.0 held**; overall **99 held**; ETA 2026-09 held. Not 100%. No persist gate. |
| Gates touched | `xhci.rs` STATUS TRB + `DESC_SPINS` + `should_reset_ep0`; [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. |
