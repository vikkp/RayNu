...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; DESC retry Toshiba named; CSW `68e16633` leftover Everest ≠ persist; port-reset + honest BOT port; iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (nested-OK closed; iron Toshiba named; leftover Everest ≠ persist; Force Off open)
...
| Field | Value |
|-------|-------|
| Commit | m8-usb-bot-port |
| Summary | **Iron leftover, not persist.** CSW COM2 `68e16633`: p11 Toshiba named + p14 hub skip + leftover Everest. Fail `portsc=0x0000000e00000e03` is packed **p14**, not LUN stuck on p10. Scan p10 was Polling (HCRST already ran). Config `08/06/50` already parsed. This EFI: always port-reset CCS (never inherit U0); hub skip never stamps cmpl; no TUR in bring-up/READ retry; COM2 `xhci hcrst` + `xhci bot pN iface=08/06/50`. Months **0.0 held**. Overall **99 held**. |
| Everest impact | months **0.0 held**; overall **99 held**; ETA 2026-09 held. Not 100%. No persist gate. |
| Gates touched | `xhci.rs` always-reset CCS + hcrst/bot-eps serial + hub never stamps; `usb_bot.rs` no TUR; [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. |
