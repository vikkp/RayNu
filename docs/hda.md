...
| Guest UEFI firmware blob | PARTIAL (RayNu-F nested F7 reboot-to-disk; iron E5 open) | Iron UDisk `2b3cc1e` / run `34300317331`: PIT hold lived then unpaced IRQ0 `soft lockup` / `reason=0x1e`. virtio stall dump PIT paced. Do not F11 `34300317331`. Nested F7 reboot-to-disk remains `fe4785a` (not iron / not Everest E5). Operator: [r640_f7_iso_iron.md](runbooks/r640_f7_iso_iron.md). |
...
| M+4 | 2026-09 | TLS/console + distro installer | remaining Everest | **ETA** (iron `2b3cc1e` / `34300317331` PIT hold lived then `soft lockup`; virtio stall dump PIT paced; `ISO-INSTALL-OK` open) |
...
| Summary | **Iron COM2 (UDisk, `2b3cc1e` / run `34300317331`):** PIT hold lived then unpaced IRQ0 `soft lockup` in `handle_softirqs` 26s/52s/78s and `reason=0x1e`. Watchdog proved jiffies move. This pin paces dump-path PIT. Do not F11 `34300317331`. Host-tested; not ISO-INSTALL-OK. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Iron apk overlay still open; do not drop months. |
| Gates touched | none. Host `cargo test --no-default-features -- --test-threads=1`. virtio stall dump PIT paced. |
| Months Δ | 0.5→0.5 |
