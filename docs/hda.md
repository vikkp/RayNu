...
|--------|------:|-------------------|
| **Overall product readiness** | **99%** | **held** — Everest **CLOSED**. M8.0 persist-first attach + nested File RAM + `MODE=keep` + DurableLun NVMe I/O (`MODE=lun`) + USB BOT (`MODE=usb`) + **`MODE=lunkeep` TCG `keep=1` after HV kill**. `MODE=usbkeep` keep-detect host (4 KiB LUN cache + poll xHCI); TCG residual. Not 100%: nested Alpine kill/restart / iron Force Off / TLS / console |
...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (`MODE=lunkeep` TCG; `MODE=usbkeep` host keep-detect; Alpine two-boot + iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (lunkeep TCG keep=1; usbkeep host; Alpine + iron Force Off open)
...
| Field | Value |
|-------|-------|
| Commit | m8-disk-persist-usb-keep |
| Summary | **M8.0 USB keep-detect.** FAT 32-byte dirents were one BOT command each and wrapped the xHCI event ring (`installed=0` despite `EFI PART`). 4 KiB LUN cache + poll-only xHCI + drain leftover events + sticky keep for TCG skip. Host 725. TCG `MODE=usbkeep` residual. QEMU USB ≠ R640. Not nested-OK. Not iron persist OK. Never `ISO-INSTALL-OK`. Do not F11. |
| Everest impact | months **0.0 held**; overall **99 held**; ETA 2026-09 held. Not 100%. |
| Gates touched | `MODE=usbkeep` keep-detect host. `./tools/sync-hda-site.sh --check`. |
...
| 2026-09-12 | m8-disk-persist-usb-keep | 0.0 | 99 | **M8.0 USB keep-detect host:** 4 KiB DurableLun cache so FAT dirents are not one BOT command each; poll-only xHCI + drain leftover events; sticky keep for TCG skip. `MODE=usbkeep` TCG residual. QEMU USB ≠ R640. Not nested-OK. Not iron persist OK. Never `ISO-INSTALL-OK`. Do not F11. months 0.0 held; overall 99 held |
...
Next move:     **M8.0** nested Alpine MODE=full + iron Force Off (`MODE=lunkeep` TCG) ([ADR-018](adr/ADR-018.md) / [m8_plan.md](m8_plan.md))
