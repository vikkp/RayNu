...
| Summary | **Iron leftover skipped, not persist.** Addrretry COM2 guest: Toshiba 298 GiB ready + leftover skip + CSW-queued READs through Alpine `[vda] 298 GiB` login then WRITE CBW `bot=cbw scsi=write cmpl=0xff` at backup-GPT; `sfdisk` I/O error. This EFI: `xhci writequeue` + `xhci writesettle`. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | Lived COM2 WRITE CBW fail after CSW-queued READs; this EFI `xhci writequeue`; [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
