...
|--------|------:|-------------------|
| **Overall product readiness** | **99%** | **held** — Everest **CLOSED**. M8.0 persist-first attach + nested File RAM + `MODE=keep` + DurableLun NVMe I/O (`MODE=lun`) + USB BOT (`MODE=usb`) + **`MODE=lunkeep` TCG `keep=1` after HV kill**. Not 100%: nested Alpine kill/restart / iron Force Off / TLS / console |
...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (`MODE=lunkeep` TCG; Alpine two-boot + iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (lunkeep TCG keep=1; Alpine + iron Force Off open)
...
| Field | Value |
|-------|-------|
| Commit | m8-disk-persist-lun-keep |
| Summary | **M8.0 DurableLun keep=1 after HV kill.** `MODE=lunkeep` TCG: plant GPT+ESP+ext4 at NVMe LUN offset 0, kill HV, second boot `virtio-blk … keep=1 (durable LUN nvme)`. NVMe I/O is post-EBS (OVMF HCRST). QEMU NVMe ≠ R640. Not nested-OK. Not iron persist OK. Never `ISO-INSTALL-OK`. Do not F11. |
| Everest impact | months **0.0 held**; overall **99 held**; ETA 2026-09 held. Not 100%. |
| Gates touched | `MODE=lunkeep` TCG + post-EBS NVMe. `./tools/sync-hda-site.sh --check`. |
