...
| Summary | **Nested `033bc0d` (run `33901852277`): Linux 6.12.13-virt boots on RayNu-F past EBS** (`efi=noruntime` skipped virtual mode; `efi: EFI v2.10 by RayNu-F`; ttyS0; PIT `inject vec=0x30`) then panics `Real mode trampoline was not allocated` in `init_real_mode` — `GetMemoryMap` reserved `[0, 8 MiB)` as one blob so e820 usable started at `0x803000`. Host fix: PC-shaped below-1M conventional `[0x1000, 0x9F000)` (158 pages, AllocateAddress-honest); AnyPages / MaxAddress(≥1MiB) do not consume it so it is still conventional at EBS (GRUB EFI heap already skips `< 1 MiB`). Nested expect: no trampoline panic. F6a ACPI still open. Not ISO-INSTALL-OK. Iron P0-14 stays 2b795a0. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Host-only map fix; nested confirmation next. E5 closes only on iron `ISO-INSTALL-OK`. |
| Gates touched | RayNu-F F6-prep: Linux on RayNu-F past EBS (nested `033bc0d`); below-1M trampoline window host-proven. F6 iron `ISO-INSTALL-OK` remains the only E5 close. |
| Months Δ | 0.5→0.5 |
