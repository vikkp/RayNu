...
last_commit: 0f886dcd5f0d439bfec70d0891e452af3f0acda5
last_commit_short: 0f886dc
updated_by: cursor
...
| Summary | **Iron COM2 (UDisk, `088ab25`) explained: RayNu-F is only entered from the retained-OVMF stop path.** Nested KVM stopped OVMF at SEC for free (`788930c` `stop n=1043 reason=0x30 rip=0xfffd4739`), so nested F7 went straight to `image=ISO-BOOTX64`. Iron OVMF has no such fault; it parked in BDS CpuSleep `rip=0x7f0680d0` `ataio=0` printing ticks toward the 16_777_216 product cap (`ea30da1`). Fix: `raynuf.txt` collapses the OVMF resume cap to 1 (`guest_uefi_raynu_f_resume_cap`) so the leg stops at its first exit and RayNu-F takes the VMCS; prints `RayNu-F direct — OVMF leg bypassed at first exit`. No OVMF state touched (ADR-016). Runbook Phase A corrected. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Root cause is code, not media or EFI pin. Host-tested only; iron re-flash from the CI pin of this commit is the next step. Nested-only evidence must not drop months. |
| Gates touched | none. Host test `raynu_f_flag_collapses_ovmf_leg_to_first_exit`. |
| Months Δ | 0.5→0.5 |
