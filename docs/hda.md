...
| M+4 | 2026-09 | TLS/console + distro installer | remaining Everest | **ETA** (iron `6cfabee` / `34218742196` apk `n=1345` after yield-until-mount lived; virtio chain all segs; `ISO-INSTALL-OK` open) |
...
| Summary | **Iron COM2 (UDisk, `6cfabee` / run `34218742196`):** `linux PIC IRQ11 yield until mount` lived, then apk freeze at virtio `n=1345`. 3:1 is not the stall. This pin copies every virtio-blk data descriptor (`DATA_SEGS` was 16). Do not F11 `34218742196`. Host-tested; not ISO-INSTALL-OK. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Iron apk overlay still open; do not drop months. |
| Gates touched | none. Host `cargo test --no-default-features -- --test-threads=1`. virtio chain all segs. |
| Months Δ | 0.5→0.5 |
