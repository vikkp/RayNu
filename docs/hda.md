...
|--------|------:|-------------------|
| **Overall product readiness** | **99%** | **held** — Everest **CLOSED**. M8.0 persist-first attach + nested File RAM + `MODE=keep` + DurableLun NVMe I/O (`MODE=lun`) + USB BOT/xHCI I/O (host). Not 100%: nested Alpine kill/restart / iron Force Off / TLS / console |
...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (NVMe + USB BOT host-proven; Alpine two-boot + iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (NVMe + USB BOT host-proven; Alpine + iron Force Off open)
...
Next move:     **M8.0** nested Alpine MODE=full + iron Force Off (NVMe + USB BOT host-proven) ([ADR-018](adr/ADR-018.md) / [m8_plan.md](m8_plan.md))
