...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; Port `96024edc` leftover Everest ≠ persist; SET_CONFIG-first + no later-port clobber; iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (nested-OK closed; Port `96024edc` leftover Everest ≠ persist; SET_CONFIG-first; Force Off open)
...
| Field | Value |
|-------|-------|
| Commit | m8-usb-bot-setcfg |
| Summary | **Iron leftover, not persist.** Port COM2 `96024edc`: p11 Toshiba BOT parse `ep_out=2 ep_in=1` then SET_CONFIG `cmd=5 cmpl=0 err=8`; fail `cmpl=0x0e801a40 bot=?` is p14 `reset_port` reset_diag. Leftover Everest `ISO-INSTALL-OK` → F7 `DISK-BOOT-OK` → `login:`. This EFI: SET_CONFIGURATION before Configure Endpoint; later-port reset does not clobber Enum/Xfer; cap `hcc1=`; cmd `6` = SET_CONFIG. Months **0.0 held**. Overall **99 held**. |
| Everest impact | months **0.0 held**; overall **99 held**; ETA 2026-09 held. Not 100%. No persist gate. |
| Gates touched | `xhci.rs` SET_CONFIG-first + `xhci setcfg` + no EP0 rewrite on CONFIG_EP; `usb_bot.rs` `store_usb_bot_diag_unless_kept`; [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. |
