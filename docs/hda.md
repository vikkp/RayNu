...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; skip-INQUIRY `bot=cbw scsi=capacity cmpl=0xff`; settle + waited INQUIRY; iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (nested-OK closed; skip-INQUIRY `bot=cbw scsi=capacity`; Force Off open)
...
| Field | Value |
|-------|-------|
| Commit | m8-usb-first-cbw |
| Summary | **Iron leftover, not persist.** Skip-INQUIRY EFI: SET_CONFIG held then `bot=cbw scsi=capacity cmpl=0xff`. First bulk OUT after SET_CONFIG timed out. This EFI: settle + waited INQUIRY (recover on fail, never ignore) then CAPACITY retry. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | `usb_bot.rs` settle + waited INQUIRY; [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. |
