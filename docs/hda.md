...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; norearm COM2 `epst ep0=3` + `scsi=inquiry` leftover DRAM; this EFI skips GET_MAX_LUN; iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (nested-OK closed; norearm `ep0=3` leftover; skip GET_MAX_LUN; Force Off open)
...
| Field | Value |
|-------|-------|
| Commit | m8-usb-bot-skipmaxlun |
| Summary | **Iron leftover, not persist.** Norearm COM2: `xhci firstcbw norearm` then `epst ep0=3` after GET_MAX_LUN DATA-then-Stop; INQUIRY CBW `cmpl=0xff` leftover 1 GiB. This EFI: skip optional GET_MAX_LUN (`xhci maxlun pN skip`) so EP0 stays Running. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | `xhci.rs` skip GET_MAX_LUN; [m8_persist_iron.md](runbooks/m8_persist_iron.md); [loihda.md](loihda.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
