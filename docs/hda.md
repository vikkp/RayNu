...
| **Confidence** | medium | E2+E3+E5 on COM2; Phase D station `:3a`; iron `link=timeout bmsr=7949` (AN incomplete); PHY reset+PWRCTL+MDIX next; HTTP-OK open |
...
| **Post-EBS durable HTTP (E3b)** | MISSING | Station `:3a` on iron; `link=timeout bmsr=7949` (no carrier, AN incomplete). `tg3` BMCR_RESET + AUXCTL PWRCTL + Auto-MDIX; HTTP-OK not claimed |
...
| P0-12 | **M7.8 / E3b** Host-owned mgmt NIC (ADR-013) | C | 1.0 | P0-4 | Station `:3a`; `bmsr=7949` no link; `tg3_phy_reset` + PWRCTL + Auto-MDIX; HTTP-OK **open** |
...
| H4 | Firmware SNP unusable after EBS | HIGH | Station `:3a` programmed; `link=timeout bmsr=7949`. BMCR_RESET + PWRCTL + Auto-MDIX; E3b = HTTP after `BOOT-OK` |
...
Now:           E2+E3+E5 stamps CLOSED; Phase 0 CLOSED; Phase D PHY reset after bmsr=7949; E3b OPEN
Months left:   1.5  (ETA ~ 2026-10)
Next move:     flash PHY-reset EFI; expect COM2 phy_reset=yes then link=up (or still timeout with bmcr/lpa)
