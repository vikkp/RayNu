...
| **Confidence** | medium | E2+E3+E3b+Phase F+P0-14 stamps on COM2; E5 iron apk sleeps on an unknown wait (device rings empty, IRQ0/INTx proven, CPU idle); SysRq probe pin asks Linux |
...
| **Deploy Linux ISO** | ~96% | Nested RayNu-F alpine-extended install + reboot-to-disk (`fe4785a`); iron apk overlay sleeps at virtio `n≈1373` (unexplained; probe pin) |
...
| Guest UEFI firmware blob | PARTIAL (RayNu-F nested F7 reboot-to-disk; iron E5 open) | Iron UDisk `09b5842` / run `34302053558`: PIT paced lived (no lockup, `PIT paced n=1375` for minutes) and apk still never kicked; rings `last==used==avail==live`, CPU idle. Ten stimulus pins refuted their own hypotheses; this pin is a **probe** (virtio stall dump ring + SysRq `w`/`m`/`t`/`l` over a COM1 BREAK + RIP samples). Heuristic PIT/INTx pins frozen. Do not F11 `34302053558`. Nested F7 reboot-to-disk remains `fe4785a` (not iron / not Everest E5). Operator: [r640_f7_iso_iron.md](runbooks/r640_f7_iso_iron.md). |
...
| M+4 | 2026-10 | TLS/console + distro installer | remaining Everest | **ETA** (iron `09b5842` / `34302053558` PIT paced lived, apk still asleep; virtio stall probe pin asks Linux what it waits on; `ISO-INSTALL-OK` open) |
...
| Summary | **Iron COM2 (UDisk, `09b5842` / run `34302053558`):** PIT paced lived (no soft lockup; `PIT paced n=1375` heartbeat for minutes), apk never issued another `0x300`; rings `last==used==avail==live`, `isr=0`, and the `2b3cc1e` watchdog showed `swapper/0` idle — apk sleeps on something that is neither our completion nor a tick. **Ten-flash review:** `6cfabee`…`09b5842` were all stimuli (PIT once/hold/paced, INTx pulse/reassert, IRQ11 yield); three tested hypotheses the previous run had already excluded. This pin adds no stimulus. It is a probe: `virtio stall dump ring` (in-guest `used.idx`, last used elem, status byte, GPA-miss counter), `virtio stall probe` (RIP/CS/CR3 samples, then SysRq `w`/`m`/`t`/`l` via a COM1 BREAK + `sysrq_always_enabled` on the patched cmdline). Also on record: `NMI testsuite local IPI: TIMEOUT` = xAPIC ICR self-IPI not emulated (Pin C candidate). Heuristic PIT/INTx pins frozen until the probe names the wait. Do not F11 `34302053558`. Host-tested; UEFI build compiles; not ISO-INSTALL-OK. |
| Everest impact | months 0.5→0.75; overall 95→94; ETA 2026-09→2026-10. E5 iron blocked since 2026-09-07 with zero progress across ten flashes; honest i8254/LAPIC-timer/ICR model (Pin C) is probable follow-on scope. |
| Gates touched | none. Host `cargo test --no-default-features -- --test-threads=1`; `cargo build --release --features uefi-bin --target x86_64-unknown-uefi`. virtio stall dump ring; virtio stall probe; UART sysrq break; linux-line sysrq_always_enabled. |
| Months Δ | 0.5→0.75 |
