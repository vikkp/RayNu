...
| M+4 | 2026-09 | TLS/console + distro installer | remaining Everest | **ETA** (iron `34968f7` / `34224368343` used.idx then apk `n=1345`; virtio shared INTx hold; `ISO-INSTALL-OK` open) |
...
| Summary | **Iron COM2 (UDisk, `34968f7` / run `34224368343`):** used.idx + stall dump lived (usbdelay dump `last==used` `isr=0` `n=1118`) then same apk freeze at `n=1345` after ISO notify + disk ISR ACK. Do not wait. This pin holds PIC 11 while sibling ISR is latched, drains before ISR ACK, dumps again after later MMIO. Do not F11 `34224368343`. Host-tested; not ISO-INSTALL-OK. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Iron apk overlay still open; do not drop months. |
| Gates touched | none. Host `cargo test --no-default-features -- --test-threads=1`. virtio shared INTx hold. |
| Months Δ | 0.5→0.5 |
