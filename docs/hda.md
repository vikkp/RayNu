...
| Summary | **Iron leftover skipped, not persist.** Writequeue COM2: Toshiba 298 GiB ready + leftover skip + WRITE lived (`wr=1` then `ISO-INSTALL-OK` on USB LUN) then `usb rw ok` flood mashed COM2. This EFI: quiet oks (`xhci_rw_ok_should_print`). Keep writequeue. Months **0.0 held**. Overall **99 held**. |
| Everest impact | months **0.0 held**; overall **99 held**; ETA 2026-09 held. Not 100%. No persist gate. LOIHDA persist **70% held** (A2 still open). USB `ISO-INSTALL-OK` is not persist. |
| Gates touched | Lived COM2 WRITE + ok-print flood; this EFI `xhci_rw_ok_should_print`; [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
