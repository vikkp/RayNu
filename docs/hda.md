...
| **Deploy Linux ISO** | ~99% | Private guest-UEFI VMCS armed; live E4 SHELL EPT not written; 4 MiB fixture not shipped OVMF.fd; VMLAUNCH insn not issued; distro later |
...
| Guest UEFI firmware blob | PARTIAL (priv-vmcs) | Private guest-UEFI VMCS selected (not E4 SHELL); live E4 SHELL EPT not written; 4 MiB fixture not shipped OVMF.fd; VMLAUNCH insn not issued |
...
|-------|-------|
| Commit | e5-priv-vmcs |
| Summary | P0-36 private guest-UEFI VMCS arm after live-ESP require; live E4 SHELL EPT not written; 4 MiB fixture not shipped OVMF.fd; VMLAUNCH insn not issued. Iron P0-14 stays 2b795a0. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Private VMCS selected ≠ live E4 SHELL EPT write ≠ shipped OVMF.fd ≠ VMLAUNCH insn ≠ installer. |
| Gates touched | `RAYNU-V-M7-E5-PRIV-VMCS-OK` (host). Not Everest E5 / not `ISO-INSTALL-OK`. |
| Months Δ | 0.5→0.5 |
...
| H3 | No live guest UEFI CD | MED | Private guest-UEFI VMCS arm closed (P0-36); live E4 SHELL EPT not written; 4 MiB fixture not shipped OVMF.fd; VMLAUNCH insn not issued; `attach_cdrom_uefi` still stub; extract-boot is lab MVP only |
