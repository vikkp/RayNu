...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; cmdptr COM2 p11 Enable Slot timeout after p10 ADDR; this EFI p11-first + abort ADDR_SPINS; iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (nested-OK closed; p11 Enable Slot timeout after p10; Force Off open)
...
| Summary | **Iron leftover, not persist.** Cmdptr COM2: p10 ADDR timeout then p11 Enable Slot `cmd=2 cmpl=0xff`. Toshiba not named. This EFI: p11-first walk + abort ADDR_SPINS, no Disable Slot wait. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | `xhci.rs` p11-first + abort; [m8_persist_iron.md](runbooks/m8_persist_iron.md); [loihda.md](loihda.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
