...
| Guest UEFI firmware blob | PARTIAL (RayNu-F: Alpine sys install completed to disk, nested; F7 reset→disk ESP wired host-only) | ADR-016/017: nested `499492c` alpine-extended `setup-disk -m sys` → `Installation is complete. Please reboot.` on a 512 MiB leftover-DRAM virtio-blk disk; F7 host-wires CF9/KBC/TF reset, FADT RESET_REG, `reset_keep_disk`, disk-before-ISO GPT ESP, serial `reboot` (nested second Linux boot not yet run; VMCS re-init + installed GRUB cmdline are nested-only risks); not distro installer / not Everest E5 |
...
| Summary | **F7 guest reset → RayNu-F relaunch wired (host).** CF9/KBC/TF classifier, FADT `RESET_REG` at `0xCF9`, serial `reboot` after `Please reboot.`, `reset_keep_disk` (install disk kept), disk-before-ISO GPT ESP staging, VMCS long-mode overlay + TSS, reset cap=1. Nested second Linux boot is **not** claimed (this environment cannot run nested KVM). E5 closes only on iron `ISO-INSTALL-OK`. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Nested-only evidence must not drop months. Known nested risks: VMCS re-init after 64-bit Linux; installed GRUB must inherit `KERNELOPTS`. |
