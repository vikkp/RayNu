...
| Guest UEFI firmware blob | PARTIAL (RayNu-F: Alpine sys install completed to disk, nested; reboot-to-disk F7 open) | ADR-016 RayNu-F: nested `499492c` alpine-extended `setup-disk -m sys` → `Installation is complete. Please reboot.` on a 512 MiB leftover-DRAM virtio-blk disk (GPT + ESP + ext4 + grub-install); Alpine GRUB → EFI stub → EBS → Linux 6.12.13-virt `efi: EFI v2.10 by RayNu-F` on nested `raynuvsrv1`; trampoline+LSTAR closed (`8f62fea`/`659bb41`); nested `2bebea7` walk: CEA present both PGDs, leaf NX GPA `0x708f000`; this slice keeps NXE after RayNu-F EBS / sticky hold; F6a ACPI + install-disk DevicePath open; not distro installer / not Everest E5 |
...
| Summary | **Nested Alpine sys install completed under RayNu-F.** Nested `499492c` (alpine-extended, `mem=4096M`): `leftover install disk hpa=0x20000000 bytes=536870912`, `BlockIo disk_bytes=536870912 disk_last_lba=1048575`, `setup-disk -m sys` → 25 packages → `initramfs: creating /boot/initramfs-lts` → `Generating grub configuration file … Found linux image: /boot/vmlinuz-lts … done` → **`Installation is complete. Please reboot.`** First complete Linux install to a virtio-blk disk under our own firmware: GPT + FAT ESP + ext4 root + `grub-install` on `/dev/vda`. This commit records the evidence; harness prints an honest `nested Alpine sys install completed …` line (not the iron marker). |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. E5 nested proof is now one stage from end-to-end: **F7 reset → boot from the installed disk** (guest `reboot` has no reset path today: no FADT reset register, KBC `0xFE` absorbed, `0xCF9` generic → triple fault → `raynu_f_stop`). E5 closes only on iron `ISO-INSTALL-OK`, which also needs media larger than the Cruzer for extended. |
| Gates touched | RayNu-F F6-prep: install completed nested (install-disk side of F6 proven under RayNu-F; not iron). F7 opened: reset → RayNu-F relaunch preferring the disk GPT ESP `\EFI\BOOT\BOOTX64.EFI` over El Torito. F6 iron `ISO-INSTALL-OK` remains the only E5 close. |
| Months Δ | 0.5→0.5 |
