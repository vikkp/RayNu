...
| **Deploy Linux ISO** | ~99% | First guest-UEFI VMLAUNCH of retained ESP OVMF.fd (QEMU/VMX); not installer; distro later |
...
| Guest UEFI firmware blob | PARTIAL (ESP retained + first VMLAUNCH) | Real ESP OVMF.fd retained; private VMCS + EPT VMLAUNCH of retained bytes on QEMU/VMX; not installer / not Everest E5 |
