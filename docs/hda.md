...
|--------|------:|-------------------|
| **Overall product readiness** | **94%** | 0 (iron apk overlay blocked since 2026-09-07; THRE-level fix `8b6ed1a` lived but changed nothing; telemetry pin names the broken link) |
...
| **Confidence** | medium | E2+E3+E3b+Phase F+P0-14 stamps on COM2; E5 iron wait still `apk` in `n_tty_write` (SysRq `t` on `f229d14` **and** `8b6ed1a`); THRE-level fix ran on iron with no visible change, so the broken THRE link is not yet named — telemetry pin next |
...
| **Deploy Linux ISO** | ~96% | Nested RayNu-F alpine-extended install + reboot-to-disk (`fe4785a`); iron apk stall is a console TX deadlock (not virtio/timer); THRE-level fix `8b6ed1a` did not clear it on iron; THRE chain telemetry + `build: sha=` pin next |
...
| M+4 | 2026-10 | TLS/console + distro installer | remaining Everest | **ETA** (iron `8b6ed1a` / `34415711199`: THRE level lived, `apk` still in `n_tty_write`; THRE chain telemetry + `build: sha=` pin next; `ISO-INSTALL-OK` open) |
...
| Summary | **Iron COM2 (UDisk, `8b6ed1a` / run `34415711199`) — THRE level lived, nothing visible changed.** Same signature as the `f229d14` probe: `apk` pid 1043 `state:S` in `n_tty_write → wait_woken` (RDI=1), apk's own text moving only around SysRq IRQ 4s (`(1/25) Installin` → `g alpine-baselayout-data (3.6.8-` → `r1)\n(2` → `/25) Installing ` → `musl (1.2.5-r9)\n  0%` → `(3/25)`), virtio stall dump at `n=1373`, `PIT paced n=1375` for minutes, `sysrq l` idle in `cpu_idle_poll`, `local IPI: TIMEOUT` unchanged. **Not a regression** (same stall, one probe deeper); **not a fix**. The level model assumes links that iron may not honour and host tests cannot tell which: Linux ETBEI state (`__stop_tx` / flow), the latch, the shared TX ring never empty when asked (our own diag bytes), COM2 (iDRAC SOL) THRE held low, 8259 IMR/ISR for IRQ 4, PIT hold starving IRQ 4, or IRQ 4 taken but answered IIR `NONE` / LSR `THRE=0`. This pin counts every link and prints them with each `PIT paced n=` heartbeat: `virtio stall dump thre ier= latch= pend= brk= rx= ring= com2_lsr= iir=RX/THRE/NONE lsr_thre=ON/OFF thr= etbei=ON/OFF raise=REASSERT/PIO lower= pic irr= imr= isr= rdy= take0= take4=` (`guest_uart::thre_chain`, `guest_irq::pic_master_snap`, `serial::com2_lsr_raw`), snapshotted **before** the heartbeat's own bytes enter the ring. Also `build: sha=` under the M0 banner (`option_env!("RAYNU_BUILD_SHA")`, exported by `tools/build.sh` from `GITHUB_SHA`) so an iron log can be tied to a commit. Decision table for the `thre` line added to the runbook. No behaviour change to inject / UART / PIC. |
