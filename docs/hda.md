...
| Summary | Stage 46 OPEN fourteenth slice: ISO patch console=ttyS0 noapic (PIC IRQ 11) + alpine_dev=vdb when present so alpine-virt uses virtio-iso /dev/vdb. Lab 73728 stub still fail-softs to E4. Not ISO-INSTALL-OK. M4.3 stays CLOSED. Iron P0-14 stays 2b795a0. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Not installer. |
| Gates touched | Stage 46 OPEN (noapic + alpine_dev=vdb ISO patch). Not Everest E5. |
| Months Δ | 0.5→0.5 |
...
| H3 | Guest UEFI CD not bootable | MED | ATAPI `sectors>0` closed (P0-59); Stage 45 El Torito closed on iron COM2 `0be7283`; P0-60 G1 EPT closed; G0 relocate closed (`M4-NVM-OK`); M4.3 host-slab closed (`M4-BLK-OK` `0x10c00000`); Stage 46 OPEN (ESP product ISO + virtio-pci queues + PIC/IOAPIC inject + 16550/ttyS0 + SOL RX + Alpine auto-answer + `console=ttyS0 noapic` + `alpine_dev=vdb` + GRUB serial + 31-sector ATAPI PIO + chained virtio OUT + read-only ISO virtio `00:03.0` + 4KiB GPA copies + virtio IOAPIC pin 11 + hold when armed; lab stub still E4); extract-boot is lab MVP only |
