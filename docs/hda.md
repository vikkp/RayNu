...
|--------|------:|-------------------|
| **Overall product readiness** | **99%** | **held** — Everest **CLOSED** (`f72b4276`). `fa6ce771` reached the installed Toshiba `login:` a fourth time, then the SPA painted and the chassis turned off. Not 100%: A4s / console / auth are M8. |
...
| Summary | **`fa6ce771` SPA painted, chassis off again.** Fourth login held the install. Paced SOL RX did not stop the power-off. Next EFI holds listen 30 s after one exchange. Months **0.0 held**. Overall **99 held**. |
...
| H2 | TLS / console polish | MED | **M8.1 CLOSED on COM2** (`928d6224` `RAYNU-V-M8-TLS-OK`). Three Firefox renders (`36d3b559` twice, `fa6ce771` once) were each followed by the chassis turning off. Paced SOL RX was not sufficient. This EFI holds the listen 30 s after one HTTP exchange and does not spin the NIC inside the vmexit. A4s stays open. A6 and A5 stay firmware-in-tree. rustls/ring stay out of `uefi-bin`. Not a reopened Everest. |
...
| H10 | Persist disk must be **reliable**, not once | **HIGH** | `fa6ce771` is the fourth installed `login:` (UUID `a0ad99ac-…`). vda2 counts match boots 2 and 3 after the 23:45 chassis off. The SPA then painted and the dashboard showed Power State OFF. Residual: unclean shutdown still needs ext4 journal recovery and clears a vfat dirty bit. Plan: [m8_state.md](m8_state.md). Evidence: [2026-09-24-36d3b559-persist-login.md](evidence/r640/2026-09-24-36d3b559-persist-login.md). |
...
| 2026-09-25 | m8-grubcfg-esp | 0.0 | 99 | **`fa6ce771` SPA then chassis off.** Fourth login, same UUID and vda2 counts, lease `.150`. Page went green. Dashboard Power State OFF. Paced SOL RX was not sufficient. This EFI holds listen 30 s after one exchange. months 0.0 held; overall 99 held |
| 2026-09-25 | m8-grubcfg-esp | 0.0 | 99 | **`fa6ce771` fourth login.** Lease `.150`. `DISK-BOOTX64`, UUID `a0ad99ac-…`, vda2 `6380/521216` `102908/2084352` matching boots 2 and 3 after the 23:45 off. Journal recovered. The page trial is the row above. months 0.0 held; overall 99 held |
...
Next move:     **Chassis is off.** Quit Firefox. Do not Power On `fa6ce771` to open the page. Next EFI prints `listen hold after HTTP` after one exchange. Do not refresh. Do not curl. Do not setup-disk. See docs/m8_state.md.
