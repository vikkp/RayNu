...
| Guest UEFI firmware blob | PARTIAL (RayNu-F nested F7 reboot-to-disk; iron E5 open) | Iron UDisk `c815ccc` / run `34078335291`: Linux 6.12.13-lts on RayNu-F reached APIC MADT then `restore host xcr0 reason=0x1e` (I/O) + `product ISO hold`. PIT hold+raise on every resume treated firmware queue-arm + PHASE_LOGIN as virtio probe and injected vec=0x30 into a half-built IDT. This pin arms PIT only after Linux DEVICE_STATUS / both DRIVER_OK. Do not F11 `34078335291`. Prior: `20e8b70` / `34076175624` froze at apk `n=1345` (`idle=poll`); `63c7e05` / `34074349118`; `46fd345` n=1281; `55a3602` cap=1 WRMSR; `088ab25` OVMF CpuSleep. Nested F7 reboot-to-disk remains `fe4785a` (not iron / not Everest E5). Operator: [r640_f7_iso_iron.md](runbooks/r640_f7_iso_iron.md). |
...
| M+4 | 2026-09 | TLS/console + distro installer | remaining Everest | **ETA** (iron `c815ccc` / `34078335291` MADT then I/O `reason=0x1e` hold; PIT after virtio probe pin; `ISO-INSTALL-OK` open) |
...
| Summary | **Iron COM2 (UDisk, `c815ccc` / run `34078335291`):** Linux reached APIC MADT then `restore host xcr0 reason=0x1e` (I/O) + `product ISO hold`. PIT hold+raise on every resume treated firmware queue-arm + PHASE_LOGIN as virtio probe and injected vec=0x30 into a half-built IDT. Fix: arm PIT only after Linux DEVICE_STATUS / both DRIVER_OK. Do not F11 `34078335291`. Host-tested; not ISO-INSTALL-OK. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Iron apk overlay still open; do not drop months. |
| Gates touched | none. Host test `product_iso_linux_early_kernel_uart_beats_pit`. |
| Months Δ | 0.5→0.5 |
