...
|--------|------:|-------------------|
| **Overall product readiness** | **99%** | **held** — Everest **CLOSED**. M8.0 persist-first attach + nested File RAM + `MODE=keep` + DurableLun NVMe I/O (`MODE=lun`) + USB BOT (`MODE=usb`) + **`MODE=lunkeep`/`usbkeep` TCG `keep=1` after HV kill**. Not 100%: nested Alpine kill/restart / iron Force Off / TLS / console |
...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (`MODE=lunkeep`/`usbkeep` TCG; Alpine two-boot + iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (lunkeep+usbkeep TCG keep=1; Alpine + iron Force Off open)
...
| Summary | **M8.0 USB keep=1 after HV kill.** `MODE=usbkeep` TCG plant GPT at USB LUN offset 0, kill HV, second boot `virtio-blk … keep=1 (durable LUN usb)`. Keep-detect is GPT header CRC + first ESP + FAT BPB + ext4 (array CRC is 32 BOT fills). 64×512 LUN cache. QEMU USB ≠ R640. Not nested-OK. Not iron persist OK. Never `ISO-INSTALL-OK`. Do not F11. |
| Everest impact | months **0.0 held**; overall **99 held**; ETA 2026-09 held. Not 100%. |
| Gates touched | `MODE=usbkeep` TCG keep=1. `./tools/sync-hda-site.sh --check`. |
...
Next move:     **M8.0** nested Alpine MODE=full + iron Force Off (`MODE=lunkeep`/`usbkeep` TCG) ([ADR-018](adr/ADR-018.md) / [m8_plan.md](m8_plan.md))
