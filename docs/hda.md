...
| **Deploy Linux ISO** | ~92% | FW size-floor staged; not EDK2; VMLAUNCH refused; real EDK2 + distro later |
...
| Guest UEFI firmware blob | PARTIAL (size-floor) | `stage_ovmf_firmware_floor` after prepare; 4 KiB not EDK2; VMLAUNCH refused |
...
|-------|-------|
| Commit | e5-fw-floor |
| Summary | P0-25 firmware size-floor after prepare; 4 KiB not EDK2; VMLAUNCH refused. Iron P0-14 stays 2b795a0. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Floor ≠ EDK2 ≠ VMLAUNCH ≠ installer. |
| Gates touched | `RAYNU-V-M7-E5-FW-FLOOR-OK` (host). Not Everest E5 / not `ISO-INSTALL-OK`. |
| Months Δ | 0.5→0.5 |
...
| H3 | No live guest UEFI CD | MED | Floor closed (P0-25); 4 KiB not EDK2; VMLAUNCH refused; `attach_cdrom_uefi` still stub; extract-boot is lab MVP only |
