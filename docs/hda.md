...
| Summary | **First `setup-disk` past apk — blocked only by disk size.** Nested `c751fbe` on alpine-extended (RayNu-F, `mem=4096M`): extended `grub.cfg` grow live, `6.12.13-0-lts` → login → SETUP → `setup-disk` resolved `grub-efi` + `dosfstools` from the ISO, partitioned `/dev/vda`, installed packages, then `No space left on device` (`boot/System.map-6.12.13-0-lts`, `grub-efi-2.12-r7` renames) — the pool disk was 64 MiB (`virtio-blk install disk bytes=67108864`) with a 48 MiB ESP. This slice carves the install disk from leftover DRAM above PRECISE **before** the report-RAM extra seed (`carve_leftover_install_disk`: 1 GiB / 512 MiB / 256 MiB, 768 MiB guest floor; nested 1.44 GiB leftover → 512 MiB; iron → 1 GiB), `try_alloc_product_iso_install_disk` prefers it, pool ladder stays the fallback; guest sees those HPAs only through virtio-blk/BlockIo (ADR-004). Harness scans `leftover install disk` / `No space left` / `Installation is complete`. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Nested run next should reach `Installation is complete` on a 512 MiB `/dev/vda` (GPT + ESP + ext4 root, grub-install). E5 still closes only on iron `ISO-INSTALL-OK`, which also needs media larger than the Cruzer. |
| Gates touched | RayNu-F F6-prep: nested alpine-extended → shell → `setup-disk` past apk + partition + package install (first time); leftover-DRAM install disk (host tests: carve ladder, floor, one-shot reserve). F6 iron `ISO-INSTALL-OK` remains the only E5 close. |
| Months Δ | 0.5→0.5 |
