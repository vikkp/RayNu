...
**Status: NEAR · ~98% · ~0.25 months residual (iron reboot-to-disk; installer DONE on iron 2026-09-10)**
...
| M+4 | 2026-10 | TLS/console + distro installer | remaining Everest | **ETA** (**installer DONE on iron 2026-09-10**: `59ac070` / `34425781629` `ISO-INSTALL-OK` + `Installation finished`; open: iron reboot-to-disk — F7 `VMCLEAR/VMPTRLD` host-stack overflow fixed this pin, flash next) |
...
**Pull-forward rule:** E2 closed 2026-08-15; E3 bring-up closed 2026-08-16; **E3b closed 2026-08-20**; **P0-14 closed 2026-08-21**; **E5 install-to-disk closed on iron 2026-09-10** (`59ac070`). Shrink further when the installed disk boots on iron (F7 `DISK-BOOT-OK`). Document why in [Changelog](#hda-changelog).
...
| Summary | **Iron COM2 (UDisk, `59ac070` / run `34425781629`) — `RAYNU-V-M7-ISO-INSTALL-OK` on the real R640.** The `916af96` console-throughput fix landed: `Installing packages to root filesystem` finished in 0.5 s (was a permanent `n_tty_write` stall), login prompt, `setup-disk -m sys /dev/vda` → GPT + ext4 + `grub-install` x86_64-efi + `initramfs-lts` → `Installation finished. No error reported.` Heartbeats: `room=3177..4096 pace=3360→5811 fifo=1 drained=38827→97346 win=6186→17488` (16 B/window, ring never full). Then `reboot` → `reboot: Restarting` → `RayNu-F relaunch after reset (F7)` → **`launch failed: VMCLEAR/VMPTRLD`**. Root cause from the `59ac070` release asm: `raynu_f_reset_relaunch` reserved an 81,024-byte frame (`RAYNU_F_STATE = FirmwareState::new()` built the 79,704-byte `PagePool` on the stack then `memcpy`) on the 16 KiB guest-UEFI host stack; the private VMCS was the next frame down, so `VMPTRLD` read a foreign revision dword. This pin: `.rdata` `RAYNU_F_STATE_TEMPLATE` memcpy (frame now 40 bytes), host stack 4→32 pages + 1 guard page checked before F7 `VMCLEAR` (`boot: guest-UEFI host stack top=… pages=32 guard=…`), split `VMCLEAR`/`VMPTRLD` failure lines (`rev=0x… want=0x… guard=ok|BREACHED`), `flush_guest_tx()` so `guest reset requested src=` reaches COM2. |
| Everest impact | months 0.75→**0.5**; overall 94→**96**; ETA 2026-10 held. Concrete DONE evidence: iron install marker + completed `setup-disk`. Scope unchanged: E5 still needs the installed disk to boot on iron (F7 `DISK-BOOT-OK`). |
| Gates touched | **`RAYNU-V-M7-ISO-INSTALL-OK` closed on iron** (install; reboot-to-disk residual). Host `cargo test --no-default-features -- --test-threads=1` 683 passed (new: `f7_relaunch_resets_firmware_state_without_a_stack_temporary`, `flush_guest_tx_empties_the_ring_before_a_mode_change`); `./tools/build.sh` (1,801,216 B, +84 KiB `.rdata` template); TCG QEMU boot gate. RayNu-F F7 template reset. guest-UEFI host stack guard. |
| Months Δ | 0.75→0.5 |
