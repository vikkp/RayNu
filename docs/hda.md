...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; epst COM2 `scsi=read` leftover DRAM; iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (nested-OK closed; epst `scsi=read` leftover; Force Off open)
...
| Field | Value |
|-------|-------|
| Commit | m8-usb-bot-firstread |
| Summary | **Iron leftover, not persist.** Epst COM2: Toshiba `epst out=1 in=1` + BOT reset + GET_MAX_LUN then INQUIRY/CAPACITY lived; first READ CBW `cmpl=0xff` leftover 1 GiB. CONFIG_EP packing is Running. This EFI: prepare_first_read + longer first-READ wait. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | `xhci.rs` / `usb_bot.rs` first-READ prepare; [m8_persist_iron.md](runbooks/m8_persist_iron.md); [loihda.md](loihda.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
...
Next move:     **M8.0 iron** COM2 `usb I/O ready` without `read-fail` + Alpine `vda` 298 GiB, then Force Off / `RAYNU-V-M8-DISK-PERSIST-OK`. Epst COM2 INQUIRY/CAPACITY then READ CBW leftover DRAM is **not** persist. Nested-OK closed on `raynuvsrv1` `ce3d8a09`.
