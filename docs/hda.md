...
| **Confidence** | medium | E2+E3+E5 on COM2; PHY-before-reset EFI `1213952` still `bmsr=7949`; skip CORECLK_RESET next; HTTP-OK open |
...
| **Post-EBS durable HTTP (E3b)** | MISSING | PHY-before-reset EFI `1213952`: `phy_reset=pre` then `CORECLK_RESET` still `bmsr=7949` / `lpa=0000`. Skip chip reset. HTTP-OK not claimed |
...
| P0-12 | **M7.8 / E3b** Host-owned mgmt NIC (ADR-013) | C | 1.0 | P0-4 | PHY-before-reset EFI `1213952` still `bmsr=7949`; skip CORECLK_RESET; HTTP-OK **open** |
...
|-------|-------|
| Commit | PENDING |
| Summary | PHY-before-reset EFI `1213952` still `lpa=0000`. Skip `CORECLK_RESET`; keep GPHY analog. HTTP-OK not claimed |
| Everest impact | months 1.5 held; ETA 2026-10; overall 88 held; E3b open (skip chip reset) |
| Gates touched | `skip_coreclk_reset`; `HOST-NIC-HTTP-OK` not claimed |
| Months Δ | 1.5 held |
...
| H4 | Firmware SNP unusable after EBS | HIGH | PHY-before-reset EFI `1213952` still `bmsr=7949` / `lpa=0000`. Skip CORECLK_RESET; E3b = HTTP after `BOOT-OK` |
...
Now:           E2+E3+E5 stamps CLOSED; Phase 0 CLOSED; Phase D skip CORECLK_RESET; E3b OPEN
Months left:   1.5  (ETA ~ 2026-10)
Next move:     flash skip-CORECLK_RESET EFI via raynuvsrv1; expect COM2 keep GPHY analog (no reset…)
