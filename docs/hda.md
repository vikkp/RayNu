...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; descabort COM2 Toshiba 298 GiB ready + virtio keep=0 then vda last_st=0x1; guestio COM2 vda1 + stall dump during ISO mount; this EFI stallquiet; iron Force Off open) |
...
| Summary | **Iron leftover skipped, not persist.** Guestio COM2 (`f2c55be4`): Toshiba 298 GiB ready + virtio keep=0; Alpine `vda` 298 GiB `vda1`; disk 3/3 ISO 556/556; leftover apk stall dump during ISO mount (not `last_st=0x1`). This EFI: hold stall dump until apk while USB LUN live; `usb rw ok`/`wait`. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | `guest_uefi.rs` USB stall-quiet; `xhci.rs` `usb rw ok`; [m8_persist_iron.md](runbooks/m8_persist_iron.md); [loihda.md](loihda.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
