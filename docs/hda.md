...
| Summary | **Iron COM2 (UDisk, `fcac0cd` / run `34145652767`):** `linux virtio INTx reassert` then apk freeze `n=1345`. Eight `inject vec=0x30` is Linux PIC IRQ0; PIC 11 log missed 0x3b. PCI INTx now level on PIC 11 + IRQ11 unmask. Do not F11 `34145652767`. Host-tested; not ISO-INSTALL-OK. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Iron apk overlay still open; do not drop months. |
| Gates touched | none. Host `cargo test --no-default-features -- --test-threads=1` → 660 passed, 1 ignored. Level INTx retriggers after EOI until ISR read; Linux ICW2 0x38 unmasks IRQ 11. |
| Months Δ | 0.5→0.5 |
