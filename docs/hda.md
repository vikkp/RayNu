...
| **Deploy Linux ISO** | ~91% | FW launch-prepare; mock VMLAUNCH refused; real EDK2 + distro later |
...
| Guest UEFI firmware blob | PARTIAL (launch-prepare) | `prepare_ovmf_firmware_launch` after bind; mock VMLAUNCH refused |
...
|-------|-------|
| Commit | e5-fw-prep |
| Summary | P0-24 firmware launch-prepare after bind; mock VMLAUNCH refused. Iron P0-14 stays 2b795a0. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Prep ≠ VMLAUNCH ≠ installer. |
| Gates touched | `RAYNU-V-M7-E5-FW-PREP-OK` (host). Not Everest E5 / not `ISO-INSTALL-OK`. |
| Months Δ | 0.5→0.5 |
...
| H3 | No live guest UEFI CD | MED | Prep closed (P0-24); mock VMLAUNCH refused; `attach_cdrom_uefi` still stub; extract-boot is lab MVP only |
