...
|--------|------:|-------------------|
| **Overall product readiness** | **99%** | **held** — Everest **CLOSED**. M8.0 persist-first attach + nested File RAM + `MODE=keep` + DurableLun NVMe/USB I/O + **`MODE=lunkeep`/`usbkeep` TCG `keep=1`**. **Nested Alpine `MODE=full` CLOSED** on `raynuvsrv1` (`ce3d8a09`). **Iron 1–3 host-ready** (R640 census / Intel PCH scratchpad ≤64 / leftover named). Iron Cap `err=1`; Cap EFI `scratch=34/64`; Enum EFI HS U0; Slot DW1 `3473a0b9` `cmpl=0x40` was MaxSlots. Recover EFI `568e8696`: Toshiba 298 GiB `usb I/O ready` + leftover skipped; peek/guest IOERR (ISO stole LUN). Isolation `06ca0f95`: leftover Everest ISO `last_st=0x0` (not persist). DESC retry `6c278e85`: p11 Toshiba named + leftover skip + ISO `vdb` `last_st=0x0` — **enum+ISO isolation proven**; peek `usb_err=8` `cmpl=0xff` at `off=0x200`. BOT READ EFI `73dc4d2e`: hub skip lived, then `usb I/O fail err=8 bot=csw cmpl=0` (p14 hub clobber); leftover Everest — **not persist**. CSW `68e16633`: packed **p14**, not LUN-on-p10. Port EFI `96024edc` (still flashed): first boot SET_CONFIG `cmd=5 cmpl=0`; second boot SET_CONFIG held then `bot=cbw scsi=read cmpl=0` (CAPACITY ok; leftover IN event retired CBW). Leftover Everest `root=UUID=45213e85-…` — **not persist**. CBW EFI `cbd9bf47` leftover Everest `vda` 1.07 GiB `root=UUID=35bfa02f-…`; p11 GET_DESC `cmd=4 cmpl=0xff` (`bot=?`) — **not persist**. STATUS-TRB EFI `6ba076cc` leftover Everest: p11 `xhci bot … ep_out=2 ep_in=1` + `xhci setcfg p11 val=1` + CAPACITY then `usb I/O fail err=8 cmpl=0xff bot=cbw scsi=read`; SNP DHCP failed → analog/coexist skip → Phase B idle `SPA Start needs HTTP` — **not persist**. DHCP EFI `0780df21` leftover Everest: coexist `HOST-NIC-HTTP-OK` `10.99.99.145:8443` → SPA Start → `ISO-INSTALL-OK` → F7 `DISK-BOOT-OK` → login `root=UUID=1bc8b57d-…`; USB `bot=csw scsi=capacity cmpl=0xff`; `vda` 1.07 GiB leftover — **not persist**. Skip-INQUIRY COM2 `bot=cbw scsi=capacity cmpl=0xff`. First-cbw COM2 SET_CONFIG held then `bot=cbw scsi=capacity cmpl=0x13` (Reset Endpoint on Running). Maxlun COM2 GET_MAX_LUN `val=0` then INQUIRY CBW `cmpl=0xff`. Epst COM2 `epst ep0=1 out=1 in=1` + `botrst` then INQUIRY/CAPACITY lived and READ CBW `cmpl=0xff` leftover 1 GiB (not persist). Skipmaxlun COM2: `xhci maxlun p11 skip` + `epst ep0=1` then INQUIRY CBW still `cmpl=0xff` leftover 1 GiB — EP0-Stopped is falsified. Overlap COM2: `xhci firstcbw p11 overlap` then `bot=cbw scsi=capacity cmpl=0xff` leftover 1 GiB — INQUIRY overlap lived. Capoverlap COM2: SET_CONFIG then CONFIG_EP `cmd=5 cmpl=0` `err=3 bot=? scsi=?` leftover 1 GiB — BOT never started (packed `cmpl=0x30500` = CC=0). Toshiba `0480:a004` **was named** on p11; leftover command event retired CONFIG_EP then Disable Slot made the mapper print need-media. Cmdptr COM2: p10 Address Device timeout then p11 Enable Slot `cmd=2 cmpl=0xff` — Toshiba not named; Disable Slot wait skipped Command Ring Stopped. slotretry COM2: nop + p11 GET_DESC `cmd=4 cmpl=0xff` then p14 hub — Enable Slot lived. Descabort COM2 (`b0c2b678`): Toshiba named + 298 GiB `usb I/O ready` + leftover skipped + virtio keep=0; Alpine `vda` mixed `last_st=0x1`; `sfdisk` I/O error. ISO `vdb` `last_st=0x0`. Guestio COM2 (`f2c55be4`): Toshiba 298 GiB ready + virtio keep=0; Alpine `vda1`; disk 3/3 ISO 556/556; leftover apk stall dump during ISO mount. Stallquiet COM2: GET_DESC timeout + descabort keep-slot then `cmd=3 cmpl=0x13` leftover 1 GiB. rstdev/udiskkick COM2: Toshiba named + firstcbw then p14/p10 leftover 1 GiB (regression vs guestio I/O ready). Stopwalk COM2 (`90af2c5b`): Toshiba 298 GiB `usb I/O ready` + leftover skip + Alpine login; `vda last_st=0x1` / `sfdisk` I/O error (not persist). Nlb1 COM2 (`920f606b`): Toshiba 298 GiB ready + peek `usb_err=0` + nlb=1 512-byte oks then `scsi=sense` after Xfer timeout; `vda` no partition table (not persist). Sensebot COM2 (`c0a726de`): nlb=1 oks then `bot=csw scsi=read cmpl=0xff`; `vda1`; no `scsi=sense`. Cswsettle COM2 (`5c51c4ea`): leftover DRAM 1.07 GiB after p11 ADDR timeout (CSW queue never ran). Addrretry COM2 (`116dbb1f`): `xhci nopretry` + `xhci slotretry p11` then Toshiba `0480:a004` named; `xhci cswqueue p11`; `usb I/O ready bytes=320072933376`; leftover skip; peek `efi=????????` `gpt_err=2` `usb_err=8` (latched Xfer, not `read-fail`). **No** leftover 1.07 GiB. **No** p14. Guest Alpine `[vda] 298 GiB` `vda1` login; CSW-queued READs lived; sequential WRITE CBW `bot=cbw scsi=write cmpl=0xff` at backup-GPT `off=0x4a85d51200` then `sfdisk` I/O error. **Not persist.** Writequeue COM2 (`f32b9238`): WRITE overlap lived; first `wr=1` then `ISO-INSTALL-OK` on the 298 GiB Toshiba; `usb rw ok wr=1` every 64 I/Os mashed COM2. **Not persist.** USB `ISO-INSTALL-OK` ≠ leftover DRAM Everest. Do not Force Off mid-install. This EFI: quiet `usb rw ok` (`xhci_rw_ok_should_print`). Keep writequeue / CSW queue / nopretry / addrretry / Bot-only SENSE / nlb=1 / stopwalk. Iron okquiet COM2 (`fba9bc49`): Toshiba named then SET_CONFIG `cmd=6 cmpl=0xff` leftover DRAM 1.07 GiB `ISO-INSTALL-OK` → F7 `DISK-BOOT-OK` → `root=UUID=53bb2054-…` — leftover Everest ≠ persist. Iron setcfgretry COM2 (`ef7e93ec`): SET_CONFIG first-try `xhci setcfg p11 val=1` (no `xhci setcfgretry` line); Toshiba 298 GiB `usb I/O ready`; leftover skip; virtio `keep=0 (durable LUN usb)`; Alpine `[vda] 298 GiB` `vda1 vda2`; WRITE then USB `ISO-INSTALL-OK`; UART mash recovered; `Installation is complete. Please reboot.` → guest `reboot` `src=kbc n=1` → F7 `BOOTX64.EFI bytes=139264` / `image=DISK-BOOTX64` → GNU GRUB 2.12 2s→1s→0s → `RAYNU-V-RAYNU-F-DISK-BOOT-OK` → second Linux `root=UUID=6d549fd4-3907-4382-a3eb-750ee9d52616` on 298 GiB `vda` (625142448) → `EXT4-fs (vda2): mounted filesystem 6d549fd4-…` → `fsck` `vda2` 6377/19537920 + `vda1` 5 files → `login:` → `cat /proc/cmdline`. After login COM2 keeps `usb rw ok wr=1` every 4096 plus virtio MMIO overlay — not a hang. Guest F7 ≠ Force Off persist. **Not persist.** Do not Force Off to stop the scroll. USB `ISO-INSTALL-OK` ≠ leftover DRAM Everest. Nested QEMU ≠ R640. Not 100%: iron Force Off / TLS / console |
...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; descabort COM2 Toshiba 298 GiB ready + virtio keep=0 then vda last_st=0x1; guestio COM2 vda1 + stall dump during ISO mount; stallquiet COM2 GET_DESC timeout + `cmd=3 cmpl=0x13`; stopwalk COM2 Toshiba 298 GiB ready + Alpine login then vda last_st=0x1; nlb1 COM2 peek usb_err=0 + nlb=1 oks then scsi=sense; sensebot COM2 nlb=1 oks then bot=csw scsi=read cmpl=0xff (no scsi=sense); cswsettle COM2 leftover DRAM 1.07 GiB ISO-INSTALL-OK (not persist; CSW queue never ran); addrretry COM2 Toshiba 298 GiB ready + leftover skip + cswqueue + Alpine `[vda] 298 GiB` login then WRITE CBW `bot=cbw scsi=write cmpl=0xff` / `sfdisk` I/O error (not persist); writequeue COM2 WRITE lived + `ISO-INSTALL-OK` on 298 GiB USB LUN then `usb rw ok` flood (not persist); okquiet COM2 Toshiba named then SET_CONFIG `cmd=6 cmpl=0xff` leftover 1.07 GiB Everest (not persist); setcfgretry COM2 SET_CONFIG first-try + 298 GiB USB `ISO-INSTALL-OK` → `Installation is complete` → F7 `DISK-BOOTX64` → `DISK-BOOT-OK` + second Linux `root=UUID=6d549fd4-…` on 298 GiB `vda` (not persist; guest F7 ≠ Force Off persist; post-login wr=1 overlay is not a hang; do not Force Off to stop the scroll); iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (nested-OK closed; p11 GET_DESC timeout then p14 hub; Force Off open)
...
| P0-64 | **M8** operator product hardening | — | IN PROGRESS | Everest closed | [ADR-018](adr/ADR-018.md) / [m8_plan.md](m8_plan.md) / [m8_persist_nested.md](runbooks/m8_persist_nested.md) / [m8_persist_iron.md](runbooks/m8_persist_iron.md). **M8.0 nested Alpine `MODE=full` CLOSED** on `raynuvsrv1` (`ce3d8a09`). Iron 1–3 host-ready (census / PCH scratchpad ≤64 / leftover named). Iron Cap `err=1` then `scratch=34/64`. Enum HS U0. Slot DW1 `3473a0b9` Parameter Error gone; `cmpl=0x40` was MaxSlots. Recover `568e8696`: Toshiba 298 GiB named, leftover skipped, peek/guest IOERR on `vda`+`vdb`. Isolation `06ca0f95`: leftover Everest ISO `last_st=0x0` (not persist). DESC retry `6c278e85`: Toshiba named + leftover skip + ISO `last_st=0x0` (enum+ISO proven); peek `usb_err=8` `cmpl=0xff`; guest `vda` IOERR. BOT READ `73dc4d2e` leftover Everest `bot=csw` `cmpl=0` (hub clobber) — not persist. CSW `68e16633` leftover Everest `scsi=tur bot=cbw` packed p14 (not LUN-on-p10). Port `96024edc`: p11 `ep_out=2 ep_in=1` then SET_CONFIG `cmd=5 cmpl=0 err=8`; second COM2 SET_CONFIG held then `bot=cbw scsi=read cmpl=0` (CAPACITY ok). Leftover Everest — not persist. CBW `cbd9bf47` leftover Everest `root=UUID=35bfa02f-…` — p11 GET_DESC `cmd=4 cmpl=0xff` (`bot=?`) not persist. STATUS-TRB `6ba076cc` leftover Everest: SET_CONFIG + CAPACITY then `bot=cbw scsi=read cmpl=0xff`; SNP DHCP failed → Phase B idle `SPA Start needs HTTP` — not persist. DHCP `0780df21` leftover Everest: coexist HTTP-OK → SPA → `ISO-INSTALL-OK` → `DISK-BOOT-OK` → login `root=UUID=1bc8b57d-…`; USB `bot=csw scsi=capacity cmpl=0xff` — not persist. Skip-INQUIRY COM2 `bot=cbw scsi=capacity cmpl=0xff`. First-cbw COM2 `bot=cbw scsi=capacity cmpl=0x13` (Reset Endpoint on Running). Maxlun COM2 INQUIRY CBW `cmpl=0xff`. Epst COM2 `epst out=1 in=1` + BOT reset; INQUIRY/CAPACITY lived; READ CBW `cmpl=0xff` leftover 1 GiB. Overlap COM2: `xhci firstcbw overlap` then CAPACITY CBW `cmpl=0xff` leftover 1 GiB. INQUIRY overlap lived. Capoverlap COM2: SET_CONFIG then CONFIG_EP `cmd=5 cmpl=0` leftover 1 GiB. Toshiba was named on p11 then Disable Slot. Cmdptr COM2: p11 Enable Slot `cmd=2 cmpl=0xff` — not named. slotretry COM2: nop + p11 GET_DESC `cmd=4 cmpl=0xff` then p14 hub — Enable Slot lived. Descabort COM2 (`b0c2b678`): Toshiba named + 298 GiB `usb I/O ready` + leftover skipped + virtio keep=0; Alpine `vda` mixed `last_st=0x1`; `sfdisk` I/O error. ISO `vdb` `last_st=0x0`. Guestio COM2 (`f2c55be4`): Toshiba 298 GiB ready + virtio keep=0; Alpine `vda1`; disk 3/3 ISO 556/556; leftover apk stall dump during ISO mount. Stallquiet COM2: GET_DESC timeout + descabort then `cmd=3 cmpl=0x13` leftover 1 GiB. rstdev/udiskkick COM2: Toshiba named + firstcbw then p14/p10 leftover 1 GiB (regression vs guestio I/O ready). Stopwalk COM2 (`90af2c5b`): Toshiba 298 GiB `usb I/O ready` + leftover skip + Alpine login; `vda last_st=0x1` / `sfdisk` I/O error (not persist). Nlb1 COM2 (`920f606b`): Toshiba 298 GiB ready + peek `usb_err=0` + nlb=1 512-byte oks then `scsi=sense` after Xfer timeout; `vda` no partition table (not persist). Sensebot COM2 (`c0a726de`): nlb=1 oks then `bot=csw scsi=read cmpl=0xff`; `vda1`; no `scsi=sense`. Cswsettle COM2 (`5c51c4ea`): leftover DRAM 1.07 GiB after p11 ADDR timeout (CSW queue never ran). Addrretry COM2 (`116dbb1f`): `xhci nopretry` + `xhci slotretry p11` then Toshiba `0480:a004` named; `xhci cswqueue p11`; `usb I/O ready bytes=320072933376`; leftover skip; peek `efi=????????` `gpt_err=2` `usb_err=8` (latched Xfer, not `read-fail`). **No** leftover 1.07 GiB. **No** p14. Guest Alpine `[vda] 298 GiB` `vda1` login; CSW-queued READs lived; sequential WRITE CBW `bot=cbw scsi=write cmpl=0xff` at backup-GPT `off=0x4a85d51200` then `sfdisk` I/O error. **Not persist.** Writequeue COM2 (`f32b9238`): WRITE overlap lived; first `wr=1` then `ISO-INSTALL-OK` on the 298 GiB Toshiba; `usb rw ok` flood mashed COM2. **Not persist.** This EFI: quiet `usb rw ok` (`xhci_rw_ok_should_print`). Keep writequeue / CSW queue / nopretry / addrretry / Bot-only SENSE / nlb=1 / stopwalk. Do not Force Off mid-install. Keep sensebot/nlb1/stopwalk/rstdev/stallquiet/FIRST_READ_SPINS/descabort/nop/slotretry/p11first/cmdptr. Nested QEMU ≠ R640. Iron COM2 `usb I/O ready` without peek `read-fail` + `RAYNU-V-M8-DISK-PERSIST-OK` open. Then TLS, auth, console UI, ISO upload, UEFI catalog, Windows later. |
...
| Summary | **Iron leftover skipped, not persist.** Setcfgretry COM2 (`ef7e93ec`): SET_CONFIG first-try `xhci setcfg p11 val=1` (no `xhci setcfgretry` line); Toshiba 298 GiB `usb I/O ready`; leftover skip; Alpine `[vda] 298 GiB`; WRITE then USB `ISO-INSTALL-OK` → `Installation is complete` → F7 `DISK-BOOTX64` → `DISK-BOOT-OK` + second Linux `root=UUID=6d549fd4-…` on 298 GiB `vda`. Months **0.0 held**. Overall **99 held**. |
| Everest impact | months **0.0 held**; overall **99 held**; ETA 2026-09 held. Not 100%. No persist gate. LOIHDA persist **70% held** (A2 still open). Guest F7 on USB LUN is not persist. Post-login `usb rw ok wr=1` overlay is not a hang. Do not Force Off to stop the scroll. |
| Gates touched | Lived setcfgretry COM2 leftover skip + USB install complete + F7 `DISK-BOOT-OK` + `root=UUID=6d549fd4`; [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
...
| 2026-09-18 | m8-usb-bot-setcfgretry | 0.0 | 99 | **Iron leftover skipped, not persist:** setcfgretry COM2 (`ef7e93ec`) SET_CONFIG first-try `xhci setcfg p11 val=1` (retry unused); Toshiba 298 GiB ready + leftover skip + Alpine `[vda] 298 GiB` WRITE + USB `ISO-INSTALL-OK` → `Installation is complete` → F7 `DISK-BOOTX64` → `DISK-BOOT-OK` + second Linux `root=UUID=6d549fd4-…` on 298 GiB `vda`. Post-login `usb rw ok wr=1` overlay is not a hang. Guest F7 ≠ persist. Do not Force Off to stop the scroll. Keep writequeue / okquiet / setcfgretry. LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. months 0.0 held; overall 99 held |
| 2026-09-18 | m8-usb-bot-okquiet | 0.0 | 99 | **Iron leftover skipped, not persist:** writequeue COM2 Toshiba 298 GiB + WRITE lived + `ISO-INSTALL-OK` on USB LUN then `usb rw ok` flood mashed COM2. This EFI: quiet oks (`xhci_rw_ok_should_print`). Keep writequeue. LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. USB `ISO-INSTALL-OK` ≠ persist. months 0.0 held; overall 99 held |
| 2026-09-17 | m8-usb-bot-writequeue | 0.0 | 99 | **Iron leftover skipped, not persist:** addrretry COM2 guest Toshiba 298 GiB + Alpine `[vda] 298 GiB` login then WRITE CBW `bot=cbw scsi=write cmpl=0xff` / `sfdisk` I/O error. This EFI: `xhci writequeue` + `xhci writesettle`. Keep CSW-queued READ. LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-17 | m8-usb-bot-cswsettle | 0.0 | 99 | **Iron leftover skipped, not persist:** sensebot COM2 Toshiba 298 GiB ready + nlb=1 oks then `bot=csw scsi=read cmpl=0xff`; `vda1`; ISO vdb; RCU/soft lockup. No `scsi=sense`. This EFI: queue CSW IN with DATA + settle between nlb=1 chunks. LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-17 | m8-usb-bot-sensebot | 0.0 | 99 | **Iron leftover skipped, not persist:** nlb1 COM2 Toshiba 298 GiB ready + peek `usb_err=0` + nlb=1 512-byte oks then `scsi=sense` after Xfer timeout; `vda` no partition table. This EFI: REQUEST SENSE only on Bot, never on Xfer timeout. LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-17 | m8-usb-bot-nlb1 | 0.0 | 99 | **Iron leftover skipped, not persist:** stopwalk COM2 Toshiba 298 GiB ready + Alpine login; `vda last_st=0x1`; `sfdisk` I/O error. This EFI: nlb=1 BOT + REQUEST SENSE after CSW fail. LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-17 | m8-usb-bot-stopwalk | 0.0 | 99 | **Iron leftover, not persist:** rstdev/udiskkick COM2 Toshiba named p11 + firstcbw then p14/p10 leftover 1 GiB — regression vs guestio I/O ready. This EFI: `xhci stopwalk` keep-slot + BOT retry. LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
...
| 2026-09-16 | m8-usb-bot-guestio | 0.0 | 99 | **Iron leftover skipped, not persist:** descabort COM2 Toshiba named + 298 GiB ready + virtio keep=0; Alpine vda last_st=0x1; sfdisk I/O error. ISO vdb last_st=0x0. This EFI: guest BOT FIRST_READ_SPINS + lun rw fail serial. LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-16 | m8-usb-bot-descabort | 0.0 | 99 | **Iron leftover, not persist:** slotretry COM2 nop + p11 EP0 GET_DESC timeout then p14 hub. Enable Slot lived. This EFI: keep-slot descabort; no Disable Slot; no p14 walk. LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-16 | m8-usb-bot-slotretry | 0.0 | 99 | **Iron leftover, not persist:** p11first COM2 p11 Enable Slot `cmd=2 cmpl=0xff` then p14 hub. Toshiba not named. This EFI: No-Op prime + Enable Slot ADDR_SPINS retry (`xhci nop` / `xhci slotretry`). LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-16 | m8-usb-bot-p11first | 0.0 | 99 | **Iron leftover, not persist:** cmdptr COM2 p10 ADDR `cmd=3 cmpl=0xff` then p11 Enable Slot `cmd=2 cmpl=0xff`. Toshiba not named. This EFI: p11-first walk + abort ADDR_SPINS, no Disable Slot wait. LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-16 | m8-usb-bot-cmdptr | 0.0 | 99 | **Iron leftover, not persist:** capoverlap COM2 named Toshiba p11 then CONFIG_EP `cmd=5 cmpl=0` `bot=?`. Not "device missing" — leftover cmd event + Disable Slot. This EFI: match Command TRB Pointer (`xhci cmdptr`). LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-16 | m8-usb-bot-cfg-retry | 0.0 | 99 | **Iron leftover, not persist:** capoverlap COM2 SET_CONFIG then CONFIG_EP `cmd=5 cmpl=0` `err=3 bot=?` leftover 1 GiB. BOT never started. This EFI: drain + skip CC=0 + CONFIG_EP retry (`xhci config` / `xhci cfgretry`). LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-16 | m8-usb-bot-cap-overlap | 0.0 | 99 | **Iron leftover, not persist:** overlap COM2 `xhci firstcbw overlap` then CAPACITY CBW `cmpl=0xff` leftover 1 GiB. INQUIRY overlap lived. This EFI: overlap CAPACITY + READ IN (`xhci capoverlap`). LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-16 | m8-usb-bot-inq-overlap | 0.0 | 99 | **Iron leftover, not persist:** skipmaxlun COM2 `xhci maxlun skip` + `epst ep0=1` then INQUIRY CBW `cmpl=0xff` leftover 1 GiB. EP0-Stopped is falsified. This EFI: overlap first INQUIRY CBW+DATA IN. LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-16 | m8-usb-bot-skipmaxlun | 0.0 | 99 | **Iron leftover, not persist:** norearm COM2 `xhci firstcbw norearm` then `epst ep0=3` after GET_MAX_LUN DATA-then-Stop; INQUIRY CBW `cmpl=0xff` leftover 1 GiB. This EFI: skip optional GET_MAX_LUN (`xhci maxlun pN skip`) so EP0 stays Running. LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-16 | m8-usb-bot-norearm | 0.0 | 99 | **Iron leftover, not persist:** firstcbw COM2 `xhci firstcbw` then INQUIRY CBW `cmpl=0xff` leftover 1 GiB. Rearm before unused CBW did not help. This EFI: `norearm` + long wait. LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-16 | m8-usb-bot-firstcbw | 0.0 | 99 | **Iron leftover, not persist:** firstread COM2 `epst`/`botrst`/`maxlun` then INQUIRY CBW `cmpl=0xff` leftover 1 GiB. No `xhci firstread`. This EFI: `prepare_first_cbw` before INQUIRY. LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-16 | m8-usb-bot-firstread | 0.0 | 99 | **Iron leftover, not persist:** epst COM2 `epst out=1 in=1` + BOT reset; INQUIRY/CAPACITY lived; READ CBW `cmpl=0xff` leftover 1 GiB. CONFIG_EP Running. This EFI: prepare_first_read + longer first-READ wait. LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-16 | m8-usb-bot-epst | 0.0 | 99 | **Iron leftover, not persist:** maxlun COM2 Toshiba + GET_MAX_LUN `val=0` then INQUIRY CBW `cmpl=0xff` leftover 1 GiB. Enum/eval/setcfg/maxlun lived. This EFI: CONFIG_EP copies Output Slot+A1; `xhci epst`; MSC BOT reset + Clear Halt. LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-15 | m8-usb-bot-maxlun | 0.0 | 99 | **Iron leftover, not persist:** ep0-eval COM2 Toshiba + eval + SET_CONFIG then CAPACITY CBW `err=8 cmpl=0`. Enum lived. This EFI: GET_MAX_LUN + required INQUIRY + longer settle. LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
...
| 2026-09-15 | m8-usb-ep0-status | 0.0 | 99 | **Iron leftover, not persist:** COM2 `cbd9bf47` leftover Everest `root=UUID=35bfa02f-…`. p11 GET_DESC `cmd=4 cmpl=0xff` (`bot=?`). This EFI: STATUS TRB ptr + `DESC_SPINS` + no EP0 reset on timeout. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-15 | m8-usb-bot-read-cbw | 0.0 | 99 | **Iron leftover, not persist:** Port `96024edc` second COM2 leftover Everest. SET_CONFIG held; `bot=cbw scsi=read cmpl=0` after CAPACITY (leftover IN event retired CBW). This EFI: slot+DCI Transfer Event match; IN bounce; no Reset EP on CBW fail; settle after CAPACITY. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-15 | m8-usb-bot-setcfg | 0.0 | 99 | **Iron leftover, not persist:** Port `96024edc` leftover Everest. p11 Toshiba BOT parse `ep_out=2 ep_in=1` then SET_CONFIG `cmd=5 cmpl=0 err=8`; fail `cmpl=0x0e801a40 bot=?` is p14 reset_diag. This EFI: SET_CONFIG before CONFIG_EP; later-port reset does not clobber Enum/Xfer; `xhci setcfg` / cmd `6` / `hcc1=`. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-14 | m8-usb-bot-port | 0.0 | 99 | **Iron leftover, not persist:** CSW `68e16633` leftover Everest. Packed `portsc=0x…0e00000e03` is p14 hub clobber, not LUN-on-p10. HCRST already ran (scan Polling). Config parse already existed. This EFI: always port-reset CCS; hub skip never stamps; no TUR; COM2 `xhci hcrst` + parsed BOT eps. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-14 | m8-usb-bot-csw | 0.0 | 99 | **Iron leftover, not persist:** BOT READ `73dc4d2e` leftover Everest (`ISO-INSTALL-OK` → F7 `DISK-BOOT-OK` → `login:`). `usb I/O fail err=8 bot=csw cmpl=0` was p14 hub clobber; START STOP + ISP + auto EP-reset regressed vs `6c278e85` `usb I/O ready lba=512`. This EFI: drop START STOP / ISP-on-512 / auto EP-reset; hub skip keeps Xfer cmpl; `scsi=` + `recover_pipes` on READ retry; keep `BULK_SPINS`. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-14 | m8-usb-bot-read | 0.0 | 99 | **Iron leftover, not persist:** DESC retry `6c278e85` Toshiba `0480:a004` named + leftover skip + ISO `last_st=0x0` (enum+ISO proven). Peek `usb_err=8` `cmpl=0xff` at `off=0x200`; guest `vda` IOERR; `sfdisk` fail. This EFI: `BULK_SPINS` + START STOP + READ probe + EP reset + `bot=` + stall frozen-n. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-14 | m8-xhci-desc-retry | 0.0 | 99 | **Iron leftover, not persist:** isolation `06ca0f95` leftover Everest ISO `last_st=0x0` (not persist). USB LUN missed: p11 GET_DESC `cmpl=0`, p14 hub `err=4`. GET_DESC retry + hub skip (`err=9`) so DESC fail is not hidden. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-14 | m8-usb-bot-io | 0.0 | 99 | **Iron leftover, not persist:** recover `568e8696` Toshiba 298 GiB `usb I/O ready` + leftover skip (enum proven). Peek `read-fail usb_err=3`; Alpine `vda`+`vdb` `last_st=0x1` — ISO virtio stole USB BOT. This EFI: ISO `allow_lun=false`, BOT TUR, 512e. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-14 | m8-xhci-enum-recover | 0.0 | 99 | **Iron leftover, not persist:** Slot DW1 `3473a0b9` HS U0 then `cmpl=0x40` (MaxSlots, not CC 64). Address Device never posted; p11/p14 Enable Slot starved on a busy ring. Timeout `0xff` + CRCR abort + vid/did + UAS detect. Toshiba unused. Do not `setup-disk` on leftover. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
| 2026-09-14 | m8-xhci-slot-dw1 | 0.0 | 99 | **Iron leftover, not persist:** Enum EFI `c6bdd671` HS U0 `sc=0xe03` then Address Device `cmpl=0x11` (Slot DW1 Number of Ports = port). Root Hub Port 23:16 only. Toshiba unused. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. Do not Force Off leftover guest. months 0.0 held; overall 99 held |
| 2026-09-14 | m8-xhci-enum | 0.0 | 99 | **Iron leftover, not persist:** Cap EFI `67db8a68` `scratch=34/64` then Enum `err=3` `cmpl=0x11` (Polling p10/p11/p14 `0x000206e1`). WPR + wait U0/speed + `xhci enum pN` diag. Toshiba unused. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. Do not Force Off leftover guest. months 0.0 held; overall 99 held |
| 2026-09-14 | m8-xhci-cap-scratch | 0.0 | 99 | **Iron leftover, not persist:** COM2 `ac3b92cd` Everest loop on leftover DRAM (`vda` 1.07 GiB); Lewisburg `8086:a1af` Cap `err=1` before PORTSC; Toshiba unused. Scratchpad 16→64 + `xhci cap caplen=` snapshot. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. Do not Force Off leftover guest. months 0.0 held; overall 99 held |
...
| 2026-09-11 | m8-disk-persist-usb-ccs | 0.0 | 99 | **M8.0 USB BOT TCG:** `MODE=usb` `durable LUN usb I/O ready bytes=1073741824`. qemu-xhci `p3=0`; PP then CCS; Address Device Slot DW1 port at 31:24 and 23:16 (QEMU 8.2). QEMU USB ≠ R640. Not nested-OK. Not iron persist OK. Never `ISO-INSTALL-OK`. Do not F11. months 0.0 held; overall 99 held |
...
Next move:     **M8.0 iron** COM2 `usb I/O ready` without `read-fail` + Alpine `vda` 298 GiB, then Force Off / `RAYNU-V-M8-DISK-PERSIST-OK`. Firstcbw COM2 INQUIRY CBW leftover DRAM is **not** persist. Nested-OK closed on `raynuvsrv1` `ce3d8a09`.
