...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; capoverlap COM2 SET_CONFIG then CONFIG_EP `cmd=5 cmpl=0` leftover DRAM; this EFI drain+skip CC=0+retry; iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (nested-OK closed; CONFIG_EP CC=0 leftover; Force Off open)
...
| Summary | **Iron leftover, not persist.** Capoverlap COM2: SET_CONFIG then CONFIG_EP `cmd=5 cmpl=0` `err=3 bot=? scsi=?` leftover 1 GiB. BOT never started. This EFI: drain + skip CC=0 + CONFIG_EP retry. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | `xhci.rs` CONFIG_EP drain/skip-CC=0/retry; [m8_persist_iron.md](runbooks/m8_persist_iron.md); [loihda.md](loihda.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
