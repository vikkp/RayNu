...
| **Deploy Linux ISO** | ~99% | Real ESP OVMF.fd required; live E4 SHELL EPT not written; 4 MiB fixture not shipped OVMF.fd; VMLAUNCH insn not issued; distro later |
...
| Guest UEFI firmware blob | PARTIAL (live-fd) | Real ESP OVMF.fd required; live ESP bytes still absent; live E4 SHELL EPT not written; 4 MiB fixture not shipped OVMF.fd; VMLAUNCH insn not issued |
...
|-------|-------|
| Commit | e5-live-fd |
| Summary | P0-39 live-ESP FD require after live-bytes probe; live E4 SHELL EPT not written; 4 MiB fixture not shipped OVMF.fd; VMLAUNCH insn not issued. Iron P0-14 stays 2b795a0. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. FD required ≠ live ESP bytes present ≠ live E4 SHELL EPT write ≠ shipped OVMF.fd ≠ VMLAUNCH insn ≠ installer. |
| Gates touched | `RAYNU-V-M7-E5-LIVE-FD-OK` (host). Not Everest E5 / not `ISO-INSTALL-OK`. |
| Months Δ | 0.5→0.5 |
...
| H3 | No live guest UEFI CD | MED | Live-ESP FD require closed (P0-39); live ESP bytes still absent; live E4 SHELL EPT not written; 4 MiB fixture not shipped OVMF.fd; VMLAUNCH insn not issued; `attach_cdrom_uefi` still stub; extract-boot is lab MVP only |
