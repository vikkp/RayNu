...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; DESC retry Toshiba named; BOT READ timeout open; iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (nested-OK closed; iron Toshiba named; BOT READ timeout; Force Off open)
...
| Field | Value |
|-------|-------|
| Commit | m8-usb-bot-read |
| Summary | **Iron leftover, not persist.** DESC retry COM2 `6c278e85`: p11 Toshiba `0480:a004` `usb I/O ready … lba=512`, leftover skip, ISO `vdb` `last_st=0x0` `Mounting boot media: ok.` — **enum+ISO isolation proven.** Peek `read-fail usb_err=8 cmpl=0xff` at `off=0x200`; guest `vda` IOERR; `sfdisk` fail; stall overlay looped `PIT paced n=6402`. This EFI: `BULK_SPINS` (100M) + START STOP UNIT + native READ probe before `usb I/O ready` + bulk EP reset on timeout + COM2 `bot=` + stall dump frozen-n cap. Months **0.0 held**. Overall **99 held**. |
| Everest impact | months **0.0 held**; overall **99 held**; ETA 2026-09 held. Not 100%. No persist gate. |
| Gates touched | `usb_bot.rs` START STOP + READ probe; `xhci.rs` `BULK_SPINS` + EP reset; stall frozen-n; [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. |
