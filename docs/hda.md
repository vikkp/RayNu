...
| Summary | **Iron leftover skipped, not persist.** Addrretry COM2 `xhci nopretry` + `xhci slotretry p11` then Toshiba `0480:a004` named; `xhci cswqueue`; `usb I/O ready` 298 GiB; leftover skip; peek `efi=????????` `gpt_err=2` `usb_err=8` (latched Xfer). Paste ends `M1-EBS-OK`. Guest Alpine not in this log. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | Lived COM2 on `xhci nopretry` / `xhci slotretry`; [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
...
| 2026-09-17 | m8-usb-bot-addrretry | 0.0 | 99 | **Iron leftover skipped, not persist:** addrretry COM2 `xhci nopretry` + `xhci slotretry p11` then Toshiba `0480:a004` named; `xhci cswqueue`; `usb I/O ready` 298 GiB; leftover skip; peek `usb_err=8` latched (not `read-fail`). Paste ends `M1-EBS-OK`. Guest Alpine not in this log. LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
