...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; p11first COM2 p11 Enable Slot timeout then p14 hub; this EFI No-Op + slotretry; iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (nested-OK closed; p11 Enable Slot timeout then p14 hub; Force Off open)
...
| Summary | **Iron leftover, not persist.** p11first COM2: p11 Enable Slot `cmd=2 cmpl=0xff` then p14 hub. Toshiba not named. This EFI: No-Op prime + Enable Slot ADDR_SPINS retry. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | `xhci.rs` nop + slotretry; [m8_persist_iron.md](runbooks/m8_persist_iron.md); [loihda.md](loihda.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
