...
| Gates touched | RayNu-F F0 DONE, F1 DONE (host), F2a DONE (host: PE32+ loader + test app + launch plan, `RAYNU-V-RAYNU-F-LOADER-OK`), F2b DONE on nested VT-x `raynuvsrv1` (serial `RAYNU-V-RAYNU-F-CONOUT-OK`, exits=2; first guest execution of our own tables; not iron). F3 **host-proven** (`RAYNU-V-RAYNU-F-SERVICES-OK`: memory services + owned TSC-calibrated firmware clock; no guest IDT/PIT/LAPIC needed), awaiting one nested run for `TIMER-OK`/`MEM-OK`. F4 **host-proven** (`RAYNU-V-RAYNU-F-BLOCKIO-GATE-OK`: protocol DB + BlockIo on CD/virtio-blk). F5 **host-proven** (FAT + SimpleFileSystem + LoadImage/StartImage; launcher boots the ISO's real BOOTX64.EFI). F6 iron `ISO-INSTALL-OK` is the only remaining stage. Stage 46 OVMF forcing family closed (ADR-016). Not Everest E5. |
| Months Δ | 0.5→0.5 |
