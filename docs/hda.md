...
| **Deploy Linux ISO** | ~86% | guest FW stub load closed; real OVMF / VMLAUNCH + distro installer later |
...
**Status: NEAR · ~86% · ~0.25–0.5 months residual (real distro installer)**
...
| Guest UEFI firmware blob | PARTIAL (stub loaded) | `load_guest_firmware` / `RAYNUFD` identity-lazy stub; not OVMF; not VMLAUNCH |
...
| Field | Value |
|-------|-------|
| Commit | e5-guest-fw-load |
| Summary | P0-19 guest FW stub payload identity-lazy load. Iron P0-14 stays 2b795a0. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Stub load ≠ OVMF ≠ installer. |
| Gates touched | `RAYNU-V-M7-E5-GUEST-FW-LOAD-OK` (host). Not Everest E5 / not `ISO-INSTALL-OK`. |
| Months Δ | 0.5→0.5 |
...
| H3 | No live guest UEFI CD | MED | Stub load closed (P0-19); `attach_cdrom_uefi` still stub; not OVMF; extract-boot is lab MVP only |
