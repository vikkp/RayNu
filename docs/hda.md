...
| Guest UEFI firmware blob | PARTIAL (RayNu-F nested F7 reboot-to-disk; iron E5 open) | Iron UDisk `20e8b70` / run `34076175624`: same freeze as `63c7e05` — `linux PIT once after DRIVER_OK`, boot media ok (~33s), `Installing packages` last virtio `n=1345`. Cmdline is `idle=poll` (never HLT); PIT-once is consumed so UART wins until the next virtio kick. This pin holds PIT until `login:` and raises PIT on overlay resume. Do not F11 `34076175624`. Prior: `63c7e05` / `34074349118` (HLT-PIT pin, same n=1345); `46fd345` / `34069352671` stalled at n=1281 with no PIT-once; `55a3602` reached `EBS-OK` then cap=1 killed Linux WRMSR; `088ab25` sat in OVMF CpuSleep. Nested F7 reboot-to-disk remains `fe4785a` (not iron / not Everest E5). Operator: [r640_f7_iso_iron.md](runbooks/r640_f7_iso_iron.md). |
...
| M+4 | 2026-09 | TLS/console + distro installer | remaining Everest | **ETA** (iron `20e8b70` / `34076175624` froze at apk `n=1345` under `idle=poll`; PIT hold until login pin; `ISO-INSTALL-OK` open) |
...
| Summary | **Iron COM2 (UDisk, `20e8b70` / run `34076175624`):** same freeze as `63c7e05` — `linux PIT once after DRIVER_OK`, boot media ok, `Installing packages` then last virtio `n=1345`. Cmdline is `idle=poll` (never HLT); PIT-once is consumed so UART wins until the next virtio kick. Fix: hold PIT over UART until getty `login:` (`prefer_pit_hold`, not consumed) and raise PIT on every overlay resume. Do not F11 `34076175624`. Host-tested; not ISO-INSTALL-OK. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Iron apk overlay still open; do not drop months. |
| Gates touched | none. Host test `product_iso_linux_pit_hold_until_login_not_consumed`. |
| Months Δ | 0.5→0.5 |
