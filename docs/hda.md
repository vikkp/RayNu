...
| Summary | **Iron COM2 (UDisk, `896424f` / run `34131274237`):** MADT + `linux virtio DRIVER_OK` live; then `virtblk_probe` `iowrite8`/`handle_softirqs` IRQ0 storm (`soft lockup`) then I/O `reason=0x1e` hold. Pace overlay resume/UART PIT ~1 ms TSC. Do not F11 `34131274237`. Host-tested; not ISO-INSTALL-OK. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Iron virtio probe / apk overlay still open; do not drop months. |
| Gates touched | none. Host test: resume PIT not due inside `LINUX_PIT_RESUME_MIN_TSC`. |
| Months Δ | 0.5→0.5 |
...
| 2026-09-07 | pit-during-apk | 0.5 | 95 | Iron UDisk `896424f` / run `34131274237`: MADT+DRIVER_OK live; virtblk_probe iowrite8 IRQ0 storm then I/O reason=0x1e hold; pace overlay resume PIT ~1ms TSC; do not F11 34131274237; not ISO-INSTALL-OK; iron P0-14 stays 2b795a0 |
