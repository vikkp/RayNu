...
| Guest UEFI firmware blob | PARTIAL (RayNu-F nested F7 reboot-to-disk; iron E5 open) | Iron UDisk `46fd345` / run `34069352671` booted Linux 6.12.13-lts on RayNu-F (`efi: EFI v2.10 by RayNu-F`) through `Mounting boot media: ok` then stalled at `Installing packages` (last virtio MMIO n=1281) because UART beat PIT after DRIVER_OK. PIT-once after DRIVER_OK is this pin; do not F11 `34069352671`. Prior: `55a3602` reached `EBS-OK` then cap=1 killed Linux WRMSR; `088ab25` sat in OVMF CpuSleep. ADR-016/017: nested `fe4785a` alpine-extended `setup-disk -m sys` → F7 relaunch → `RAYNU-V-RAYNU-F-DISK-BOOT-OK` → second Linux `root=UUID=698a922a-3a7d-45ea-9da3-2b11403af82a` (ext4 `/dev/vda2`, FAT ESP `/dev/vda1`); nested `3492ebc` had GRUB rescue `disk `,gpt2' not found`; re-run `088ab25` shows the reset path (`src=kbc`, i8042 `0xFE`); not iron / not Everest E5. 4 GB front-USB2 UDisk unblocks alpine-extended retain (977.5 MiB Micro cannot hold ~994 MiB). Operator: [r640_f7_iso_iron.md](runbooks/r640_f7_iso_iron.md). Evidence: [fe4785a](evidence/nested/2026-09-05-fe4785a-f7-reboot-to-disk.md), [088ab25](evidence/nested/2026-09-05-088ab25-f7-reset-src-kbc.md) |
...
| M+4 | 2026-09 | TLS/console + distro installer | remaining Everest | **ETA** (iron `46fd345` reached apk overlay then UART starved PIT; PIT-during-apk pin; `ISO-INSTALL-OK` open) |
...
| Summary | **Iron COM2 (UDisk, `46fd345` / run `34069352671`): Linux 6.12.13-lts on RayNu-F** reached `Installing packages to root filesystem...` then stalled (last virtio MMIO `n=1281`). Cap restore worked; after both virtio DRIVER_OK, UART beat PIT so Alpine printk THRE latched and `idle=poll`/`sleep` never saw jiffies. Fix: `guest_uefi_linux_prefer_pit_during_apk` arms PIT-once on virtio MMIO + VMX preempt after DRIVER_OK; HLT keeps UART first for `login:` / auto-answer. DRIVER_OK line is nowait; virtio MMIO heartbeat every 64. Do not F11 `34069352671`. Host-tested; not ISO-INSTALL-OK. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Iron reached apk overlay then a self-inflicted jiffies freeze. Nested-only evidence must not drop months. |
| Gates touched | none. Host tests `product_iso_linux_pit_once_after_driver_ok_beats_uart_then_uart_wins` + residual/heartbeat helpers. |
| Months Δ | 0.5→0.5 |
