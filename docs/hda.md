...
|--------|------:|-------------------|
| **Overall product readiness** | **99%** | **held** — Everest **CLOSED**. M8.0 backend chosen + host round-trip (`RAYNU-V-M8-DISK-PERSIST-HOST-OK`). Not 100%: iron persist / TLS / console |
...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (host round-trip exists) |
...
2026-10 ░░░░░░░░  M8.0 persist (host round-trip; iron COM2 open)
...
| P0-64 | **M8** operator product hardening | — | IN PROGRESS | Everest closed | [ADR-018](adr/ADR-018.md) / [m8_plan.md](m8_plan.md). **M8.0** backend chosen: `File` nested / `DurableLun` iron (not PERC) / leftover DRAM fallback. Host round-trip `RAYNU-V-M8-DISK-PERSIST-HOST-OK`. Iron `RAYNU-V-M8-DISK-PERSIST-OK` open. Then TLS, auth, console UI, ISO upload, UEFI catalog, Windows later. |
...
| Field | Value |
|-------|-------|
| Commit | m8-disk-persist-host |
| Summary | **M8.0 backend chosen + host round-trip.** virtio-blk HPA is durable (`File` nested / `DurableLun` iron USB-or-NVMe, not PERC Ubuntu); leftover DRAM is the fallback. Iron marker `RAYNU-V-M8-DISK-PERSIST-OK`. Host marker `RAYNU-V-M8-DISK-PERSIST-HOST-OK` (GPT+ESP+ext4 after HV reboot). Never `ISO-INSTALL-OK`. Nested + iron still open. |
| Everest impact | months **0.0 held**; overall **99 held**; ETA 2026-09 held. Not 100%. |
| Gates touched | `mgmt/disk_persist.rs` + `mgmt/m8_disk_persist_gate.rs`. `./tools/sync-hda-site.sh --check`. |
...
| H10 | Leftover-DRAM disk dies on **HV** reboot | MED | **M8.0** first gate ([m8_plan.md](m8_plan.md)): backend chosen (file nested / durable LUN iron / leftover fallback); host `RAYNU-V-M8-DISK-PERSIST-HOST-OK`; iron `RAYNU-V-M8-DISK-PERSIST-OK` still open. Guest F7 persist already closed (ADR-017). |
