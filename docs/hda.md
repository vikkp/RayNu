...
velocity_commits_30d: 611
velocity_gates_30d: 62
overall_pct: 96
confidence: medium
...
|--------|------:|-------------------|
| **Overall product readiness** | **96%** | **+2** — `RAYNU-V-M7-ISO-INSTALL-OK` printed on the **real R640** (UDisk `59ac070` / run `34425781629`, 2026-09-10): Alpine 3.21 ISO → `apk` 25 pkgs in 0.5 s → `setup-disk` to virtio-blk `vda` (GPT, grub x86_64-efi, initramfs) `Installation finished. No error reported.`; the `916af96` console-throughput fix landed. Residual: F7 reboot-to-disk failed `VMCLEAR/VMPTRLD` (host-stack overflow into the VMCS; fixed this pin, not yet iron-proven) |
| **Months to Mount Everest** | **0.5** | **−0.25** (iron install-to-disk DONE; only reboot-to-disk on iron + TLS/console polish remain on the E5/E4 path) |
| **ETA month** | **2026-10** | held |
| **Confidence** | medium | E2+E3+E3b+Phase F+P0-14+E5-install stamps on COM2. The F7 failure has a named, asm-proven root cause (81,024-byte `FirmwareState::new()` frame on a 16 KiB host stack; the VMCS was the next frame down) and a host-side fix (`.rdata` template memcpy, 32-page stack + guard); reboot-to-disk on iron is still unproven |
...
| **Deploy Linux ISO** | ~98% | **Iron install-to-disk DONE** (`59ac070`: `ISO-INSTALL-OK` on R640, `Installation finished. No error reported.`); heartbeat confirmed the console model (`room=3177..4096 pace=3360→5811 fifo=1 drained=38827→97346 win=6186→17488`). Open: iron reboot-to-disk — F7 relaunch `VMCLEAR/VMPTRLD` fixed (template reset + host stack guard), next flash |
...
**Status: NEAR · ~98% · ~0.25 months residual (iron reboot-to-disk; installer DONE on iron 2026-09-10; F7 relaunch + installed GRUB menu reached on iron `975f8fc`)**
...
| Guest UEFI firmware blob | **DONE on iron for install (RayNu-F ADR-016); iron reboot-to-disk PARTIAL — relaunch + installed GRUB menu reached** | **Iron UDisk `975f8fc` / run `34474850361` (2026-09-10): F7 relaunch worked on the R640** — `host stack top=0x142d000 pages=32 guard=0x140c000`; install re-ran clean and the install marker printed again; `reboot` → `guest reset requested src=kbc n=1` → `relaunch after reset` → `GPT ESP lba=2048 sectors=98304 part=1` → `disk whole-disk path` → `BOOTX64.EFI bytes=139264` → `image=DISK-BOOTX64` → GRUB 2.12 from `vda` drew its menu (`*Alpine Linux v3.21, with Linux lts … executed automatically in 2s`, mirrored on COM1 via Alpine's `terminal_input serial console`) → **`RayNu-F stop exit-cap exits=1048577 svc=492778 blk_rd=59`**. GRUB's `run_menu` polls `ReadKeyStroke` + serial LSR with no idle = 2 exits/iteration at ~2 exits/µs on iron, so the fixed 1 M exit-count cap fired inside GRUB's own 2 s timeout (nested KVM exits 10–50× slower never reached it; GRUB clock honest via our TSC `Stall`). Fix pin: **RayNu-F wall cap** — loader phase bounded by 180 s wall from `rdtsc` at launch/relaunch (checked every 4096 exits); exit count only a u32-wrap guard (`1 << 30`); stop lines print `wall_ms=`. Iron `DISK-BOOT-OK` not claimed. **Prior:** **Iron UDisk `59ac070` / run `34425781629` (2026-09-10): `RAYNU-V-M7-ISO-INSTALL-OK` on the R640.** RayNu-F BlockIo/FAT/StartImage → Linux 6.12 → `apk` 25 packages in 0.5 s (`916af96` console fix landed: `room=3177..4096 pace=3360→5811 fifo=1 drained=38827→97346 win=6186→17488`) → `setup-disk -m sys /dev/vda` (GPT, ext4, `grub-install` x86_64-efi, `initramfs-lts`, `Installation finished. No error reported.`). `reboot` → `src=kbc` → F7 relaunch **failed `VMCLEAR/VMPTRLD`**: release asm shows `raynu_f_reset_relaunch` reserved 81,024 bytes (`RAYNU_F_STATE = FirmwareState::new()` built the 79,704-byte `PagePool` on the stack) on the 16 KiB host stack at `0x140c000`; the private VMCS at `0x140b000` was the next frame down, so `VMPTRLD` saw a foreign revision dword. Fix pin: `.rdata` template memcpy (frame 40 bytes), 32-page host stack + guard page checked before F7 `VMCLEAR`, split `VMCLEAR`/`VMPTRLD` lines with `rev=/want=/guard=`, `flush_guest_tx()` before share-off. Iron `DISK-BOOT-OK` not claimed. **History:** Iron UDisk `916af96` / run `34420783162` **telemetry named the link**: `build: sha=916af960b985`; stall `n=1370`, `apk` in `n_tty_write`; heartbeats `ier=0x7 latch=1 pend=0 ring=2048→0→1988 com2_lsr=0x0 iir THRE≈take4≈lsr_thre ON (1:1) thr +16/IRQ etbei set pic irr=0x5 imr=0xe8 isr=0x4 take0 +30/2 s take4 stuck`. Every counted link agreed when it fired; the level was almost never true because guest THRE was host `com2_lsr` bit 5 **and** an empty ring, sampled only at VM exits — under `idle=poll` ~15 preemption ticks a second, each draining one byte (LSR polled per byte); own per-hit `virtio MMIO gpa=` lines filled the 2 KiB ring. Fix pin (host-side only, ADR-016): guest UART TX ring room (guest THRE/IRQ level = `room ≥ 1024+16`, 4 KiB ring, 1 KiB HV reserve; COM2 LSR no longer gates the guest) + guest UART line-rate pace (VMX preemption timer clamped to 250 µs while the ring holds bytes; `IA32_VMX_MISC` shift at launch, TSC calibrated pre-EBS; paced ticks drain 16 + inject, no HPET/PIT/exit-count) + guest UART COM2 FIFO burst (16 bytes per THRE window on a 16550A; 1 on floating/no-FIFO) + virtio MMIO kick throttle (≤1 kick line / ~250 ms). Heartbeat gains `room= pace= fifo= drained= win=`. Do not F11 `34420783162`. Prior `8b6ed1a` / run `34415711199` **THRE level lived, COM2 identical to the probe** (`apk` still in `n_tty_write → wait_woken`, text 16–32 bytes per SysRq IRQ 4, stall dump `n=1373`, `PIT paced n=1375`): not a regression, not a fix. Next pin prints every THRE link with the heartbeat (`virtio stall dump thre …`: IER/latch/pending, ring, COM2 LSR, IIR/LSR classes, THR/ETBEI counts, 8259 IRR/IMR/ISR, IRQ 0/4 INTA counts — UART THRE chain telemetry) and `build: sha=` under the banner. Do not F11 `34415711199`. Prior `f229d14` / run `34354322953` **probe answered**: SysRq `t` shows `apk` `state:S` in `n_tty_write → wait_woken` (stdout = ttyS0), ring lines `mem_used==used last_st=0x0 xlate_fail=0`, userspace output moved exactly one 16-byte `tx_loadsz` per SysRq IRQ 4. Root cause: guest 16550 THRE interrupt lost when the IIR read cleared the latch and LSR then read THRE=0 (SOL back-pressure / our diag bytes in the shared ring); 4 KiB xmit buffer filled, `apk` blocked, no more virtio kicks. Not virtio, not timer. Fix: THRE is a gated level (pending while ETBEI and ring empty; cleared by `__stop_tx`), re-asserted per resume — UART THRE level until stop_tx. Do not F11 `34354322953`. Nested F7 reboot-to-disk remains `fe4785a` (not iron / not Everest E5). Operator: [r640_f7_iso_iron.md](runbooks/r640_f7_iso_iron.md). |
...
| M+4 | 2026-10 | TLS/console + distro installer | remaining Everest | **ETA** (**installer DONE on iron 2026-09-10**: `59ac070` / `34425781629` `ISO-INSTALL-OK` + `Installation finished`; open: iron reboot-to-disk — `975f8fc` relaunched into the installed GRUB menu on iron, exit-count cap fired inside GRUB's 2 s timeout; RayNu-F wall cap this pin, flash next) |
...
2026-09 ██████░░  distro installer DONE on iron; reboot-to-disk + TLS/console  ← months_to_everest ≈ 0.5
2026-10 ░░░░░░░░  buffer
2026-11 ░░░░░░░░  buffer
```

**Pull-forward rule:** E2 closed 2026-08-15; E3 bring-up closed 2026-08-16; **E3b closed 2026-08-20**; **P0-14 closed 2026-08-21**; **E5 install-to-disk closed on iron 2026-09-10** (`59ac070`); **F7 relaunch + installed-disk GRUB reached on iron 2026-09-10** (`975f8fc`, exit-cap ended it). Shrink further when the installed disk boots to a second Linux on iron (F7 `DISK-BOOT-OK`). Document why in [Changelog](#hda-changelog).
...
| Summary | **Iron COM2 (UDisk, `975f8fc` / run `34474850361`) — the F7 relaunch worked and RayNu-F booted the installed disk's GRUB on the real R640; the firmware-phase exit-count cap ended it inside GRUB's 2 s menu.** `host stack top=0x142d000 pages=32 guard=0x140c000`; install re-ran clean (`RAYNU-V-M7-ISO-INSTALL-OK` again); `reboot` → `guest reset requested src=kbc n=1` → `relaunch after reset` → `GPT ESP lba=2048 sectors=98304 part=1` → `disk whole-disk path` → `BOOTX64.EFI bytes=139264` → `image=DISK-BOOTX64` → `MEM-OK`/`BLOCKIO-OK`/`CONOUT-OK` → GRUB 2.12 menu `*Alpine Linux v3.21, with Linux lts … executed automatically in 2s` → `RayNu-F stop exit-cap exits=1048577 svc=492778 blk_rd=59`. Cause (GRUB source + counters): `run_menu` polls `grub_getkey_noblock()` with no idle — `ReadKeyStroke` svc + serial LSR `inb` = 2 exits per iteration (`svc` ≈ `exits`/2) at ~2 exits/µs on the R640, so `RAYNU_F_EXIT_CAP = 1_048_576` fired in under a second; nested KVM exits are 10–50× slower so `fe4785a` never approached it. GRUB's clock is honest (no ACPI; port `0x61` reads `0xff` so `grub_pit_wait` bails; `grub_tsc_calibrate_from_efi` on our TSC `Stall`). This pin: **RayNu-F wall cap** — `RAYNU_F_WALL_CAP_S = 180` s from `rdtsc` at launch/relaunch, sampled every 4096 exits (`raynu_f_stop("wall-cap")`); `RAYNU_F_EXIT_CAP` = `1 << 30` (u32-wrap guard only); stop lines print `wall_ms=`. `bli` `unable to determine partition UUID` (our `SetVariable` = `EFI_UNSUPPORTED`) and `no suitable video mode` are harmless. |
| Everest impact | months **0.5 held**; overall **96 held**; ETA 2026-10 held. Real iron progress (relaunch, GPT ESP, installed GRUB) but the loop is not closed: no second Linux from `vda` yet. Under-claim until `RAYNU-V-RAYNU-F-DISK-BOOT-OK` prints on COM2. |
| Gates touched | None closed. Host `cargo test --no-default-features -- --test-threads=1` (new: `raynu_f_wall_cap_bounds_the_loader_phase_by_time_not_exits`; F7 gate checks `RAYNU_F_EXIT_CAP ≥ 1<<28`, wall cap 60–600 s, hit/not-hit); `./tools/build.sh`; TCG QEMU boot gate. RayNu-F wall cap. |
| Months Δ | 0.5→0.5 |
