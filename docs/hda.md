...
|--------|------:|-------------------|
| **Overall product readiness** | **99%** | **held** — Everest **CLOSED**. M8.0 persist-first attach + nested File RAM + `MODE=keep` + DurableLun NVMe/USB I/O + **`MODE=lunkeep`/`usbkeep` TCG `keep=1`**. **Nested Alpine `MODE=full` CLOSED** on `raynuvsrv1` (`ce3d8a09`). **Iron 1–3 host-ready** (R640 census / Intel PCH scratchpad ≤64 / leftover named). Iron `ac3b92cd` Cap `err=1`; Cap EFI `67db8a68` `scratch=34/64` then Enum `err=3` `cmpl=0x11` (Polling p10/p11/p14). Nested QEMU ≠ R640. Not 100%: iron Force Off / TLS / console |
...
| P0-64 | **M8** operator product hardening | — | IN PROGRESS | Everest closed | [ADR-018](adr/ADR-018.md) / [m8_plan.md](m8_plan.md) / [m8_persist_nested.md](runbooks/m8_persist_nested.md) / [m8_persist_iron.md](runbooks/m8_persist_iron.md). **M8.0 nested Alpine `MODE=full` CLOSED** on `raynuvsrv1` (`ce3d8a09`). Iron 1–3 host-ready (census / PCH scratchpad ≤64 / leftover named). Iron Cap `err=1` (`ac3b92cd`) then Cap EFI `67db8a68` `scratch=34/64` + Enum `err=3` `cmpl=0x11` (USB3 Polling; this EFI waits U0 + names `xhci enum pN`). Nested QEMU ≠ R640. Iron COM2 `I/O ready` + `RAYNU-V-M8-DISK-PERSIST-OK` open. Then TLS, auth, console UI, ISO upload, UEFI catalog, Windows later. |
...
|-------|-------|
| Commit | m8-xhci-enum |
| Summary | **Iron leftover, not persist.** Cap EFI `67db8a68` proved `scratch=34/64`; then Enum `err=3` `cmpl=0x11` on Polling PORTSC `0x000206e1` (p10/p11/p14). This EFI: WPR on PLS=Polling, wait U0+speed, full slot/port counts, `xhci enum pN sc=` (not `portsc=0`). Toshiba unused. Months **0.0 held**. Overall **99 held**. |
...
| H10 | Leftover-DRAM disk dies on **HV** reboot | MED | **M8.0** first gate ([m8_plan.md](m8_plan.md)): persist-first attach; nested File RAM + `MODE=keep`; DurableLun NVMe/USB I/O; **`MODE=lunkeep`/`usbkeep` TCG keep=1**; leftover/File persist RAM **3584M**; **`MODE=full` nested-OK CLOSED on `raynuvsrv1` `ce3d8a09`**. **Iron 1–3 host-ready** ([m8_persist_iron.md](runbooks/m8_persist_iron.md)). Iron Cap `err=1` (`ac3b92cd`) then Cap EFI `67db8a68` `scratch=34/64` + Enum `err=3` `cmpl=0x11` (Polling; leftover DRAM Everest loop). Do not Force Off that guest expecting persist. Nested QEMU ≠ R640. Iron `RAYNU-V-M8-DISK-PERSIST-OK` still open. Guest F7 persist already closed (ADR-017). |
