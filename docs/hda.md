...
| Summary | **Iron leftover skipped, not persist.** Nlb1 COM2 Toshiba 298 GiB ready + peek `usb_err=0` + nlb=1 512-byte oks then `scsi=sense` after Xfer timeout; `vda` no partition table. This commit: REQUEST SENSE only on `UsbBotError::Bot`, never on Xfer timeout; keep nlb=1. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | `usb_bot.rs` Bot-only REQUEST SENSE; [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
