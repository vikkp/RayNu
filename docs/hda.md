...
| Guest UEFI firmware blob | PARTIAL (RayNu-F nested F7 reboot-to-disk; iron E5 open) | Iron UDisk `896424f` / run `34131274237`: MADT/`DRIVER_OK` live, then `virtblk_probe` `iowrite8` IRQ0 storm (`soft lockup`) then I/O `reason=0x1e` hold. Pace overlay resume PIT ~1 ms TSC. Do not F11 `34131274237`. Prior `69102aa` / `34080595540` stormed at first DEVICE_STATUS. Nested F7 reboot-to-disk remains `fe4785a` (not iron / not Everest E5). Operator: [r640_f7_iso_iron.md](runbooks/r640_f7_iso_iron.md). |
...
| M+4 | 2026-09 | TLS/console + distro installer | remaining Everest | **ETA** (iron `ba5bf8f` / `34290078274` stall dump PIT lived then apk `n=1373`; virtio stall dump INTx; `ISO-INSTALL-OK` open) |
...
| Summary | **Iron COM2 (UDisk, `ba5bf8f` / run `34290078274`):** stall dump PIT lived (`disk_live=1093 iso_live=1052` matched last) then still apk `n=1373`. Do not wait. This pin pulses shared virtio INTx. Do not F11 `34290078274`. Host-tested; not ISO-INSTALL-OK. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Iron apk overlay still open; do not drop months. |
| Gates touched | none. Host `cargo test --no-default-features -- --test-threads=1` (671 passed). virtio stall dump INTx. |
| Months Δ | 0.5→0.5 |
