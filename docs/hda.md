...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; descabort COM2 Toshiba 298 GiB ready + virtio keep=0 then vda last_st=0x1; this EFI guestio FIRST_READ_SPINS; iron Force Off open) |
...
| Summary | **Iron leftover skipped, not persist.** Descabort COM2: Toshiba named + 298 GiB `usb I/O ready` + virtio `keep=0`; Alpine `vda` then `last_st=0x1` / `sfdisk` I/O error. ISO `vdb` `last_st=0x0`. This EFI: guest BOT `FIRST_READ_SPINS` + `usb rw fail` serial. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | `xhci.rs` guest BOT `FIRST_READ_SPINS` + `usb rw fail`; [m8_persist_iron.md](runbooks/m8_persist_iron.md); [loihda.md](loihda.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
