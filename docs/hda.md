...
|--------|------:|-------------------|
| **Overall product readiness** | **99%** | **held** — Everest **CLOSED**. M8.0 persist-first attach + host round-trip (`RAYNU-V-M8-DISK-PERSIST-HOST-OK`). Not 100%: nested Alpine kill/restart / iron persist / TLS / console |
...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (attach persist-first; nested Alpine + iron open) |
...
2026-10 ░░░░░░░░  M8.0 persist (attach persist-first; iron COM2 open)
...
| P0-64 | **M8** operator product hardening | — | IN PROGRESS | Everest closed | [ADR-018](adr/ADR-018.md) / [m8_plan.md](m8_plan.md). **M8.0** persist-first attach: `File` nested NVDIMM / `DurableLun` iron (not PERC) / leftover DRAM fallback. `attach_disk_keep` when GPT ESP + BOOTX64 + ext4. Host round-trip `RAYNU-V-M8-DISK-PERSIST-HOST-OK`. Nested Alpine kill/restart + iron `RAYNU-V-M8-DISK-PERSIST-OK` open. Then TLS, auth, console UI, ISO upload, UEFI catalog, Windows later. |
...
| Field | Value |
|-------|-------|
| Commit | m8-disk-persist-attach |
| Summary | **M8.0 persist-first attach.** virtio-blk prefers firmware PersistentMemory (`attach_disk_keep` when GPT ESP + BOOTX64 + ext4); leftover DRAM remains the fallback. Nested `M8_PERSIST_IMG` is NVDIMM (default off). Host `RAYNU-V-M8-DISK-PERSIST-HOST-OK` still the only host marker. Nested Alpine kill/restart + iron `RAYNU-V-M8-DISK-PERSIST-OK` still open. Never `ISO-INSTALL-OK`. |
| Everest impact | months **0.0 held**; overall **99 held**; ETA 2026-09 held. Not 100%. |
| Gates touched | `mgmt/disk_persist.rs` persist reserve/take + `attach_disk_keep` + handoff PersistentMemory + `M8_PERSIST_IMG` NVDIMM. `./tools/sync-hda-site.sh --check`. |
...
| H10 | Leftover-DRAM disk dies on **HV** reboot | MED | **M8.0** first gate ([m8_plan.md](m8_plan.md)): persist-first attach (`attach_disk_keep`); leftover DRAM fallback; host `RAYNU-V-M8-DISK-PERSIST-HOST-OK`; nested Alpine kill/restart + iron `RAYNU-V-M8-DISK-PERSIST-OK` still open. Guest F7 persist already closed (ADR-017). |
...
Next move:     **M8.0** persist-first attach wired; nested Alpine + iron COM2 open ([ADR-018](adr/ADR-018.md) / [m8_plan.md](m8_plan.md))
