...
| Guest UEFI firmware blob | PARTIAL (RayNu-F nested F7 reboot-to-disk; iron E5 open) | Iron UDisk `b8ac361` / run `34297632324`: notify-reset lived; apk empty-ring `n=1370` then INTx + ISR, no `0x300`. virtio stall dump PIT hold. Do not F11 `34297632324`. Nested F7 reboot-to-disk remains `fe4785a` (not iron / not Everest E5). Operator: [r640_f7_iso_iron.md](runbooks/r640_f7_iso_iron.md). |
...
| M+4 | 2026-09 | TLS/console + distro installer | remaining Everest | **ETA** (iron `b8ac361` / `34297632324` notify-reset lived then apk empty-ring `n=1370`; virtio stall dump PIT hold; `ISO-INSTALL-OK` open) |
...
| Summary | **Iron COM2 (UDisk, `b8ac361` / run `34297632324`):** notify-reset lived (no flood). Apk dump `live==last n=1370` then INTx + ISR and no `0x300`. Do not wait. This pin holds PIT while dumped. Do not F11 `34297632324`. Host-tested; not ISO-INSTALL-OK. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Iron apk overlay still open; do not drop months. |
| Gates touched | none. Host `cargo test --no-default-features -- --test-threads=1`. virtio stall dump PIT hold. |
| Months Δ | 0.5→0.5 |
