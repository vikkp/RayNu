...
| Summary | **Iron leftover, not persist.** rstdev/udiskkick COM2 named Toshiba p11 then walked p14/p10 after INQUIRY CSW wait — regression vs guestio `usb I/O ready`. This commit: `xhci stopwalk` keep-slot + BOT retry; no Disable Slot; no p14. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | `xhci.rs` named-MSC stop-walk; [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
