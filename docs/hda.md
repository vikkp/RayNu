...
|--------|------:|-------------------|
| **Overall product readiness** | **99%** | **held** — Everest **CLOSED**. M8.0 persist-first attach + nested File RAM + `MODE=keep` + DurableLun NVMe/USB I/O + **`MODE=lunkeep`/`usbkeep` TCG `keep=1`**. **Nested Alpine `MODE=full` CLOSED** on `raynuvsrv1` (`ce3d8a09`). **Iron 1–3 host-ready** (R640 census / Intel PCH scratchpad ≤64 / leftover named). Iron `ac3b92cd` leftover DRAM Everest loop; xHCI Cap `err=1` on `8086:a1af`. Nested QEMU ≠ R640. Not 100%: iron Force Off / TLS / console |
...
| P0-64 | **M8** operator product hardening | — | IN PROGRESS | Everest closed | [ADR-018](adr/ADR-018.md) / [m8_plan.md](m8_plan.md) / [m8_persist_nested.md](runbooks/m8_persist_nested.md) / [m8_persist_iron.md](runbooks/m8_persist_iron.md). **M8.0 nested Alpine `MODE=full` CLOSED** on `raynuvsrv1` (`ce3d8a09`). Iron 1–3 host-ready (census / PCH scratchpad ≤64 / leftover named). Iron `ac3b92cd` leftover DRAM + xHCI Cap `err=1` on `8086:a1af`. Nested QEMU ≠ R640. Iron COM2 `I/O ready` + `RAYNU-V-M8-DISK-PERSIST-OK` open. Then TLS, auth, console UI, ISO upload, UEFI catalog, Windows later. |
...
| Field | Value |
|-------|-------|
| Commit | m8-xhci-cap-scratch |
| Summary | **Iron leftover, not persist.** COM2 `ac3b92cd`: Everest loop on leftover DRAM (`vda` 1.07 GiB); Lewisburg xHCI `8086:a1af` Cap `err=1` before PORTSC; Toshiba unused. Scratchpad budget 16→64 + always-on `xhci cap caplen=` snapshot. Months **0.0 held**. Overall **99 held**. |
...
| H10 | Leftover-DRAM disk dies on **HV** reboot | MED | **M8.0** first gate ([m8_plan.md](m8_plan.md)): persist-first attach; nested File RAM + `MODE=keep`; DurableLun NVMe/USB I/O; **`MODE=lunkeep`/`usbkeep` TCG keep=1**; leftover/File persist RAM **3584M**; **`MODE=full` nested-OK CLOSED on `raynuvsrv1` `ce3d8a09`**. **Iron 1–3 host-ready** ([m8_persist_iron.md](runbooks/m8_persist_iron.md)). Iron `ac3b92cd` (2026-09-14): leftover DRAM Everest loop; xHCI Cap `err=1` on `8086:a1af` (16-page budget → 64). Do not Force Off that guest expecting persist. Nested QEMU ≠ R640. Iron `RAYNU-V-M8-DISK-PERSIST-OK` still open. Guest F7 persist already closed (ADR-017). |
