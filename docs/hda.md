...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; DESC retry Toshiba named; BOT READ `73dc4d2e` leftover Everest ≠ persist; CSW bring-up open; iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (nested-OK closed; iron Toshiba named; BOT CSW bring-up; Force Off open)
...
| Field | Value |
|-------|-------|
| Commit | m8-usb-bot-csw |
| Summary | **Iron leftover, not persist.** BOT READ COM2 `73dc4d2e`: p11 Toshiba named + p14 hub skip + leftover Everest (`ISO-INSTALL-OK` → F7 `DISK-BOOT-OK` → `login:`). `usb I/O fail err=8 bot=csw cmpl=0` — p14 hub clobbered p11 CSW; START STOP + ISP + auto EP-reset regressed vs `6c278e85` `usb I/O ready lba=512`. This EFI: drop START STOP from bring-up; ISP only on CSW; no auto EP-reset; hub skip keeps Xfer cmpl; `scsi=` + `recover_pipes` on READ retry; keep `BULK_SPINS` + READ probe. Months **0.0 held**. Overall **99 held**. |
| Everest impact | months **0.0 held**; overall **99 held**; ETA 2026-09 held. Not 100%. No persist gate. |
| Gates touched | `usb_bot.rs` no START STOP bring-up + `scsi=` + `recover_pipes`; `xhci.rs` ISP-on-CSW + hub keep-diag; [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. |
...
Next move:     **M8.0 iron** COM2 peek without `read-fail` + Alpine mounts `vdb`, then Force Off / `RAYNU-V-M8-DISK-PERSIST-OK`. Recover `568e8696` Toshiba attach is **not** persist. Nested-OK closed on `raynuvsrv1` `ce3d8a09`.
