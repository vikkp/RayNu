...
| Summary | **Iron leftover, not persist.** Okquiet COM2: Toshiba named then SET_CONFIG `cmd=6 cmpl=0xff` leftover DRAM 1.07 GiB `ISO-INSTALL-OK` → F7 `DISK-BOOT-OK` → `root=UUID=53bb2054-…`. This EFI: abort + No-Op + EP0 reset + retry SET_CONFIG (`xhci setcfgretry`). Keep writequeue / okquiet. Months **0.0 held**. Overall **99 held**. |
| Everest impact | months **0.0 held**; overall **99 held**; ETA 2026-09 held. Not 100%. No persist gate. LOIHDA persist **70% held** (A2 still open). Leftover DRAM `ISO-INSTALL-OK` is not persist. |
| Gates touched | Lived COM2 SET_CONFIG timeout leftover; this EFI `xhci setcfgretry`; [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
...
| 2026-09-18 | m8-usb-bot-setcfgretry | 0.0 | 99 | **Iron leftover, not persist:** okquiet COM2 Toshiba named then SET_CONFIG `cmd=6 cmpl=0xff` leftover DRAM 1.07 GiB `ISO-INSTALL-OK` → F7 `DISK-BOOT-OK` → `root=UUID=53bb2054-…`. This EFI: `xhci setcfgretry`. Keep writequeue / okquiet. LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Leftover DRAM Everest ≠ persist. months 0.0 held; overall 99 held |
