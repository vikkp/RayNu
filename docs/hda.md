...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; slotretry COM2 p11 GET_DESC timeout then p14 hub; Enable Slot lived; this EFI descabort; iron Force Off open) |
...
| Field | Value |
|-------|-------|
| Commit | m8-usb-bot-descabort |
| Summary | **Iron leftover, not persist.** Slotretry COM2: nop + p11 EP0 GET_DESC timeout (`cmd=4`, not Eval) then p14 hub. Enable Slot lived. This EFI: keep-slot descabort; no p14 walk. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | `xhci.rs` descabort; [m8_persist_iron.md](runbooks/m8_persist_iron.md); [loihda.md](loihda.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
