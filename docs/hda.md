...
|--------|------:|-------------------|
| **Overall product readiness** | **99%** | **held** — Everest **CLOSED**. M8.0 persist-first attach + nested File RAM + `MODE=keep` + DurableLun census. Not 100%: nested Alpine kill/restart / iron persist I/O / TLS / console |
...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (DurableLun census; Alpine two-boot + iron I/O open) |
...
2026-10 ░░░░░░░░  M8.0 persist (DurableLun census; Alpine + iron I/O open)
...
| Field | Value |
|-------|-------|
| Commit | m8-disk-persist-durable-lun |
| Summary | **M8.0 DurableLun mapper (host).** PCI census picks NVMe (`01:08`) or USB ≥ 1 GiB; refuses PERC/MegaRAID and the ESP Cruzer. Post-EBS NVMe/USB I/O still residual (leftover DRAM attach). Not nested-OK. Not iron persist OK. Never `ISO-INSTALL-OK`. Do not F11. |
| Everest impact | months **0.0 held**; overall **99 held**; ETA 2026-09 held. Not 100%. |
| Gates touched | `mgmt/durable_lun.rs` + M8 persist host gate. `./tools/sync-hda-site.sh --check`. |
...
Next move:     **M8.0** DurableLun census (not iron I/O); Alpine MODE=full + iron COM2 open ([ADR-018](adr/ADR-018.md) / [m8_plan.md](m8_plan.md))
