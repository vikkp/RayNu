...
| Summary | **Iron leftover skipped, not persist.** Sensebot COM2 Toshiba 298 GiB ready + nlb=1 oks then `bot=csw scsi=read cmpl=0xff`; `vda1`; ISO vdb FAT; RCU/soft lockup in `nlplug-findfs`. No `scsi=sense`. This commit: queue CSW IN with DATA (`xhci cswqueue`) + settle between nlb=1 chunks; keep Bot-only SENSE. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | `xhci.rs` CSW queue with DATA (`xhci cswqueue`); `usb_bot.rs` settle between nlb=1 chunks; [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
