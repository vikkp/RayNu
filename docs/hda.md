...
| Summary | **Iron leftover skipped, not persist.** Setcfgretry COM2 (`ef7e93ec`): SET_CONFIG first-try `xhci setcfg p11 val=1` (no `xhci setcfgretry` line); Toshiba 298 GiB `usb I/O ready`; leftover skip; Alpine `[vda] 298 GiB`; WRITE then USB `ISO-INSTALL-OK` → `Installation is complete` → F7 `DISK-BOOTX64` GRUB 2s. Waiting `DISK-BOOT-OK` + `root=UUID=` on 298 GiB `vda`. Months **0.0 held**. Overall **99 held**. |
| Everest impact | months **0.0 held**; overall **99 held**; ETA 2026-09 held. Not 100%. No persist gate. LOIHDA persist **70% held** (A2 still open). Guest F7 on USB LUN is not persist. Do not Force Off during GRUB. |
| Gates touched | Lived setcfgretry COM2 leftover skip + USB install complete + F7 `DISK-BOOTX64`; [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
