...
| Summary | **Iron leftover skipped, not persist.** Setcfgretry COM2 (`ef7e93ec`): SET_CONFIG first-try `xhci setcfg p11 val=1` (no `xhci setcfgretry` line); Toshiba 298 GiB `usb I/O ready`; leftover skip; Alpine `[vda] 298 GiB`; WRITE then USB `ISO-INSTALL-OK`; UART mash during setup-disk. In progress at paste. Months **0.0 held**. Overall **99 held**. |
| Everest impact | months **0.0 held**; overall **99 held**; ETA 2026-09 held. Not 100%. No persist gate. LOIHDA persist **70% held** (A2 still open). USB `ISO-INSTALL-OK` is not persist. Do not Force Off mid-install. |
| Gates touched | Lived setcfgretry COM2 leftover skip + USB `ISO-INSTALL-OK`; [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
...
| 2026-09-18 | m8-usb-bot-setcfgretry | 0.0 | 99 | **Iron leftover skipped, not persist:** setcfgretry COM2 (`ef7e93ec`) SET_CONFIG first-try `xhci setcfg p11 val=1` (retry unused); Toshiba 298 GiB ready + leftover skip + Alpine `[vda] 298 GiB` WRITE + USB `ISO-INSTALL-OK` then UART mash during setup-disk. In progress. Do not Force Off. Keep writequeue / okquiet / setcfgretry. LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. USB `ISO-INSTALL-OK` ≠ persist. months 0.0 held; overall 99 held |
