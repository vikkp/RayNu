...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; skipmaxlun COM2 `epst ep0=1` + `scsi=inquiry` leftover DRAM; this EFI overlaps first INQUIRY; iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (nested-OK closed; skipmaxlun `ep0=1` leftover; overlap INQUIRY; Force Off open)
...
| Summary | **Iron leftover, not persist.** Skipmaxlun COM2: `xhci maxlun skip` + `epst ep0=1` then INQUIRY CBW `cmpl=0xff` leftover 1 GiB. EP0-Stopped is falsified. This EFI: overlap first INQUIRY CBW OUT + DATA IN. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | `xhci.rs` / `usb_bot.rs` first INQUIRY overlap; [m8_persist_iron.md](runbooks/m8_persist_iron.md); [loihda.md](loihda.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
