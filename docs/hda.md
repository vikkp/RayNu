...
| **Confidence** | medium | E2+E3+E5 on COM2; post-EBS `cand bmsr=7949` both ports; skip `BMCR_RESET` when `ape-ncsi=yes`; HTTP-OK open |
...
| **Post-EBS durable HTTP (E3b)** | MISSING | Post-EBS `cand bmsr=7949` on both ports immediately after EBS. Skip `BMCR_RESET` when `ape-ncsi=yes`. HTTP-OK not claimed |
...
| P0-12 | **M7.8 / E3b** Host-owned mgmt NIC (ADR-013) | C | 1.0 | P0-4 | Post-EBS `cand bmsr=7949`; skip `BMCR_RESET` when `ape-ncsi=yes`; HTTP-OK **open** |
...
|-------|-------|
| Commit | PENDING |
| Summary | Post-EBS `cand bmsr=7949` immediately after EBS. Skip `BMCR_RESET` when `ape-ncsi=yes`. HTTP-OK not claimed |
| Everest impact | months 1.5 held; ETA 2026-10; overall 88 held; E3b open (APE NCSI PHY) |
| Gates touched | `skip_bmcr_reset`; `HOST-NIC-HTTP-OK` not claimed |
| Months Δ | 1.5 held |
...
| H4 | Firmware SNP unusable after EBS | HIGH | Post-EBS `cand bmsr=7949` both PHYs immediately after EBS. Skip `BMCR_RESET` when `ape-ncsi=yes`; E3b = HTTP after `BOOT-OK` |
...
|------|--------|-------:|----------:|------|
| 2026-08-19 | PENDING | 1.5 | 88 | Post-EBS `cand bmsr=7949` at EBS; skip `BMCR_RESET` when `ape-ncsi=yes`; HTTP-OK not claimed |
...
Now:           E2+E3+E5 stamps CLOSED; Phase 0 CLOSED; Phase D skip BMCR_RESET when ape-ncsi; E3b OPEN
Months left:   1.5  (ETA ~ 2026-10)
Next move:     flash skip-BMCR_RESET EFI via raynuvsrv1; expect COM2 phy_reset=pre skip (ape-ncsi)
