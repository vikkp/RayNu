...
| Summary | **Iron leftover, not persist.** Cswsettle COM2 No-Op timeout + abort then p11 ADDR `cmd=3 cmpl=0xff`; leftover DRAM 1.07 GiB `ISO-INSTALL-OK` → F7 `DISK-BOOT-OK` → login `root=UUID=d43dd928-…`. CSW queue never ran. This commit: `xhci nopretry` + `xhci addrretry`; keep CSW queue. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | `xhci.rs` No-Op re-prime + Address Device retry; [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
...
| 2026-09-17 | m8-usb-bot-addrretry | 0.0 | 99 | **Iron leftover, not persist:** cswsettle COM2 No-Op timeout + abort then p11 ADDR `cmd=3 cmpl=0xff`; leftover DRAM 1.07 GiB `ISO-INSTALL-OK` → F7 `DISK-BOOT-OK` → login `root=UUID=d43dd928-…`. CSW queue never ran. This EFI: `xhci nopretry` + `xhci addrretry`; keep CSW queue. LOIHDA Bar A 42% held. Nested QEMU ≠ R640. Iron persist open. Never `ISO-INSTALL-OK`. months 0.0 held; overall 99 held |
