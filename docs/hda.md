...
|--------|------:|-------------------|
| **Overall product readiness** | **94%** | 0 (iron apk overlay blocked since 2026-09-07; telemetry `916af96` named the link — guest THRE gated on host COM2 LSR + empty ring, sampled only at ~15 idle exits/s; fix pin: ring room + line-rate pace + FIFO burst, not yet on iron) |
| **Months to Mount Everest** | **0.75** | 0 (held: console TX deadlock still the named wait; fix did not land on iron; next pin is measurement, not stimulus) |
| **ETA month** | **2026-10** | held |
| **Confidence** | medium | E2+E3+E3b+Phase F+P0-14 stamps on COM2; E5 iron wait still `apk` in `n_tty_write`; `916af96` telemetry named the broken link (console-throughput model: guest THRE = host COM2 THRE ∧ empty ring, sampled at ~15 exits/s, 1 byte/exit; own diag lines filled the 2 KiB ring) — the fix pin is host-side only and not yet iron-proven |
...
| **Deploy Linux ISO** | ~96% | Nested RayNu-F alpine-extended install + reboot-to-disk (`fe4785a`); iron apk stall is a console-throughput deadlock (not virtio/timer) — `916af96` heartbeats: `pend=0` with `ring=0` and `com2_lsr=0x0`, ~15 exits/s each draining 1 byte; fix pin: guest UART TX ring room + line-rate pace (preemption timer 250 µs while ring non-empty) + COM2 FIFO burst + kick throttle |
...
| M+4 | 2026-10 | TLS/console + distro installer | remaining Everest | **ETA** (iron `916af96` / `34420783162`: telemetry named the link — guest THRE gated on host COM2 LSR + empty ring at ~15 exits/s; fix pin ring room + line-rate pace + FIFO burst next; `ISO-INSTALL-OK` open) |
...
| Summary | **Iron COM2 (UDisk, `916af96` / run `34420783162`) — THRE chain telemetry named the broken link.** `build: sha=916af960b985` tied the log to the commit. Stall again at `n=1370`, `apk` in `n_tty_write`. Heartbeats: `ier=0x7 latch=1 pend=0 ring=2048→1→0→1988 com2_lsr=0x0 iir=0/17/17→4/32/32 lsr_thre=15/0→30/12 thr=21478→73633 etbei=13/433→942/1362 raise=144/72→2280/2000 pic irr=0x5 imr=0xe8 isr=0x4 rdy=1 take0=2795→3381 take4=14→29`. Every counted link agreed when it fired (`take4` ≈ `iir` THRE ≈ `lsr_thre` ON, one THRE IRQ per 16 `thr`; `iir` NONE is the loop-exit read; 8259 sane; ETBEI set). The **level** was almost never true: guest THRE was `com2_lsr` bit 5 **and** an empty ring, evaluated only at VM exits, and with `idle=poll` the only exits during the stall were ~15 preemption ticks a second (`take0` +30 per 2 s) — each drained one byte because the drain polled LSR before every byte. Our own per-hit `virtio MMIO gpa=` lines and SysRq printk filled the 2 KiB ring (`ring=2048`, drop-oldest → shredded SOL). Net ~8–240 B/s into apk's tty; its 4 KiB xmit never emptied. Not virtio, not the timer: the console-throughput model. **This commit changes that model and nothing else (host-side, ADR-016):** (1) **guest UART TX ring room** — guest LSR THRE/TEMT and the THRE IRQ level are `ring room ≥ GUEST_TX_RESERVE(1024) + 16`; ring 4 KiB; host COM2 LSR no longer gates the guest (`serial::guest_tx_room_has_thre`). (2) **guest UART line-rate pace** — while the ring holds bytes, `arm_preempt_on_resume` clamps the VMX preemption timer to `UART_PACE_US=250` (`IA32_VMX_MISC` bits 4:0 read at launch; TSC calibrated pre-EBS against `Stall` unconditionally); paced ticks drain `GUEST_TX_DRAIN_PACE=16` and run the normal inject path but step no HPET, raise no PIT (`tick_hpet_on_exit` / `linux_raise_overlay_pit_paced` return early), skip the 1 GiB split walk and do not bump `EXIT_COUNT`; the nominal tick keeps its deadline (`guest_uefi_preempt_arm`). (3) **guest UART COM2 FIFO burst** — `init_port(COM2)` reads IIR after FCR 0xC7; bits 7:6 set ⇒ 16 bytes per THRE=1 window in `drain_guest_tx` (Linux `tx_loadsz`); `0xFF`/no-FIFO stays 1. (4) **virtio MMIO kick throttle** — overlay kick lines ≤ 1 per `VIRTIO_MMIO_KICK_LINE_MIN_TSC` (~250 ms); every-64 heartbeat unchanged. Heartbeat `thre` line gains `room= pace= fifo= drained= win=`; launch prints `preemption timer … shift= pace_ticks=`; first clamp prints `guest UART line-rate pace ticks= fifo=`. Host tests: ring-room level + reassert after drain, FIFO window bursts, IIR FIFO detect, pace/arm helpers, kick throttle (681 pass). Runbook: `916af96` reading + post-fix heartbeat table. |
| Everest impact | months 0.75 held; overall 94 held; ETA 2026-10 held. No DONE evidence; no scope growth. Confidence in the exact THRE link dropped, so this is a measurement pin, not a stimulus pin. Timer/ICR gap (`local IPI: TIMEOUT`) still not implicated in the stall. |
| Gates touched | none. Host `cargo test --no-default-features -- --test-threads=1` (new: `thre_chain_counts_every_link`, `pic_master_snap_counts_irq0_and_irq4_takes`); `./tools/build.sh` (banner shows `build: sha=`). UART THRE chain telemetry. build sha banner. |
| Months Δ | 0.75→0.75 |
