...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (nested-OK closed; iron 1–3 host-ready; Force Off open)
...
| Field | Value |
|-------|-------|
| Commit | m8-xhci-desc-retry |
| Summary | **Iron leftover, not persist.** Isolation COM2 `06ca0f95`: ISO `vdb` `last_st=0x0`, leftover Everest `ISO-INSTALL-OK` → F7 `DISK-BOOT-OK` → `login:` — **ISO isolation proven, not persist.** USB LUN missed: p11 GET_DESC `cmd=4 cmpl=0`, p14 hub `1604:10c0` class 09 `err=4` hid the DESC fail. This EFI: GET_DESC retry + EP0 reset; skip hubs (`err=9`); hub skip does not overwrite an earlier DESC fail. Toshiba attach still open. Months **0.0 held**. Overall **99 held**. |
| Everest impact | months **0.0 held**; overall **99 held**; ETA 2026-09 held. Not 100%. No persist gate. |
| Gates touched | `xhci.rs` GET_DESC retry + hub skip + [m8_persist_iron.md](runbooks/m8_persist_iron.md). `./tools/sync-hda-site.sh --check`. |
