...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; firstcbw COM2 `scsi=inquiry` leftover DRAM; iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (nested-OK closed; firstcbw `scsi=inquiry` leftover; Force Off open)
...
| Summary | **Iron leftover, not persist.** Firstcbw COM2: `xhci firstcbw` then INQUIRY CBW `cmpl=0xff` leftover 1 GiB. Rearm before unused CBW did not retire a Transfer Event. This EFI: `norearm` + long wait. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | `xhci.rs` / `usb_bot.rs` firstcbw norearm; [m8_persist_iron.md](runbooks/m8_persist_iron.md); [loihda.md](loihda.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
