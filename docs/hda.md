...
| M+5 | 2026-10 | **M8.0** persist (file nested / LUN iron / leftover fallback) | `RAYNU-V-M8-DISK-PERSIST-OK` on COM2 | **NEXT** (nested-OK closed; iron 1–3 host-ready; firstread COM2 `scsi=inquiry` leftover DRAM; iron Force Off open) |
...
2026-10 ░░░░░░░░  M8.0 persist (nested-OK closed; firstread `scsi=inquiry` leftover; Force Off open)
...
| Field | Value |
|-------|-------|
| Commit | m8-usb-bot-firstcbw |
| Summary | **Iron leftover, not persist.** Firstread COM2: `epst`/`botrst`/`maxlun` then INQUIRY CBW `cmpl=0xff` leftover 1 GiB. No `xhci firstread` (prepare was after CAPACITY). This EFI: `prepare_first_cbw` before INQUIRY. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | `xhci.rs` / `usb_bot.rs` first-CBW prepare; [m8_persist_iron.md](runbooks/m8_persist_iron.md); [loihda.md](loihda.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
...
Next move:     **M8.0 iron** COM2 `usb I/O ready` without `read-fail` + Alpine `vda` 298 GiB, then Force Off / `RAYNU-V-M8-DISK-PERSIST-OK`. Firstread COM2 INQUIRY CBW leftover DRAM is **not** persist. Nested-OK closed on `raynuvsrv1` `ce3d8a09`.
