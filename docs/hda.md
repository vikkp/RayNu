...
| Guest UEFI firmware blob | PARTIAL (RayNu-F nested F7 reboot-to-disk; iron E5 open) | Iron UDisk `6d0c58a` / run `34292570282`: stall dump INTx lived then dump/ISR flood (`n=+2` no `0x300`). virtio stall dump notify reset. Do not F11 `34292570282`. Nested F7 reboot-to-disk remains `fe4785a` (not iron / not Everest E5). Operator: [r640_f7_iso_iron.md](runbooks/r640_f7_iso_iron.md). |
...
| M+4 | 2026-09 | TLS/console + distro installer | remaining Everest | **ETA** (iron `6d0c58a` / `34292570282` stall dump INTx lived then dump/ISR flood; virtio stall dump notify reset; `ISO-INSTALL-OK` open) |
...
| Summary | **Iron COM2 (UDisk, `6d0c58a` / run `34292570282`):** stall dump INTx lived then dump/ISR flood (`n=+2` ISR ACK re-armed dump). Do not wait. This pin re-arms dump only on notify (`0x300`). Do not F11 `34292570282`. Host-tested; not ISO-INSTALL-OK. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Iron apk overlay still open; do not drop months. |
| Gates touched | none. Host `cargo test --no-default-features -- --test-threads=1` (672 passed). virtio stall dump notify reset. |
| Months Δ | 0.5→0.5 |
