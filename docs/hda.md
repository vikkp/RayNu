...
| Summary | **Iron COM2 (UDisk, `c61942b` / run `34135448354`):** paced PIT lived (`DRIVER_OK` + `linux PIT resume paced`, no `soft lockup`); apk froze at virtio `n=1345` with **no** `linux PIC IRQ0` (`MADT or MP tables are not detected`). Linux PIC despite leftover firmware GSI 2. Do not F11 `34135448354`. Host-tested; not ISO-INSTALL-OK. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Iron apk overlay still open; do not drop months. |
| Gates touched | none. Host test: leftover GSI 2 does not count as Linux-programmed pin 2. |
| Months Δ | 0.5→0.5 |
