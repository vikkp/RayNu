...
2026-10 ░░░░░░░░  M8.0 persist (nested-OK closed; CAPACITY CBW `err=8`; Force Off open)
...
| Summary | **Iron leftover, not persist.** Maxlun COM2: Toshiba + eval + BOT parse + SET_CONFIG + GET_MAX_LUN `val=0` then INQUIRY CBW `err=8 cmpl=0xff` leftover 1 GiB. Enum lived. This EFI: CONFIG_EP copies Output Slot+A1 live EP0; `xhci epst`; MSC BOT reset + Clear Halt before INQUIRY. Months **0.0 held**. Overall **99 held**. |
...
| Gates touched | `xhci.rs` CONFIG_EP copy + epst + BOT reset; [m8_persist_iron.md](runbooks/m8_persist_iron.md); [loihda.md](loihda.md). `./tools/sync-hda-site.sh --check`. `./tools/sync-loihda-site.sh --check`. |
...
Next move:     **M8.0 iron** COM2 `usb I/O ready` without `read-fail` + Alpine `vda` 298 GiB, then Force Off / `RAYNU-V-M8-DISK-PERSIST-OK`. Ep0-eval SET_CONFIG then CAPACITY CBW leftover DRAM is **not** persist. Nested-OK closed on `raynuvsrv1` `ce3d8a09`.
