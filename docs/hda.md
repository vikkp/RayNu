...
| Guest UEFI firmware blob | PARTIAL (RayNu-F nested F7 reboot-to-disk; iron E5 open) | Iron UDisk `69102aa` / run `34080595540`: MADT/FPU/`Freeing initrd`/Alpine Init live, then `virtio_pci` probe `vp_set_status`/`iowrite8` IRQ0 storm (`soft lockup` 26s/52s/78s) then I/O `reason=0x1e` hold. Raise PIT on resume only after both DRIVER_OK (overlay); MMIO/HLT/preempt still raise during probe. Do not F11 `34080595540`. Prior `c815ccc` / `34078335291` died at MADT. Nested F7 reboot-to-disk remains `fe4785a` (not iron / not Everest E5). Operator: [r640_f7_iso_iron.md](runbooks/r640_f7_iso_iron.md). |
...
| M+4 | 2026-09 | TLS/console + distro installer | remaining Everest | **ETA** (iron `69102aa` / `34080595540` virtio_pci_probe soft lockup; raise-on-resume after DRIVER_OK; `ISO-INSTALL-OK` open) |
...
| Summary | **Iron COM2 (UDisk, `69102aa` / run `34080595540`):** MADT/FPU/`Freeing initrd`/Alpine Init live; then `virtio_pci` `vp_set_status`/`iowrite8` IRQ0 storm (`soft lockup` 26s/52s/78s) then I/O `reason=0x1e` hold. Raise PIT on resume only after both DRIVER_OK. Do not F11 `34080595540`. Host-tested; not ISO-INSTALL-OK. |
...
| Gates touched | none. Host test: raise-on-resume false during virtio probe. |
| Months Δ | 0.5→0.5 |
