...
| M+4 | 2026-09 | TLS/console + distro installer | remaining Everest | **ETA** (iron `896424f` / `34131274237` virtblk_probe soft lockup; paced resume PIT; `ISO-INSTALL-OK` open) |
...
| Summary | **Iron COM2 (UDisk, `9652262` / run `34149809660`):** `linux PIC IRQ0` + `linux virtio PIC 11` + `Mounting boot media: ok.` then apk freeze at virtio `n=1345` after disk ISR ACK. Drain now raises INTx on virtio FLUSH (0 OUT bytes). Do not F11 `34149809660`. Host-tested; not ISO-INSTALL-OK. |
| Everest impact | months 0.5 held; overall 95 held; ETA 2026-09 held. Iron apk overlay still open; do not drop months. |
| Gates touched | none. Host `cargo test --no-default-features -- --test-threads=1` → 663 passed, 1 ignored. FLUSH without notify still latches ISR + PIC 11. |
| Months Δ | 0.5→0.5 |
