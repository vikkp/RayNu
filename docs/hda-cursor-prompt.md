...
Public tracker: [`site/hda.html`](../site/hda.html) (fed by [`site/hda.json`](../site/hda.json)).
...
3. Update docs/hda.md in this same commit:
   - YAML frontmatter (last_updated, last_commit, months_to_everest_prev → months_to_everest, overall_pct, everest_eta_month, summit_*_pct)
...
5. Sync the public site tracker (required):
   ./tools/sync-hda-site.sh
   ./tools/sync-hda-site.sh --check
   Stage site/hda.json with docs/hda.md.
6. Honesty locks:
   - Latitude/QEMU ≠ R640
...
Run ./tools/sync-hda-site.sh so site/hda.json stays fresh.
...
```text
Open docs/hda.md scoreboard only. Reply with: overall %, months_to_everest, ETA month, next P0, top blocker. Then offer to full-refresh HDA + site sync.
```
...
```text
We shipped faster than the timeline. Re-estimate docs/hda.md residuals for open P0 items using actual commits/gates since baseline_date. Shrink months_to_everest and pull everest_eta_month earlier. Cite evidence in changelog. Run ./tools/sync-hda-site.sh. Do not invent closed R640/ISO/UI work.
```
...
```text
We are blocked on <reason>. Update docs/hda.md blockers, raise months_to_everest with velocity_factor ≥ 1.3, push everest_eta_month, note slip in changelog, run ./tools/sync-hda-site.sh.
```
...
```text
RayNu-V: On every commit to this repo, update docs/hda.md per .cursor/rules/hda-update.mdc, then run ./tools/sync-hda-site.sh so site/hda.html stays fresh. Mount Everest = EFI ship + real R640 + network UI + Linux ISO deploy + M6.9. Keep the month timeline honest; faster Everest-path work pulls ETA closer.
```
