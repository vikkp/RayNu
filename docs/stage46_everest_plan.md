...
| F2b | **IN PROGRESS** (wired, unproven) | Long-mode launch on the stopped private VMCS (no new allocations) behind ESP `raynuf.txt`; CI QEMU stages it (`RAYNU_F=1`, observational). Closes on a serial `RAYNU-V-RAYNU-F-CONOUT-OK` (guest VM-exit only; never host/CI). Runs `33826341793` / `33826787787` landed on GitHub runners **without nested VT-x** (`VMXON-SKIP`) — flag probe proven (`RayNu-F launch requested`), launch not exercised. Deterministic nested proof: `RAYNU_F=1 REQUIRE_VMX=1 QEMU_ACCEL=kvm ./tools/qemu-boot-test.sh` on `raynuvsrv1` with the CI EFI at `target/x86_64-unknown-uefi/release/`. |
