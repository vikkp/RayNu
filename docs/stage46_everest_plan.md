...
| 3m | **IN PROGRESS** | Firmware WFE preempt skip | Skip `endbr64` + `cmp [rip],imm8` after WFE. Print RIP on noskip. |
...
| Right next move | Skip the post-WFE preempt spin (`endbr64` + `cmp [rip],imm8`). Print RIP on noskip. |
| Wrong next moves | dest, HLT policy, COMMAND, CF8/ROM, hide-slot0/PIIX, ConIn, WFE unwind, another ZeroMem EPT RIP, #231 |
...
Firmware left CpuSleep, resumed PciBus CF8, then sat in a preemption
timer spin (`reason=0x34`, not EPT `0x30`). `c8d504d` named the
ZeroMem RIP as an EPT fill; that print never fired. Last COM2:
`preempt noskip same=8 insn=f30f1efa48833d…` then
`reason=0x34 rip=0x7ec8f639`. This #229 HEAD skips that
`endbr64` + `cmp [rip],imm8` after WFE and prints RIP on noskip.
Wait CI, then flash. Do not F11 `c8d504d`. Do not F11 `e0d5c55`.
Do not F11 `0b770cd`. Do not flash `3b1cf51`.

Next proof: `firmware WFE preempt skip len=` and/or noskip **with RIP**,
then `ataio>0` or a new hang. Still not `ISO-INSTALL-OK`. After step 6,
E5 can close. TLS and guest console stay residual; they are not this
ladder.
