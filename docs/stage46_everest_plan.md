...
| F5 | **HOST-PROVEN** (`RAYNU-F-FS-GATE-OK` + `RAYNU-F-IMAGE-GATE-OK`) | FAT12/16/32 reader; `SimpleFileSystem`/`EFI_FILE_PROTOCOL` exposed to guests; `LoadImage` over `GuestMem` with DIR64 relocs + `LoadedImage`; `StartImage` guest redirect. **The launcher now boots the ISO's real `\EFI\BOOT\BOOTX64.EFI`** (El Torito → FAT ESP → stage → load), test app as fallback. Closes on nested `RAYNU-F-START-IMAGE-OK` with GRUB running. |
