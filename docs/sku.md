...
| Network SPA + REST | Coexist `10.99.99.x:8443`. Flashed `4af78b43` CURL NOW is `http://` (**plaintext HTTP**). In-tree TLS 1.2 EFI is `https://` and is not yet on that stick. |
...
- **Iron HTTPS (A4) is open.** In-tree firmware is TLS 1.2 (`Tls12Listen`); rustls cannot join `uefi-bin`. Flashed iron is still **plaintext HTTP** on the wire the CISO can reach.
