...
| `2026-08-15-confirm-rebuild-com2.txt` | `dist/` rebuild (same EFI SHA as xsavesfix) | Confirming retest on Virtual Floppy: SHELL→M4→`VMXOFF` |
...
# Expect markers through RAYNU-V-M4-SMP-OK + boot: VMXOFF ok
```

Closing EFI SHA256 (xsavesfix + confirming rebuild):
