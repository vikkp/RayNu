...
Boot path today (M3.6 in flight): UEFI entry → load → VMLAUNCH → … → proto-init shell → **continuous HLT exit loop** → VMXOFF. Verification: L2 specs + Kani.
...
**M3.6 gate:** `RAYNU-V-M3-LOOP-OK` — survive N HLT resumes after SHELL-OK (fuller GPR save). Then real bzImage (M3.7–M3.10). Plan: [m3_plan.md](m3_plan.md).
