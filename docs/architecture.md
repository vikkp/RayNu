...
Boot path today (M3.9 in flight): UEFI entry → tiny bzImage → earlyprintk → MSR firewall → host LAPIC GTIMER2 → VMXOFF. Verification: L2 specs + Kani.
...
**M3.9 gate:** `RAYNU-V-M3-GTIMER2-OK` — MSR allow-list emulate + post-earlyprintk host LAPIC. Plan: [m3_plan.md](m3_plan.md).
