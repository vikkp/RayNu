...
Boot path today (M3.10 in flight): UEFI entry → tiny bzImage + initrd → earlyprintk → GTIMER2 → real `/init` SHELL → VMXOFF. Verification: L2 specs + Kani.
...
**M3.10 gate:** `RAYNU-V-M3-SHELL-OK` from real `/init` on initrd. Plan: [m3_plan.md](m3_plan.md).
