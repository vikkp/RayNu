...
| E5 Stage 44 | `RAYNU-V-M7-E5-OVMF-ATAPI-OK` | **OPEN** host: iron COM2 `d5f9431` DXE then n=8192 `reason=0x34` `rip=0x6e81ca` (CpuDeadLoop). Preempt skip includes GCC `eb fc` / `0F 84` rel32 (`e2af81e` missed those). Nested VT-x `2674629` BOTH-OK then ACPI BdsWait; fw_cfg `etc/boot-menu-wait` 0 ms. Not El Torito boot. Not installer (2026-08-23). |
...
**P0-59 / E5 Stage 44 open (host):** firmware ATAPI READ (`RAYNU-V-M7-E5-OVMF-ATAPI-OK`). Iron COM2 `d5f9431`: `#UD` gone, DXE, then n=1280..8192 `reason=0x34` `rip=0x6e81ca` (CpuDeadLoop; no BOTH-OK). `e2af81e` skipped only `pause`/`jcc rel8`/`eb f3`/`eb fe`; GCC is often `eb fc` or `0F 84` rel32. Nested VT-x `2674629` BOTH-OK then ACPI BdsWait; fw_cfg `etc/boot-menu-wait` 0 ms. Not El Torito boot. Not installer. Not Everest E5.  
...
| P0-59 / E5 Stage 44 | `RAYNU-V-M7-E5-OVMF-ATAPI-OK` | **OPEN (host).** Iron COM2 `d5f9431` `rip=0x6e81ca` CpuDeadLoop. Preempt `eb fc` / `0F 84` skip. Stop on `sectors>0`. Not El Torito boot. Not installer. Not Everest E5. |
