...
**P0-60 closed (iron COM2 after `5147222`, not an E5 stage):** G0 SHELL then G1 `RAYNU-V-M4-SHELL-G1` / `RAYNU-V-M4-2VM-OK` (`EPTP=0x1040601e` CPUID `rip=0x1040000a`); G2/G3 latched; no `GPA=0x10403000`. Stage 45 El Torito held (`RN-ELT` n=197992). **G0 VMCS relocate closed** on later iron COM2 after `b7259c1`/`10e7984`: `E4 G0 VMCS relocated HPA=0x10a00000`; `M4-SLICE-G0`; `M4-NVM-OK`. Residual: M4.3 virtio-blk probe triple-fault `reason=0x02` `guest_code=0xfc0f000` (e820 scribble) — host-slab + fail-soft. Not Stage 46. Not installer. Not Everest E5. Next: M4.3 blk host-slab → Stage 46 `ISO-INSTALL-OK`.  
...
| P0-60 | M4.2 G1 shell EPT / fail-soft | **CLOSED (iron COM2 after `5147222`, not an E5 stage).** G1 `RAYNU-V-M4-SHELL-G1` / `RAYNU-V-M4-2VM-OK` `EPTP=0x1040601e` CPUID `rip=0x1040000a`; G2/G3 latched; no `GPA=0x10403000`. Stage 45 El Torito held (`RN-ELT` n=197992). G0 VMCS relocate **CLOSED** on later iron after `b7259c1`/`10e7984` (`M4-NVM-OK` / `SLICE-G0` / HPA=`0x10a00000`). Next: M4.3 blk probe host-slab (iron `reason=0x02` at `0xfc0f000`). Not Stage 46. |
| P0-62 / E5 Stage 46 | `ISO-INSTALL-OK` | **OPEN.** Linux ISO install to virtio-blk. Everest E5. Waits on Stage 45 + P0-60 + G0 relocate (all closed) and a live M4.3 ladder (host-slab / fail-soft). |
