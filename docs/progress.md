...
**P0-59 / E5 Stage 44 open (host):** firmware ATAPI READ (`RAYNU-V-M7-E5-OVMF-ATAPI-OK`). Iron COM2 `e2af81e`: `insn=ebec` (`jmp rel8` −20). `932e8fc` skips that on preemption. fw_cfg CD path is master `drive@0` (not slave `drive@1`) so BDS can Connect ATAPI after BOTH-OK. Nested VT-x `2674629` BOTH-OK then ACPI BdsWait; fw_cfg `etc/boot-menu-wait` 0 ms. Not El Torito boot. Not installer. Not Everest E5.  
...
| P0-59 / E5 Stage 44 | `RAYNU-V-M7-E5-OVMF-ATAPI-OK` | **OPEN (host).** Iron `e2af81e` `insn=ebec`. CD bootorder master `drive@0`. Stop on `sectors>0`. Not El Torito boot. Not installer. Not Everest E5. |
