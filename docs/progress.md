...
| E5 Stage 43 | `RAYNU-V-M7-E5-OVMF-BOTH-OK` | **OPEN (host):** firmware-simultaneous virtio `00:00.0` + IDE `00:00.1` on one boot. Nested VT-x `105ffbe`: live HPET + preemption, n=2048 `reason=0x34` `rip=0x6e812d` `pci_ide=0` (10 ms step too small). 1 s HPET step + stop RIP insn dump. Nested VT-x must print both DIDs (`0x1042` and `0x7010`). Still `sectors=0`. Not installer. |
...
**P0-58 / E5 Stage 43 open (host):** firmware-simultaneous PCI enum (`RAYNU-V-M7-E5-OVMF-BOTH-OK`). Virtio stays `00:00.0` (multifunction); IDE is `00:00.1`. Nested VT-x `699c9a6`: n=2048 `pci_ide=0 virtio=1` only `pci select 00:00.00` — `AcpiTimerLibConstructor` rejects DID `0x1042` and `IoRead32(0)` was `0xFFFFFFFF`. ACPI PM timer (port 0 dword + PIIX `0x408`) plus PIIX4 PM `00:01.3` and guest-private i440FX DID remap (hardware DID stays `0x1042`; not two-phase DID). Nested VT-x `1991a27`: remap n=1, `acpi=13`, `dxe=1`, then EPT `gpa=0xffc00000` (VARS gap). 4 MiB flash window + empty VARS `_FVH`. Nested VT-x `20763e4`: VARS mapped, then 300 s kill (HPET sink was zeros). Nested VT-x `105ffbe`: live HPET + preemption, n=2048 `reason=0x34` `rip=0x6e812d` `pci_ide=0` — 10 ms HPET step burned Delay. 1 s step so Delay can end. Stop dumps identity RIP bytes. Nested VT-x must show both DIDs on one serial. Still `sectors=0`. Not installer. Not Everest E5.  
...
| P0-58 / E5 Stage 43 | `RAYNU-V-M7-E5-OVMF-BOTH-OK` | **OPEN (host).** Firmware-simultaneous virtio `00:00.0` + IDE `00:00.1`. Nested VT-x `105ffbe`: live HPET + preemption, n=2048 `reason=0x34` `rip=0x6e812d` `pci_ide=0`. 1 s HPET step + stop RIP insn dump. Still `sectors=0`. Not installer. Not Everest E5. |
