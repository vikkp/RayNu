...
| E5 Stage 44 | `RAYNU-V-M7-E5-OVMF-ATAPI-OK` | **OPEN** host: iron COM2 LIVE-BYTES-OK then #UD RIP `0x109D` `pci_ide=0` `com=15515` (guest-UEFI lacked INVPCID/XSAVES/RDTSCP; XSETBV must execute). Nested VT-x `2674629` BOTH-OK then ACPI BdsWait; fw_cfg `etc/boot-menu-wait` 0 ms. Not El Torito boot. Not installer (2026-08-23). |
...
**P0-59 / E5 Stage 44 open (host):** firmware ATAPI READ (`RAYNU-V-M7-E5-OVMF-ATAPI-OK`). Iron COM2 2026-08-23 (Cruzer with ESP `OVMF.fd`): `LIVE-BYTES-PRESENT-OK`, VMLAUNCH/ALIVE/PAST-SEC/VIRTIO/DXE, then `#UD` RIP `0x109D` ImageBase `0x1000`, stop n=32768 `pci_ide=0` `com=15515` `port=0x3fd` `bdfs=2`. Guest-UEFI VMCS had EPT+unrestricted only; host CPUID (Xeon Silver 4110) advertises INVPCID/XSAVES/RDTSCP — same #UD class as E4 Linux without those secondary bits. XSETBV must execute XCR0 (not `skip_insn`). Nested VT-x `2674629` BOTH-OK then ACPI BdsWait (`acpi=16612`); fw_cfg `etc/boot-menu-wait` 0 ms. Not El Torito boot. Not installer. Not Everest E5.  
...
| P0-59 / E5 Stage 44 | `RAYNU-V-M7-E5-OVMF-ATAPI-OK` | **OPEN (host).** Iron COM2 #UD RIP `0x109D` `pci_ide=0`. Guest-UEFI INVPCID/RDTSCP/XSAVES + XSETBV XCR0. fw_cfg `etc/boot-menu-wait` 0 ms. Stop on `sectors>0`. Not El Torito boot. Not installer. Not Everest E5. |
