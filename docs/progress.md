...
| P0-60 | M4.2 G1 shell EPT / fail-soft | **OPEN (not an E5 stage).** Iron COM2 after Stage 45: G1 `GPA=0x10403000` (slab CR3) then `boot gate failed`. Ladder now uses SPA slab-local 2 MiB EPT + VMCS in the punched slab (not FrameAllocator pages in G0 e820). Shell EPT faults fail-soft (no VMXOFF). Nested KVM resume cap 65536 and withholds report-RAM from E4 so CI SHELL does not reload bzImage onto firmware-scratched HPAs (`0x8200000` / CR2 `0x8a00000`). Not closed until iron G1 does not `boot gate failed`. Not Stage 46. |
