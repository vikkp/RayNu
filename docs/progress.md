...
**After Stage 44 (Stage 45 in progress):** Stage 45 El Torito (`OVMF-ELTORITO-OK`) host package — do not stop on first ATAPI sector; checksummed catalog + FAT12 ESP `\EFI\BOOT\BOOTX64.EFI` writes `RN-ELT` on COM1. Nested VT-x or iron COM2 to close. Then P0-60 G1 EPT (not an E5 stage) → Stage 46 `ISO-INSTALL-OK`.  
