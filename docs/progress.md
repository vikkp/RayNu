...
**P0-58 / E5 Stage 43 open (host):** firmware-simultaneous PCI enum (`RAYNU-V-M7-E5-OVMF-BOTH-OK`). Virtio stays `00:00.0` (multifunction); IDE is `00:00.1`. Nested VT-x `699c9a6`: n=2048 `pci_ide=0 virtio=1` only `pci select 00:00.00` — `AcpiTimerLibConstructor` rejects DID `0x1042` and `IoRead32(0)` was `0xFFFFFFFF`. ACPI PM timer (port 0 dword + PIIX `0x408`) plus PIIX4 PM `00:01.3` and guest-private i440FX DID remap (hardware DID stays `0x1042`; not two-phase DID). Nested VT-x `1991a27`: remap n=1, `acpi=13`, `dxe=1`, then EPT `gpa=0xffc00000` (VARS gap). 4 MiB flash window + empty VARS `_FVH`. Nested VT-x must show both DIDs on one serial. Still `sectors=0`. Not installer. Not Everest E5.  
