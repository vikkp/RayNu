...
**P0-60 closed (iron COM2 after `5147222`, not an E5 stage):** G0 SHELL then G1 `RAYNU-V-M4-SHELL-G1` / `RAYNU-V-M4-2VM-OK` (`EPTP=0x1040601e` CPUID `rip=0x1040000a`); G2/G3 latched; no `GPA=0x10403000`. Stage 45 El Torito held (`RN-ELT` n=197992). Residual: credit scheduler `VMPTRLD` G0 `0xfc38000` `rev=0` error 11 — relocate G0 VMCS to host slab before G1. Not Stage 46. Not installer. Not Everest E5. Next: G0 VMCS relocate → Stage 46 `ISO-INSTALL-OK`.  
