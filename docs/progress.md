...
| E5 Stage 44 | `RAYNU-V-M7-E5-OVMF-ATAPI-OK` | **OPEN** host: iron `aee545f` DXE assert skip then `#UD` `0x109d`. Revert skip. MTRR power-on `E=0` VCNT=8. Not installer (2026-08-23). |
...
**P0-59 / E5 Stage 44 open (host):** firmware ATAPI READ (`RAYNU-V-M7-E5-OVMF-ATAPI-OK`). Iron COM2 `aee545f`: `DXE assert skip caller=0x1d25193` then `#UD` `linear=0x109d` stop n=5364 `sectors=0` (same as `891eb5b`). Revert iron `eb ec` skip. MTRR power-on `E=0` VCNT=8, no UC hole. Not El Torito boot. Not installer. Not Everest E5.  
...
| P0-59 / E5 Stage 44 | `RAYNU-V-M7-E5-OVMF-ATAPI-OK` | **OPEN (host).** Iron `aee545f` DXE skip then `#UD` `0x109d`. Revert skip. MTRR power-on `E=0` VCNT=8. Stop on `sectors>0`. Not El Torito boot. Not installer. Not Everest E5. |
