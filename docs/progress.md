...
| E5 Stage 45 | `RAYNU-V-M7-E5-OVMF-ELTORITO-OK` | **OPEN** (in progress). Host package: keep VMCS after first ATAPI sector; checksummed catalog; 2048-byte FAT12 ESP + ISO9660 `\EFI\BOOT\BOOTX64.EFI`; catalog+load READ + `RN-ELT`; PE SectionAlignment `0x1000` for DxeCore ProtectUefiImage; do not apply the 32768 post-ATAPI tail after PACKET (first sector is often LBA 0 dummy); 262144-exit cap after iron+nested hit the 131072-exit cap. Iron COM2 `df7d158`: ATAPI-OK n=30769 then catalog=1 bootimg=1 elt=0 stop n=131072 (512-byte BPB / empty PVD root; not ELTORITO-OK). Nested VT-x or iron COM2 to close. Not installer (2026-08-27). |
...
**After Stage 44 (Stage 45 in progress):** Stage 45 El Torito (`OVMF-ELTORITO-OK`) host package — do not stop on first ATAPI sector; do not apply the 32768 post-ATAPI tail after PACKET (first sector is often LBA 0 dummy); checksummed catalog + 2048-byte FAT12 ESP + ISO9660 `\EFI\BOOT\BOOTX64.EFI` writes `RN-ELT` on COM1; PE SectionAlignment `0x1000` for DxeCore ProtectUefiImage. Iron COM2 `df7d158`: ATAPI-OK n=30769 then catalog=1 bootimg=1 elt=0 stop n=131072 (131072-exit cap; 512-byte BPB / empty PVD root; not ELTORITO-OK). Hard resume 262144. Nested VT-x or iron COM2 to close. Then P0-60 G1 EPT (not an E5 stage) → Stage 46 `ISO-INSTALL-OK`.  
...
| P0-61 / E5 Stage 45 | `RAYNU-V-M7-E5-OVMF-ELTORITO-OK` | **OPEN (in progress).** Host package: do not stop on first ATAPI sector; checksummed catalog; 2048-byte FAT12 ESP + ISO9660 `\EFI\BOOT\BOOTX64.EFI`; catalog+load READ + `RN-ELT`; PE SectionAlignment `0x1000` for DxeCore ProtectUefiImage; do not apply the 32768 post-ATAPI tail after PACKET; 262144-exit cap after iron+nested hit the 131072-exit cap. Iron COM2 `df7d158`: catalog=1 bootimg=1 elt=0 stop n=131072 (512-byte BPB / empty PVD root; not ELTORITO-OK). Nested VT-x or iron COM2 to close. Not installer. Not Everest E5. Work order: 45 → P0-60 → 46. |
