...
**P0-58 / E5 Stage 43 open (host):** firmware-simultaneous PCI enum (`RAYNU-V-M7-E5-OVMF-BOTH-OK`). Virtio stays `00:00.0` (multifunction); IDE is `00:00.1`. Nested VT-x `707a849`: 1s HPET + RIP dump, n=2048 `rip=0x6e812d insn=ebf3` `pci_ide=0` — unconditional backward `jmp rel8` (CpuDeadLoop), not Delay. Skip that spin so firmware can fall through to PciBus. Nested VT-x must show both DIDs on one serial. Still `sectors=0`. Not installer. Not Everest E5.  
...
| P0-58 / E5 Stage 43 | `RAYNU-V-M7-E5-OVMF-BOTH-OK` | **OPEN (host).** Firmware-simultaneous virtio `00:00.0` + IDE `00:00.1`. Nested VT-x `707a849`: n=2048 `rip=0x6e812d insn=ebf3` `pci_ide=0`. Spin-jmp skip. Still `sectors=0`. Not installer. Not Everest E5. |
