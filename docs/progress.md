...
| E5 Stage 43 | `RAYNU-V-M7-E5-OVMF-BOTH-OK` | **OPEN (host):** firmware-simultaneous virtio `00:00.0` + IDE `00:00.1` on one boot. Virtio-alone no longer stops DXE. 2048-exit cap (`699c9a6` n=2048 still only `00:00.0`). ACPI PM timer + PIIX4 PM `00:01.3` + guest-private i440FX DID remap so PEI can leave the virtio DID loop. HLT skip + CR-access resume. Nested VT-x must print both DIDs (`0x1042` and `0x7010`). Still `sectors=0`. Not installer. |
...
**P0-58 / E5 Stage 43 open (host):** firmware-simultaneous PCI enum (`RAYNU-V-M7-E5-OVMF-BOTH-OK`). Virtio stays `00:00.0` (multifunction); IDE is `00:00.1`. Nested VT-x `699c9a6`: n=2048 `pci_ide=0 virtio=1` only `pci select 00:00.00` — `AcpiTimerLibConstructor` rejects DID `0x1042` and `IoRead32(0)` was `0xFFFFFFFF`. ACPI PM timer (port 0 dword + PIIX `0x408`) plus PIIX4 PM `00:01.3` and guest-private i440FX DID remap (hardware DID stays `0x1042`; not two-phase DID). HLT skip + CR-access resume. Nested VT-x must show both DIDs on one serial. Still `sectors=0`. Not installer. Not Everest E5.  
...
| P0-58 / E5 Stage 43 | `RAYNU-V-M7-E5-OVMF-BOTH-OK` | **OPEN (host).** Firmware-simultaneous virtio `00:00.0` + IDE `00:00.1` on one boot. ACPI PM timer + PIIX4 PM `00:01.3` + guest-private i440FX DID remap after `699c9a6` n=2048 `pci_ide=0`. Nested VT-x must print both DIDs. Still `sectors=0`. Not installer. Not Everest E5. |
