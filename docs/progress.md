...
| E5 Stage 44 | `RAYNU-V-M7-E5-OVMF-ATAPI-OK` | **OPEN** host: PIIX3 ISA PIRQ after nested VT-x `8e55abf` `cf8=0x80000838` ISA `00:01.0:0x38`. Iron COM2 2026-08-23 skipped guest-UEFI (`bytes_present` false; Cruzer lacked `EFI/RayNu/OVMF.fd`). Flash now stages host OVMF. Not El Torito boot. Not installer (2026-08-23). |
...
**P0-59 / E5 Stage 44 open (host):** firmware ATAPI READ (`RAYNU-V-M7-E5-OVMF-ATAPI-OK`). Stop predicate is honest `sectors>0`. Nested VT-x `8e55abf` BOTH-OK then n=2048 `ata=0x0` `unh=0` `cf8=0x80000838` (PIIX ISA `00:01.0` offset `0x38` — PciBus programming, not empty-slot scan). PIIX3 ISA PIRQ `0x60-0x63` reset `0x80` (QEMU) so IRQ assign is not IRQ0. 8192-exit cap. Iron COM2 2026-08-23: VMXON then immediate `M1.1 complete` with no `LIVE-BYTES-PRESENT-OK` — Cruzer `RAYNUV` had no `\EFI\RayNu\OVMF.fd` (`flash-cruzer-esp.sh` copied BOOTX64 only). That paste is E4 + HOST-NIC, not Stage 44. Flash now stages a host 1–4 MiB `_FVH` `OVMF.fd`; missing file prints COM2. Not El Torito boot. Not installer. Not Everest E5.  
...
| P0-59 / E5 Stage 44 | `RAYNU-V-M7-E5-OVMF-ATAPI-OK` | **OPEN (host).** Nested VT-x `8e55abf` n=2048 `cf8=0x80000838` ISA `00:01.0:0x38`. PIIX PIRQ `0x80`. 8192-exit cap. Iron Cruzer lacked `EFI/RayNu/OVMF.fd` (COM2 2026-08-23 skipped guest-UEFI). Flash now stages host OVMF. Stop on `sectors>0`. Not El Torito boot. Not installer. Not Everest E5. |
| Everest residual | firmware El Torito CD boot + TLS/console + distro installer | After P0-59 (`sectors>0`). Product ISO: [ADR-014](adr/ADR-014.md). |
