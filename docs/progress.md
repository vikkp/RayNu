...
| E5 Stage 44 | `RAYNU-V-M7-E5-OVMF-ATAPI-OK` | **OPEN** host: nested VT-x `2674629` BOTH-OK then n=32768 `ataio=0` `acpi=16612` `port=0` `hpet=10` (BdsWait on 18ms ACPI). ACPI PM 1s step. Not El Torito boot. Not installer (2026-08-23). |
...
**P0-59 / E5 Stage 44 open (host):** firmware ATAPI READ (`RAYNU-V-M7-E5-OVMF-ATAPI-OK`). Nested VT-x `2674629` (push QEMU, real VMXON): BOTH-OK then stop n=32768 `ataio=0` `packet=0` `acpi=16612` `port=0` `hpet=10` `unh=0` (`in eax,dx` at `rip=0x6fb153`). Two-speed HPET + 8042 + 32768 let PciBus finish; firmware then burned the cap on `InternalAcpiDelay` (64 Ki PM step ≈ 18 ms; 1 s BdsWait ≈ 55 INs). ACPI PM step is now ≥ 1 s of PM ticks. Iron COM2 2026-08-23 skipped guest-UEFI (Cruzer lacked `OVMF.fd`). Not El Torito boot. Not installer. Not Everest E5.  
...
| P0-59 / E5 Stage 44 | `RAYNU-V-M7-E5-OVMF-ATAPI-OK` | **OPEN (host).** Nested VT-x `2674629` n=32768 `ataio=0` `acpi=16612` `port=0`. ACPI PM 1s step. Stop on `sectors>0`. Not El Torito boot. Not installer. Not Everest E5. |
