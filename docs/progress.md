...
| E5 Stage 44 | `RAYNU-V-M7-E5-OVMF-ATAPI-OK` | **OPEN** host: nested VT-x `5d9e346` BOTH-OK then n=8192 `ataio=0` `unh=3` `port=0xcf8` (empty-slot walk + KBC). HPET 1s only on preemption/HLT; 8042; 32768-exit cap. Iron COM2 skipped guest-UEFI (no ESP OVMF.fd). Not El Torito boot. Not installer (2026-08-23). |
...
**P0-59 / E5 Stage 44 open (host):** firmware ATAPI READ (`RAYNU-V-M7-E5-OVMF-ATAPI-OK`). Nested VT-x `5d9e346` (push QEMU, real VMXON): BOTH-OK then stop n=8192 `ataio=0` `packet=0` `unh=3` `port=0xcf8` after walking `00:02`–`00:1f` and probing KBC `0x60`/`0x64`. PIRQ `0x80` let PciBus past ISA `0x38`. 1 s HPET per PCI I/O jumped guest time ~2 h before AtaAtapiPassThru Start; now 1 s only on preemption/HLT/HPET-EPT. 8042 status `0x10`. 32768-exit cap. Iron COM2 2026-08-23 skipped guest-UEFI (Cruzer lacked `OVMF.fd`). Not El Torito boot. Not installer. Not Everest E5.  
...
| P0-59 / E5 Stage 44 | `RAYNU-V-M7-E5-OVMF-ATAPI-OK` | **OPEN (host).** Nested VT-x `5d9e346` n=8192 `ataio=0` `port=0xcf8`. HPET 1s on preemption only; 8042; 32768-exit cap. Stop on `sectors>0`. Not El Torito boot. Not installer. Not Everest E5. |
