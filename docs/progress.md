...
**P0-59 / E5 Stage 44 open (host):** firmware ATAPI READ (`RAYNU-V-M7-E5-OVMF-ATAPI-OK`). Iron COM2 `d5f9431` (Cruzer ESP OVMF): `LIVE-BYTES-PRESENT-OK`, VMLAUNCH/ALIVE/PAST-SEC/VIRTIO/DXE, **no** `#UD` at `0x109D` (INVPCID/XSAVES/RDTSCP bits). Then tick n=1280 `reason=0x34` `rip=0x6e81ca` (preemption; paste had no `stop n=` / no BOTH-OK). Prior iron paste `#UD` RIP `0x109D` `pci_ide=0` `com=15515`. Nested VT-x `2674629` BOTH-OK then ACPI BdsWait (`acpi=16612`); fw_cfg `etc/boot-menu-wait` 0 ms. Not El Torito boot. Not installer. Not Everest E5.  
