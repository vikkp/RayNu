...
**After Stage 44 (Stage 45 in progress):** Stage 45 El Torito (`OVMF-ELTORITO-OK`) host package — do not stop on first ATAPI sector; do not apply the 32768 post-ATAPI tail after PACKET (first sector is often LBA 0 dummy); checksummed catalog + FAT12 ESP `\EFI\BOOT\BOOTX64.EFI` writes `RN-ELT` on COM1; PE SectionAlignment `0x1000` for DxeCore ProtectUefiImage. Iron COM2 `df7d158`: ATAPI-OK n=30769 then catalog=1 bootimg=1 elt=0 stop n=131072 (131072-exit cap; not ELTORITO-OK). Hard resume 262144. Nested VT-x or iron COM2 to close. Then P0-60 G1 EPT (not an E5 stage) → Stage 46 `ISO-INSTALL-OK`.  
