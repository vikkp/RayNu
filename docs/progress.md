...
| E5 Stage 45 | `RAYNU-V-M7-E5-OVMF-ELTORITO-OK` | **OPEN** (in progress). Host package: keep VMCS after first ATAPI sector; PE32+ CD EFI; catalog+load READ + `RN-ELT` COM magic. Nested VT-x or iron COM2 to close. Not installer (2026-08-27). |
...
**After Stage 44 (Stage 45 in progress):** Stage 45 El Torito (`OVMF-ELTORITO-OK`) host package — do not stop on first ATAPI sector; PE32+ CD EFI writes `RN-ELT` on COM1. Nested VT-x or iron COM2 to close. Then P0-60 G1 EPT (not an E5 stage) → Stage 46 `ISO-INSTALL-OK`.  
...
| P0-61 / E5 Stage 45 | `RAYNU-V-M7-E5-OVMF-ELTORITO-OK` | **OPEN (in progress).** Host package: do not stop on first ATAPI sector; PE32+ CD EFI; catalog+load READ + `RN-ELT`. Nested VT-x or iron COM2 to close. Not installer. Not Everest E5. Work order: 45 → P0-60 → 46. |
