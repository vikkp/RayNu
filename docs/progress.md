...
| E5 Stage 42 | `RAYNU-V-M7-E5-OVMF-VIRTIO-OK` | Empty PCI virtio-blk at `00:00.0` (PEI DID probe) + fw_cfg bootorder CD then disk; not installer (2026-08-23). |
...
**P0-57 / E5 Stage 42 (host + QEMU):** empty virtio-blk + boot order CD then disk (`RAYNU-V-M7-E5-OVMF-VIRTIO-OK`). Nested VT-x: PEI only `inw` DID of `00:00.0` (`0x7010` then `0x1237` both looped; `virtio=0`). That slot is virtio `1AF4:1042`; IDE is `00:00.1` / GuestVisible; fw_cfg `bootorder` is CD then disk. Not a completed firmware CD boot. Not installer. Not Everest E5.  
...
| P0-57 / E5 Stage 42 | `RAYNU-V-M7-E5-OVMF-VIRTIO-OK` | **Host + QEMU.** Empty PCI virtio-blk at `00:00.0` (PEI DID probe) + fw_cfg bootorder CD then disk. Not a completed firmware CD boot. Not installer. Not Everest E5. |
