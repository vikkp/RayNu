...
live-present / live-admit / live-read fixture.

**Closed host:** Stage 0–26 as before · Stage 27 `RAYNU-V-M7-E5-LIVE-READ-OK`
(`read_ovmf_live_esp` + `POST /fw/live-read`;
`try_vmlaunch_guest_uefi_ovmf` → `LiveEspReadAbsent`).
`attach_cdrom_uefi` stays `UnsupportedOnFirmware`. Stage 27 **reads**
...
**Next after Stage 27:** real ESP `OVMF.fd` presence **or** TLS/console polish.
Product ISO is
