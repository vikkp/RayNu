...
live-present / live-admit fixture.

**Closed host:** Stage 0–25 as before · Stage 26 `RAYNU-V-M7-E5-LIVE-ADMIT-OK`
(`admit_ovmf_live_esp` + `POST /fw/live-admit`;
`try_vmlaunch_guest_uefi_ovmf` → `LiveEspAdmitAbsent`).
`attach_cdrom_uefi` stays `UnsupportedOnFirmware`. Stage 26 **admits**
...
**Next after Stage 26:** real ESP `OVMF.fd` presence **or** TLS/console polish.
Product ISO is
