...
**Status:** **M7.5 + M7.6 + M7.7 stamp-persist + M7.8 / E3b + ADR-013 Stage 1 (Phases 0–G) + E4 SPA VMLAUNCH (P0-14) + E5 Stage 0–44 closed**. Stage 44 / P0-59 ATAPI closed on iron COM2 `bf696ca` (`OVMF-ATAPI-OK` `sectors=1` `packet=9` `scsi=0x28` stop n=30769). Stage 45 / P0-61 El Torito **OPEN** (host package in progress). Phase G is the accepted-risk note (shared LOM). **P0-15**–**P0-59** are closed. Residual: Stage 45 El Torito close on nested VT-x or iron COM2 → P0-60 G1 EPT (not an E5 stage) → Stage 46 `ISO-INSTALL-OK`, plus TLS/console. Optional: `VMRESUME` instead of VMLAUNCH-every-quantum.  
...
Iron COM2: ATAPI-OK n=30769 then `eltorito-progress catalog=1 bootimg=0` (Stage 45
kept the VMCS; not `OVMF-ELTORITO-OK`). Close when nested VT-x **or iron COM2**
...
**Next after Stage 44:** Stage 45 El Torito (host package in progress; close on
nested VT-x or iron COM2 `OVMF-ELTORITO-OK`), then P0-60 G1 EPT, then Stage 46
