...
**Status:** **M7.5 + M7.6 + M7.7 stamp-persist + M7.8 / E3b + ADR-013 Stage 1 (Phases 0–G) + E4 SPA VMLAUNCH (P0-14) + E5 Stage 0–41 closed**. Phase G is the accepted-risk note (shared LOM). **P0-15**–**P0-56** are host/QEMU gates. Residual: virtio-blk + boot order CD→disk + TLS/console + distro installer. Optional: `VMRESUME` instead of VMLAUNCH-every-quantum.  
...
**First action (after Stage 41 past-PEI/DXE or CD boot attempt):**
virtio-blk + boot order CD→disk, not another `*Absent` bookkeeping
stage or SPA flag button. Do **not** open another
...
Stage 41 `RAYNU-V-M7-E5-OVMF-DXE-OK` (CMOS/fw_cfg + i440FX at `00:08.0` +
IDE at `00:00.0` for the PEI Device ID probe + EPT sink-resume; post-DXE
tail then E4; past-PEI/DXE or CD boot attempt; not installer).
