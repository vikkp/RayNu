...
**Status:** **M7.5 + M7.6 + M7.7 stamp-persist + M7.8 / E3b + ADR-013 Stage 1 (Phases 0–G) + E4 SPA VMLAUNCH (P0-14) + E5 Stage 0/1/2 (host) closed**. Phase G is the accepted-risk note (shared LOM). **P0-15** boot spec + **P0-16** host CD attach + **P0-17** firmware CD arm are host gates. Residual: guest UEFI VMLAUNCH / OVMF + TLS/console + distro installer. Optional: `VMRESUME` instead of VMLAUNCH-every-quantum.  
...
**First action (E5 Stage 3):** guest UEFI firmware blob sized under ADR-003
(OVMF-style, lazy/zstd) **or** TLS/console polish. Do **not** VMLAUNCH guest
UEFI until the blob is boxed. Do **not** claim Everest E5 / `ISO-INSTALL-OK`.
...
`POST /iso/{id}/firmware`). `attach_cdrom_uefi` stays `UnsupportedOnFirmware`.

**Next after Stage 2:** OVMF-sized guest firmware **or** TLS/console polish.
Product ISO is
