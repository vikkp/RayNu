...
live-present / live-admit / live-read / live-copy fixture.

**Closed host:** Stage 0–27 as before · Stage 28 `RAYNU-V-M7-E5-LIVE-COPY-OK`
(`copy_ovmf_live_esp` + `POST /fw/live-copy`;
`try_vmlaunch_guest_uefi_ovmf` → `LiveEspCopyAbsent`).
`attach_cdrom_uefi` stays `UnsupportedOnFirmware`. Stage 28 **copies**
...
**Next after Stage 28:** real ESP `OVMF.fd` presence **or** TLS/console polish.
Product ISO is
