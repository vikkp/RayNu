...
**Closed host:** Stage 0–30 as before · Stage 31 `RAYNU-V-M7-E5-LIVE-COMMIT-OK`
(`commit_ovmf_live_esp` + `POST /fw/live-commit`;
`try_vmlaunch_guest_uefi_ovmf` → `LiveEspCommitAbsent`).
`attach_cdrom_uefi` stays `UnsupportedOnFirmware`. Stage 31 **commits**
...
**Next after Stage 31:** real ESP `OVMF.fd` presence **or** TLS/console polish.
Product ISO is
