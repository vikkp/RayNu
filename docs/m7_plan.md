...
live-commit / live-latch fixture.

**Closed host:** Stage 0–31 as before · Stage 32 `RAYNU-V-M7-E5-LIVE-LATCH-OK`
(`latch_ovmf_live_esp` + `POST /fw/live-latch`;
`try_vmlaunch_guest_uefi_ovmf` → `LiveEspLatchAbsent`).
`attach_cdrom_uefi` stays `UnsupportedOnFirmware`. Stage 32 **latches**
...
**Next after Stage 32:** real ESP `OVMF.fd` presence **or** TLS/console polish.
Product ISO is
