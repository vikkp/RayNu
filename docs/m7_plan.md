...
**Status:** **M7.5 + M7.6 + M7.7 stamp-persist + M7.8 / E3b + ADR-013 Stage 1 (Phases 0–G) + E4 SPA VMLAUNCH (P0-14) + E5 Stage 0–5 (host) closed**. Phase G is the accepted-risk note (shared LOM). **P0-15**–**P0-20** are host gates. Residual: real EDK2 OVMF bytes on ESP / guest UEFI VMLAUNCH + TLS/console + distro installer. Optional: `VMRESUME` instead of VMLAUNCH-every-quantum.  
...
**First action (E5 Stage 6):** place a real EDK2 `OVMF.fd` on ESP
`\\EFI\\RayNu\\OVMF.fd` (ADR-003 split-mode) **or** TLS/console polish.
Do **not** VMLAUNCH guest UEFI until those bytes exist. Do **not** claim
Everest E5 / `ISO-INSTALL-OK`. `iso=0` E4 SHELL start stays valid.

**Closed host:** Stage 0–4 as before · Stage 5 `RAYNU-V-M7-E5-OVMF-PROBE-OK`
(`probe_ovmf_firmware` + `POST /fw/ovmf` + host mock `_FVH`).
`attach_cdrom_uefi` stays `UnsupportedOnFirmware`. Stage 5 does **not**
embed a 4 MiB EDK2 image.

**Next after Stage 5:** real ESP OVMF.fd **or** TLS/console polish.
Product ISO is
