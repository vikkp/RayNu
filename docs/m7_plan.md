...
**Status:** **M7.5 + M7.6 + M7.7 stamp-persist + M7.8 / E3b + ADR-013 Stage 1 (Phases 0–G) + E4 SPA VMLAUNCH (P0-14) + E5 Stage 0–41 closed**. Phase G is the accepted-risk note (shared LOM). **P0-15**–**P0-57** are host/QEMU gates. Residual: firmware CD boot + TLS/console + distro installer. Optional: `VMRESUME` instead of VMLAUNCH-every-quantum.  
...
**First action (after Stage 42 virtio-blk + CD→disk order):**
firmware CD boot (ATAPI/El Torito read), not another `*Absent` bookkeeping
...
`00:00.0` + fw_cfg bootorder CD then disk; this PEI only probes that
DID; not installer).
