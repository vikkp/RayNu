...
pub enum GuestUefiLaunchError {
    ...
    RealLaunchNotIssued,
    /// Live ESP `\EFI\RayNu\OVMF.fd` bytes are required before the VMLAUNCH
    ...
    /// Live-ESP VMLAUNCH issue path is armed. Live ESP bytes are still
    /// absent. The VMLAUNCH instruction is not issued.
    LiveEspBytesNotPresent,
    /// Live ESP `\EFI\RayNu\OVMF.fd` bytes were probed. They are still
    /// absent. The heap fixture is not those bytes. The VMLAUNCH
    /// instruction is not issued.
    LiveEspBytesAbsent,
    /// A real ESP `\EFI\RayNu\OVMF.fd` is required. The heap fixture
    /// is not that file. The VMLAUNCH instruction is not issued.
    LiveEspFdAbsent,
    /// Real ESP `\EFI\RayNu\OVMF.fd` bytes were presented. They are
    ...
    /// Real ESP `\EFI\RayNu\OVMF.fd` bytes were admitted. They are
    /// still absent in a private guest-UEFI VMCS + EPT. The heap
    /// fixture is not those bytes. The VMLAUNCH instruction is not issued.
    LiveEspAdmitAbsent,
    /// Real ESP `\EFI\RayNu\OVMF.fd` bytes were read-attempted. They are
    /// still absent in a private guest-UEFI VMCS + EPT. The heap
    /// fixture is not those bytes. The VMLAUNCH instruction is not issued.
    LiveEspReadAbsent,
    /// Real ESP `\EFI\RayNu\OVMF.fd` bytes were copy-attempted. They are
    /// still absent in a private guest-UEFI VMCS + EPT. The heap
    /// fixture is not those bytes. The VMLAUNCH instruction is not issued.
    LiveEspCopyAbsent,
    ...
    /// fixture is not those bytes. The VMLAUNCH instruction is not issued.
    LiveEspPlaceAbsent,
    /// Real ESP `\EFI\RayNu\OVMF.fd` bytes were apply-attempted. They are
    /// still absent in a private guest-UEFI VMCS + EPT. The heap
    /// fixture is not those bytes. The VMLAUNCH instruction is not issued.
    LiveEspApplyAbsent,
    /// Real ESP `\EFI\RayNu\OVMF.fd` bytes were commit-attempted. They are
    /// still absent in a private guest-UEFI VMCS + EPT. The heap
    /// fixture is not those bytes. The VMLAUNCH instruction is not issued.
    LiveEspCommitAbsent,
    /// Real ESP `\EFI\RayNu\OVMF.fd` bytes were latch-attempted. They are
    /// still absent in a private guest-UEFI VMCS + EPT. The heap
    /// fixture is not those bytes. The VMLAUNCH instruction is not issued.
    LiveEspLatchAbsent,
    /// Real ESP `\EFI\RayNu\OVMF.fd` bytes were seal-attempted. They are
    /// still absent in a private guest-UEFI VMCS + EPT. The heap
    /// fixture is not those bytes. The VMLAUNCH instruction is not issued.
    LiveEspSealAbsent,
    ...
    /// fixture is not those bytes. The VMLAUNCH instruction is not issued.
    LiveEspLockAbsent,
    /// Real ESP `\EFI\RayNu\OVMF.fd` bytes were hold-attempted. They are
    /// still absent in a private guest-UEFI VMCS + EPT. The heap
    /// fixture is not those bytes. The VMLAUNCH instruction is not issued.
    LiveEspHoldAbsent,
...
static GUEST_UEFI_REAL_LAUNCH_ARMED: AtomicBool = AtomicBool::new(false);
...
/// Stage 22: live-ESP VMLAUNCH issue path armed. Bytes still absent.
static GUEST_UEFI_LIVE_ISSUE_ARMED: AtomicBool = AtomicBool::new(false);
/// Stage 23: live ESP `\EFI\RayNu\OVMF.fd` bytes probed. Still absent.
static GUEST_UEFI_LIVE_BYTES_PROBED: AtomicBool = AtomicBool::new(false);
/// Stage 24: real ESP `\EFI\RayNu\OVMF.fd` required. Still absent.
static GUEST_UEFI_LIVE_FD_REQUIRED: AtomicBool = AtomicBool::new(false);
/// Stage 25: real ESP `\EFI\RayNu\OVMF.fd` present-attempt. Still absent.
static GUEST_UEFI_LIVE_ESP_PRESENTED: AtomicBool = AtomicBool::new(false);
/// Stage 26: real ESP `\EFI\RayNu\OVMF.fd` admit-attempt. Still absent.
static GUEST_UEFI_LIVE_ESP_ADMITTED: AtomicBool = AtomicBool::new(false);
/// Stage 27: real ESP `\EFI\RayNu\OVMF.fd` read-attempt. Still absent.
...
/// Stage 30: real ESP `\EFI\RayNu\OVMF.fd` apply-attempt. Still absent.
static GUEST_UEFI_LIVE_ESP_APPLIED: AtomicBool = AtomicBool::new(false);
/// Stage 31: real ESP `\EFI\RayNu\OVMF.fd` commit-attempt. Still absent.
static GUEST_UEFI_LIVE_ESP_COMMITTED: AtomicBool = AtomicBool::new(false);
/// Stage 32: real ESP `\EFI\RayNu\OVMF.fd` latch-attempt. Still absent.
...
/// Stage 35: real ESP `\EFI\RayNu\OVMF.fd` hold-attempt. Still absent.
static GUEST_UEFI_LIVE_ESP_HELD: AtomicBool = AtomicBool::new(false);
...
pub fn reset_live_esp_ovmf_mapping() {
    ...
    GUEST_UEFI_LIVE_ISSUE_ARMED.store(false, Ordering::Release);
    GUEST_UEFI_LIVE_BYTES_PROBED.store(false, Ordering::Release);
    GUEST_UEFI_LIVE_FD_REQUIRED.store(false, Ordering::Release);
    GUEST_UEFI_LIVE_ESP_PRESENTED.store(false, Ordering::Release);
    ...
    GUEST_UEFI_LIVE_ESP_APPLIED.store(false, Ordering::Release);
...
/// True after a successful [`install_guest_uefi_alias_ept`].
pub fn guest_uefi_alias_ept_is_installed() -> bool {
    GUEST_UEFI_ALIAS_EPT_INSTALLED.load(Ordering::Acquire)
}

/// Record a private guest-UEFI alias-EPT install after program (ADR-014 Stage 17).
///
/// INVARIANTS:
/// - Requires a prior alias-EPT program
/// - Requires `firmware_alias_gpa(bytes_len)` and reset-vector coverage
/// - Does not write the E4 SHELL EPT, does not VMWRITE, and does not issue VMLAUNCH
pub fn install_guest_uefi_alias_ept(bytes_len: u64) -> Result<(), GuestUefiLaunchError> {
    ...
    GUEST_UEFI_ALIAS_EPT_GPA.store(gpa, Ordering::Release);
    GUEST_UEFI_ALIAS_EPT_INSTALLED.store(true, Ordering::Release);
    Ok(())
}
...
/// True after a successful [`arm_guest_uefi_real_launch`].
pub fn guest_uefi_real_launch_is_armed() -> bool {
    GUEST_UEFI_REAL_LAUNCH_ARMED.load(Ordering::Acquire)
}

/// Arm the guest-UEFI VMLAUNCH insn path after real-ESP qualify (ADR-014 Stage 19).
///
/// INVARIANTS:
/// - Requires a prior real-ESP qualify
/// - Requires `firmware_alias_gpa(bytes_len)` and reset-vector coverage
/// - Selects [`GUEST_UEFI_VMLAUNCH_OPCODE`] for `\\EFI\\RayNu\\OVMF.fd` only
/// - Does not write the E4 SHELL EPT, does not VMWRITE, and does not issue VMLAUNCH
/// - A heap fixture is not a shipped EDK2 `OVMF.fd`
pub fn arm_guest_uefi_real_launch(bytes_len: u64) -> Result<(), GuestUefiLaunchError> {
    if !guest_uefi_real_esp_is_qualified() {
        return Err(GuestUefiLaunchError::RealEspNotLaunched);
    }
    let Some(gpa) = firmware_alias_gpa(bytes_len) else {
        return Err(GuestUefiLaunchError::MissingEspFirmware);
    };
    ...
    GUEST_UEFI_ALIAS_EPT_GPA.store(gpa, Ordering::Release);
    GUEST_UEFI_REAL_LAUNCH_ARMED.store(true, Ordering::Release);
    Ok(())
}

/// True after a successful [`require_guest_uefi_live_esp`].
pub fn guest_uefi_live_esp_is_required() -> bool {
...
pub fn guest_uefi_live_fd_is_required() -> bool {
    GUEST_UEFI_LIVE_FD_REQUIRED.load(Ordering::Acquire)
}

/// Require a real ESP `\EFI\RayNu\OVMF.fd` after live-bytes probe (ADR-014 Stage 24).
///
/// INVARIANTS:
/// - Requires a prior live-bytes probe
/// - Requires `firmware_alias_gpa(bytes_len)` and reset-vector coverage
...
/// - A heap fixture is not a shipped EDK2 `OVMF.fd`
pub fn require_guest_uefi_live_fd(bytes_len: u64) -> Result<(), GuestUefiLaunchError> {
    if !guest_uefi_live_bytes_is_probed() {
        return Err(GuestUefiLaunchError::LiveEspBytesAbsent);
    }
    let Some(gpa) = firmware_alias_gpa(bytes_len) else {
        return Err(GuestUefiLaunchError::MissingEspFirmware);
    };
    ...
    GUEST_UEFI_ALIAS_EPT_GPA.store(gpa, Ordering::Release);
    GUEST_UEFI_LIVE_FD_REQUIRED.store(true, Ordering::Release);
    Ok(())
}

/// True after a successful [`present_guest_uefi_live_esp`].
pub fn guest_uefi_live_esp_is_presented() -> bool {
    GUEST_UEFI_LIVE_ESP_PRESENTED.load(Ordering::Acquire)
}

/// Present real ESP `\EFI\RayNu\OVMF.fd` bytes after live-FD require (ADR-014 Stage 25).
///
/// INVARIANTS:
/// - Requires a prior live-FD require
/// - Requires `firmware_alias_gpa(bytes_len)` and reset-vector coverage
/// - Does not allocate a VMCS, does not VMWRITE, and does not issue VMLAUNCH
/// - Does not write the E4 SHELL EPT
/// - Does not flip [`guest_uefi_live_esp_bytes_present`]
/// - A heap fixture is not a shipped EDK2 `OVMF.fd`
pub fn present_guest_uefi_live_esp(bytes_len: u64) -> Result<(), GuestUefiLaunchError> {
    if !guest_uefi_live_fd_is_required() {
        return Err(GuestUefiLaunchError::LiveEspFdAbsent);
    }
    let Some(gpa) = firmware_alias_gpa(bytes_len) else {
        return Err(GuestUefiLaunchError::MissingEspFirmware);
    };
    ...
    GUEST_UEFI_ALIAS_EPT_GPA.store(gpa, Ordering::Release);
    GUEST_UEFI_LIVE_ESP_PRESENTED.store(true, Ordering::Release);
    Ok(())
}

/// True after a successful [`admit_guest_uefi_live_esp`].
pub fn guest_uefi_live_esp_is_admitted() -> bool {
    GUEST_UEFI_LIVE_ESP_ADMITTED.load(Ordering::Acquire)
}

/// Admit real ESP `\EFI\RayNu\OVMF.fd` bytes after present-attempt (ADR-014 Stage 26).
///
/// INVARIANTS:
/// - Requires a prior live-ESP present-attempt
/// - Requires `firmware_alias_gpa(bytes_len)` and reset-vector coverage
/// - Does not allocate a VMCS, does not VMWRITE, and does not issue VMLAUNCH
/// - Does not write the E4 SHELL EPT
/// - Does not flip [`guest_uefi_live_esp_bytes_present`]
/// - A heap fixture is not a shipped EDK2 `OVMF.fd`
pub fn admit_guest_uefi_live_esp(bytes_len: u64) -> Result<(), GuestUefiLaunchError> {
    if !guest_uefi_live_esp_is_presented() {
        return Err(GuestUefiLaunchError::LiveEspPresentAbsent);
    }
    let Some(gpa) = firmware_alias_gpa(bytes_len) else {
        return Err(GuestUefiLaunchError::MissingEspFirmware);
    };
    ...
    GUEST_UEFI_ALIAS_EPT_GPA.store(gpa, Ordering::Release);
    GUEST_UEFI_LIVE_ESP_ADMITTED.store(true, Ordering::Release);
    Ok(())
}

/// True after a successful [`read_guest_uefi_live_esp`].
pub fn guest_uefi_live_esp_is_read() -> bool {
    GUEST_UEFI_LIVE_ESP_READ.load(Ordering::Acquire)
}

/// Read-attempt real ESP `\EFI\RayNu\OVMF.fd` bytes after admit (ADR-014 Stage 27).
///
/// INVARIANTS:
/// - Requires a prior live-ESP admit-attempt
/// - Requires `firmware_alias_gpa(bytes_len)` and reset-vector coverage
/// - Does not allocate a VMCS, does not VMWRITE, and does not issue VMLAUNCH
/// - Does not write the E4 SHELL EPT
/// - Does not flip [`guest_uefi_live_esp_bytes_present`]
/// - A heap fixture is not a shipped EDK2 `OVMF.fd`
pub fn read_guest_uefi_live_esp(bytes_len: u64) -> Result<(), GuestUefiLaunchError> {
    if !guest_uefi_live_esp_is_admitted() {
        return Err(GuestUefiLaunchError::LiveEspAdmitAbsent);
    }
    let Some(gpa) = firmware_alias_gpa(bytes_len) else {
        return Err(GuestUefiLaunchError::MissingEspFirmware);
    };
    ...
    GUEST_UEFI_ALIAS_EPT_GPA.store(gpa, Ordering::Release);
    GUEST_UEFI_LIVE_ESP_READ.store(true, Ordering::Release);
    Ok(())
}

/// True after a successful [`copy_guest_uefi_live_esp`].
pub fn guest_uefi_live_esp_is_copied() -> bool {
    GUEST_UEFI_LIVE_ESP_COPIED.load(Ordering::Acquire)
}

/// Copy-attempt real ESP `\EFI\RayNu\OVMF.fd` bytes after read (ADR-014 Stage 28).
///
/// INVARIANTS:
/// - Requires a prior live-ESP read-attempt
/// - Requires `firmware_alias_gpa(bytes_len)` and reset-vector coverage
/// - Does not allocate a VMCS, does not VMWRITE, and does not issue VMLAUNCH
/// - Does not write the E4 SHELL EPT
/// - Does not flip [`guest_uefi_live_esp_bytes_present`]
/// - A heap fixture is not a shipped EDK2 `OVMF.fd`
pub fn copy_guest_uefi_live_esp(bytes_len: u64) -> Result<(), GuestUefiLaunchError> {
    if !guest_uefi_live_esp_is_read() {
    ...
    GUEST_UEFI_ALIAS_EPT_GPA.store(gpa, Ordering::Release);
    GUEST_UEFI_LIVE_ESP_COPIED.store(true, Ordering::Release);
    Ok(())
}
...
/// True after a successful [`apply_guest_uefi_live_esp`].
pub fn guest_uefi_live_esp_is_applied() -> bool {
    GUEST_UEFI_LIVE_ESP_APPLIED.load(Ordering::Acquire)
}

/// Apply-attempt real ESP `\EFI\RayNu\OVMF.fd` bytes after place (ADR-014 Stage 30).
///
/// INVARIANTS:
/// - Requires a prior live-ESP place-attempt
/// - Requires `firmware_alias_gpa(bytes_len)` and reset-vector coverage
/// - Does not allocate a VMCS, does not VMWRITE, and does not issue VMLAUNCH
/// - Does not write the E4 SHELL EPT
/// - Does not flip [`guest_uefi_live_esp_bytes_present`]
/// - A heap fixture is not a shipped EDK2 `OVMF.fd`
pub fn apply_guest_uefi_live_esp(bytes_len: u64) -> Result<(), GuestUefiLaunchError> {
    ...
    GUEST_UEFI_ALIAS_EPT_GPA.store(gpa, Ordering::Release);
    GUEST_UEFI_LIVE_ESP_APPLIED.store(true, Ordering::Release);
    Ok(())
}

/// True after a successful [`commit_guest_uefi_live_esp`].
pub fn guest_uefi_live_esp_is_committed() -> bool {
    GUEST_UEFI_LIVE_ESP_COMMITTED.load(Ordering::Acquire)
}

/// Commit-attempt real ESP `\EFI\RayNu\OVMF.fd` bytes after apply (ADR-014 Stage 31).
///
/// INVARIANTS:
/// - Requires a prior live-ESP apply-attempt
/// - Requires `firmware_alias_gpa(bytes_len)` and reset-vector coverage
/// - Does not allocate a VMCS, does not VMWRITE, and does not issue VMLAUNCH
/// - Does not write the E4 SHELL EPT
/// - Does not flip [`guest_uefi_live_esp_bytes_present`]
/// - A heap fixture is not a shipped EDK2 `OVMF.fd`
pub fn commit_guest_uefi_live_esp(bytes_len: u64) -> Result<(), GuestUefiLaunchError> {
    if !guest_uefi_live_esp_is_applied() {
        return Err(GuestUefiLaunchError::LiveEspApplyAbsent);
    }
    let Some(gpa) = firmware_alias_gpa(bytes_len) else {
        return Err(GuestUefiLaunchError::MissingEspFirmware);
    };
    ...
    GUEST_UEFI_ALIAS_EPT_GPA.store(gpa, Ordering::Release);
    GUEST_UEFI_LIVE_ESP_COMMITTED.store(true, Ordering::Release);
    Ok(())
}

/// True after a successful [`latch_guest_uefi_live_esp`].
pub fn guest_uefi_live_esp_is_latched() -> bool {
    GUEST_UEFI_LIVE_ESP_LATCHED.load(Ordering::Acquire)
}

/// Latch-attempt real ESP `\EFI\RayNu\OVMF.fd` bytes after commit (ADR-014 Stage 32).
///
/// INVARIANTS:
/// - Requires a prior live-ESP commit-attempt
/// - Requires `firmware_alias_gpa(bytes_len)` and reset-vector coverage
/// - Does not allocate a VMCS, does not VMWRITE, and does not issue VMLAUNCH
/// - Does not write the E4 SHELL EPT
/// - Does not flip [`guest_uefi_live_esp_bytes_present`]
/// - A heap fixture is not a shipped EDK2 `OVMF.fd`
pub fn latch_guest_uefi_live_esp(bytes_len: u64) -> Result<(), GuestUefiLaunchError> {
    if !guest_uefi_live_esp_is_committed() {
        return Err(GuestUefiLaunchError::LiveEspCommitAbsent);
    }
    let Some(gpa) = firmware_alias_gpa(bytes_len) else {
        return Err(GuestUefiLaunchError::MissingEspFirmware);
    };
    ...
    GUEST_UEFI_ALIAS_EPT_GPA.store(gpa, Ordering::Release);
    GUEST_UEFI_LIVE_ESP_LATCHED.store(true, Ordering::Release);
    Ok(())
}

/// True after a successful [`seal_guest_uefi_live_esp`].
pub fn guest_uefi_live_esp_is_sealed() -> bool {
    GUEST_UEFI_LIVE_ESP_SEALED.load(Ordering::Acquire)
}

/// Seal-attempt real ESP `\EFI\RayNu\OVMF.fd` bytes after latch (ADR-014 Stage 33).
///
/// INVARIANTS:
/// - Requires a prior live-ESP latch-attempt
/// - Requires `firmware_alias_gpa(bytes_len)` and reset-vector coverage
/// - Does not allocate a VMCS, does not VMWRITE, and does not issue VMLAUNCH
/// - Does not write the E4 SHELL EPT
/// - Does not flip [`guest_uefi_live_esp_bytes_present`]
/// - A heap fixture is not a shipped EDK2 `OVMF.fd`
pub fn seal_guest_uefi_live_esp(bytes_len: u64) -> Result<(), GuestUefiLaunchError> {
    if !guest_uefi_live_esp_is_latched() {
    ...
    GUEST_UEFI_ALIAS_EPT_GPA.store(gpa, Ordering::Release);
    GUEST_UEFI_LIVE_ESP_SEALED.store(true, Ordering::Release);
    Ok(())
}
...
/// True after a successful [`hold_guest_uefi_live_esp`].
pub fn guest_uefi_live_esp_is_held() -> bool {
    GUEST_UEFI_LIVE_ESP_HELD.load(Ordering::Acquire)
}

/// Hold-attempt real ESP `\EFI\RayNu\OVMF.fd` bytes after lock (ADR-014 Stage 35).
///
/// INVARIANTS:
/// - Requires a prior live-ESP lock-attempt
/// - Requires `firmware_alias_gpa(bytes_len)` and reset-vector coverage
/// - Does not allocate a VMCS, does not VMWRITE, and does not issue VMLAUNCH
/// - Does not write the E4 SHELL EPT
/// - Does not flip [`guest_uefi_live_esp_bytes_present`]
/// - A heap fixture is not a shipped EDK2 `OVMF.fd`
pub fn hold_guest_uefi_live_esp(bytes_len: u64) -> Result<(), GuestUefiLaunchError> {
    ...
    GUEST_UEFI_ALIAS_EPT_GPA.store(gpa, Ordering::Release);
    GUEST_UEFI_LIVE_ESP_HELD.store(true, Ordering::Release);
    Ok(())
}
...
///
/// INVARIANTS:
/// - Requires a prior live-issue arm
/// - Requires `firmware_alias_gpa(bytes_len)` and reset-vector coverage
/// - Does not allocate a VMCS, does not VMWRITE, and does not issue VMLAUNCH
/// - Does not write the E4 SHELL EPT
/// - Does not flip [`guest_uefi_live_esp_bytes_present`]
/// - A heap fixture is not a shipped EDK2 `OVMF.fd`
pub fn probe_guest_uefi_live_bytes(bytes_len: u64) -> Result<(), GuestUefiLaunchError> {
    if !guest_uefi_live_issue_is_armed() {
        return Err(GuestUefiLaunchError::LiveEspBytesNotPresent);
    }
    let Some(gpa) = firmware_alias_gpa(bytes_len) else {
        return Err(GuestUefiLaunchError::MissingEspFirmware);
    };
    ...
    GUEST_UEFI_ALIAS_EPT_GPA.store(gpa, Ordering::Release);
    GUEST_UEFI_LIVE_BYTES_PROBED.store(true, Ordering::Release);
    Ok(())
}

/// True after a successful [`arm_guest_uefi_live_issue`].
pub fn guest_uefi_live_issue_is_armed() -> bool {
    GUEST_UEFI_LIVE_ISSUE_ARMED.load(Ordering::Acquire)
}

/// Arm the live-ESP VMLAUNCH issue path after private VMCS (ADR-014 Stage 22).
///
/// INVARIANTS:
/// - Requires a prior private-VMCS arm
/// - Requires `firmware_alias_gpa(bytes_len)` and reset-vector coverage
/// - Does not allocate a VMCS, does not VMWRITE, and does not issue VMLAUNCH
/// - Does not write the E4 SHELL EPT
/// - Does not flip [`guest_uefi_live_esp_bytes_present`]
/// - A heap fixture is not a shipped EDK2 `OVMF.fd`
pub fn arm_guest_uefi_live_issue(bytes_len: u64) -> Result<(), GuestUefiLaunchError> {
    ...
    GUEST_UEFI_ALIAS_EPT_GPA.store(gpa, Ordering::Release);
    GUEST_UEFI_LIVE_ISSUE_ARMED.store(true, Ordering::Release);
    Ok(())
}
...
///
/// INVARIANTS:
/// - Requires a prior insn-path arm
/// - Requires `firmware_alias_gpa(bytes_len)` and reset-vector coverage
/// - Does not write the E4 SHELL EPT, does not VMWRITE, and does not issue VMLAUNCH
/// - A heap fixture is not a shipped EDK2 `OVMF.fd`
pub fn require_guest_uefi_live_esp(bytes_len: u64) -> Result<(), GuestUefiLaunchError> {
    if !guest_uefi_real_launch_is_armed() {
    ...
    GUEST_UEFI_ALIAS_EPT_GPA.store(gpa, Ordering::Release);
    GUEST_UEFI_LIVE_ESP_REQUIRED.store(true, Ordering::Release);
    Ok(())
}
...
fn issue_guest_uefi_vmlaunch() -> Result<(), GuestUefiLaunchError> {
    if !guest_uefi_live_esp_bytes_present() {
        if guest_uefi_live_esp_is_held() {
            return Err(GuestUefiLaunchError::LiveEspHoldAbsent);
        }
        if guest_uefi_live_esp_is_locked() {
            return Err(GuestUefiLaunchError::LiveEspLockAbsent);
        }
...
/// Guest UEFI VMLAUNCH from ESP `\\EFI\\RayNu\\OVMF.fd` (ADR-014 Stage 32).
///
/// INVARIANTS:
...
/// - Private VMCS selected, no live-issue arm →
///   [`GuestUefiLaunchError::PrivateVmcsNotLaunched`]
/// - Live-issue armed, no live-bytes probe →
///   [`GuestUefiLaunchError::LiveEspBytesNotPresent`]
/// - Live bytes probed, no live-FD require →
///   [`GuestUefiLaunchError::LiveEspBytesAbsent`]
/// - Live FD required, no present-attempt →
///   [`GuestUefiLaunchError::LiveEspFdAbsent`]
/// - Live ESP presented, no admit-attempt →
///   [`GuestUefiLaunchError::LiveEspPresentAbsent`]
/// - Live ESP admitted, no read-attempt →
...
/// - Live ESP place-attempted, no apply-attempt →
///   [`GuestUefiLaunchError::LiveEspPlaceAbsent`]
/// - Live ESP apply-attempted, no commit-attempt →
///   [`GuestUefiLaunchError::LiveEspApplyAbsent`]
/// - Live ESP commit-attempted, no latch-attempt →
...
/// - Live ESP lock-attempted, no hold-attempt →
///   [`GuestUefiLaunchError::LiveEspLockAbsent`]
/// - Live ESP hold-attempted, still absent →
///   [`GuestUefiLaunchError::LiveEspHoldAbsent`]
...
mod launch_test {
    ...
    fn marker_stable() {
        ...
        assert_eq!(
            install_guest_uefi_alias_ept(MIN_LIVE_ESP_OVMF_BYTES as u64),
            Err(GuestUefiLaunchError::MissingEspFirmware)
        );
        assert!(!guest_uefi_alias_ept_is_installed());
        assert_eq!(
            install_guest_uefi_alias_ept(MIN_FIRMWARE_ALIAS_BYTES as u64),
            Ok(())
        );
        assert!(guest_uefi_alias_ept_is_installed());
        assert_eq!(
        ...
        assert_eq!(
            arm_guest_uefi_real_launch(MIN_LIVE_ESP_OVMF_BYTES as u64),
            Err(GuestUefiLaunchError::MissingEspFirmware)
        );
        assert!(!guest_uefi_real_launch_is_armed());
        assert_eq!(
            arm_guest_uefi_real_launch(MIN_FIRMWARE_ALIAS_BYTES as u64),
            Ok(())
        );
        assert!(guest_uefi_real_launch_is_armed());
        assert_eq!(
            try_vmlaunch_guest_uefi_ovmf(),
            Err(GuestUefiLaunchError::RealLaunchNotIssued)
        );
        assert_eq!(GUEST_UEFI_VMLAUNCH_OPCODE, [0x0F, 0x01, 0xC2]);
        assert!(!guest_uefi_live_esp_is_required());
        assert!(!guest_uefi_live_esp_bytes_present());
        assert_eq!(
            require_guest_uefi_live_esp(MIN_LIVE_ESP_OVMF_BYTES as u64),
            Err(GuestUefiLaunchError::MissingEspFirmware)
        );
        assert!(!guest_uefi_live_esp_is_required());
        assert_eq!(
            require_guest_uefi_live_esp(MIN_FIRMWARE_ALIAS_BYTES as u64),
            Ok(())
        );
        assert!(guest_uefi_live_esp_is_required());
        assert_eq!(
        ...
        assert!(!guest_uefi_live_issue_is_armed());
        assert_eq!(
            arm_guest_uefi_live_issue(MIN_LIVE_ESP_OVMF_BYTES as u64),
            Err(GuestUefiLaunchError::MissingEspFirmware)
        );
        assert!(!guest_uefi_live_issue_is_armed());
        assert_eq!(
            arm_guest_uefi_live_issue(MIN_FIRMWARE_ALIAS_BYTES as u64),
            Ok(())
        );
        assert!(guest_uefi_live_issue_is_armed());
        assert!(!guest_uefi_live_esp_bytes_present());
        assert_eq!(
            try_vmlaunch_guest_uefi_ovmf(),
            Err(GuestUefiLaunchError::LiveEspBytesNotPresent)
        );
        assert!(!guest_uefi_live_bytes_is_probed());
        assert_eq!(
            probe_guest_uefi_live_bytes(MIN_LIVE_ESP_OVMF_BYTES as u64),
            Err(GuestUefiLaunchError::MissingEspFirmware)
        );
        assert!(!guest_uefi_live_bytes_is_probed());
        assert_eq!(
            probe_guest_uefi_live_bytes(MIN_FIRMWARE_ALIAS_BYTES as u64),
            Ok(())
        );
        assert!(guest_uefi_live_bytes_is_probed());
        assert!(!guest_uefi_live_esp_bytes_present());
        assert_eq!(
            try_vmlaunch_guest_uefi_ovmf(),
            Err(GuestUefiLaunchError::LiveEspBytesAbsent)
        );
        assert!(!guest_uefi_live_fd_is_required());
        assert_eq!(
            require_guest_uefi_live_fd(MIN_LIVE_ESP_OVMF_BYTES as u64),
            Err(GuestUefiLaunchError::MissingEspFirmware)
        );
        assert!(!guest_uefi_live_fd_is_required());
        assert_eq!(
            require_guest_uefi_live_fd(MIN_FIRMWARE_ALIAS_BYTES as u64),
            Ok(())
        );
        assert!(guest_uefi_live_fd_is_required());
        assert!(!guest_uefi_live_esp_bytes_present());
        assert_eq!(
            try_vmlaunch_guest_uefi_ovmf(),
            Err(GuestUefiLaunchError::LiveEspFdAbsent)
        );
        assert!(!guest_uefi_live_esp_is_presented());
        assert_eq!(
            present_guest_uefi_live_esp(MIN_LIVE_ESP_OVMF_BYTES as u64),
            Err(GuestUefiLaunchError::MissingEspFirmware)
        );
        assert!(!guest_uefi_live_esp_is_presented());
        assert_eq!(
            present_guest_uefi_live_esp(MIN_FIRMWARE_ALIAS_BYTES as u64),
            Ok(())
        );
        assert!(guest_uefi_live_esp_is_presented());
        assert!(!guest_uefi_live_esp_bytes_present());
        assert_eq!(
            try_vmlaunch_guest_uefi_ovmf(),
            Err(GuestUefiLaunchError::LiveEspPresentAbsent)
        );
        assert!(!guest_uefi_live_esp_is_admitted());
        assert_eq!(
            admit_guest_uefi_live_esp(MIN_LIVE_ESP_OVMF_BYTES as u64),
            Err(GuestUefiLaunchError::MissingEspFirmware)
        );
        assert!(!guest_uefi_live_esp_is_admitted());
        assert_eq!(
            admit_guest_uefi_live_esp(MIN_FIRMWARE_ALIAS_BYTES as u64),
            Ok(())
        );
        assert!(guest_uefi_live_esp_is_admitted());
        assert!(!guest_uefi_live_esp_bytes_present());
        assert_eq!(
            try_vmlaunch_guest_uefi_ovmf(),
            Err(GuestUefiLaunchError::LiveEspAdmitAbsent)
        );
        assert!(!guest_uefi_live_esp_is_read());
        assert_eq!(
            read_guest_uefi_live_esp(MIN_LIVE_ESP_OVMF_BYTES as u64),
            Err(GuestUefiLaunchError::MissingEspFirmware)
        );
        assert!(!guest_uefi_live_esp_is_read());
        assert_eq!(
            read_guest_uefi_live_esp(MIN_FIRMWARE_ALIAS_BYTES as u64),
            Ok(())
        );
        assert!(guest_uefi_live_esp_is_read());
        assert!(!guest_uefi_live_esp_bytes_present());
        assert_eq!(
            try_vmlaunch_guest_uefi_ovmf(),
            Err(GuestUefiLaunchError::LiveEspReadAbsent)
        );
        assert!(!guest_uefi_live_esp_is_copied());
        assert_eq!(
            copy_guest_uefi_live_esp(MIN_LIVE_ESP_OVMF_BYTES as u64),
            Err(GuestUefiLaunchError::MissingEspFirmware)
        );
        assert!(!guest_uefi_live_esp_is_copied());
        assert_eq!(
            copy_guest_uefi_live_esp(MIN_FIRMWARE_ALIAS_BYTES as u64),
            Ok(())
        );
        assert!(guest_uefi_live_esp_is_copied());
        assert!(!guest_uefi_live_esp_bytes_present());
        ...
        assert!(!guest_uefi_live_esp_is_applied());
        assert_eq!(
            apply_guest_uefi_live_esp(MIN_LIVE_ESP_OVMF_BYTES as u64),
            Err(GuestUefiLaunchError::MissingEspFirmware)
        );
        assert!(!guest_uefi_live_esp_is_applied());
        assert_eq!(
            apply_guest_uefi_live_esp(MIN_FIRMWARE_ALIAS_BYTES as u64),
            Ok(())
        );
        assert!(guest_uefi_live_esp_is_applied());
        assert!(!guest_uefi_live_esp_bytes_present());
        assert_eq!(
            try_vmlaunch_guest_uefi_ovmf(),
            Err(GuestUefiLaunchError::LiveEspApplyAbsent)
        );
        assert!(!guest_uefi_live_esp_is_committed());
        assert_eq!(
            commit_guest_uefi_live_esp(MIN_LIVE_ESP_OVMF_BYTES as u64),
            Err(GuestUefiLaunchError::MissingEspFirmware)
        );
        assert!(!guest_uefi_live_esp_is_committed());
        assert_eq!(
            commit_guest_uefi_live_esp(MIN_FIRMWARE_ALIAS_BYTES as u64),
            Ok(())
        );
        assert!(guest_uefi_live_esp_is_committed());
        assert!(!guest_uefi_live_esp_bytes_present());
        assert_eq!(
            try_vmlaunch_guest_uefi_ovmf(),
            Err(GuestUefiLaunchError::LiveEspCommitAbsent)
        );
        assert!(!guest_uefi_live_esp_is_latched());
        assert_eq!(
            latch_guest_uefi_live_esp(MIN_LIVE_ESP_OVMF_BYTES as u64),
            Err(GuestUefiLaunchError::MissingEspFirmware)
        );
        assert!(!guest_uefi_live_esp_is_latched());
        assert_eq!(
            latch_guest_uefi_live_esp(MIN_FIRMWARE_ALIAS_BYTES as u64),
            Ok(())
        );
        assert!(guest_uefi_live_esp_is_latched());
        assert!(!guest_uefi_live_esp_bytes_present());
        assert_eq!(
            try_vmlaunch_guest_uefi_ovmf(),
            Err(GuestUefiLaunchError::LiveEspLatchAbsent)
        );
        assert!(!guest_uefi_live_esp_is_sealed());
        assert_eq!(
            seal_guest_uefi_live_esp(MIN_LIVE_ESP_OVMF_BYTES as u64),
            Err(GuestUefiLaunchError::MissingEspFirmware)
        );
        assert!(!guest_uefi_live_esp_is_sealed());
        assert_eq!(
            seal_guest_uefi_live_esp(MIN_FIRMWARE_ALIAS_BYTES as u64),
            Ok(())
        );
        assert!(guest_uefi_live_esp_is_sealed());
        assert!(!guest_uefi_live_esp_bytes_present());
        ...
        assert_eq!(
            hold_guest_uefi_live_esp(MIN_LIVE_ESP_OVMF_BYTES as u64),
            Err(GuestUefiLaunchError::MissingEspFirmware)
        );
        assert!(!guest_uefi_live_esp_is_held());
        assert_eq!(
            hold_guest_uefi_live_esp(MIN_FIRMWARE_ALIAS_BYTES as u64),
            Ok(())
        );
        assert!(guest_uefi_live_esp_is_held());
        assert!(!guest_uefi_live_esp_bytes_present());
        assert_eq!(
            try_vmlaunch_guest_uefi_ovmf(),
            Err(GuestUefiLaunchError::LiveEspHoldAbsent)
        );
        reset_live_esp_ovmf_mapping();
        assert!(!live_esp_ovmf_is_mapped());
        assert!(!guest_uefi_reset_vector_is_armed());
