...
/// CMOS/fw_cfg LowMemory 2 GiB (P5; not EPT-map the gap).
...
/// Tests / P2: allow firmware UC variable MTRRs (default **false** = hold).
...
unsafe fn handle_ept(gpa: u64, qual: u64) -> bool {
    ...
    // Iron f9a08c9: CMOS 2GiB LowMemory is not EPT-backed. Do not sink
    // [32MiB, 2GiB) as MMIO zeros (that would look like RAM).
    if crate::devices::guest_platform::is_unbacked_report_ram_gpa(gpa) {
        serial::write_str("boot: guest-UEFI EPT unbacked report-RAM gpa=0x");
        write_hex(gpa);
        serial::write_byte(b'\n');
        return false;
    }
