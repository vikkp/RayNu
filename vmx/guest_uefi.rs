...
/// `ataio=0`). Else LAPIC timer `0x20` after force LVT. Cap shots with
/// [`guest_uefi_firmware_hlt_inject_cap`] (CI `33464757885` unlimited
/// `0x20` CPUID livelock). nested iso=0 firmware HLT 0x68 miss.
/// nested iso=0 firmware HLT EDK2 0x68. nested iso=0 firmware HLT no PIT
/// inject. nested iso=0 firmware LAPIC timer. nested iso=0 EDK2 IRQ0.
...
/// CI `33469144799` VMXON: skip-after-inject proved (`hlt=0` sit on
/// CpuSleep `rip=0x7f10c0d0`) but 8 at-HLT `0x20` shots left `ataio=0`
/// until POST_DXE_TAIL. Cover the remaining HLT window (first HLT ~n=16384).
/// Product stays [`guest_uefi_firmware_hlt_inject_cap`] (8). Do not go
/// unlimited (CI `33464757885` skip-first CPUID livelock).
/// nested iso=0 firmware HLT inject cap. nested iso=0 firmware HLT skip after inject.
/// firmware HLT inject cap. Not `ISO-INSTALL-OK`.
pub const NESTED_ISO0_FIRMWARE_HLT_INJECT_CAP: u32 = 16384;
...
unsafe fn try_inject_nested_iso0_firmware_hlt_pit(reason: u32) {
    ...
    // unlimited 0x20 CPUID livelock. CI 33466890874 VMXON: 0x68 CR
    // livelock. Cap timer 0x20 after force LVT.
    ...
    // (0x68 miss). Do not pass that take into inject_vec. Force LVT 0x20.
    // Product skip_pit / wait_for_irq stay. Nested PIT skip_after_inject
    // waits for this inject (CI 33468177902 skip-first). nested iso=0
    // firmware HLT skip after inject. nested iso=0 firmware HLT EDK2 0x68.
    // nested iso=0 EDK2 IRQ0. nested iso=0 firmware HLT 0x68 miss.
