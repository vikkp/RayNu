...
unsafe fn try_inject_guest_irq() {
    ...
    if let Ok(info) = crate::sched::interrupt::prepare_external_inject(vec) {
        ...
        if n < 8 {
            serial::write_str("boot: Stage 46 inject vec=0x");
            write_hex(u64::from(vec));
            serial::write_line(" (not ISO-INSTALL-OK)");
        }
    } else {
