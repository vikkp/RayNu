...
/// `cr2=0x205f18`). skip HV identity PML4 dest. COM2 prints dest= and n=
/// so a 2d6b109 boot shows whether QemuFwCfgInitialize's signature was
/// skipped. n<=16 can overlay then restore on the next VM-exit (PEI stack
/// in the reserved window; PD[3] is not on the compare walk). fw_cfg
/// string skip HV identity dest. fw_cfg identity overlay. Not `ISO-INSTALL-OK`.
