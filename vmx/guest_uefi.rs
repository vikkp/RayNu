...
/// nested iso=0 EDK2 IRQ0. Not `ISO-INSTALL-OK`.
...
unsafe fn try_inject_nested_iso0_firmware_hlt_pit(reason: u32) {
    ...
    // take_nested_iso0_pit needs ICW4 ready; take_nested_iso0_pit_or_edk2
    // injects nested iso=0 EDK2 IRQ0 (0x68) when take is None.
    let rflags = guest_uefi_firmware_force_if_for_inject(
        linux,
        ops::vmread(GUEST_RFLAGS).unwrap_or(0),
    );
    ...
    // nested iso=0 EDK2 IRQ0: pic_take needs ICW4 ready. If the shadow
    // 8259 never finishes ICW, inject EDK2 Legacy8259 IRQ0 0x68 (or the
    // shadowed ICW2) so WaitForInterrupt returns. Product skip_pit /
    // wait_for_irq / skip_after_inject(false) stay unchanged.
    let vec = crate::devices::guest_irq::take_nested_iso0_pit_or_edk2();
