...
/// Keep overlay jiffies moving after an empty-ring dump. Iron `b8ac361`
/// / `34297632324`: one apk INTx, then ISR ACK, no `0x300`. Do not pulse
/// INTx again. virtio stall dump PIT hold. Not `ISO-INSTALL-OK`.
#[cfg(target_os = "uefi")]
unsafe fn maybe_virtio_stall_dump_pit_hold() {
    if !guest_uefi_virtio_stall_dump_pit_hold(
        true,
        crate::devices::guest_serial_answer::apk_packages_overlay_active(),
    ) {
        return;
    ...
    {
        serial::write_line_nowait(
            "boot: guest-UEFI virtio stall dump PIT hold (apk jiffies; not ISO-INSTALL-OK)",
        );
    }
    let now = cpu::rdtsc();
    ...
    serial::write_str_nowait("boot: guest-UEFI virtio stall dump PIT hold n=");
    write_dec_nowait(u64::from(LAST_VIRTIO_MMIO_HIT.load(Ordering::Acquire)));
    serial::write_str_nowait(" (not ISO-INSTALL-OK)\n");
}
