...
/// `0x20`/`0x2E`/`0xEF`. Default EDK2 IRQ0 `0x68` (iron `ea30da1` leftover
/// `0x20` is the timer ISR; CI `33464757885` nested same). PIC ICW2 never
/// ran (`pic=0`) so do not force LVT `0x20`. skip-only-after-inject skips
/// HLT so IRET lands on RET. Main-path skip_pit still drops `0x20`.
...
unsafe fn try_inject_product_firmware_hlt_wake(
    ...
    // product ISO firmware HLT EDK2 0x68: do not force leftover LVT 0x20
    // (iron ea30da1 timer ISR; CI 33464757885 nested CPUID ataio=0).
    if guest_uefi_product_firmware_hlt_wake_lvt_unmask(linux)
        && !guest_uefi_product_firmware_hlt_edk2_irq0(linux, pci_ide, ataio, reason)
    {
        let _ = crate::devices::lapic_virt::force_firmware_lapic_timer_expiry();
    }
