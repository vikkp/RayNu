...
/// ~34k exits after n=30769, and the 32768 post-ATAPI tail could cut BDS
/// StartImage even after catalog+load READs. 131072 keeps the private VMCS
/// until `RN-ELT` or the hard cap.
pub const GUEST_UEFI_RESUME_CAP: u32 = 131072;
...
/// After the first honest ATAPI sector, keep running this many more exits
/// unless El Torito boot evidence lands. Stage 44 stopped on `sectors>0`.
...
/// `pci_ide=0` (never `identity SPLIT n=2`). Only ATA data-register
/// FIFOs (`IoReadFifo16` IDENTIFY / PACKET), including BAR-relocated
/// command blocks. Other string I/O stays one-shot + `skip_insn`
/// (same as `06b011a`). Status/control ports are not FIFOs.
...
/// - catalog+load READ without payload does **not** apply the short tail
///   (BDS FatDxe/LoadImage/StartImage still needs the 131072-exit cap)
/// - `true` after [`GUEST_UEFI_POST_ATAPI_TAIL`] past the first ATAPI exit
///   when catalog or load LBA was never READ (PACKET without El Torito)
...
pub fn post_atapi_should_stop(
    ...
    if atapi_read_evidence(sectors) {
        if catalog && boot_image {
            return false;
        }
        if atapi_at == 0 {
            return false;
        }
        return exit_n.saturating_sub(atapi_at) >= GUEST_UEFI_POST_ATAPI_TAIL;
    }
