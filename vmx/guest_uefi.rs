...
/// Nested iso=0: PIC take except leftover `0x20`/`0x2E`/`0xEF`, else EDK2
/// IRQ0 `0x68`. Do not prefer leftover LVT `0x20` (CI `33464757885` timer
/// ISR CPUID `ataio=0`). nested iso=0 firmware HLT EDK2 0x68.
/// nested iso=0 firmware HLT no PIT inject. nested iso=0 firmware
...
/// `skip_after_inject(false)` stays false. nested iso=0 firmware HLT EDK2 0x68.
...
unsafe fn try_inject_nested_iso0_firmware_hlt_pit(reason: u32) {
    ...
    // force LVT then inject vec=0x20 ran timer ISR CPUID ataio=0.
    // nested iso=0 firmware HLT EDK2 0x68: PIC take else 0x68, never LVT.
    ...
    // Do not force LVT (CI 33464757885 leftover 0x20).
    // Product skip_pit / wait_for_irq / skip_after_inject(false) stay.
    // nested iso=0 firmware HLT EDK2 0x68. nested iso=0 EDK2 IRQ0.
    let vec = if guest_uefi_nested_iso0_firmware_hlt_edk2_irq0(
        false, linux, pci_ide, ataio, reason,
    ) {
        guest_uefi_nested_iso0_inject_vec(
            Some(crate::devices::guest_irq::take_nested_iso0_pit_or_edk2()),
            None,
        )
    } else {
        ...
        guest_uefi_nested_iso0_inject_vec(pic, lapic)
    };
