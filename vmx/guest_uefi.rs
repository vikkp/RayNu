...
/// - Iron `eb4b27d` `err=0xb` `CR2=0x80000008` is true (RSVD 1GiB in MTRR UC)
pub fn guest_uefi_pf_should_identity_map(err: u64, cr2: u64) -> bool {
    ...
    let in_mtrr_uc = cr2 >= crate::vmx::guest_pt::IDENTITY_MTRR_UC_FLOOR
        && cr2 < crate::vmx::guest_pt::IDENTITY_FLASH_FLOOR;
    (in_ram || in_flash || (in_mtrr_uc && guest_uefi_pf_error_is_reserved(err))) && np_or_rsvd
}

/// Iron `fdf07ba`: after EPT sink, 4G WB identity then ASSERT `lastmsr=0x23f`.
/// NP or RSVD `#PF` in a sink GPA gets a UC 2 MiB leaf; EPT still sink-resumes.
...
pub fn guest_uefi_pf_should_map_mmio(err: u64, cr2: u64) -> bool {
    (guest_uefi_pf_error_is_not_present(err) || guest_uefi_pf_error_is_reserved(err))
        && crate::devices::guest_platform::is_platform_sink_gpa(cr2)
}
...
unsafe fn handle_pf(rip: u64, linear: u64, cr2: u64) -> bool {
    ...
    if !guest_uefi_pf_should_identity_map(err, cr2) {
        ...
        match r {
            ...
            Ok(_) => true,
            Err(_) => {
                serial::write_line("boot: guest-UEFI #PF identity MMIO fail");
                false
            }
        }
    } else {
