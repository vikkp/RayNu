...
/// take and LAPIC ATA IRR are empty (`pic=0` APIC-mode CpuSleep).
/// Iron COM2 `b5c3a9c`: `ataio=0` `inj=0` `pic=0` skip-after-inject.
/// Force LVT expiry then inject that vector unless leftover `0x20`/`0xEF`
/// (then `0x68`). Latch into LAPIC IRR then inject.
/// skip_pit still drops leftover `0x20`. `wait_for_irq` stays false.
/// product ISO firmware HLT wake. product ISO firmware HLT wake LAPIC.
...
unsafe fn try_inject_product_firmware_hlt_wake(
    ...
    // Nested iso=0 already force_firmware_lapic_timer_expiry; product
    // hardcoded 0x68 into an IDT slot PIC ICW2 never installed.
    // Leftover 0x20/0xEF still substitutes 0x68 (iron ea30da1).
