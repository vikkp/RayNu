...
/// (not timer `0x20`). CpuSleep HLT still uses IDT `0x20`. BAR/CF8 false.
...
/// CI `33460343555` VMXON-SKIP.
/// firmware skip PIT inject. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_product_firmware_ide_cmd_inject_ata(ide_cmd_wr: bool) -> bool {
    ide_cmd_wr
}
...
unsafe fn try_inject_product_firmware_hlt_wake(
    ...
    let vec = if guest_uefi_product_firmware_ide_cmd_inject_ata(ide_cmd_wr) {
        // product ISO firmware IDE cmd inject ATA: IdeBus Start wants IRQ 14.
