...
/// `prehex=` 16 bytes at CpuDxe `ret-16` started at `mov word, 6`
/// / `jmp` / `call DebugAssert` — Status is not there. Dump 32 bytes
/// immediately before `callerrip=0x7fd25193` plus `rax` (EFI_STATUS
/// after `CoreStartImage` `call [rax+0x20]`; DebugAssert may clobber).
/// Keep PEI i440FX `0x1237` / DXE virtio `0x1042`. Keep FIX WB hold.
/// No DID flip. No new PAT-UC.
