...
/// leftover `0x20`. ATA 14 (`0x2E`) and virtio INTx still inject.
...
/// Never leftover LVT `0xEF`. `wait_for_irq` stays false. HLT only.
/// product ISO firmware HLT wake. product ISO firmware HLT wake IDT 0x20.
...
/// Product firmware HLT `ataio==0` wake vector: PIC / LAPIC except leftover
/// LVT `0xEF`. Keep PIT `0x20` (iron `ea30da1` IDT[0x20] timer ISR). PIC
/// ICW2 never ran (`pic=0`) so EDK2 `0x68` may be empty. Default `0x20`.
/// skip-only-after-inject skips HLT so IRET lands on RET, not the ISR
/// livelock. Main-path skip_pit still drops `0x20`. `wait_for_irq` stays
/// false. product ISO firmware HLT wake. product ISO firmware HLT wake IDT 0x20.
/// firmware skip PIT inject. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_firmware_hlt_ataio0_wake_vec(pic: Option<u8>, lapic: Option<u8>) -> u8 {
    if let Some(v) = pic {
        if v >= 16 && v != 0xEF {
            return v;
        }
...
/// Product ISO firmware HLT after BOTH-OK: inject IDT `0x20` (or OVMF LVT
/// if unmasked) when PIC take and LAPIC ATA IRR are empty (`pic=0`).
/// Iron COM2 `b5c3a9c`: `ataio=0` `inj=0` `pic=0` skip-after-inject.
/// Force LVT expiry; leftover `0xEF` still substitutes `0x20`. Keep PIT
/// `0x20` on this HLT wake only (iron `ea30da1` IDT[0x20]; skip-only-after-inject
...
unsafe fn try_inject_product_firmware_hlt_wake(
    ...
    // product ISO firmware HLT wake LAPIC timer: pic=0 APIC-mode CpuSleep.
    // Nested iso=0 already force_firmware_lapic_timer_expiry. Leftover
    // 0xEF substitutes 0x20. Keep 0x20 (iron ea30da1 IDT[0x20]).
    let lapic = if guest_uefi_product_firmware_hlt_wake_lapic_timer(None) {
        let _ = crate::devices::lapic_virt::force_firmware_lapic_timer_expiry();
        crate::devices::lapic_virt::take_highest_irr().map(|v| v as u8)
    } else {
        None
    };
    let vec = guest_uefi_firmware_hlt_ataio0_wake_vec(None, lapic);
