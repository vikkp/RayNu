...
pub unsafe extern "C" fn guest_uefi_vmexit() -> ! {
    ...
    if !entry_fail && !tf && !fetch_fail && n < guest_uefi_resume_cap(guest_uefi_host_hypervisor_present()) {
        resume = match basic {
            ...
            // CPU_BASED_INVLPG_EXITING (Xeon allowed0=1). Linux pause raises
            // PIT so poll_idle jiffies move (idle=poll never HLT).
            13 | 16 | 54 => skip_cpuid_msr(),
            40 => {
                linux_product_iso_raise_pit(false);
                skip_cpuid_msr()
            }
            14 => skip_invlpg(),
...
fn linux_prefer_pit_until_driver_ok() {
    crate::devices::guest_irq::prefer_pit_until_driver_ok(
