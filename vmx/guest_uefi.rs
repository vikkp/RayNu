...
fn tick_hpet_on_exit(basic: u32, gpa: u64, qual: u64) {
    ...
    let step = guest_uefi_hpet_step_for_exit(
        basic,
...
/// CPUID clobbers EAX–EDX; `hypervisor_cpuid_base` keeps `base` in a
/// callee-saved GPR. Snap it so `base += 0x100` leaves `[0x40000000, 0x40010000)`.
/// Firmware CPUID (low RIP) is not touched. Not `ISO-INSTALL-OK`.
#[cfg(target_os = "uefi")]
unsafe fn linux_hypervisor_scan_bump_callee_gprs(leaf: u32) {
    let mut gprs = [
        SAVED_RBP, SAVED_R10, SAVED_R11, SAVED_R12, SAVED_R13, SAVED_R14, SAVED_R15,
    ];
    if !guest_uefi_linux_hypervisor_scan_bump_gprs(leaf, &mut gprs) {
        return;
    }
    SAVED_RBP = gprs[0];
    SAVED_R10 = gprs[1];
