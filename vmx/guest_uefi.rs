...
/// `ataio=0`); [`guest_uefi_nested_iso0_firmware_hlt_pit`] injects PIT.
...
/// Nested iso=0 firmware HLT after BOTH-OK: inject PIT so CpuSleep
/// WaitForInterrupt returns and IdeBus can PIO IDENTIFY.
///
...
/// `skip_after_inject(false)` stays false. nested iso=0 firmware HLT PIT.
/// nested iso=0 EDK2 IRQ0. nested iso=0 firmware LAPIC timer.
/// Not `ISO-INSTALL-OK`.
...
/// IDENTIFY WaitForInterrupt can return. Delay wake is
/// [`guest_uefi_nested_iso0_firmware_hlt_pit`] (`ataio==0`).
...
/// Nested iso=0: PIC take, else LAPIC timer, else EDK2 IRQ0 `0x68`.
/// CI `33440951898` `pic=0` `gsi2=0` `tpr=0x0`: CpuSleep is APIC-mode,
/// not 8259. Product skip_pit still drops `0x20`. nested iso=0 firmware
/// LAPIC timer. nested iso=0 EDK2 IRQ0. Not `ISO-INSTALL-OK`.
...
/// Product ISO firmware HLT after BOTH-OK: inject IDT `0x20` so
/// CpuSleep WaitForInterrupt returns and IdeBus can PIO IDENTIFY.
/// Nested iso=0 uses [`guest_uefi_nested_iso0_firmware_hlt_pit`].
/// Main-path skip_pit still drops leftover `0x20` (iron `ea30da1`).
/// This HLT wake keeps `0x20` ([`guest_uefi_product_firmware_hlt_wake_idt20`]).
/// Never leftover LVT `0xEF`. `wait_for_irq` stays false.
/// Also VMX preemption (CF8 walk / Delay before CpuSleep HLT).
...
/// Product firmware `ataio==0` wake on HLT or VMX preemption.
/// Iron CF8 walk after BOTH-OK is I/O + preempt, then CpuSleep HLT.
/// Unmask LVT / inject `0x20` before the first HLT. Do not skip RIP on
/// preempt (skip_after_inject is HLT-only). CI `33456465331` VMXON-SKIP.
...
/// Product firmware `ataio==0` wake on ACPI PM timer I/O (Delay via
/// `0xB008` / `0x408` / `0xB000` dword). Iron `084430f` Delay then HLT;
/// a tight timer poll never HLT/preempt. Do not wake on CF8 (PciBus
/// must not take IRQ 0 mid-walk). skip_after_inject stays HLT-only.
...
/// on this HLT. Consumes `take_ide_pci_cmd_ata_hlt()`. CpuSleep without
/// that latch still uses IDT `0x20`. product ISO firmware IDE cmd ATA on HLT.
/// firmware skip PIT inject. Not `ISO-INSTALL-OK`.
...
/// so EDK2 `0x68` may be empty. skip-only-after-inject skips HLT so IRET
/// lands on RET. Main-path skip_pit still drops `0x20`. `wait_for_irq`
/// stays false. product ISO firmware HLT wake. product ISO firmware HLT
/// wake IDT 0x20. product ISO firmware HLT wake IDT 0x20 only.
/// firmware skip PIT inject. Not `ISO-INSTALL-OK`.
...
/// Nested iso=0 firmware HLT PIT wakes CpuSleep after BOTH-OK (`pci_ide=1`).
...
unsafe fn try_inject_guest_irq(reason: u32) {
    ...
    if guest_uefi_firmware_skip_pit_inject(linux, vec) {
        ...
        // VM-entry them (iron ea30da1). Substitute EDK2 0x68 on firmware
        // HLT ataio==0 so CpuSleep still WaitForInterrupt. product ISO
        // firmware HLT wake. PIC prefers slave IRQ 14 over PIT when both
...
unsafe fn try_inject_nested_iso0_firmware_hlt_pit(reason: u32) {
    ...
    // take_nested_iso0_pit_or_edk2: PIC take else EDK2 0x68.
    // nested iso=0 firmware LAPIC timer when pic=0 (CI 33440951898).
    ...
    // PIC take when ready; else force LVT expiry; else EDK2 0x68.
    // Product skip_pit / wait_for_irq / skip_after_inject(false) stay.
    // nested iso=0 firmware LAPIC timer. nested iso=0 EDK2 IRQ0.
    let pic = crate::devices::guest_irq::take_nested_iso0_pit();
    if pic.is_none()
        && guest_uefi_nested_iso0_firmware_lapic_timer(false, linux, pci_ide, ataio, reason)
    {
        let _ = crate::devices::lapic_virt::force_firmware_lapic_timer_expiry();
    }
    let lapic = crate::devices::lapic_virt::take_highest_irr().map(|v| v as u8);
    let vec = guest_uefi_nested_iso0_inject_vec(pic, lapic);
...
unsafe fn try_inject_product_firmware_hlt_wake(
    ...
    // after a successful 0x76 inject so IF=0 interrupt-window does not drop it.
    ...
    // without injecting. ATA 0x76 waits for CpuSleep.
    ...
    // product ISO firmware HLT wake LVT unmask: masked 0xEF becomes
    // periodic 0x20. Do not take_highest_irr (virtio can steal).
    // CI 33455373334 VMXON-SKIP. Do not F11 f37674f / --run 33455373334.
    if guest_uefi_product_firmware_hlt_wake_lvt_unmask(linux) {
        let _ = crate::devices::lapic_virt::force_firmware_lapic_timer_expiry();
    }
    let _ = guest_uefi_product_firmware_hlt_wake_idt20_only();
    let use_ata = guest_uefi_product_firmware_ide_cmd_ata_on_hlt(
        linux,
        pci_ide,
        ataio,
        reason,
    ...
    let vec = if use_ata {
        // product ISO firmware IDE cmd ATA on HLT: IdeBus Start wants IRQ 14
        // at CpuSleep, not mid-PciIo. product ISO firmware IDE cmd inject ATA.
