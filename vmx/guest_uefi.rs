...
/// Vector for IdeBus Start HLT while `ataio==0`: IDT `0x20`, not `0x76`.
/// product ISO firmware IDE cmd HLT 0x20.
...
unsafe fn try_inject_product_firmware_hlt_wake(
    ...
    let vec = if use_cmd_hlt {
        // product ISO firmware IDE cmd HLT 0x20: IDENTIFY has not run
        // (iron cmd=0x00 ataio=0 pic=0). Do not inject ATA 0x76 here.
