...
/// (ATAPI-OK missing). Delay/PciBus already skip HLT so PM timer IN can
/// finish; a timer IRQ here is the same class as product no LVT inject I/O.
/// Keep [`guest_uefi_nested_iso0_firmware_hlt_ata`] for `ataio>0`.
...
/// Nested iso=0 firmware HLT no PIT inject: skip CpuSleep after BOTH-OK; do not inject 0x20.
...
unsafe fn try_inject_nested_iso0_firmware_hlt_pit(reason: u32) {
    ...
    // nested iso=0 firmware HLT no PIT inject. CI 33464757885 VMXON:
    // inject vec=0x20 after HLT skip ran timer ISR CPUID ataio=0.
    // skip_hlt already advanced CpuSleep; do not VM-entry leftover 0x20.
    if guest_uefi_nested_iso0_firmware_hlt_no_pit_inject(
