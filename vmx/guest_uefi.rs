...
/// was WB vs coerced VGA FIX UC (`mtrr259=0x0`).
...
/// True for PEI VGA IoMemory / coerced FIX `[0xA0000, 1MiB)`.
pub fn guest_uefi_gpa_in_vga_fix_uc(gpa: u64) -> bool {
    gpa >= crate::vmx::guest_pt::IDENTITY_VGA_A0000
        && gpa < crate::vmx::guest_pt::IDENTITY_FIXED_MTRR_1M
}
...
/// Paint live CR3 GPA0 4 K VGA leaves to PAT-UC (iron `ddbd866` WB vs FIX UC).
///
/// Requires PD[0] already split to a 4 K table. Do **not** skip `ebecc9c3`.
    ...
    Poke: Fn(u64, u64) -> bool,
{
    ...
    while gpa < crate::vmx::guest_pt::IDENTITY_FIXED_MTRR_1M {
        let i = gpa >> 12;
...
/// True for FIX covering PEI VGA IoMemory `[0xA0000, 1MiB)` (`0x259`, `0x268`–`0x26F`).
/// `0x250`/`0x258` stay WB (`[0, 640KiB)`).
...
unsafe fn launch_uefi(
    ...
    // variable UC (2GiB hole) and coerce VGA FIX to UC.
...
/// coerced FIX UC. PAT-UC those 4 K leaves on the live CR3.
#[cfg(target_os = "uefi")]
