...
use crate::arch::cpu::{
    ...
    IA32_GS_BASE, IA32_PAT, IA32_SYSENTER_CS, IA32_SYSENTER_EIP, IA32_SYSENTER_ESP, IA32_VMX_BASIC,
    IA32_VMX_CR0_FIXED0,
...
/// IA32_VMX_MISC (SDM A.6): bits 4:0 = X, the preemption timer counts down
/// once every 2^X TSC cycles.
#[cfg(target_os = "uefi")]
const IA32_VMX_MISC: u32 = 0x485;
/// Preemption-timer rate shift used until IA32_VMX_MISC is read (Skylake-SP
/// reports 5).
const PREEMPT_SHIFT_DEFAULT: u32 = 5;
/// TSC rate assumed when the pre-EBS `Stall` calibration is missing.
const UART_PACE_TSC_HZ_FALLBACK: u64 = 2_000_000_000;
/// UART line-rate pacing period. While the shared guest TX ring holds bytes
/// the VMX preemption timer is clamped to this so the ring drains toward
/// iDRAC SOL at the port's pace even when the guest is `idle=poll` with no
/// other VM exit (iron `916af96`: ~15 exits/s, one byte each). 250 µs ×
/// 16-byte FIFO windows = 64 kB/s ceiling, above 115200 baud; without a
/// FIFO 4 kB/s, still 250× the stall. guest UART line-rate pace.
/// Not `ISO-INSTALL-OK`.
pub const UART_PACE_US: u64 = 250;

/// ADR-016: RayNu-F does not mutate a third-party firmware's internal control
/// state to manufacture progress. The Stage 46 `3k–3o` OVMF forcing
/// (WaitForEvent force-return, gState=4 poke, event-#PF inject) self-inflicted
/// the NULL-event `#PF cr2=0xffffffffffffffb8` + `CpuDeadLoop` on iron
/// `9474ab6` / `4e16b59`. Those mutations are disabled; the retained-OVMF
/// VMLAUNCH stays only as a read-only diagnostic/fallback. The E5 close path is
/// RayNu-F (be the guest firmware ourselves), not puppeting OVMF.
/// Not `ISO-INSTALL-OK`.
const RAYNU_F_NO_FW_STATE_MUTATION: bool = true;
...
/// After DRIVER_OK, virtio MMIO must not raise unpaced PIT (iron `896424f`
/// `virtblk_probe` lockup). Probe still raises. Overlay jiffies come from
/// paced resume. linux virtio MMIO PIT until DRIVER_OK. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_linux_virtio_mmio_raises_pit(
    linux: bool,
    product_iso: bool,
    need_until_driver_ok: bool,
) -> bool {
    guest_uefi_virtio_mmio_raises_pit(linux, product_iso) && need_until_driver_ok
}

/// After both virtio functions reach DRIVER_OK, still prefer PIT once so
/// Alpine apk overlay / `sleep` keep jiffies moving while UART THRE is
/// latched. UART wins the following inject (auto-answer / printk).
/// Call on virtio MMIO, VMX preempt, HLT (until login:), and UART I/O.
/// linux PIT once after DRIVER_OK. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_linux_prefer_pit_during_apk(need_until_driver_ok: bool) -> bool {
    if need_until_driver_ok {
        crate::devices::guest_irq::prefer_pit_hold(true);
        false
    } else {
...
/// When Linux product-ISO should prefer/raise PIT for jiffies.
///
/// Firmware queue-arm + PHASE_LOGIN is not enough: that is true from the
/// first Linux instruction. Iron `c815ccc` / `34078335291` injected PIT
/// during APIC setup (`MADT` then I/O `reason=0x1e` `leave_to_e4`).
/// Arm only after Linux writes DEVICE_STATUS, or after both DRIVER_OK
/// while overlay apk has not reached `login:`.
/// linux PIT after virtio probe. linux PIT hold after DRIVER_OK.
/// linux PIT hold until login. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_linux_pit_jiffies_now(
    virtio_need: bool,
    overlay_needs_pit: bool,
    probe_started: bool,
    both_driver_ok: bool,
) -> bool {
    (virtio_need && probe_started) || (overlay_needs_pit && both_driver_ok)
}

/// Hold PIT over UART while Linux is probing virtio or apk overlay
/// (`idle=poll`) after DRIVER_OK. Iron `20e8b70` / `34076175624`: PIT-once
/// consumed, UART won, last virtio `n=1345`. linux PIT hold until login.
/// Not `ISO-INSTALL-OK`.
pub fn guest_uefi_linux_prefer_pit_hold(
    virtio_need: bool,
    overlay_needs_pit: bool,
    probe_started: bool,
    both_driver_ok: bool,
) -> bool {
    let need = guest_uefi_linux_pit_jiffies_now(
        virtio_need,
        overlay_needs_pit,
        probe_started,
        both_driver_ok,
    );
    crate::devices::guest_irq::prefer_pit_hold(need);
    need
}

/// Raise PIT on Linux product-ISO resume only after both DRIVER_OK
/// while overlay apk has not reached `login:`. Iron `69102aa` /
/// `34080595540` raised on every resume as soon as Linux wrote
/// DEVICE_STATUS (`vp_set_status` / `iowrite8`); IRQ0 storm then
/// `watchdog: soft lockup` in `modprobe` virtio_pci_probe, then I/O
/// `reason=0x1e` `leave_to_e4`. Virtio MMIO / HLT / preempt still
/// raise during probe. `idle=poll` never HLT after DRIVER_OK.
/// Iron UDisk `896424f`: MADT + `DRIVER_OK` lived, then the same
/// `virtblk_probe` `iowrite8` / `handle_softirqs` lockup because
/// resume still raised PIT on every VM-entry. Callers must also
/// pass [`guest_uefi_linux_pit_resume_elapsed`].
/// linux PIT raise after DRIVER_OK not probe.
/// linux PIT raise on overlay resume. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_linux_raise_pit_on_resume(
    overlay_needs_pit: bool,
    _virtio_need: bool,
    _probe_started: bool,
    both_driver_ok: bool,
) -> bool {
    overlay_needs_pit && both_driver_ok
}

/// ~1 ms at 2 GHz. Overlay `idle=poll` still needs periodic PIT
/// (iron `20e8b70` / `n=1345`); every-exit is a nested-IRQ livelock
/// (iron `896424f` `virtblk_probe`). linux PIT resume min tsc.
/// Not `ISO-INSTALL-OK`.
pub const LINUX_PIT_RESUME_MIN_TSC: u64 = 2_000_000;

/// True when `min_delta` host TSC ticks have passed since `last`.
/// `last == 0` is due (first overlay resume injects once).
/// linux PIT resume paced. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_linux_pit_resume_elapsed(now: u64, last: u64, min_delta: u64) -> bool {
    last == 0 || now.wrapping_sub(last) >= min_delta
}

/// Policy [`guest_uefi_linux_raise_pit_on_resume`] and TSC cadence.
/// linux PIT resume paced. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_linux_raise_pit_on_resume_due(
    overlay_needs_pit: bool,
    both_driver_ok: bool,
    tsc_now: u64,
    tsc_last: u64,
    ...
    guest_uefi_linux_raise_pit_on_resume(overlay_needs_pit, false, false, both_driver_ok)
        && guest_uefi_linux_pit_resume_elapsed(tsc_now, tsc_last, min_delta)
}

/// HLT after DRIVER_OK still prefers PIT while overlay apk has not
/// reached getty `login:`. Iron `63c7e05` mounted media (jiffies via
/// virtio MMIO) then froze at `Installing packages` `n=1345` because
/// cpuidle HLT kept UART first. linux HLT PIT during apk.
/// Not `ISO-INSTALL-OK`.
pub fn guest_uefi_linux_hlt_prefer_pit_during_apk(
    linux: bool,
    product_iso: bool,
    need_until_driver_ok: bool,
    overlay_needs_pit: bool,
) -> bool {
    linux && product_iso && !need_until_driver_ok && overlay_needs_pit
}

/// UART LSR/`IN` after DRIVER_OK must re-arm PIT-once: printk polls
/// LSR in a tight I/O loop that resets the VMX preempt timer.
/// linux UART PIT during apk. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_linux_uart_prefer_pit_during_apk(
    uart: bool,
    need_until_driver_ok: bool,
    overlay_needs_pit: bool,
) -> bool {
    uart && !need_until_driver_ok && overlay_needs_pit
}

/// HLT after `login:` keeps UART ahead of PIT so auto-answer is not starved.
/// Overlay apk still prefers PIT (see [`guest_uefi_linux_hlt_prefer_pit_during_apk`]).
/// linux HLT UART after DRIVER_OK. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_linux_hlt_uart_after_driver_ok(
    linux: bool,
    product_iso: bool,
    need_until_driver_ok: bool,
    overlay_needs_pit: bool,
...
/// After the first 32 virtio MMIO hits, heartbeat every 64 so iron SOL
/// still shows apk overlay I/O. virtio MMIO heartbeat 64.
/// Not `ISO-INSTALL-OK`.
pub fn guest_uefi_virtio_mmio_heartbeat(n: u32) -> bool {
    n < 32 || n % 64 == 0
}
...
/// hid whether ISO ISR was ACKed. virtio MMIO heartbeat kick.
/// Not `ISO-INSTALL-OK`.
///
/// `kick_due` rate-limits the overlay kick lines
/// ([`guest_uefi_virtio_mmio_kick_line_due`]): iron `916af96` printed one
/// ~60-byte line per apk virtio hit and filled the shared TX ring
/// (`ring=2048`), evicting the guest's own console text. The every-64
/// heartbeat is never throttled. virtio MMIO kick throttle.
pub fn guest_uefi_virtio_mmio_heartbeat_kick(
    n: u32,
    off: u16,
    overlay: bool,
    kick_due: bool,
) -> bool {
    guest_uefi_virtio_mmio_heartbeat(n)
        || (overlay && kick_due && (off == 0x100 || off == 0x300))
}

/// Minimum TSC gap between two overlay kick lines (~250 ms at 2 GHz).
pub const VIRTIO_MMIO_KICK_LINE_MIN_TSC: u64 = 500_000_000;

/// One overlay kick line per [`VIRTIO_MMIO_KICK_LINE_MIN_TSC`]; the first
/// one (`last == 0`) is always due. virtio MMIO kick throttle.
pub fn guest_uefi_virtio_mmio_kick_line_due(now: u64, last: u64, min: u64) -> bool {
    last == 0 || now.wrapping_sub(last) >= min
}

/// Clamp the preemption timer to the UART pacing period only while Linux
/// earlycon share is on, the product ISO window is armed and the shared
/// guest TX ring holds bytes. iso=0 / firmware never pace.
/// guest UART line-rate pace. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_uart_pace_wanted(share: bool, product_iso: bool, ring_len: usize) -> bool {
    share && product_iso && ring_len > 0
}

/// Preemption-timer ticks for [`UART_PACE_US`] at `tsc_hz` with rate shift
/// `shift` (IA32_VMX_MISC bits 4:0). Never 0.
pub fn guest_uefi_uart_pace_ticks(tsc_hz: u64, shift: u32) -> u32 {
    let hz = if tsc_hz == 0 { UART_PACE_TSC_HZ_FALLBACK } else { tsc_hz };
    let tsc = hz.saturating_mul(UART_PACE_US) / 1_000_000;
    let ticks = tsc >> shift.min(31);
    ticks.clamp(1, u64::from(u32::MAX)) as u32
}

/// Value to arm on resume and whether that arming is a pacing clamp.
///
/// `due` is the TSC at which the nominal HPET/PIT preemption tick is owed
/// (set at the last un-paced resume). Paced exits fire early and must not
/// step HPET or raise PIT; when the nominal tick is closer than one pacing
/// period the remaining time is armed instead and that exit is a real tick.
/// guest UART line-rate pace.
pub fn guest_uefi_preempt_arm(
    reload: u32,
    pace: bool,
    pace_ticks: u32,
    now: u64,
    ...
    let remaining = if due > now {
        ((due - now) >> shift.min(31)).min(u64::from(reload)) as u32
    } else {
        0
    };
    if remaining > pace_ticks {
        (pace_ticks, true)
    } else {
...
/// Firmware virtual-wire leftover GSI 2 must not steal Linux PIC virtio.
/// Iron `c61942b` / `34135448354`: `DRIVER_OK` + `linux PIT resume paced`,
/// `APIC: ACPI MADT or MP tables are not detected`, **no** `linux PIC IRQ0`,
/// apk overlay froze at virtio `n=1345`. `gsi2_armed` leftover is ignored
/// until Linux writes pin 2. linux PIC before leftover GSI 2.
/// Not `ISO-INSTALL-OK`.
pub fn guest_uefi_linux_pic_before_leftover_gsi2(
    pic_ready: bool,
    leftover_gsi2: bool,
    linux_wrote_gsi2: bool,
) -> bool {
    let _ = leftover_gsi2;
    guest_uefi_linux_pic_before_lapic(pic_ready, linux_wrote_gsi2)
}

/// Overlay apk (`idle=poll`) must re-raise virtio INTx while ISR is still 1.
/// Iron `0a9b552` / `34141401594`: leftover GSI 2 was a no-op (RayNu-F never
/// armed pin 2); apk froze at virtio `n=1345` after a shared IRQ 11 ISR read
/// dropped the sibling. linux virtio INTx reassert. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_linux_reassert_virtio_intx(
    overlay_needs_pit: bool,
    both_driver_ok: bool,
    isr_latched: bool,
) -> bool {
    overlay_needs_pit && both_driver_ok && isr_latched
}

/// Linux / firmware 8259 slave ICW2 + IRQ 11. linux virtio PIC 11.
///
/// Firmware/OVMF ICW2 0x28 → 0x2b. Linux x86_64 `IRQ0_VECTOR` is 0x30 so
/// slave ICW2 is 0x38 → IRQ 11 is 0x3b. Iron `fcac0cd` / `34145652767`:
/// eight `inject vec=0x30` during NMI selftest (PIC IRQ0), then
/// `linux virtio INTx reassert`, then apk freeze `n=1345` with **no**
/// `linux virtio PIC 11` because this predicate only matched 0x2b.
/// Not `ISO-INSTALL-OK`.
pub fn guest_uefi_linux_virtio_pic_vec(vec: u32) -> bool {
    let irq = u32::from(crate::devices::guest_irq::VIRTIO_PIC_IRQ) - 8;
    vec == 0x28 + irq || vec == 0x38 + irq
}
...
/// CpuSleep is `hlt; ret` (`f4c3`). `[RSP]` is the caller.
/// Iron COM2 `27eda8c` hid slot-0 (CDROM-OK via PIIX) and still hung
/// `rip=0x7f0680d0` `ataio=0`. BAR conflict is closed. print HLT retaddr.
/// Iron COM2 `2d4ab51` `ret=0x7ff0e055` (DxeCore-range; AcpiTimer at
/// `0x7ff635d2`, not PciBus `0x7f01`) still `ataio=0`. firmware ConIn CR.
/// Not `ISO-INSTALL-OK`.
pub fn guest_uefi_cpusleep_retaddr(slot: u64, long_mode: bool) -> u64 {
    if long_mode {
        slot
    } else {
        slot & 0xffff_ffff
...
pub fn last_hlt_retaddr() -> u64 {
    LAST_HLT_RET.load(Ordering::Acquire)
}

/// 8 bytes immediately before CpuSleep return (`ret-8`).
/// Iron COM2 `6c4bfde` ConIn CR fired; HLT `ret=0x7ff0e055` still
/// `ataio=0`. DxeCore WaitForEvent is a timer wait, not serial ConIn.
/// print HLT callsite. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_cpusleep_callsite(bytes: [u8; 8]) -> u64 {
    u64::from_le_bytes(bytes)
}

pub fn last_hlt_callsite() -> u64 {
    LAST_HLT_CALL.load(Ordering::Acquire)
}

/// Iron COM2 `2d4ab51` HLT `ret=0x7ff0e055` is DxeCore WaitForEvent
/// (same image as AcpiTimer `0x7ff635d2`), not PciBus `0x7f01xxxx`.
/// EnableAttributes + ROM probe already ran; IdeBus never issued ATA.
/// One CR into guest COM1 so a serial ConIn wait can return. Not HLT
/// policy. Not PIT. iso=0 does not push. firmware ConIn CR.
/// Not `ISO-INSTALL-OK`.
pub fn guest_uefi_firmware_conin_cr_on_hlt(
    product_iso: bool,
    pci_ide: bool,
    ataio: u32,
    already: bool,
) -> bool {
    product_iso && pci_ide && ataio == 0 && !already
}

/// Iron COM2 `0b770cd` `rethx=0xe056ff41b84d8b48` =
/// `48 8b 4d b8 41 ff 56 e0` = `mov rcx,[rbp-0x48]; call [r14-0x20]`.
/// DxeCore WaitForEvent invokes CpuSleep that way. CpuSleep returns
/// (HLT skip after one-shot) and the wait loops — the event never
/// signals. PIT `0x20` and ConIn CR did not complete it.
pub const GUEST_UEFI_WFE_HLT_RET: u64 = 0x7ff0_e055;
pub const GUEST_UEFI_WFE_CALLSITE: u64 = 0xe056_ff41_b84d_8b48;

/// One-shot: unwind that WaitForEvent as `EFI_SUCCESS` (`xor eax,eax;
/// leave; ret`). Not HLT policy. Not another print. iso=0 does not
/// unwind. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_waitforevent_force_return(
    product_iso: bool,
    pci_ide: bool,
    ataio: u32,
    already: bool,
    ...
    product_iso && pci_ide && ataio == 0 && !already && retaddr == GUEST_UEFI_WFE_HLT_RET
}

/// Iron COM2 `e0d5c55` WaitForEvent return `caller=0x7feffe28`, then
/// EPT `reason=0x34` `rip=0x7ec8f6ff`
/// `insn=31c031ff8903eb0348ffc7493b7e1873` still `ataio=0`.
/// `xor eax,eax; xor edi,edi; mov [rbx],eax` is firmware ZeroMem.
/// Not a second unwind. Not `ISO-INSTALL-OK`.
pub const GUEST_UEFI_WFE_CALLER: u64 = 0x7fef_fe28;
pub const GUEST_UEFI_ZEROMEM_EPT_RIP: u64 = 0x7ec8_f6ff;
pub const GUEST_UEFI_ZEROMEM_FILL_CAP: u64 = 16 * 1024 * 1024;

/// COM2 when the post-WFE ZeroMem EPT is filled host-side.
pub const GUEST_UEFI_ZEROMEM_EPT_PREFIX: &str = "boot: guest-UEFI firmware ZeroMem ept gpa=0x";

/// Named iron RIP after `e0d5c55` WFE return. iso=0 does not fill.
pub fn guest_uefi_firmware_zeromem_ept(
    product_iso: bool,
    pci_ide: bool,
    ataio: u32,
    wfe_done: bool,
    ...
    product_iso && pci_ide && ataio == 0 && wfe_done && rip == GUEST_UEFI_ZEROMEM_EPT_RIP
}

/// `31 c0 31 ff 89 03` = `xor eax,eax; xor edi,edi; mov [rbx],eax`.
pub fn guest_uefi_firmware_zeromem_insn(b: [u8; 6]) -> bool {
    b == [0x31, 0xc0, 0x31, 0xff, 0x89, 0x03]
}

/// Dest must be low identity or leftover report-RAM. Cap 16 MiB so a
/// smashed `rbx` / `[r14+0x18]` cannot wipe the pool. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_firmware_zeromem_dest_ok(dest: u64, count: u64) -> bool {
    if dest == 0 || count == 0 || count > GUEST_UEFI_ZEROMEM_FILL_CAP {
        return false;
    }
    let end = dest.saturating_add(count);
    if end < dest || end == dest {
        return false;
    }
    if dest < GUEST_UEFI_LOW_RAM_BYTES {
        return end <= GUEST_UEFI_LOW_RAM_BYTES;
    }
    guest_uefi_report_ram_should_map(dest) && guest_uefi_report_ram_should_map(end.saturating_sub(1))
}

/// Iron COM2 `c8d504d`: after WFE return, preemption timer (`reason=0x34`,
/// not EPT `0x30`) at firmware report-RAM. Noskip
/// `insn=f30f1efa48833d4c9b00000448b80300` =
/// `endbr64; cmp qword [rip+disp], 4; mov rax, …`. Existing
/// [`preempt_deadloop_skip_len`] only skips pause / backward jcc /
/// delay_loop (`cmp [rip], 0`). Forward `je +0x6c` at `0x7ec8f639`
/// is not skipped. Not `ISO-INSTALL-OK`.
pub const GUEST_UEFI_WFE_PREEMPT_SKIP_PREFIX: &str =
    "boot: guest-UEFI firmware WFE preempt skip len=";

/// Bytes to advance after WFE when the named noskip pattern matches.
///
/// INVARIANTS:
/// - 0 unless product ISO + IDE enumerated + `ataio==0` + WFE already returned
/// - 12: `endbr64` + `cmp qword [rip+disp32], imm8`
/// - 4: `endbr64` alone
/// - 8: `cmp qword [rip+disp32], imm8` alone
/// - never a forward `jcc`
pub fn guest_uefi_firmware_wfe_preempt_skip_len(
    product_iso: bool,
    pci_ide: bool,
    ataio: u32,
    wfe_done: bool,
    ...
    if !(product_iso && pci_ide && ataio == 0 && wfe_done) {
        return 0;
    }
    if bytes.len() >= 12
        && bytes[0] == 0xF3
    ...
    if bytes.len() >= 8 && bytes[0] == 0x48 && bytes[1] == 0x83 && bytes[2] == 0x3D {
        return 8;
    }
    0
}

/// Iron COM2 `d0e44d4`: WFE preempt skip **hit**
/// (`len=12 rip=0x7ff0e7e8`) then the same `endbr64+cmp [rip],4`
/// re-entered (`spin jmp skip` ×8) still `ataio=0`. Skip-12 lands on
/// `mov rax, 3` (not-ready). Do not skip RIP. Host-store `4` at the
/// RIP-relative dest so the cmp can succeed. Not `ISO-INSTALL-OK`.
pub const GUEST_UEFI_WFE_STATE4_RIP: u64 = 0x7ff0_e7e8;
pub const GUEST_UEFI_WFE_STATE4_DEST: u64 = 0x7ff1_8340;
pub const GUEST_UEFI_WFE_STATE4_VAL: u64 = 4;
pub const GUEST_UEFI_WFE_STATE4_POKE_PREFIX: &str =
    "boot: guest-UEFI firmware WFE state4 poke dest=0x";

/// Dest of `endbr64; cmp qword [rip+disp32], 4`. `None` if the bytes
/// are not that encoding. Iron `d0e44d4` `rip=0x7ff0e7e8` → `0x7ff18340`.
pub fn guest_uefi_firmware_wfe_state4_dest(rip: u64, bytes: &[u8]) -> Option<u64> {
    if bytes.len() < 12 {
        return None;
    }
    ...
    if bytes[4] != 0x48 || bytes[5] != 0x83 || bytes[6] != 0x3D || bytes[11] != 0x04 {
        return None;
    }
    let disp = i32::from_le_bytes([bytes[7], bytes[8], bytes[9], bytes[10]]);
    Some(rip.wrapping_add(12).wrapping_add(disp as i64 as u64))
}

/// Iron COM2 `9474ab6`: `cr2=0xffffffffffffffb8` is `[r14-0x48]` with
/// `r14=0` (`cmp qword [r14-0x48], 'evnt'`). `identity_signext32_gpa`
/// would treat this as hole GPA `0xffffffb8`. Not a 32-bit MMIO leftover.
pub fn guest_uefi_pf_is_null_relative(cr2: u64) -> bool {
    let s = cr2 as i64;
    s < 0 && s > -0x1000
}

/// Iron COM2 `9474ab6` after state4 poke: HV printed
/// `#PF MMIO skip — RIP left 32MiB RAM` then stopped. RIP `0x7ff0e018`
/// is DxeCore in report-RAM (same image as WaitForEvent `0x7ff0e055`).
/// `insn=49817eb865766e74` = `cmp qword [r14-0x48], 0x746e7665` (`evnt`).
/// Deliver `#PF` to the guest IDT. Do not MMIO-map. Do not skip RIP.
/// Not `ISO-INSTALL-OK`.
pub const GUEST_UEFI_WFE_EVENT_PF_CR2: u64 = 0xffff_ffff_ffff_ffb8;
pub const GUEST_UEFI_WFE_EVENT_PF_RIP: u64 = 0x7ff0_e018;
pub const GUEST_UEFI_EVENT_SIG: u32 = 0x746e_7665;
pub const GUEST_UEFI_WFE_EVENT_PF_PREFIX: &str =
    "boot: guest-UEFI firmware WFE event #PF cr2=0x";

/// `cmp qword [r14-0x48], imm32` (CoreCheckEvent signature).
pub fn guest_uefi_firmware_wfe_event_pf_insn(b: [u8; 4]) -> bool {
    b == [0x49, 0x81, 0x7e, 0xb8]
}

/// Exception bitmap bit 14 (`#PF`). Firmware inject drops this so the
/// VM-entry event reaches the guest IDT; restore it on the next non-#PF
...
/// After the one-shot firmware event `#PF` inject, OR bit 14 back.
pub fn guest_uefi_firmware_wfe_event_pf_restore_bitmap(
    event_pf_done: bool,
    exit_is_pf: bool,
    bitmap: u64,
) -> Option<u64> {
    if event_pf_done && !exit_is_pf && (bitmap & GUEST_UEFI_EXC_BITMAP_PF) == 0 {
        Some(bitmap | GUEST_UEFI_EXC_BITMAP_PF)
    } else {
        None
    }
}

/// One-shot after WFE+state4. iso=0 does not deliver. RIP must be
/// leftover report-RAM (DxeCore), not the 32 MiB identity hole.
pub fn guest_uefi_firmware_wfe_event_pf(
    product_iso: bool,
    pci_ide: bool,
    ataio: u32,
    wfe_done: bool,
    ...
    product_iso
        ...
        && wfe_done
        && state4_done
        && guest_uefi_pf_is_null_relative(cr2)
        && guest_uefi_report_ram_should_map(rip)
}

/// One-shot after WFE. Dest must already be a valid report-RAM / low-RAM
/// qword. iso=0 does not poke. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_firmware_wfe_state4_poke(
    product_iso: bool,
    pci_ide: bool,
    ataio: u32,
    wfe_done: bool,
    ...
    product_iso
        ...
        && !already
        && dest != 0
        && guest_uefi_firmware_zeromem_dest_ok(dest, 8)
}

/// WaitForEvent frame: `[RBP]=prev`, `[RBP+8]=caller`, `leave` →
/// `RSP=RBP+16`. Reject a smashed or null frame.
pub fn guest_uefi_waitforevent_leave(rbp: u64, prev_rbp: u64, caller: u64) -> Option<(u64, u64)> {
    if rbp < 16 || prev_rbp == 0 || caller < 0x1000 {
        None
    } else {
        Some((caller, rbp.wrapping_add(16)))
    }
}
...
/// until the first PIT. Iron COM2 `e3cbfa5` / `--run 33558261624`:
/// one-shot `0x20` printed, firmware IRET'd to `rip=0x7f0680d0`
/// (`reason=0xc` `insn=f4c3` `inj=1` `ataio=0` through n>800k). Waiting
/// after that is a deadlock (further PIT is skipped). After the first
/// wake, do not wait — skip HLT without more PIT. Nested iso=0
/// (`pci_ide` false) still skips without inject. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_firmware_hlt_wait_for_irq(
    ...
    guest_uefi_firmware_hlt_wait_for_irq_oneshot(
        ...
        pci_ide,
        ataio,
        FIRMWARE_PIT_ONESHOT.load(Ordering::Acquire),
    )
}

/// Wait only before the first firmware PIT. `pit_oneshot` is iron
/// COM2 `e3cbfa5` (one inject, then HLT hang).
pub fn guest_uefi_firmware_hlt_wait_for_irq_oneshot(
    product_iso: bool,
    _n: u32,
    reason: u32,
    pci_ide: bool,
...
/// Drop firmware PIT `vec=0x20` after the first wake, or after ATA starts.
/// Iron COM2 `b5c3a9c` skip-PIT from the start left BDS in CpuSleep
/// (`ataio=0`). Allow the **first** `0x20` so virtual-wire can leave HLT.
/// Iron COM2 `24c5fa6` / `--run 33555104832`: wait-for-irq woke
/// `rip=0x7f0680d0`, then stacked `vec=0x20` livelocked APIC/CR8
/// (`rip=0x7f03f641` / `0x7f03fbe5` / `0x7f0697a9`, `ataio=0` through
/// `n>2.5M`). After that one wake, drop further PIT until PACKET.
/// After `ataio>0`, keep dropping PIT (`ea30da1` was `pci_ide=0`).
...
/// Pure PIT-skip predicate. `pit_oneshot` is the first firmware `0x20`
/// already injected while `ataio==0` (iron COM2 `24c5fa6` storm).
pub fn guest_uefi_firmware_skip_pit_inject_oneshot(
    linux: bool,
    vec: u32,
    ataio: u32,
    pit_oneshot: bool,
) -> bool {
    if linux || !crate::devices::guest_irq::firmware_is_pit_vec(vec as u8) {
        return false;
    }
    if ataio > 0 {
        return guest_uefi_firmware_hlt_skip_without_inject(linux);
    }
    pit_oneshot
}

/// Latch after the first firmware PIT inject before ATA. Host tests use
/// the pure [`guest_uefi_firmware_skip_pit_inject_oneshot`] flag.
pub fn guest_uefi_firmware_note_pit_injected(linux: bool, vec: u32, ataio: u32) -> bool {
    if linux || ataio != 0 || !crate::devices::guest_irq::firmware_is_pit_vec(vec as u8) {
        return false;
    }
    FIRMWARE_PIT_ONESHOT
...
/// Nested iso=0 does not take skip_after_inject. skip_hlt uses VMCS
/// insn_len; when that is 0, firmware RIP is not Linux high-half so
/// linux_hlt_skip returns 0 and CpuSleep parks at `f4c3` (CI 33437881901
/// / 33438918646 `ataio=0 packet=0`). Skip 1 so Delay re-reads 0xB008.
/// firmware HLT insn_len 0 skip. Not `ISO-INSTALL-OK`.
...
/// Linux PIC IRQ 0. Firmware ICW2 0x20 → 0x20. Linux x86_64
/// `IRQ0_VECTOR` is 0x30 (iron `fcac0cd` / `34145652767` eight
/// `inject vec=0x30` during NMI selftest). linux PIC IRQ0.
/// linux PIC IRQ0 vec 0x30. Not `ISO-INSTALL-OK`.
...
pub fn guest_uefi_pf_should_map_mmio(err: u64, cr2: u64) -> bool {
    ...
    // Iron 9474ab6: `cr2=0xffffffffffffffb8` is NULL-relative, not hole MMIO.
    if guest_uefi_pf_is_null_relative(cr2) {
        return false;
    }
...
/// Drain after (and before) ISR ACK so a sibling ISO used-idx is not missed.
/// Iron `e717fb4` / `34220740109`: DATA_SEGS=128 still froze at apk `n=1345`
/// after disk ISR `off=0x100 wr=0`. virtio drain on ISR.
/// Not `ISO-INSTALL-OK`.
pub fn guest_uefi_virtio_drain_on_isr(is_write: bool) -> bool {
    !is_write
}

/// One-shot COM2 dump after ~2s without virtio MMIO (2.1GHz TSC ≈ 4e9).
/// Iron `34968f7` / `34224368343`: dump fired at usbdelay `n=1118` then apk
...
/// dump consumed the one-shot before apk. virtio stall dump again.
/// Not `ISO-INSTALL-OK`.
pub fn guest_uefi_virtio_stall_dump_reset_on_mmio(had_mmio: bool) -> bool {
    had_mmio
}

/// Re-arm the stall dump only on a queue notify, never on ISR ACK.
/// Iron `6d0c58a` / `34292570282`: dump + INTx pulse then Linux ISR
/// `off=0x100` reset `dumped` so the dump fired again every ~2s (`n=+2`
/// ISR pair, no `0x300`). virtio stall dump notify reset.
/// Not `ISO-INSTALL-OK`.
pub fn guest_uefi_virtio_stall_dump_reset_on_notify(off: u64, wr: bool) -> bool {
    wr && (off == 0x300 || off == 0x50 || off == 0x64)
}

/// When apk virtio MMIO has been quiet ~2s, raise PIT so overlay jiffies
/// move. Iron `a580299` / `34227607779`: INTx hold lived past `n=1345`;
/// second dump `last==used==avail` `isr=0` `kick=0` `n=1373` during
/// `Installing packages`. virtio stall dump PIT.
/// Not `ISO-INSTALL-OK`.
pub fn guest_uefi_virtio_stall_dump_pit(dumped: bool) -> bool {
    dumped
}

/// After an empty-ring apk dump, keep raising PIT on every resume so
/// overlay `sleep` jiffies move. Iron `b8ac361` / `34297632324`:
/// notify-reset lived (no ISR flood); usbdelay dump PIT-only; apk dump
/// `disk_last=1162 iso_last=1111 live==last n=1370` then INTx + ISR ACK
/// `n=1371/1372` and no further `0x300`. Harvest was a no-op. Do not
/// pulse INTx again. virtio stall dump PIT hold.
/// Not `ISO-INSTALL-OK`.
pub fn guest_uefi_virtio_stall_dump_pit_hold(dumped: bool, packages: bool) -> bool {
    dumped && packages
}

/// Pace dump-path PIT to [`LINUX_PIT_RESUME_MIN_TSC`]. Iron `2b3cc1e` /
/// `34300317331`: `virtio stall dump PIT hold` lived (apk dump+INTx then
/// `PIT hold n=1375`) then unpaced `raise_pit` every resume stormed IRQ0:
/// `soft lockup` in `handle_softirqs` 26s/52s/78s and `reason=0x1e`.
/// Watchdog proved jiffies already move; overlay is not a timer wait.
/// virtio stall dump PIT paced.
/// Not `ISO-INSTALL-OK`.
pub fn guest_uefi_virtio_stall_dump_pit_paced(dumped: bool, packages: bool) -> bool {
    dumped && packages
}
...
pub enum StallProbe {
    /// Print guest RIP / CS / CR3 / RFLAGS (idle loop vs user vs kernel wait).
    Sample,
    /// COM1 BREAK + this magic-SysRq key (UART sysrq break).
    Sysrq(u8),
}

/// R640 Xeon here runs TSC at ~2.1 GHz; the hold heartbeat already uses raw
/// `4_000_000_000` as ~2 s. virtio stall probe.
pub const STALL_PROBE_TSC_PER_SEC: u64 = 2_100_000_000;

/// Probe schedule after the dump, indexed by `step`, gated on `elapsed`
/// TSC since the dump. Iron `09b5842` / `34302053558`: paced IRQ0 lived for
/// minutes, rings stayed `last==used==avail==live`, no `0x300`, and the
/// `2b3cc1e` watchdog trace showed `swapper/0` in `cpu_idle_poll` — apk is
/// asleep on something that is neither our completion nor a tick. Ten
/// stimulus pins (PIT once/hold/paced, INTx pulse/reassert, IRQ11 yield)
/// never asked Linux what it waits on. This does: three RIP samples, then
/// SysRq `w` (blocked tasks), `m` (memory; guest has 288 MiB and a tmpfs
/// sysroot), `t` (every task with stack), `l` (running CPU), then a final
/// sample. Serial BREAK + key is an architected console input (ADR-016).
/// `sysrq_always_enabled` is on the patched cmdline; Alpine lts has
/// `CONFIG_MAGIC_SYSRQ=y` and no `DETECT_HUNG_TASK`, so waiting longer
/// prints nothing by itself. virtio stall probe. UART sysrq break.
/// Not `ISO-INSTALL-OK`.
pub fn guest_uefi_virtio_stall_probe(elapsed_tsc: u64, step: u8) -> Option<StallProbe> {
    const S: u64 = STALL_PROBE_TSC_PER_SEC;
    let (at, probe) = match step {
        0 => (S / 4, StallProbe::Sample),
        1 => (S / 2, StallProbe::Sample),
        2 => (3 * S / 4, StallProbe::Sample),
        3 => (S, StallProbe::Sysrq(b'w')),
        4 => (4 * S, StallProbe::Sysrq(b'm')),
        5 => (8 * S, StallProbe::Sysrq(b't')),
        6 => (40 * S, StallProbe::Sysrq(b'l')),
        7 => (60 * S, StallProbe::Sample),
        _ => return None,
    };
    if elapsed_tsc >= at {
        Some(probe)
...
/// Live avail.idx matches last drain on both queues: Linux is not
/// kicking. Iron `ba5bf8f` / `34290078274`: `disk_live=` matched
/// `disk_last=` (and iso) at apk `n=1373` after `virtio stall dump PIT`.
/// virtio stall dump INTx.
/// Not `ISO-INSTALL-OK`.
pub fn guest_uefi_virtio_stall_empty(
    disk_live: u16,
    disk_last: u16,
    iso_live: u16,
    iso_last: u16,
) -> bool {
    disk_live == disk_last && iso_live == iso_last
}

/// Pulse shared virtio INTx after an empty-ring stall dump so Linux
/// re-runs `vring_interrupt` (missed used.idx harvest). Iron `ba5bf8f`
/// / `34290078274`: PIT lived, live==last, still froze at `n=1373`.
/// virtio stall dump INTx.
/// Not `ISO-INSTALL-OK`.
pub fn guest_uefi_virtio_stall_dump_intx(dumped: bool, empty: bool) -> bool {
    dumped && empty
}
...
static SKIP_AFTER_ONESHOT_LOGGED: AtomicBool = AtomicBool::new(false);
static FIRMWARE_PIT_ONESHOT: AtomicBool = AtomicBool::new(false);
static FIRMWARE_CONIN_CR: AtomicBool = AtomicBool::new(false);
static FIRMWARE_WFE_RETURN: AtomicBool = AtomicBool::new(false);
static FIRMWARE_WFE_STATE4: AtomicBool = AtomicBool::new(false);
static FIRMWARE_WFE_EVENT_PF: AtomicBool = AtomicBool::new(false);
static FIRMWARE_ZEROMEM_EPT: AtomicBool = AtomicBool::new(false);
...
/// Last CF8 with CONFIG_ADDRESS enable (bit 31). Iron COM2 `61991be`
/// HLT `cf8=0x0` (deselect). Need the last live BDF before that write.
static LAST_CF8_EN: AtomicU32 = AtomicU32::new(0);
/// Last enabled CF8 that selected IDE `00:00.1` / `00:01.1`.
/// Iron COM2 `5de9e1c` `cf8en=0x80004008` is i440FX host `00:08.0+08`.
static LAST_CF8_IDE: AtomicU32 = AtomicU32::new(0);
/// CpuSleep `[RSP]` return address. Iron COM2 `6c4bfde` ConIn CR still
/// `ret=0x7ff0e055` `ataio=0`. print HLT callsite.
static LAST_HLT_RET: AtomicU64 = AtomicU64::new(0);
static LAST_HLT_CALL: AtomicU64 = AtomicU64::new(0);
...
/// IA32_VMX_MISC bits 4:0 read at launch (preemption timer = TSC >> shift).
static PREEMPT_SHIFT: AtomicU32 = AtomicU32::new(PREEMPT_SHIFT_DEFAULT);
/// TSC at which the nominal (HPET/PIT) preemption tick is owed while the
/// timer is clamped for UART pacing. 0 = not tracking.
static PREEMPT_NOMINAL_DUE_TSC: AtomicU64 = AtomicU64::new(0);
/// The last resume armed a UART pacing clamp, not the nominal reload.
static PREEMPT_UART_PACED: AtomicBool = AtomicBool::new(false);
/// This VM exit is a UART pacing tick (drain + inject only; no HPET step,
/// no PIT raise, no exit-count bump). Cleared at resume.
static UART_PACE_EXIT: AtomicBool = AtomicBool::new(false);
/// Pacing ticks taken since reset (stall heartbeat `pace=`).
static UART_PACE_EXITS: AtomicU32 = AtomicU32::new(0);
static UART_PACE_LOG: AtomicBool = AtomicBool::new(false);
...
/// F2b/F5/F7: secondary exit-count cap. Iron `975f8fc` (run `34474850361`)
/// proved a count is not a time bound: GRUB's menu loop on the installed
/// disk polls `ReadKeyStroke` **and** the serial terminal's LSR with no idle
/// (`run_menu` → `grub_getkey_noblock`, no `grub_cpu_idle`), so on an R640
/// that is ~2 exits per µs and the old `1_048_576` cap fired
/// (`stop exit-cap exits=1048577 svc=492778`) before the 2 s GRUB timeout
/// elapsed — nested KVM exits are 10–50× slower, so the same loop never
/// reached the cap there. The wall clock is the bound now
...
/// TSC at the RayNu-F launch/relaunch (wall cap base; `raynu_f_stop` prints
/// the elapsed ms).
static RAYNU_F_LAUNCH_TSC: AtomicU64 = AtomicU64::new(0);
...
/// Const-evaluated pristine [`crate::raynu_f::FirmwareState`] (`.rdata`).
/// F7 resets `RAYNU_F_STATE` by `copy_nonoverlapping` from here.
///
/// Iron `59ac070` / `34425781629`: `RAYNU_F_STATE = FirmwareState::new()`
/// compiled to an **81,024-byte stack frame** in `raynu_f_reset_relaunch`
/// (`__rust_probestack` then `memcpy` of the 79,704-byte `PagePool`), on a
/// 16 KiB host stack whose next-lower frame was the private VMCS — the
/// overflow rewrote the VMCS revision dword and `VMPTRLD` failed
/// (`launch failed: VMCLEAR/VMPTRLD`). A `.rdata` template is a plain
/// `memcpy` with no stack temporary. RayNu-F F7 template reset.
#[cfg(target_os = "uefi")]
static RAYNU_F_STATE_TEMPLATE: crate::raynu_f::FirmwareState = crate::raynu_f::FirmwareState::new();
#[cfg(target_os = "uefi")]
...
static LINUX_PIT_DURING_APK_LOG: AtomicBool = AtomicBool::new(false);
#[cfg(target_os = "uefi")]
static LINUX_PIT_HOLD_LOG: AtomicBool = AtomicBool::new(false);
#[cfg(target_os = "uefi")]
static LINUX_PIT_PACED_LOG: AtomicBool = AtomicBool::new(false);
#[cfg(target_os = "uefi")]
static LINUX_PIT_RESUME_TSC: AtomicU64 = AtomicU64::new(0);
#[cfg(target_os = "uefi")]
static LAST_VIRTIO_MMIO_TSC: AtomicU64 = AtomicU64::new(0);
#[cfg(target_os = "uefi")]
static LAST_VIRTIO_MMIO_HIT: AtomicU32 = AtomicU32::new(0);
#[cfg(target_os = "uefi")]
static VIRTIO_STALL_DUMPED: AtomicBool = AtomicBool::new(false);
#[cfg(target_os = "uefi")]
static VIRTIO_STALL_PIT_HOLD_LOG: AtomicBool = AtomicBool::new(false);
#[cfg(target_os = "uefi")]
static VIRTIO_STALL_PIT_HOLD_TSC: AtomicU64 = AtomicU64::new(0);
/// TSC at the apk empty-ring dump (virtio stall probe schedule base).
#[cfg(target_os = "uefi")]
static VIRTIO_STALL_DUMP_TSC: AtomicU64 = AtomicU64::new(0);
...
#[cfg(target_os = "uefi")]
static LINUX_VIRTIO_REASSERT_LOG: AtomicBool = AtomicBool::new(false);
#[cfg(target_os = "uefi")]
static LINUX_VIRTIO_PIC_LOG: AtomicBool = AtomicBool::new(false);
#[cfg(target_os = "uefi")]
...
/// Host stack pages for the guest-UEFI VM-exit path (`HOST_RSP` points at
/// the top). Every exit re-enters at the top, so only the deepest single
/// exit matters. Was 4 (16 KiB); iron `59ac070` / `34425781629` F7 relaunch
/// needed ~81 KiB (`FirmwareState::new()` stack temporary) and overflowed
/// into the private VMCS one frame below. 32 pages leaves headroom above
/// the largest release frame (~74 KiB) even if such a temporary returns.
/// guest-UEFI host stack pages.
pub const GUEST_UEFI_HOST_STACK_PAGES: u64 = 32;
/// One canary page below the host stack. Filled with
/// [`GUEST_UEFI_HOST_STACK_GUARD_BYTE`]; checked before F7 `VMCLEAR`.
/// guest-UEFI host stack guard.
pub const GUEST_UEFI_HOST_STACK_GUARD_PAGES: u64 = 1;
pub const GUEST_UEFI_HOST_STACK_GUARD_BYTE: u8 = 0x5A;
/// Physical base of the guard page (0 = not armed).
static HOST_STACK_GUARD: AtomicU64 = AtomicU64::new(0);
...
pub fn reset_guest_uefi_launch() {
    ...
    PREEMPT_NOMINAL_DUE_TSC.store(0, Ordering::Release);
    PREEMPT_UART_PACED.store(false, Ordering::Release);
    UART_PACE_EXIT.store(false, Ordering::Release);
    UART_PACE_EXITS.store(0, Ordering::Release);
    UART_PACE_LOG.store(false, Ordering::Release);
    ...
    if let Some(bytes) = crate::mgmt::durable_lun::take_durable_lun_install_disk() {
        ...
        if crate::devices::guest_virtio_blk::attach_lun(bytes as usize, !keep) {
            ...
            if crate::mgmt::nvme::nvme_io_ready() {
                ...
                serial::write_line(" (durable LUN usb; not ISO-INSTALL-OK)");
            }
            return;
        }
    }
...
pub unsafe fn attach_persist_keep_on_vmx_skip(alloc: &mut FrameAllocator) {
    if crate::mgmt::durable_lun::durable_lun_install_reserved() {
        ...
        let Some(bytes) = crate::mgmt::durable_lun::take_durable_lun_install_disk() else {
            return;
        };
        let _ = crate::devices::guest_virtio_blk::attach_lun(bytes as usize, false);
        serial::write_str("boot: Stage 46 virtio-blk install disk bytes=");
        write_dec(crate::devices::guest_virtio_blk::disk_bytes());
        serial::write_line(" keep=1 (durable LUN skip; not ISO-INSTALL-OK)");
        return;
    }
...
unsafe fn launch_uefi(
    ...
    // Guard page first (lowest), stack above it; an overflow hits the canary
    // before the VMCS / EPT frames allocated earlier (iron `59ac070`).
    let stack_total = GUEST_UEFI_HOST_STACK_GUARD_PAGES + GUEST_UEFI_HOST_STACK_PAGES;
    let Some(stack_frame) = alloc.allocate_contiguous(stack_total) else {
        return Err(GuestUefiLaunchError::LaunchSetupFailed);
    };
    let guard = stack_frame.to_phys();
    core::ptr::write_bytes(
        guard as *mut u8,
        GUEST_UEFI_HOST_STACK_GUARD_BYTE,
        (GUEST_UEFI_HOST_STACK_GUARD_PAGES * 4096) as usize,
    );
    HOST_STACK_GUARD.store(guard, Ordering::Release);
    let host_stack = guard + GUEST_UEFI_HOST_STACK_GUARD_PAGES * 4096;
    core::ptr::write_bytes(
        host_stack as *mut u8,
        0,
        (GUEST_UEFI_HOST_STACK_PAGES * 4096) as usize,
    );
    let host_rsp = (host_stack + GUEST_UEFI_HOST_STACK_PAGES * 4096) & !0xFu64;
    ...
    write_dec(fw_len);
    serial::write_byte(b'\n');
    serial::write_str("boot: guest-UEFI host stack top=0x");
    write_hex(host_rsp);
    serial::write_str(" pages=");
    write_dec(GUEST_UEFI_HOST_STACK_PAGES);
    serial::write_str(" guard=0x");
    write_hex(guard);
...
unsafe fn setup_guest_uefi_vmcs(
    ...
    if pin & PIN_BASED_VMX_PREEMPTION_TIMER != 0 {
        ...
        // SAFETY: VMX-root on a CPU that advertised VMX; IA32_VMX_MISC is
        // architectural (SDM A.6). KANI-TARGET: guest-UEFI timer rate read
        // (outside Proven Core).
        let shift = (unsafe { cpu::rdmsr(IA32_VMX_MISC) } & 0x1F) as u32;
        PREEMPT_SHIFT.store(shift, Ordering::Release);
        serial::write_str("boot: guest-UEFI VMX preemption timer for live HPET shift=");
        write_dec(u64::from(shift));
        serial::write_str(" pace_ticks=");
        write_dec(u64::from(guest_uefi_uart_pace_ticks(
            crate::boot::raynu_f_flag::tsc_hz(),
            shift,
        )));
        serial::write_line(" (guest UART line-rate pace; not ISO-INSTALL-OK)");
    }
...
/// Re-arm the VMX preemption timer for the next resume.
///
/// Nominal: [`PREEMPT_RELOAD`] ticks (live HPET / PIT cadence; restarts at
/// every un-paced resume as the hardware countdown always did). While
/// [`guest_uefi_uart_pace_wanted`] the value is clamped to
/// [`guest_uefi_uart_pace_ticks`] so the shared guest TX ring drains toward
/// iDRAC SOL at the port's pace with no help from guest exits; the nominal
/// tick keeps its deadline through the clamps ([`guest_uefi_preempt_arm`]).
/// Host-side timer only — no guest state is touched (ADR-016).
/// guest UART line-rate pace. Not `ISO-INSTALL-OK`.
#[cfg(target_os = "uefi")]
fn arm_preempt_on_resume() {
    ...
    let shift = PREEMPT_SHIFT.load(Ordering::Acquire);
    let now = cpu::rdtsc();
    let pace = guest_uefi_uart_pace_wanted(
        serial::linux_earlycon_share(),
        crate::devices::ide_cdrom::product_iso_window_armed(),
        serial::guest_tx_len(),
    );
    if !PREEMPT_UART_PACED.load(Ordering::Acquire)
        || PREEMPT_NOMINAL_DUE_TSC.load(Ordering::Acquire) == 0
    {
        PREEMPT_NOMINAL_DUE_TSC.store(
            now.wrapping_add(u64::from(reload) << shift.min(31)),
            Ordering::Release,
        );
    }
    let (value, paced) = guest_uefi_preempt_arm(
        reload,
        pace,
        guest_uefi_uart_pace_ticks(crate::boot::raynu_f_flag::tsc_hz(), shift),
        now,
        PREEMPT_NOMINAL_DUE_TSC.load(Ordering::Acquire),
        shift,
    );
    PREEMPT_UART_PACED.store(paced, Ordering::Release);
    // SAFETY: VMX-root with the guest-UEFI VMCS current (called from the
    // resume path only). KANI-TARGET: guest-UEFI preemption timer arm
    // (outside Proven Core).
    let _ = unsafe { ops::vmwrite(VMX_PREEMPTION_TIMER_VALUE, u64::from(value)) };
    if paced
        && UART_PACE_LOG
            .compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire)
            .is_ok()
    {
        serial::write_str_nowait("boot: guest-UEFI guest UART line-rate pace ticks=");
        write_dec_nowait(u64::from(value));
        serial::write_str_nowait(" fifo=");
        write_dec_nowait(u64::from(serial::com2_fifo()));
        serial::write_str_nowait(" (ring drains on the preemption timer; not ISO-INSTALL-OK)\n");
    }
}
...
fn tick_hpet_on_exit(basic: u32, gpa: u64, qual: u64) {
    ...
    // guest UART line-rate pace: not a time tick.
    if UART_PACE_EXIT.load(Ordering::Acquire) {
        return;
    }
    let now = cpu::rdtsc();
        ...
        && (basic == EXIT_REASON_PREEMPTION_TIMER || basic == EXIT_REASON_HLT)
    {
        ...
        if serial::linux_earlycon_share() {
            ...
            let overlay = crate::devices::guest_serial_answer::apk_overlay_needs_pit();
            ...
            if basic == EXIT_REASON_PREEMPTION_TIMER
                || (basic == EXIT_REASON_HLT
                    && guest_uefi_linux_hlt_prefer_pit_during_apk(true, true, need, overlay))
            {
                linux_prefer_pit_during_apk();
            }
        }
    }
}
...
pub unsafe extern "C" fn guest_uefi_vmexit() -> ! {
    ...
    // guest UART line-rate pace: a preemption exit that fired because the
    // last resume clamped the timer for the TX ring is a drain+inject tick
    // only. It steps no HPET, raises no PIT and does not count as an exit.
    let pace_exit = (reason & 0xFFFF) == EXIT_REASON_PREEMPTION_TIMER
        && PREEMPT_UART_PACED.load(Ordering::Acquire);
    UART_PACE_EXIT.store(pace_exit, Ordering::Release);
    if pace_exit {
        UART_PACE_EXITS.fetch_add(1, Ordering::AcqRel);
    }
    tick_hpet_on_exit(reason & 0xFFFF, gpa, qual);
    // linux earlycon hush HV (do not drain CHUNK on every exit).
    let _ = serial::drain_guest_tx(if pace_exit {
        serial::GUEST_TX_DRAIN_PACE
    } else {
        guest_uefi_linux_earlycon_drain(serial::linux_earlycon_share())
    });
    ...
    let n = if pace_exit {
        ...
        EXIT_COUNT.fetch_add(1, Ordering::AcqRel) + 1
    };
    let basic = reason & 0xFFFF;
    if let Some(bmp) = guest_uefi_firmware_wfe_event_pf_restore_bitmap(
        FIRMWARE_WFE_EVENT_PF.load(Ordering::Acquire),
        guest_uefi_exit_is_page_fault(basic, intr),
        ops::vmread(EXCEPTION_BITMAP).unwrap_or(0),
    ) {
        let _ = ops::vmwrite(EXCEPTION_BITMAP, bmp);
    }
        ...
        resume = match basic {
            ...
            EXIT_REASON_HLT => {
                ...
                if hlt_should_resume() {
                    ...
                    if guest_uefi_firmware_conin_cr_on_hlt(
                        crate::devices::ide_cdrom::product_iso_window_armed(),
                        pci_ide,
                        ataio,
                        FIRMWARE_CONIN_CR.load(Ordering::Acquire),
                    ) && FIRMWARE_CONIN_CR
                        .compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire)
                        .is_ok()
                    {
                        let _ = crate::devices::guest_uart::push_host_rx(b'\r');
                        serial::write_line(
                            "boot: guest-UEFI firmware ConIn CR (Stage 46; not ISO-INSTALL-OK)",
                        );
                    }
                    ...
                    let mut wfe_ok = false;
                    // ADR-016: do not force OVMF's WaitForEvent to return
                    // (unwinding without writing *Index self-inflicted the
                    // NULL-event #PF on 9474ab6). note_hlt_retaddr() is a
                    // read-only peek and stays for diagnostics.
                    let _ = note_hlt_retaddr();
                    if !RAYNU_F_NO_FW_STATE_MUTATION
                        && guest_uefi_waitforevent_force_return(
                        crate::devices::ide_cdrom::product_iso_window_armed(),
                        pci_ide,
                        ataio,
                        FIRMWARE_WFE_RETURN.load(Ordering::Acquire),
                        note_hlt_retaddr(),
                    ) && FIRMWARE_WFE_RETURN
                        .compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire)
                        .is_ok()
                    {
                        // SAFETY: guest-UEFI is VMX-halted in CpuSleep;
                        // RBP is the WaitForEvent frame (callsite uses
                        // [rbp-0x48]). Unwind is leave+ret + RAX=0.
                        // KANI-TARGET: WaitForEvent force return
                        // (outside Proven Core).
                        let caller = unsafe { apply_waitforevent_return() };
                        match caller {
                            Some(rip) => {
                                serial::write_str(
                                    "boot: guest-UEFI firmware WaitForEvent return caller=0x",
                                );
                                write_hex(rip);
                            ...
                            None => {
                                serial::write_line(
                                    "boot: guest-UEFI firmware WaitForEvent return miss (Stage 46; not ISO-INSTALL-OK)",
                                );
                            }
                        }
                    }
                            ...
                            .is_ok()
                    {
                        ...
                        serial::write_str(" ata_vec=0x");
                        write_hex_u8(crate::devices::guest_irq::firmware_ata_vec());
                        serial::write_str(" cmdwr=");
                        write_dec(crate::devices::ide_cdrom::pci_cmd_writes() as u64);
                        serial::write_str(" pcicmd=0x");
                        write_hex(u64::from(crate::devices::ide_cdrom::pci_cmd()));
                        serial::write_str(" wr=0x");
                        write_hex(u64::from(crate::devices::ide_cdrom::last_pci_cmd_wr()));
                        serial::write_str(" seq=");
                        let seq_n = crate::devices::ide_cdrom::pci_cmd_writes().min(8);
                        for i in 0..seq_n {
                            if i > 0 {
                                serial::write_str(",");
                            }
                            write_hex(u64::from(crate::devices::ide_cdrom::pci_cmd_wr_at(
                                i as usize,
                            )));
                        }
                        // Iron COM2 61991be: HLT cf8=0x0 (CONFIG_ADDRESS
                        // cleared). Print last enabled CF8 (bit 31).
                        serial::write_str(" cf8=0x");
                        write_hex_u32(LAST_CF8.load(Ordering::Acquire));
                        serial::write_str(" cf8en=0x");
                        write_hex_u32(LAST_CF8_EN.load(Ordering::Acquire));
                        serial::write_str(" cf8ide=0x");
                        write_hex_u32(LAST_CF8_IDE.load(Ordering::Acquire));
                        serial::write_str(" romwr=0x");
                        write_hex_u32(crate::devices::ide_cdrom::last_pci_rom_wr());
                        serial::write_str(" ret=0x");
                        write_hex(note_hlt_retaddr());
                        serial::write_str(" rethx=0x");
                        write_hex(note_hlt_callsite());
                        serial::write_line(" (Stage 46; not ISO-INSTALL-OK)");
                    }
                    if wfe_ok {
                        true
                        ...
                        // firmware HLT skip after ataio. Skip HLT +
                        // activity Active so ConnectAll and PACKET continue
                        // after ataio>0. Do not inject PIT: iron COM2
                        // ea30da1 vec=0x20 livelock. firmware HLT skip
                        // without inject. iron COM2 eac424b IRET-to-HLT.
                        // do not F11 90da03d. do not F11 8e81c2e.
                        if !skip_hlt() {
                        ...
                        // firmware HLT activity active. HLT completed.
                        let _ = ops::vmwrite(
                        ...
                        // Iron COM2 e3cbfa5: after PIT one-shot, wait-for-irq
                        // stayed true and CpuSleep hung (reason=0xc same RIP).
                        // Skip HLT without more PIT so ConnectAll can move.
                        if FIRMWARE_PIT_ONESHOT.load(Ordering::Acquire)
                            && SKIP_AFTER_ONESHOT_LOGGED
                                .compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire)
                                .is_ok()
                        {
                            serial::write_line(
                                "boot: guest-UEFI HLT skip after PIT one-shot (Stage 46; not ISO-INSTALL-OK)",
                            );
                        }
                        ...
                        // firmware HLT insn_len 0 skip: nested iso=0
                        // skip_after_inject is false; skip_hlt alone
                        // parked CpuSleep at f4c3 (ataio=0).
                        if !skip_hlt() {
                            let hlt_rip = ops::vmread(GUEST_RIP).unwrap_or(0);
            ...
            EXIT_REASON_PREEMPTION_TIMER => {
                ...
                // guest UART line-rate pace ticks skip the walk.
                let ram_hpa = RAM_HPA.load(Ordering::Acquire);
                let cr3 = ops::vmread(GUEST_CR3).unwrap_or(0);
                if !UART_PACE_EXIT.load(Ordering::Acquire)
                    && ram_hpa != 0
                    && cr3 != 0
                    && !guest_uefi_rip_is_hole_execute(rip)
                {
    ...
    for i in 0..seq_n {
        ...
        write_hex(u64::from(crate::devices::ide_cdrom::pci_cmd_wr_at(i as usize)));
    }
...
pub fn guest_uefi_host_stack_guard_ok(guard: &[u8]) -> bool {
    !guard.is_empty() && guard.iter().all(|&b| b == GUEST_UEFI_HOST_STACK_GUARD_BYTE)
}

/// Bytes the guest-UEFI host stack offers above `frame_bytes` of a single
/// exit's deepest frame (negative = overflow into the guard/VMCS). Pure
/// form so host tests pin the F7 `59ac070` arithmetic: 81,024 bytes on a
/// 16 KiB stack overflowed by ~64 KiB; on 32 pages it fits.
pub fn guest_uefi_host_stack_headroom(stack_pages: u64, frame_bytes: u64) -> i64 {
    (stack_pages * 4096) as i64 - frame_bytes as i64
}

/// RayNu-F wall cap: elapsed ms since `start_tsc` at `tsc_hz` (0 Hz → the
/// 1 GHz fallback the service clock also uses). Pure; host-tested.
pub fn raynu_f_wall_elapsed_ms(start_tsc: u64, now_tsc: u64, tsc_hz: u64) -> u64 {
    let hz = if tsc_hz == 0 { 1_000_000_000 } else { tsc_hz };
    let d = now_tsc.saturating_sub(start_tsc);
    (d / hz).saturating_mul(1000) + ((d % hz).saturating_mul(1000)) / hz
}

/// True when the RayNu-F firmware phase has run longer than `cap_s` seconds.
/// The exit **count** is not the bound (iron `975f8fc`: GRUB's menu poll
/// loop made 1 M exits in under 2 s); this is.
pub fn raynu_f_wall_cap_hit(start_tsc: u64, now_tsc: u64, tsc_hz: u64, cap_s: u64) -> bool {
    start_tsc != 0 && raynu_f_wall_elapsed_ms(start_tsc, now_tsc, tsc_hz) > cap_s.saturating_mul(1000)
}

/// True when this exit index should sample `rdtsc` for the wall cap.
pub fn raynu_f_wall_check_due(n: u32) -> bool {
    n & RAYNU_F_WALL_CHECK_MASK == 0
}

/// True when the guard page below the host stack is untouched (or not armed).
#[cfg(target_os = "uefi")]
fn guest_uefi_host_stack_guard_intact() -> bool {
    let guard = HOST_STACK_GUARD.load(Ordering::Acquire);
    if guard == 0 {
        return true;
    }
    // SAFETY: HV-owned identity-mapped guard page(s) written at launch.
    // KANI-TARGET: guest-UEFI host stack guard read (outside Proven Core).
    let bytes = unsafe {
    ...
    guest_uefi_host_stack_guard_ok(bytes)
}

/// One line naming which VMX instruction refused the private VMCS on the
/// RayNu-F launch path and why it plausibly did (iron `59ac070`: the
/// revision dword no longer matched `IA32_VMX_BASIC` after a host-stack
/// overflow). RayNu-F launch VMCS fail.
#[cfg(target_os = "uefi")]
fn raynu_f_launch_vmcs_fail(which: &str, rev_seen: u32, rev_want: u32, guard_ok: bool) {
    serial::write_str("boot: RayNu-F launch failed: ");
    serial::write_str(which);
    serial::write_str(" rev=0x");
    write_hex_u32(rev_seen);
    serial::write_str(" want=0x");
    write_hex_u32(rev_want);
    serial::write_str(" guard=");
    serial::write_str(if guard_ok { "ok" } else { "BREACHED" });
...
unsafe fn raynu_f_launch_on_stopped_vmcs() -> ! {
    ...
    // Iron `59ac070`: report the guard and the VMCS revision dword *before*
    // touching the VMCS so a host-stack overflow names itself on COM2.
    let guard_ok = guest_uefi_host_stack_guard_intact();
    // SAFETY: SAVED_VMCS is the HV-owned identity-mapped private VMCS page.
    // KANI-TARGET: RayNu-F F7 VMCS revision peek (outside Proven Core).
    let rev_seen = core::ptr::read_volatile(vmcs as *const u32);
    let rev_want = (cpu::rdmsr(IA32_VMX_BASIC) & 0x7FFF_FFFF) as u32;
    if !guard_ok {
        serial::write_line(
            "boot: guest-UEFI WARN host stack guard breached (guest-UEFI host stack guard; F7)",
        );
    }
    if ops::vmclear(vmcs).is_err() {
        raynu_f_launch_vmcs_fail("VMCLEAR", rev_seen, rev_want, guard_ok);
        leave_to_e4();
    }
    if ops::vmptrld(vmcs).is_err() {
        raynu_f_launch_vmcs_fail("VMPTRLD", rev_seen, rev_want, guard_ok);
        leave_to_e4();
    }
...
/// | Symbol | Class | Why |
/// |---|---|---|
/// | `RAYNU_F_STATE` | RESET | `memcpy` from `RAYNU_F_STATE_TEMPLATE` (`.rdata`) — never a stack temporary (iron `59ac070`) |
...
unsafe fn raynu_f_reset_relaunch(_src: crate::devices::guest_platform::ResetSrc) -> ! {
    // Push the queued `guest reset requested src=` line (and the guest's
    // own `reboot: Restarting`) to COM2 before leaving share mode — iron
    // `59ac070` lost it because no VM exit drained the ring in between.
    ...
    LINUX_PIT_DURING_APK_LOG.store(false, Ordering::Release);
    LINUX_PIT_HOLD_LOG.store(false, Ordering::Release);
    LINUX_PIT_PACED_LOG.store(false, Ordering::Release);
    LINUX_PIT_RESUME_TSC.store(0, Ordering::Release);
    LAST_VIRTIO_MMIO_TSC.store(0, Ordering::Release);
    LAST_VIRTIO_MMIO_HIT.store(0, Ordering::Release);
    VIRTIO_STALL_DUMPED.store(false, Ordering::Release);
    VIRTIO_STALL_PIT_HOLD_LOG.store(false, Ordering::Release);
    ...
    // SAFETY: BSP-only firmware state; guest is halted for relaunch. A
    // `.rdata` template copy — never `= FirmwareState::new()`, which built
    // an 81,024-byte stack temporary and overflowed the host stack into the
    // VMCS on iron `59ac070` (RayNu-F F7 template reset).
    // KANI-TARGET: RayNu-F F7 FirmwareState reset (outside Proven Core).
    core::ptr::copy_nonoverlapping(
        core::ptr::addr_of!(RAYNU_F_STATE_TEMPLATE),
        core::ptr::addr_of_mut!(RAYNU_F_STATE),
        1,
    );
    guest_uefi_mtrr_reset();
    crate::devices::guest_platform::reset();
...
unsafe fn raynu_f_stop(why: &str) -> ! {
    ...
    write_dec(u64::from(RAYNU_F_EXITS.load(Ordering::Acquire)));
    serial::write_str(" wall_ms=");
    write_dec(raynu_f_wall_elapsed_ms(
        RAYNU_F_LAUNCH_TSC.load(Ordering::Acquire),
        cpu::rdtsc(),
        crate::boot::raynu_f_flag::tsc_hz(),
    ));
    serial::write_str(" svc=");
...
unsafe fn raynu_f_vmexit(reason: u32, qual: u64, rip: u64, intr: u64) -> ! {
    ...
    // RayNu-F wall cap: time, not exits, bounds the loader phase (iron
    // `975f8fc` hit the old count cap inside GRUB's 2 s menu timeout).
    if raynu_f_wall_check_due(n)
        && raynu_f_wall_cap_hit(
            RAYNU_F_LAUNCH_TSC.load(Ordering::Acquire),
            cpu::rdtsc(),
            crate::boot::raynu_f_flag::tsc_hz(),
            RAYNU_F_WALL_CAP_S,
...
/// Inject the named post-WFE+state4 NULL-event `#PF` into firmware.
/// Drop bitmap bit 14 so the inject does not re-exit. Park PIC/LAPIC
/// via `PF_LINUX_CR2` only (do not set linux guest-active). Not
/// `ISO-INSTALL-OK`.
#[cfg(target_os = "uefi")]
unsafe fn deliver_firmware_wfe_event_pf(cr2: u64, err: u64, rip: u64) -> bool {
    // SAFETY: VMX-root; cr2 is the guest #PF linear (canonical -0x48).
    // KANI-TARGET: firmware WFE event #PF inject (outside Proven Core).
    cpu::write_cr2(cr2);
    PF_LINUX_CR2.store(cr2, Ordering::Release);
    let bmp = ops::vmread(EXCEPTION_BITMAP).unwrap_or(0);
    let _ = ops::vmwrite(EXCEPTION_BITMAP, bmp & !GUEST_UEFI_EXC_BITMAP_PF);
    let _ = ops::vmwrite(VM_ENTRY_EXCEPTION_ERROR_CODE, err);
    let _ = ops::vmwrite(
        VM_ENTRY_INTERRUPTION_INFO,
        u64::from(guest_uefi_linux_pf_entry_info()),
    );
    if FIRMWARE_WFE_EVENT_PF
        .compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire)
        .is_ok()
    {
        serial::write_str(GUEST_UEFI_WFE_EVENT_PF_PREFIX);
        write_hex(cr2);
        serial::write_str(" rip=0x");
        write_hex(rip);
...
unsafe fn handle_pf(rip: u64, linear: u64, cr2: u64) -> bool {
    ...
    // ADR-016: injecting this #PF (steering firmware control flow) is a
    // foreign-firmware state mutation. On iron 4e16b59 it made OVMF's own
    // handler dump a real #PF then CpuDeadLoop. Disabled; the `#PF cr2=` dump
    // above already records the fault read-only, and the guest then stops
    // honestly instead of being steered into a NULL-event walk.
    if !RAYNU_F_NO_FW_STATE_MUTATION
        && guest_uefi_firmware_wfe_event_pf(
        crate::devices::ide_cdrom::product_iso_window_armed(),
        guest_uefi_product_iso_pci_ready(
            crate::devices::ide_cdrom::pci_enumerated(),
            crate::devices::guest_virtio_blk::pci_enumerated(),
        ...
        crate::devices::ide_cdrom::ata_io_accesses(),
        FIRMWARE_WFE_RETURN.load(Ordering::Acquire),
        FIRMWARE_WFE_STATE4.load(Ordering::Acquire),
        cr2,
        rip,
    ) {
        return deliver_firmware_wfe_event_pf(cr2, err, rip);
    }
    if guest_uefi_pf_should_deliver_to_guest(rip) {
...
/// Peek CpuSleep caller at `[RSP]`. `hlt; ret` so the slot is the return
/// address. Iron COM2 `2d4ab51` `ret=0x7ff0e055`. firmware ConIn CR.
#[cfg(target_os = "uefi")]
unsafe fn peek_hlt_retaddr() -> u64 {
    let rsp = ops::vmread(GUEST_RSP).unwrap_or(0);
    let mut buf = [0u8; 8];
    let n = copy_guest_linear_bytes(rsp, &mut buf);
    if n < 4 {
        return 0;
    }
    let long = guest_uefi_cs_ar_is_long(ops::vmread(GUEST_CS_ACCESS_RIGHTS).unwrap_or(0));
    ...
    guest_uefi_cpusleep_retaddr(slot, long)
}

fn note_hlt_retaddr() -> u64 {
    #[cfg(target_os = "uefi")]
    {
        // SAFETY: guest-UEFI is VMX-halted on this HLT exit; RSP is the
        // firmware CpuSleep frame (`hlt; ret`).
        // KANI-TARGET: HLT retaddr peek (outside Proven Core).
        let v = unsafe { peek_hlt_retaddr() };
        LAST_HLT_RET.store(v, Ordering::Release);
        v
    }
    #[cfg(not(target_os = "uefi"))]
    {
...
/// Peek 8 bytes at `ret-8` (call site before CpuSleep return).
/// Iron COM2 `6c4bfde` ConIn CR still `ataio=0`. print HLT callsite.
#[cfg(target_os = "uefi")]
unsafe fn peek_hlt_callsite() -> u64 {
    let ret = LAST_HLT_RET.load(Ordering::Acquire);
    if ret < 8 {
        return 0;
    }
    let mut buf = [0u8; 8];
    let n = copy_guest_linear_bytes(ret - 8, &mut buf);
    if n < 8 {
        return 0;
    }
    guest_uefi_cpusleep_callsite(buf)
}

fn note_hlt_callsite() -> u64 {
    #[cfg(target_os = "uefi")]
    {
        // SAFETY: guest-UEFI is VMX-halted; retaddr already peeked.
        // KANI-TARGET: HLT callsite peek (outside Proven Core).
        let v = unsafe { peek_hlt_callsite() };
        LAST_HLT_CALL.store(v, Ordering::Release);
        v
    }
    #[cfg(not(target_os = "uefi"))]
    {
...
/// Unwind DxeCore WaitForEvent as EFI_SUCCESS. Iron COM2 `0b770cd`
/// `rethx=0xe056ff41b84d8b48` still `ataio=0`. firmware WaitForEvent
/// return. Not `ISO-INSTALL-OK`.
#[cfg(target_os = "uefi")]
unsafe fn apply_waitforevent_return() -> Option<u64> {
    let rbp = SAVED_RBP;
    let mut buf = [0u8; 8];
    if copy_guest_linear_bytes(rbp, &mut buf) < 8 {
        return None;
    }
    let prev_rbp = u64::from_le_bytes(buf);
    if copy_guest_linear_bytes(rbp.wrapping_add(8), &mut buf) < 8 {
        return None;
    }
    let caller = u64::from_le_bytes(buf);
    let (rip, rsp) = guest_uefi_waitforevent_leave(rbp, prev_rbp, caller)?;
    SAVED_RAX = 0;
    SAVED_RBP = prev_rbp;
    if ops::vmwrite(GUEST_RIP, rip).is_err() || ops::vmwrite(GUEST_RSP, rsp).is_err() {
        return None;
    }
    let _ = ops::vmwrite(
        GUEST_ACTIVITY_STATE,
...
unsafe fn skip_preempt_deadloop(linear: u64, rip: u64) -> bool {
    ...
    // Iron d0e44d4: skip-12 of endbr64+cmp [rip],4 is a no-op
    // (falls into mov rax,3). The gState=4 poke is a foreign-firmware state
    // mutation — disabled per ADR-016 (it fed the 9474ab6 NULL-event #PF).
    if !RAYNU_F_NO_FW_STATE_MUTATION {
        let _ = try_firmware_wfe_state4_poke(rip, &buf[..n]);
    }
...
/// Host-store 4 at the RIP-relative `cmp [rip],4` dest after WFE.
/// Do not skip RIP. Not `ISO-INSTALL-OK`.
#[cfg(target_os = "uefi")]
unsafe fn try_firmware_wfe_state4_poke(rip: u64, bytes: &[u8]) -> bool {
    let Some(dest) = guest_uefi_firmware_wfe_state4_dest(rip, bytes) else {
        return false;
    };
    if !guest_uefi_firmware_wfe_state4_poke(
        crate::devices::ide_cdrom::product_iso_window_armed(),
        guest_uefi_product_iso_pci_ready(
            crate::devices::ide_cdrom::pci_enumerated(),
            crate::devices::guest_virtio_blk::pci_enumerated(),
    ...
    if FIRMWARE_WFE_STATE4
        .compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire)
        .is_err()
    {
        return false;
    }
    if !poke_guest_u64(dest, GUEST_UEFI_WFE_STATE4_VAL) {
        FIRMWARE_WFE_STATE4.store(false, Ordering::Release);
        return false;
    }
    serial::write_str(GUEST_UEFI_WFE_STATE4_POKE_PREFIX);
    write_hex(dest);
    serial::write_str(" rip=0x");
    write_hex(rip);
    serial::write_line(" (Stage 46; not ISO-INSTALL-OK)");
...
/// COM2 dump when apk virtio MMIO goes quiet: device counters, live ring
/// state, then the paced PIT hold and the SysRq probe on later resumes.
/// virtio stall dump. virtio stall dump ring. virtio stall probe.
/// Not `ISO-INSTALL-OK`.
#[cfg(target_os = "uefi")]
unsafe fn maybe_virtio_stall_dump() {
    if !crate::devices::guest_serial_answer::apk_media_mounted() {
        return;
    }
    if VIRTIO_STALL_DUMPED.load(Ordering::Acquire) {
    ...
    if VIRTIO_STALL_DUMPED
        .compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire)
        .is_err()
    {
        return;
    }
    let s = crate::devices::guest_virtio_blk::virtio_stall_snap();
    serial::write_str_nowait("boot: guest-UEFI virtio stall dump disk_last=");
    write_dec_nowait(u64::from(s.disk_last));
    serial::write_str_nowait(" iso_last=");
    write_dec_nowait(u64::from(s.iso_last));
    serial::write_str_nowait(" disk_used=");
    write_dec_nowait(u64::from(s.disk_used));
    serial::write_str_nowait(" iso_used=");
    write_dec_nowait(u64::from(s.iso_used));
    serial::write_str_nowait(" disk_avail=");
    write_dec_nowait(u64::from(s.disk_avail));
    serial::write_str_nowait(" iso_avail=");
    write_dec_nowait(u64::from(s.iso_avail));
    serial::write_str_nowait(" disk_isr=");
    write_dec_nowait(u64::from(s.disk_isr));
    serial::write_str_nowait(" iso_isr=");
    write_dec_nowait(u64::from(s.iso_isr));
    serial::write_str_nowait(" disk_kick=");
    write_dec_nowait(if s.disk_kick { 1 } else { 0 });
    serial::write_str_nowait(" iso_kick=");
    write_dec_nowait(if s.iso_kick { 1 } else { 0 });
    let disk_live = crate::devices::guest_virtio_blk::virtio_live_avail_idx(
        false,
        guest_uefi_gpa_to_hpa,
    );
    let iso_live = crate::devices::guest_virtio_blk::virtio_live_avail_idx(
        true,
        guest_uefi_gpa_to_hpa,
    );
    serial::write_str_nowait(" disk_live=");
    write_dec_nowait(u64::from(disk_live));
    serial::write_str_nowait(" iso_live=");
    write_dec_nowait(u64::from(iso_live));
    serial::write_str_nowait(" n=");
    write_dec_nowait(u64::from(LAST_VIRTIO_MMIO_HIT.load(Ordering::Acquire)));
    serial::write_str_nowait(" (not ISO-INSTALL-OK)\n");
    VIRTIO_STALL_DUMP_TSC.store(cpu::rdtsc(), Ordering::Release);
    VIRTIO_STALL_PROBE_STEP.store(0, Ordering::Release);
    virtio_stall_dump_ring(false);
    virtio_stall_dump_ring(true);
    if guest_uefi_virtio_stall_dump_pit(true) {
        crate::devices::guest_irq::raise_pit();
        linux_prefer_pit_during_apk();
        serial::write_line_nowait(
            "boot: guest-UEFI virtio stall dump PIT (apk jiffies; not ISO-INSTALL-OK)",
        );
    }
    if guest_uefi_virtio_stall_dump_intx(
        true,
        guest_uefi_virtio_stall_empty(disk_live, s.disk_last, iso_live, s.iso_last),
    ) && crate::devices::guest_serial_answer::apk_packages_overlay_active()
    {
        crate::devices::guest_virtio_blk::virtio_stall_pulse_intx();
        serial::write_line_nowait(
            "boot: guest-UEFI virtio stall dump INTx (apk harvest; not ISO-INSTALL-OK)",
        );
    }
    serial::write_line_nowait(
...
/// Keep overlay jiffies moving after an empty-ring dump, paced.
/// Iron `2b3cc1e` / `34300317331`: unpaced hold lived then `soft lockup`
/// in `handle_softirqs`. Share [`linux_raise_overlay_pit_paced`].
/// virtio stall dump PIT paced. Not `ISO-INSTALL-OK`.
#[cfg(target_os = "uefi")]
unsafe fn maybe_virtio_stall_dump_pit_hold() {
    if !guest_uefi_virtio_stall_dump_pit_paced(
        true,
        crate::devices::guest_serial_answer::apk_packages_overlay_active(),
    ) {
        return;
    }
    linux_raise_overlay_pit_paced();
    linux_prefer_pit_during_apk();
    if VIRTIO_STALL_PIT_HOLD_LOG
        .compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire)
        .is_ok()
    {
        serial::write_line_nowait(
            "boot: guest-UEFI virtio stall dump PIT paced (apk jiffies; not ISO-INSTALL-OK)",
        );
    }
    virtio_stall_probe_step();
    let now = cpu::rdtsc();
    let last = VIRTIO_STALL_PIT_HOLD_TSC.load(Ordering::Acquire);
    if last != 0 && now.wrapping_sub(last) < 4_000_000_000 {
        return;
    }
...
unsafe fn virtio_stall_dump_thre_chain(
    ...
    // guest UART line-rate pace / COM2 FIFO burst telemetry.
    let (drained, windows) = serial::guest_tx_drain_stats();
    serial::write_str_nowait(" room=");
    write_dec_nowait(serial::GUEST_TX_CAP.saturating_sub(ring) as u64);
    serial::write_str_nowait(" pace=");
    write_dec_nowait(u64::from(UART_PACE_EXITS.load(Ordering::Acquire)));
    serial::write_str_nowait(" fifo=");
    write_dec_nowait(u64::from(serial::com2_fifo()));
    serial::write_str_nowait(" drained=");
    write_dec_nowait(u64::from(drained));
    serial::write_str_nowait(" win=");
    write_dec_nowait(u64::from(windows));
    serial::write_str_nowait(" (not ISO-INSTALL-OK)\n");
}

/// One line per function with what Linux sees in its own ring.
/// virtio stall dump ring. Not `ISO-INSTALL-OK`.
#[cfg(target_os = "uefi")]
unsafe fn virtio_stall_dump_ring(iso: bool) {
    let r = crate::devices::guest_virtio_blk::virtio_ring_live(iso, guest_uefi_gpa_to_hpa);
    serial::write_str_nowait(if iso {
        "boot: guest-UEFI virtio stall dump ring iso qsize="
    } else {
        "boot: guest-UEFI virtio stall dump ring disk qsize="
    });
    write_dec_nowait(u64::from(r.qsize));
    serial::write_str_nowait(" mem_used=");
    write_dec_nowait(u64::from(r.used_idx_mem));
    serial::write_str_nowait(" avail_flags=0x");
    write_hex_nowait(u64::from(r.avail_flags));
    serial::write_str_nowait(" used_flags=0x");
    write_hex_nowait(u64::from(r.used_flags));
    serial::write_str_nowait(" last_head=");
    write_dec_nowait(u64::from(r.last_head));
    serial::write_str_nowait(" last_id=");
    write_dec_nowait(u64::from(r.last_id));
    serial::write_str_nowait(" last_len=");
    write_dec_nowait(u64::from(r.last_len));
    serial::write_str_nowait(" last_st=0x");
    write_hex_nowait(u64::from(r.last_status));
    serial::write_str_nowait(" last_ty=");
    write_dec_nowait(u64::from(r.last_ty));
    serial::write_str_nowait(" last_sec=");
    write_dec_nowait(r.last_sector);
    serial::write_str_nowait(" xlate_fail=");
    write_dec_nowait(u64::from(
        crate::devices::guest_virtio_blk::virtio_xlate_fail_count(),
    ));
    serial::write_str_nowait(" (not ISO-INSTALL-OK)\n");
}

/// Advance the post-dump probe: RIP samples, then SysRq `w`/`m`/`t`/`l`
/// through the COM1 BREAK path. A step that cannot inject yet (previous
/// BREAK unread) is retried on the next resume. virtio stall probe.
/// UART sysrq break. Not `ISO-INSTALL-OK`.
#[cfg(target_os = "uefi")]
unsafe fn virtio_stall_probe_step() {
    let base = VIRTIO_STALL_DUMP_TSC.load(Ordering::Acquire);
    if base == 0 {
        return;
    }
    let step = VIRTIO_STALL_PROBE_STEP.load(Ordering::Acquire);
    let elapsed = cpu::rdtsc().wrapping_sub(base);
    let Some(probe) = guest_uefi_virtio_stall_probe(elapsed, step) else {
        return;
    };
    match probe {
        StallProbe::Sample => {
            serial::write_str_nowait("boot: guest-UEFI virtio stall probe rip=0x");
            ...
            serial::write_str_nowait(" rflags=0x");
            write_hex_nowait(ops::vmread(GUEST_RFLAGS).unwrap_or(0));
            serial::write_str_nowait(" step=");
            write_dec_nowait(u64::from(step));
            serial::write_str_nowait(" (not ISO-INSTALL-OK)\n");
        }
        StallProbe::Sysrq(key) => {
            if !crate::devices::guest_uart::inject_sysrq(key) {
                return;
            }
            serial::write_str_nowait("boot: guest-UEFI virtio stall dump sysrq ");
            serial::write_byte_nowait(key);
            serial::write_str_nowait(" (Linux task state; not ISO-INSTALL-OK)\n");
        }
    }
    VIRTIO_STALL_PROBE_STEP.store(step.saturating_add(1), Ordering::Release);
}
...
/// unless [`linux_prefer_pit_during_apk`] arms a one-shot.
/// linux PIT prefer until DRIVER_OK. Not `ISO-INSTALL-OK`.
#[cfg(target_os = "uefi")]
fn linux_prefer_pit_until_driver_ok() {
    let virtio = crate::devices::guest_virtio_blk::virtio_needs_pit_over_uart();
    let overlay = crate::devices::guest_serial_answer::apk_overlay_needs_pit();
    ...
    let need = guest_uefi_linux_prefer_pit_hold(virtio, overlay, probe, both_ok);
    if !virtio
    ...
    {
        serial::write_line_nowait(
            "boot: guest-UEFI linux PIT hold until login (idle=poll apk jiffies; not ISO-INSTALL-OK)",
        );
    }
}

/// Raise overlay PIT at most once per [`LINUX_PIT_RESUME_MIN_TSC`].
/// linux PIT resume paced. Not `ISO-INSTALL-OK`.
#[cfg(target_os = "uefi")]
fn linux_raise_overlay_pit_paced() {
    // guest UART line-rate pace ticks are not jiffies.
    if UART_PACE_EXIT.load(Ordering::Acquire) {
        return;
    }
    let now = cpu::rdtsc();
    let last = LINUX_PIT_RESUME_TSC.load(Ordering::Acquire);
    if !guest_uefi_linux_pit_resume_elapsed(now, last, LINUX_PIT_RESUME_MIN_TSC) {
        return;
    }
    LINUX_PIT_RESUME_TSC.store(now, Ordering::Release);
    crate::devices::guest_irq::raise_pit();
    if LINUX_PIT_PACED_LOG
        .compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire)
        .is_ok()
    {
        serial::write_line_nowait(
            "boot: guest-UEFI linux PIT resume paced (apk jiffies; not ISO-INSTALL-OK)",
        );
    }
}

/// After DRIVER_OK, arm PIT-once so apk overlay reads keep jiffies moving.
/// Called from virtio MMIO, preempt, HLT (until login:), and UART LSR.
/// UART still wins the following inject. linux PIT once after DRIVER_OK.
/// linux HLT PIT during apk. Not `ISO-INSTALL-OK`.
#[cfg(target_os = "uefi")]
fn linux_prefer_pit_during_apk() {
    linux_prefer_pit_until_driver_ok();
    if guest_uefi_linux_prefer_pit_during_apk(
        crate::devices::guest_virtio_blk::virtio_needs_pit_over_uart(),
    ) && LINUX_PIT_DURING_APK_LOG
        .compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire)
        .is_ok()
    {
        serial::write_line_nowait(
            "boot: guest-UEFI linux PIT once after DRIVER_OK (apk jiffies; not ISO-INSTALL-OK)",
        );
    }
}
...
unsafe fn linux_product_iso_raise_pit(uart: bool) {
    let need = crate::devices::guest_virtio_blk::virtio_needs_pit_over_uart();
    let overlay = crate::devices::guest_serial_answer::apk_overlay_needs_pit();
    if guest_uefi_linux_uart_prefer_pit_during_apk(uart, need, overlay) {
        linux_raise_overlay_pit_paced();
        linux_prefer_pit_during_apk();
        return;
    }
...
unsafe fn handle_pci(port: u16, is_in: bool, size: u8) {
    if port == 0xCF8 {
        if is_in {
            ...
            if (addr & 0x8000_0000) != 0 {
                LAST_CF8_EN.store(addr, Ordering::Release);
                if crate::devices::ide_cdrom::pci_addr_selects_cd(addr) {
                    LAST_CF8_IDE.store(addr, Ordering::Release);
                }
            }
            note_pci_cf8(addr);
        }
...
unsafe fn handle_virtio_bar_ept(gpa: u64, qual: u64) -> bool {
    ...
    if guest_uefi_linux_virtio_mmio_raises_pit(
        linux,
        crate::devices::ide_cdrom::product_iso_window_armed(),
        crate::devices::guest_virtio_blk::virtio_needs_pit_over_uart(),
    ) {
    ...
    LAST_VIRTIO_MMIO_TSC.store(cpu::rdtsc(), Ordering::Release);
    LAST_VIRTIO_MMIO_HIT.store(n.saturating_add(1), Ordering::Release);
    let off = gpa.wrapping_sub(bar);
    if guest_uefi_virtio_stall_dump_reset_on_notify(off, is_write) {
        if VIRTIO_STALL_DUMPED.swap(false, Ordering::AcqRel) {
            VIRTIO_STALL_PIT_HOLD_LOG.store(false, Ordering::Release);
            VIRTIO_STALL_PIT_HOLD_TSC.store(0, Ordering::Release);
            VIRTIO_STALL_DUMP_TSC.store(0, Ordering::Release);
            VIRTIO_STALL_PROBE_STEP.store(0, Ordering::Release);
    ...
    let overlay = crate::devices::guest_serial_answer::apk_media_mounted();
    static LAST_KICK_LINE_TSC: AtomicU64 = AtomicU64::new(0);
    let now = cpu::rdtsc();
    let kick_due = guest_uefi_virtio_mmio_kick_line_due(
        now,
    ...
    let Some(op) = decode_mmio_or_retry(&buf, n, insn_len) else {
        ...
        if skip != 0 {
            ...
            if is_write {
                ...
                if guest_uefi_virtio_drain_on_isr(false) {
                    drain_virtio_product_iso();
                }
...
/// Latch Linux IOAPIC pin-2 writes so leftover firmware GSI 2 does not count.
#[cfg(target_os = "uefi")]
fn ioapic_linux_write(off: u16, val: u32, linux: bool) {
    crate::devices::guest_irq::ioapic_write(off, val);
    if linux {
        crate::devices::guest_irq::note_linux_ioapic_write(off);
    }
}
...
unsafe fn handle_ioapic_ept(gpa: u64, qual: u64) -> bool {
    ...
    let Some(op) = decode_mmio_or_retry(&buf, n, insn_len) else {
        ...
        if skip != 0 {
            MMIO_INSN_LEN.store(skip, Ordering::Relaxed);
            if is_write {
                ioapic_linux_write(off, cr_gpr(0) as u32, linux);
            } else {
    ...
    if crate::devices::guest_virtio_blk::mmio_alu_is_sse(op.alu) {
        if op.is_write {
            ioapic_linux_write(off, mmio_xmm_in(op.reg) as u32, linux);
        } else {
    ...
    if crate::devices::guest_virtio_blk::mmio_alu_is_string(op.alu) {
        let Some(done) = mmio_string_step(
            ...
            |val| ioapic_linux_write(off, val as u32, linux),
            gpa,
        ) else {
            return skip_insn();
        };
    ...
    if crate::devices::guest_virtio_blk::mmio_alu_is_pop(op.alu) {
        ...
        if let Some(val) = mmio_stack_pop(size) {
            ioapic_linux_write(off, val as u32, linux);
        }
        return skip_insn();
    }
    if crate::devices::guest_virtio_blk::mmio_alu_is_call(op.alu)
    ...
    if op.xchg {
        ...
        ioapic_linux_write(off, oldr as u32, linux);
        mmio_gpr_out(op, oldm);
        return skip_insn();
    }
    if op.atomic != 0 {
...
unsafe fn try_inject_guest_irq() {
    ...
    if linux {
        ...
        if guest_uefi_linux_reassert_virtio_intx(
            overlay,
            both_ok,
            crate::devices::guest_virtio_blk::virtio_isr_latched(),
        ) {
            crate::devices::guest_virtio_blk::reassert_virtio_shared_intx();
            if LINUX_VIRTIO_REASSERT_LOG
                .compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire)
                .is_ok()
            {
                serial::write_line_nowait(
                    "boot: guest-UEFI linux virtio INTx reassert (apk; not ISO-INSTALL-OK)",
                );
            }
        }
        if guest_uefi_linux_raise_pit_on_resume(overlay, virtio_need, probe, both_ok) {
            linux_raise_overlay_pit_paced();
        }
        linux_prefer_pit_until_driver_ok();
    }
    ...
    let leftover_gsi2 = crate::devices::guest_irq::ioapic_gsi2_armed();
    let linux_gsi2 = crate::devices::guest_irq::linux_ioapic_gsi2_programmed();
    let gsi2_armed = if linux { linux_gsi2 } else { leftover_gsi2 };
    ...
    if linux
        ...
        && !linux_gsi2
        && use_pic
        && LINUX_PIC_LEFTOVER_GSI2_LOG
            .compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire)
            .is_ok()
    {
        serial::write_line_nowait(
            "boot: guest-UEFI linux PIC before leftover GSI 2 (Stage 46; not ISO-INSTALL-OK)",
        );
    }
    ...
    if guest_uefi_firmware_skip_pit_inject(linux, vec, ataio) {
        ...
        // firmware take IOAPIC ATA. firmware PIC ATA. do not F11 eaa580d.
        // do not F11 bce5bbb.
        // do not F11 489d938. do not F11 e70a295.
        // firmware PIT one-shot after first wake. do not F11 24c5fa6.
        // Not ISO-INSTALL-OK.
        let _ = set_guest_uefi_interrupt_window(false);
        return;
    }
    if let Ok(info) = crate::sched::interrupt::prepare_external_inject(vec) {
        ...
        if guest_uefi_firmware_note_pit_injected(linux, vec, ataio) {
            serial::write_line_nowait(
                "boot: Stage 46 PIT one-shot (not ISO-INSTALL-OK)",
            );
        }
        ...
        if linux
            && use_pic
            && guest_uefi_linux_virtio_pic_vec(vec)
            && LINUX_VIRTIO_PIC_LOG
                .compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire)
                .is_ok()
        {
            serial::write_line_nowait(
                "boot: guest-UEFI linux virtio PIC 11 (Stage 46; not ISO-INSTALL-OK)",
            );
        }
        if linux
            && gsi2_armed
...
unsafe fn handle_ept(gpa: u64, qual: u64) -> bool {
    ...
    // Iron COM2 e0d5c55: WFE return hit, then EPT 0x34 at ZeroMem
    // rip=0x7ec8f6ff. Host-zero dest + RWE-replace report-RAM so the
    // dword loop does not stall. Not a second WaitForEvent unwind.
    let rip = ops::vmread(GUEST_RIP).unwrap_or(0);
    let pci_ide = guest_uefi_product_iso_pci_ready(
        crate::devices::ide_cdrom::pci_enumerated(),
        crate::devices::guest_virtio_blk::pci_enumerated(),
    );
    if guest_uefi_firmware_zeromem_ept(
        crate::devices::ide_cdrom::product_iso_window_armed(),
        pci_ide,
        crate::devices::ide_cdrom::ata_io_accesses(),
        FIRMWARE_WFE_RETURN.load(Ordering::Acquire),
        ...
        firmware_zeromem_ept_fill(gpa, qual, rip);
    }
...
/// Iron `e0d5c55`: already-present report-RAM fetch/write may lack X.
/// Replace the 2 MiB WB leaf with RWE. firmware ZeroMem ept fill.
#[cfg(target_os = "uefi")]
unsafe fn ept_map_2m_report_ram_replace(gpa: u64) -> bool {
    if !guest_uefi_report_ram_should_map(gpa) {
        return false;
    }
    let g = guest_uefi_report_ram_gpa_2m(gpa);
    ...
    ept_map_2m_hpa_mt(g, hpa, true, true, GUEST_UEFI_EPT_MT_WB, true)
}

/// Host-zero firmware ZeroMem dest after WFE. Print once. Map dest and
/// the fault GPA as RWE report-RAM. Existing handle_ept still runs.
/// Not `ISO-INSTALL-OK`.
#[cfg(target_os = "uefi")]
unsafe fn firmware_zeromem_ept_fill(gpa: u64, qual: u64, rip: u64) {
    let dest = SAVED_RBX;
    let mut cntb = [0u8; 8];
    let npeek = copy_guest_linear_bytes(SAVED_R14.wrapping_add(0x18), &mut cntb);
    let count = if npeek >= 8 {
        u64::from_le_bytes(cntb)
    } else {
        0
    };
    if FIRMWARE_ZEROMEM_EPT
        .compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire)
        .is_ok()
    {
        serial::write_str(GUEST_UEFI_ZEROMEM_EPT_PREFIX);
        write_hex(gpa);
        serial::write_str(" qual=0x");
        write_hex(qual);
        serial::write_str(" rip=0x");
        ...
        serial::write_str(" rbx=0x");
        write_hex(dest);
        serial::write_str(" n=0x");
        write_hex(count);
        serial::write_line(" (Stage 46; not ISO-INSTALL-OK)");
    }
    if guest_uefi_firmware_zeromem_dest_ok(dest, count) {
        let _ = firmware_zeromem_store_zeros(dest, count);
        let mut page = dest;
        let end = dest.saturating_add(count);
        while page < end {
            if guest_uefi_report_ram_should_map(page) {
                let _ = ept_map_2m_report_ram_replace(page);
            }
            page = page.saturating_add(GUEST_UEFI_REPORT_RAM_PAGE);
        }
    }
    if guest_uefi_report_ram_should_map(gpa) {
        let _ = ept_map_2m_report_ram_replace(gpa);
    }
    if dest != 0 && guest_uefi_report_ram_should_map(dest) {
        let _ = ept_map_2m_report_ram_replace(dest);
    }
}

/// Zero `count` bytes at guest dest (identity or leftover report-RAM).
#[cfg(target_os = "uefi")]
unsafe fn firmware_zeromem_store_zeros(dest: u64, count: u64) -> u64 {
    let mut done = 0u64;
    while done < count {
        let gpa = dest.wrapping_add(done);
        let Some(hpa) = guest_uefi_gpa_to_hpa(gpa) else {
            break;
        };
        let chunk = (4096 - (gpa & 0xfff)).min(count - done);
        if chunk == 0 {
            break;
        }
        // SAFETY: dest_ok restricted this GPA to guest-UEFI RAM / report-RAM
        // already translated to an exclusive HPA.
        // KANI-TARGET: firmware ZeroMem host fill (outside Proven Core).
        core::ptr::write_bytes(hpa as *mut u8, 0, chunk as usize);
