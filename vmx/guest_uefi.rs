...
/// `[2GiB, 4GiB)` PAT-UC PCD+PWT at 4G; `[ram_len, 2GiB)` stays NP.
...
unsafe fn setup_guest_uefi_vmcs(
    ...
    // GetAllMtrrs (lastmsr=0x23f): PA0=UC vs MTRR WB RAM. E4 launch.rs
    // writes PAT when SAVE/LOAD PAT is set; guest-UEFI RDMSR 0x277 is
    // also VmcsPat, so the field must be the SDM reset even if LOAD_PAT
    // is not forced.
