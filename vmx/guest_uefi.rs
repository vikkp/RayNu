...
/// after CMOS 2 GiB (`c70768b`); iron `f0781bb` FIX is WB `0x06`, so the
/// live path holds UC again (uniform GetMemoryAttributes on a spanning GCD).
...
unsafe fn launch_uefi(
    ...
    guest_uefi_mtrr_reset();
    // Iron `f0781bb`: CpuFlush `n=2` `9090` still ASSERT `ebecc9c3`.
    // FIX MTRRs are WB `0x06` (not P1 power-on 0). Hold variable UC so
    // GetMemoryAttributes is uniform on a spanning GCD. P2 admit
    // (`c70768b`) left `mtrr0=0x80000000` mixed with WB RAM.
    guest_uefi_mtrr_set_admit_uc(false);
    serial::write_line("boot: guest-UEFI MTRR UC held after FIX WB (GCD)");
