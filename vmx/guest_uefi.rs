...
/// True for classic VGA framebuffer / FIX16K_A0000 `[0xA0000, 0xC0000)`.
/// Iron `e368e86` PAT-UC `[0xA0000, 1MiB)` including option-ROM `C0000`
/// (`pte_c0000=0xc007f`) and still ASSERTed. Leave `[0xC0000, 1MiB)` WB.
pub fn guest_uefi_gpa_in_vga_fix_uc(gpa: u64) -> bool {
    gpa >= crate::vmx::guest_pt::IDENTITY_VGA_A0000
        && gpa < crate::vmx::guest_pt::IDENTITY_VGA_C0000
}
...
static MTRR_VGA_FIX_UC: AtomicU32 = AtomicU32::new(0);
...
/// True for FIX16K_A0000 (`0x259`, `[0xA0000, 0xC0000)`).
/// Iron `e368e86` coerced `0x268`–`0x26F` (option-ROM) to UC and PAT-UC
/// `pte_c0000`; firmware WRMSR was WB `0x06`. `0x250`/`0x258` stay WB.
/// `0x268`–`0x26F` keep the firmware value (dump `mtrr268=`).
...
pub fn guest_uefi_mtrr_reset() {
    ...
    MTRR_VGA_FIX_UC.store(0, Ordering::Release);
}
...
/// and coerces VGA FIX to UC so GCD IoMemory matches.
...
pub fn guest_uefi_mtrr_write(msr: u32, value: u64) -> bool {
    ...
    if let Some(i) = mtrr_fixed_index(msr) {
        let v = if guest_uefi_mtrr_fixed_is_vga_hole(msr) {
            let n = MTRR_VGA_FIX_UC.fetch_add(1, Ordering::AcqRel);
            #[cfg(target_os = "uefi")]
            if n == 0 {
                serial::write_line("boot: guest-UEFI MTRR VGA FIX UC (GCD)");
            }
            GUEST_UEFI_MTRR_UC_PACKED
        } else {
            value
        };
...
unsafe fn launch_uefi(
    ...
    // RefreshGcd failure. PEI VGA IoMemory is GCD UC while firmware
    // FIX is WB; hold also left Uc32Base GCD UC vs default WB. Admit
    // variable UC (2GiB hole). Coerce only FIX `0x259` (VGA
    // A0000-BFFFF); `0x268`-`0x26F` stay firmware WB (iron `e368e86`).
    guest_uefi_mtrr_set_admit_uc(true);
    serial::write_line("boot: guest-UEFI MTRR UC admitted (GCD)");
    serial::write_line("boot: guest-UEFI MTRR VGA FIX UC armed (GCD)");
