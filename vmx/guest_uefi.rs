...
/// VM-entry injects PIC IRQ 0 before leftover IOAPIC→LAPIC.
///
/// Iron `a525340`: `Freeing initrd` then silent (`rest_init` needs PIT).
/// linux PIC before LAPIC. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_linux_pic_before_lapic(pic_ready: bool) -> bool {
    pic_ready
}
...
unsafe fn try_inject_guest_irq() {
    ...
    // PIC first (Linux virtual-wire / no MADT). Iron `a525340`: OVMF left
    // IOAPIC unmasked, `raise_pit` latched pin 0 into LAPIC, and inject
    // preferred that vector so PIC IRQ 0 never reached `rest_init`
    // (`Freeing initrd`, no `/init`). IOAPIC → LAPIC only when PIC is
    // quiet (ACPI / PIC masked). linux PIC before LAPIC.
    // Not `ISO-INSTALL-OK`.
