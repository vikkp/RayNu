...
/// HLT stall quiet tick print-only. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_hlt_stall_quiet_tick(n: u32, reason: u32, pci_ide: bool, ataio: u32) -> bool {
    reason == crate::vmx::fields::EXIT_REASON_HLT && pci_ide && ataio == 0 && n > 16384
}
...
/// IRETs to HLT (iron COM2 `eac424b`). Hardware HLT completes, then the
/// IRQ is recognized: skip AND inject. DXE before `pci_ide` still skips
/// HLT so PciBus can walk IDE. Nested iso=0 has no inject — keep skip.
/// firmware HLT stall waits for IRQ. firmware HLT skip after inject.
/// Not `ISO-INSTALL-OK`.
pub fn guest_uefi_firmware_hlt_wait_for_irq(
    ...
    product_iso && guest_uefi_hlt_stall_quiet_tick(n, reason, pci_ide, ataio)
}

/// Hardware HLT wakeup: skip the 1-byte `F4` and keep resume so
/// `try_inject_guest_irq` still runs. IRET then lands on CpuSleep `RET`
/// (`insn=f4c3`). Iron COM2 `eac424b` injected at HLT RIP; IRET-to-HLT;
/// `ataio=0`. Nested iso=0: wait_for_irq is false — skip without inject.
/// firmware HLT skip after inject. do not F11 8e81c2e. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_firmware_hlt_skip_after_inject(
    ...
    guest_uefi_firmware_hlt_wait_for_irq(product_iso, n, reason, pci_ide, ataio)
}
...
/// firmware LAPIC timer expiry. do not F11 b26c86a. flash 2ae4544. Not `ISO-INSTALL-OK`.
...
/// IOAPIC I/O over PIT. firmware virtual-wire GSI 14. flash 2ae4544
/// wakeup still needed; this is the PACKET path after that. Not `ISO-INSTALL-OK`.
        ...
        resume = match basic {
            ...
            EXIT_REASON_HLT => {
                ...
                if hlt_should_resume() {
                    ...
                    let pci_ide = crate::devices::ide_cdrom::pci_enumerated();
                    ...
                    if crate::devices::ide_cdrom::pci_enumerated()
                        && HLT_STALL_LOGGED
                            .compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire)
                            .is_ok()
                    {
                    ...
                    if guest_uefi_firmware_hlt_skip_after_inject(
                        ...
                        // firmware HLT skip after inject. Hardware wakeup:
                        // skip so IRET returns past HLT (CpuSleep RET), then
                        // resume so try_inject_guest_irq still runs.
                        // iron COM2 eac424b IRET-to-HLT. do not F11 8e81c2e.
                        if !skip_hlt() {
