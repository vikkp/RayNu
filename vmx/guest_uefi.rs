...
/// skip_pit still drops leftover `0x20` (iron `ea30da1`). Never leftover
/// LVT `0xEF`. `wait_for_irq` stays false. HLT only: do not inject mid-CF8.
/// product ISO firmware HLT wake. firmware skip PIT inject.
/// Not `ISO-INSTALL-OK`.
...
/// Product firmware HLT `ataio==0` wake vector: PIC / LAPIC if not leftover
/// `0x20`/`0xEF`, else EDK2 IRQ0 `0x68`. product ISO firmware HLT wake.
...
pub fn guest_uefi_firmware_hlt_ataio0_wake_vec(pic: Option<u8>, lapic: Option<u8>) -> u8 {
    if let Some(v) = pic {
        if !guest_uefi_firmware_leftover_timer_vec(u32::from(v)) {
            return v;
        }
    }
    if let Some(v) = lapic {
...
/// Product ATA: PIC IRQ 14 take, else IOAPIC pin 14 take, else EDK2 `0x76`.
/// pic=0 APIC-mode CpuSleep (CI `33440951898`) has no PIC take; raise_ata
/// latches pin 14. do not inject leftover 0x2E.
...
unsafe fn try_inject_nested_iso0_firmware_hlt_ata(linux: bool) {
    ...
    let vec = guest_uefi_nested_iso0_ata_inject_vec(pic);
    if let Ok(info) = crate::sched::interrupt::prepare_external_inject(u32::from(vec)) {
        let _ = set_guest_uefi_interrupt_window(false);
        let _ = ops::vmwrite(VM_ENTRY_INTERRUPTION_INFO, info as u64);
        let _ = ops::vmwrite(GUEST_INTERRUPTIBILITY_STATE, 0);
...
/// pic=0 APIC-mode: take IOAPIC pin 14 after raise_ata.
...
/// Product ISO firmware HLT after BOTH-OK: inject EDK2 `0x68` when PIC
/// take and LAPIC ATA IRR are empty (`pic=0` APIC-mode CpuSleep, same
/// IdeBus stall as CI `33440951898`). skip_pit still drops leftover
/// `0x20`. `wait_for_irq` stays false. Nested ATA inject is iso=0 only.
/// product ISO firmware HLT wake. firmware skip PIT inject.
/// Not `ISO-INSTALL-OK`.
#[cfg(target_os = "uefi")]
unsafe fn try_inject_product_firmware_hlt_wake(
    ...
    let vec = u32::from(guest_uefi_firmware_hlt_ataio0_wake_vec(None, None));
    if guest_uefi_firmware_skip_pit_inject(linux, vec) {
        return false;
    }
    if let Ok(info) = crate::sched::interrupt::prepare_external_inject(vec) {
