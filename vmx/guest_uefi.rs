...
/// SIGSEGV at 64/exit). After Linux `#PF`, linux earlycon drain CHUNK so
/// printk reaches SOL while THRE is set. Nested iso=0 never sets share.
/// Not `ISO-INSTALL-OK`.
pub fn guest_uefi_linux_earlycon_drain(share: bool) -> usize {
    if share {
        crate::boot::serial::GUEST_TX_DRAIN_CHUNK
    } else {
...
pub unsafe extern "C" fn guest_uefi_vmexit() -> ! {
    ...
    // linux earlycon drain CHUNK while share (iso=0 stays 4/exit).
