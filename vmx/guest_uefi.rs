...
unsafe fn launch_uefi(
    ...
    if let Some(sink_frame) = alloc.allocate_contiguous_aligned(512, 512) {
        ...
        // Present EPT leaves so early hole walks do not EPT-fault, but use
        // scratch HPAs so PT stores persist (iron 577c9eb zero-sink lost
        // 0xC0200000; iron 471391f skipped these and hit 0x1e9000 1GiB).
        // Iron 0bad45d: pool=8 used 0x80000000 + 0xC0000000..0xC0A00000
        // then sink 0xC0C00000; leftover CR2; #DE RIP 0xCFFF9E DIV RCX=0.
        // Pre-scratch the known PT GPAs. Do not bulk 2–4GiB (73576cc).
        for n in 0u64..8 {
            let _ = ept_map_2m_scratch(0xC000_0000 + n * 0x20_0000);
        }
        for &mm in &[0x8000_0000u64, 0xFCE0_0000] {
            let _ = ept_map_2m_scratch(mm);
        }
...
unsafe fn handle_ept(gpa: u64) -> bool {
    ...
    // land on a dedicated scratch HPA. The shared sink is the HPET zero page.
