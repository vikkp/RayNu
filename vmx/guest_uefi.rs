...
/// before connecting Pci(1,1). Trailing still bound the PCI handle
/// (NOT_FOUND children OK) then IdeBus never Start. CI `33486002459`
/// VMXON-SKIP (`7661d22` BM unproven). do not F11 7661d22.
/// Not `ISO-INSTALL-OK`.
...
/// nested iso=0 firmware IdeBus BM: BAR4 is I/O with a non-zero base so
/// IdeBus EnableAttributes BusMaster can run and Start can write `0x3F6`.
/// Address 0 (`bar4=1`) is not a command BAR. CI `33484950374` VMXON-SKIP
/// (`f9163b7` ConnectAll unproven). do not F11 f9163b7. Dump `bar4=`.
/// Not `ISO-INSTALL-OK`.
