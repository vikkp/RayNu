...
/// Firmware BDS keeps PIC-first even when leftover IOAPIC pin 2 is unmasked.
/// Linux after MADT still uses [`guest_uefi_linux_pic_before_lapic`] so GSI 2
/// wins. Iron COM2 `084430f`: BOTH-OK then HLT `rip=0x7f0680d0` `ataio=0`
/// (`CpuSleep`); inject preferred IOAPIC while TPR/CR8 blocked PIC IRQ 0.
/// firmware PIC before GSI 2. Not `ISO-INSTALL-OK`.
...
/// After BOTH-OK, `skip_hlt` turns CpuSleep into a busy `RET` loop so the
/// timer ISR never runs as a wait. Keep the guest in HLT and let inject
/// wake BDS (PIC-first or TPR-ignored IOAPIC). DXE before `pci_ide` still
/// skips HLT so PciBus can walk IDE. Nested iso=0 has no inject — keep
/// skip. firmware HLT stall waits for IRQ. Not `ISO-INSTALL-OK`.
        ...
        resume = match basic {
            ...
            EXIT_REASON_HLT => {
                ...
                if hlt_should_resume() {
                    ...
                    if guest_uefi_firmware_hlt_wait_for_irq(
                        crate::devices::ide_cdrom::product_iso_window_armed(),
                        n,
                        basic,
                        crate::devices::ide_cdrom::pci_enumerated(),
                        ...
                        // firmware HLT stall waits for IRQ. Do not skip_hlt.
                        true
                    } else {
...
unsafe fn try_inject_guest_irq() {
    ...
    // firmware virtual-wire AEOI. do not F11 eac424b.
    let linux = guest_uefi_linux_guest_active(
        serial::linux_earlycon_share(),
        guest_uefi_pf_should_deliver_to_guest(ops::vmread(GUEST_RIP).unwrap_or(0)),
        PF_LINUX_DELIVER.load(Ordering::Acquire) != 0,
    ...
    let use_pic = guest_uefi_pic_before_lapic(pic_ready, gsi2_armed, linux);
    let ignore_tpr = guest_uefi_firmware_hlt_ignores_tpr(
        linux,
        crate::devices::ide_cdrom::pci_enumerated(),
        crate::devices::ide_cdrom::ata_io_accesses(),
    );
