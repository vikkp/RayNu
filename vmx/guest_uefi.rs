...
/// After PACKET (`ataio>0`), take ATA `0x2E` from IRR ignoring TPR.
...
/// (iron `ea30da1` timer ISR). wait_for_irq stays false. Linux keeps TPR.
/// firmware prefer ATA IRR. flash 489d938. flash bce5bbb.
/// do not F11 489d938. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_firmware_prefer_ata_irr(linux: bool, ataio: u32) -> bool {
    !linux && ataio > 0
}
...
unsafe fn try_inject_guest_irq() {
    ...
    if guest_uefi_firmware_skip_pit_inject(linux, vec) {
        ...
        // flash 5227ad9. flash 489d938. flash bce5bbb. firmware prefer ATA IRR.
