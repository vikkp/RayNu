...
/// nested 36/40 stays. Iron COM2 after keep_4k: `pde20=0x20000e7`
/// `pde40=0x40000e7` `maxpa=32` `pml4e=0x1a02023` still ASSERT
/// `callerrip=0x1d25193` — 2 MiB at GPA 0 spans the 1 MiB fixed-MTRR.
...
/// `pci_ide=1` `ataio=0` 3/3 (ATAPI-OK missing). Nested keeps 36/40 and
/// the 2 MiB leaf; iron `maxpa=32` still splits.
...
unsafe fn guest_uefi_split_gpa0_1m_mtrr(walk: u64, ram_hpa: u64) -> bool {
    if ram_hpa == 0 || !guest_uefi_gpa0_fixed_mtrr_split(guest_uefi_phys_width()) {
        return false;
    }
