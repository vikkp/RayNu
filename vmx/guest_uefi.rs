...
/// Scratch HPA is for stores (and fetches) in the hole. A data-read walk
/// of 2–4 GiB must not consume the pool (iron `5837243`).
pub fn guest_uefi_ept_scratch_on_qual(qual: u64) -> bool {
    guest_uefi_ept_qual_is_write(qual) || guest_uefi_ept_qual_is_fetch(qual)
}
...
unsafe fn launch_uefi(
    ...
    if let Some(sink_frame) = alloc.allocate_contiguous_aligned(512, 512) {
        ...
        // known PT-store GPA. Hole reads are R+X sink (handle_ept).
        // Do not bulk 2–4GiB (73576cc).
...
unsafe fn handle_ept(gpa: u64, qual: u64) -> bool {
    ...
    // land on a dedicated scratch HPA. Iron 5837243: a data-read walk of
    // 0xC1000000..0xC3C00000 filled pool=32; scratch only write/fetch.
    // Hole reads get R+X sink so a later store EPT-faults and upgrades.
...
/// Hole data-read: shared sink as R+X. A store upgrades via scratch.
#[cfg(target_os = "uefi")]
unsafe fn ept_map_2m_hole_ro_sink(gpa: u64) -> bool {
    ...
    let g = guest_uefi_pf_gpa32(gpa);
    ept_map_2m_hpa_rwe(g, sink, false, false)
}
