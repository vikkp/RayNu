...
/// Do not inject PIT: iron COM2 `ea30da1` `inject vec=0x20` never returned
/// to the boot media Start. firmware HLT skip after inject.
/// firmware HLT skip without inject. skip-after-inject uses pci_ready.
...
/// firmware HLT skip without inject. do not F11 ea30da1. Not `ISO-INSTALL-OK`.
            ...
            EXIT_REASON_HLT => {
                ...
                if hlt_should_resume() {
                    ...
                    if guest_uefi_firmware_hlt_skip_after_inject(
                        ...
                        // firmware HLT skip without inject. Skip HLT +
                        // activity Active so ConnectAll continues. Do not
                        // inject PIT: iron COM2 ea30da1 vec=0x20 livelock.
...
unsafe fn try_inject_guest_irq() {
    ...
    if guest_uefi_firmware_hlt_skip_without_inject(linux) {
        // firmware HLT skip without inject. Iron COM2 ea30da1 inject vec=0x20
        // livelocked the timer ISR through the 16_777_216 cap.
        return;
    }
    let pci_ide = guest_uefi_product_iso_pci_ready(
        crate::devices::ide_cdrom::pci_enumerated(),
