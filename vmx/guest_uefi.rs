...
/// Hold PIT over UART while virtio probe or apk overlay (`idle=poll`).
/// Iron `20e8b70` / `34076175624`: PIT-once consumed, UART won, last virtio
/// `n=1345`. linux PIT hold until login. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_linux_prefer_pit_hold(virtio_need: bool, overlay_needs_pit: bool) -> bool {
    let need = virtio_need || overlay_needs_pit;
    crate::devices::guest_irq::prefer_pit_hold(need);
    need
}

/// Raise PIT on every Linux product-ISO resume while overlay or virtio
/// probe still needs jiffies. `idle=poll` never HLT.
/// linux PIT raise on overlay resume. Not `ISO-INSTALL-OK`.
pub fn guest_uefi_linux_raise_pit_on_resume(overlay_needs_pit: bool, virtio_need: bool) -> bool {
    overlay_needs_pit || virtio_need
}
...
fn linux_prefer_pit_until_driver_ok() {
    ...
    let overlay = crate::devices::guest_serial_answer::apk_overlay_needs_pit();
    let need = guest_uefi_linux_prefer_pit_hold(virtio, overlay);
    if !virtio
...
unsafe fn try_inject_guest_irq() {
    ...
    if linux {
        ...
        if guest_uefi_linux_raise_pit_on_resume(overlay, virtio_need) {
            crate::devices::guest_irq::raise_pit();
        }
        linux_prefer_pit_until_driver_ok();
    }
