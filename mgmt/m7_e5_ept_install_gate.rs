//! E5 Stage 17 — private alias-EPT install contract (outside Proven Core).
//!
//! Pillar: [Z] [A]
//! Proven Core: **outside** mgmt bookkeeping; launch.rs entry is L0 (ADR-014)
//! VERIFICATION: N/A
//!
//! Host/CI: `install_ovmf_alias_ept` records a private guest-UEFI alias-EPT
//! install after program. `try_vmlaunch_ovmf_firmware` then calls
//! `try_vmlaunch_guest_uefi_ovmf`, which refuses to issue VMLAUNCH
//! (`AliasEptInstalledNotLaunched`). A 2 MiB fixture is `TooSmall`.
...
use super::guest_fw::{
    ...
    guest_fw_bytes, install_ovmf_alias_ept, load_guest_firmware, load_ovmf_from_esp,
    ovmf_alias_ept_is_installed, prepare_ovmf_firmware_launch, probe_ovmf_firmware, reset_guest_fw,
...
use super::m7_e5_alias_ept_gate::run_m7_e5_alias_ept_gate;
...
use super::guest_fw::{
    ...
    program_ovmf_alias_ept, stage_edk2_ovmf_firmware, try_vmlaunch_ovmf_firmware,
...
/// Host / CI marker when the E5 Stage 17 private EPT-install gate passes.
pub const M7_E5_EPT_INSTALL_OK_MARKER: &str = "RAYNU-V-M7-E5-EPT-INSTALL-OK";

/// Honest residual: private EPT install recorded; live E4 SHELL EPT not written.
pub const E5_EPT_INSTALL_RESIDUAL_NOTE: &str =
    "residual: private alias-EPT install is recorded; 4 MiB fixture is not a shipped OVMF.fd; live EPT is not written; VMLAUNCH insn not issued; attach_cdrom_uefi stays UnsupportedOnFirmware; no guest UEFI VMLAUNCH";

fn stage_through_alias_ept() -> bool {
    reset_guest_fw();
    ...
    #[cfg(test)]
    {
        ...
        return arm_ovmf_firmware_alias(&alias).is_ok() && program_ovmf_alias_ept(&alias).is_ok();
    }
    #[cfg(not(test))]
    {
        false
...
/// Alias-EPT program then private install. VMLAUNCH stays AliasEptInstalledNotLaunched.
pub fn prop_ept_install_after_program() -> bool {
    #[cfg(not(test))]
    {
        return false;
    }
    #[cfg(test)]
    {
        if !stage_through_alias_ept() {
            return false;
        }
        if try_vmlaunch_ovmf_firmware() != Err(GuestFwError::AliasEptNotLaunched) {
            return false;
        }
        let mut live = vec![0u8; MIN_LIVE_ESP_OVMF_BYTES];
        if write_live_esp_ovmf_fv(&mut live).is_err() || write_reset_vector_stub(&mut live).is_err()
        ...
        if install_ovmf_alias_ept(&live) != Err(GuestFwError::TooSmall) {
            return false;
        }
        let mut alias = vec![0u8; MIN_FIRMWARE_ALIAS_BYTES];
        if write_firmware_alias_fv(&mut alias).is_err() {
        ...
        match install_ovmf_alias_ept(&alias) {
            Ok(a) => {
                a.bytes_len == MIN_FIRMWARE_ALIAS_BYTES as u64
                    && a.gpa == 0xFFC0_0000
                    && ovmf_alias_ept_is_installed()
...
/// Private EPT install without program, reject. attach_cdrom_uefi stays closed.
pub fn prop_ept_install_rejects() -> bool {
    reset_guest_fw();
    if install_ovmf_alias_ept(&[]) != Err(GuestFwError::LaunchNotWired) {
        return false;
    }
    attach_cdrom_uefi(1) == Err(IsoError::UnsupportedOnFirmware)
}

/// Bearer REST: program then install. Without program → 409.
/// `POST /fw/vmlaunch` → 409 (AliasEptInstalledNotLaunched). `iso=0` SHELL stays 201.
pub fn prop_rest_ept_install() -> bool {
    reset_guest_fw();
    ...
    let installed = dispatch_guest_fw_rest(RestRequest {
        method: RestMethod::Post,
        path: "/fw/ept-install",
        auth_token: tok,
    });
    if installed.status != 201 || !ovmf_alias_ept_is_installed() {
        return false;
    }
...
/// SPA + ADR-014 Stage 17 phrases. Private EPT install recorded; VMLAUNCH insn not issued.
pub fn ept_install_surface_present() -> bool {
    ...
    spa.contains("Install FW EPT")
        ...
        && adr.contains("Stage 17")
        && adr.contains("install_ovmf_alias_ept")
        && adr.contains("AliasEptInstalledNotLaunched")
        && src.contains("fn install_ovmf_alias_ept")
        && src.contains("ovmf_alias_ept_is_installed")
        && launch.contains("fn try_vmlaunch_guest_uefi_ovmf")
        && launch.contains("fn install_guest_uefi_alias_ept")
...
/// Full E5 Stage 17 package. Host gate only — not iron, not Everest E5.
pub fn run_m7_e5_ept_install_gate() -> bool {
    let _ = (M7_E5_EPT_INSTALL_OK_MARKER, E5_EPT_INSTALL_RESIDUAL_NOTE);
    reset_guest_fw();
    let ok = prop_ept_install_after_program()
        && prop_ept_install_rejects()
        && prop_rest_ept_install()
        && ept_install_surface_present()
        && run_m7_e5_alias_ept_gate()
        && E5_EPT_INSTALL_RESIDUAL_NOTE.contains("UnsupportedOnFirmware")
        && E5_EPT_INSTALL_RESIDUAL_NOTE.contains("no guest UEFI VMLAUNCH")
        && E5_EPT_INSTALL_RESIDUAL_NOTE.contains("not a shipped OVMF.fd")
        && E5_EPT_INSTALL_RESIDUAL_NOTE.contains("VMLAUNCH insn not issued")
        && E5_EPT_INSTALL_RESIDUAL_NOTE.contains("live EPT is not written")
        && M7_E5_EPT_INSTALL_OK_MARKER == "RAYNU-V-M7-E5-EPT-INSTALL-OK";
    reset_guest_fw();
    ok
}

#[cfg(test)]
#[path = "m7_e5_ept_install_gate_test.rs"]
mod m7_e5_ept_install_gate_test;
