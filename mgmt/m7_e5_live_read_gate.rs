//! E5 Stage 27 — live-ESP read-attempt (outside Proven Core).
//!
//! Pillar: [Z] [A]
//! Proven Core: **outside** mgmt bookkeeping; launch.rs entry is L0 (ADR-014)
//! VERIFICATION: N/A
//!
//! Host/CI: `read_ovmf_live_esp` records that real ESP
//! `\\EFI\\RayNu\\OVMF.fd` bytes were read-attempted for a private guest-UEFI
...
//! are still absent (`LiveEspReadAbsent`). A 2 MiB fixture is
...
use super::guest_fw::{
    ...
    guest_fw_bytes, load_guest_firmware, load_ovmf_from_esp, ovmf_live_esp_is_read,
    prepare_ovmf_firmware_launch, probe_ovmf_firmware, read_ovmf_live_esp, reset_guest_fw,
...
use super::m7_e5_live_admit_gate::run_m7_e5_live_admit_gate;
use super::VmTable;
...
use super::guest_fw::{
    ...
    qualify_real_esp_ovmf, require_ovmf_live_esp, require_ovmf_live_fd, stage_edk2_ovmf_firmware,
...
/// Host / CI marker when the E5 Stage 27 live-read gate passes.
pub const M7_E5_LIVE_READ_OK_MARKER: &str = "RAYNU-V-M7-E5-LIVE-READ-OK";

/// Honest residual: live ESP read-attempted; still absent; VMLAUNCH insn not issued.
pub const E5_LIVE_READ_RESIDUAL_NOTE: &str =
    "residual: real ESP OVMF.fd bytes were read-attempted; live ESP bytes are not present; 4 MiB fixture is not a shipped OVMF.fd; live EPT is not written; VMLAUNCH insn not issued; attach_cdrom_uefi stays UnsupportedOnFirmware; no guest UEFI VMLAUNCH";

fn stage_through_live_admit() -> bool {
    reset_guest_fw();
...
/// Live-admit then live-ESP read. VMLAUNCH stays LiveEspReadAbsent.
pub fn prop_live_read_after_live_admit() -> bool {
    #[cfg(not(test))]
    {
        return false;
    }
    #[cfg(test)]
    {
        if !stage_through_live_admit() {
            return false;
        }
        if try_vmlaunch_ovmf_firmware() != Err(GuestFwError::LiveEspAdmitAbsent) {
            return false;
        }
        let mut live = vec![0u8; MIN_LIVE_ESP_OVMF_BYTES];
        if write_live_esp_ovmf_fv(&mut live).is_err() || write_reset_vector_stub(&mut live).is_err()
        ...
        if read_ovmf_live_esp(&live) != Err(GuestFwError::TooSmall) {
            return false;
        }
        let mut alias = vec![0u8; MIN_FIRMWARE_ALIAS_BYTES];
        if write_firmware_alias_fv(&mut alias).is_err() {
        ...
        match read_ovmf_live_esp(&alias) {
            Ok(a) => {
                a.bytes_len == MIN_FIRMWARE_ALIAS_BYTES as u64
                    && a.gpa == 0xFFC0_0000
                    && ovmf_live_esp_is_read()
                    && try_vmlaunch_ovmf_firmware() == Err(GuestFwError::LiveEspReadAbsent)
            }
            Err(_) => false,
        }
    }
...
/// Live-ESP read without admit-attempt, reject. attach_cdrom_uefi stays closed.
pub fn prop_live_read_rejects() -> bool {
    reset_guest_fw();
    if read_ovmf_live_esp(&[]) != Err(GuestFwError::LaunchNotWired) {
        return false;
    }
    attach_cdrom_uefi(1) == Err(IsoError::UnsupportedOnFirmware)
}

/// Bearer REST: live-admit then live-read. Without admit → 409.
/// `POST /fw/vmlaunch` → 409 (LiveEspReadAbsent). `iso=0` SHELL stays 201.
pub fn prop_rest_live_read() -> bool {
    reset_guest_fw();
    ...
    let read = dispatch_guest_fw_rest(RestRequest {
        method: RestMethod::Post,
        path: "/fw/live-read",
        auth_token: tok,
    });
    if read.status != 201 || !ovmf_live_esp_is_read() {
        return false;
    }
...
/// SPA + ADR-014 Stage 27 phrases. Live ESP read-attempted; VMLAUNCH insn not issued.
pub fn live_read_surface_present() -> bool {
    ...
    spa.contains("Read live ESP")
        ...
        && adr.contains("Stage 27")
        && adr.contains("read_ovmf_live_esp")
        && adr.contains("LiveEspReadAbsent")
        && src.contains("fn read_ovmf_live_esp")
        && src.contains("ovmf_live_esp_is_read")
        && launch.contains("fn try_vmlaunch_guest_uefi_ovmf")
        && launch.contains("fn read_guest_uefi_live_esp")
        && launch.contains("GUEST_UEFI_PRIVATE_VMCS_ID")
        && launch.contains("LiveEspReadAbsent")
        && launch.contains("LiveEspAdmitAbsent")
...
/// Full E5 Stage 27 package. Host gate only — not iron, not Everest E5.
pub fn run_m7_e5_live_read_gate() -> bool {
    let _ = (M7_E5_LIVE_READ_OK_MARKER, E5_LIVE_READ_RESIDUAL_NOTE);
    reset_guest_fw();
    let ok = prop_live_read_after_live_admit()
        && prop_live_read_rejects()
        && prop_rest_live_read()
        && live_read_surface_present()
        && run_m7_e5_live_admit_gate()
        && E5_LIVE_READ_RESIDUAL_NOTE.contains("UnsupportedOnFirmware")
        && E5_LIVE_READ_RESIDUAL_NOTE.contains("no guest UEFI VMLAUNCH")
        && E5_LIVE_READ_RESIDUAL_NOTE.contains("not a shipped OVMF.fd")
        && E5_LIVE_READ_RESIDUAL_NOTE.contains("VMLAUNCH insn not issued")
        && E5_LIVE_READ_RESIDUAL_NOTE.contains("live EPT is not written")
        && M7_E5_LIVE_READ_OK_MARKER == "RAYNU-V-M7-E5-LIVE-READ-OK";
    reset_guest_fw();
    ok
}

#[cfg(test)]
#[path = "m7_e5_live_read_gate_test.rs"]
mod m7_e5_live_read_gate_test;
