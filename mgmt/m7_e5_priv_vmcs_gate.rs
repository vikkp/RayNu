//! E5 Stage 21 — private guest-UEFI VMCS arm (outside Proven Core).
//!
//! Pillar: [Z] [A]
//! Proven Core: **outside** mgmt bookkeeping; launch.rs entry is L0 (ADR-014)
//! VERIFICATION: N/A
//!
//! Host/CI: `arm_ovmf_private_vmcs` records that a private guest-UEFI
//! VMCS (not the E4 SHELL) is selected for real ESP `\\EFI\\RayNu\\OVMF.fd`.
//! `try_vmlaunch_ovmf_firmware` then calls `try_vmlaunch_guest_uefi_ovmf`,
//! which refuses to issue VMLAUNCH (`PrivateVmcsNotLaunched`). A 2 MiB
...
use super::guest_fw::{
    arm_ovmf_firmware_slot, arm_ovmf_private_vmcs, bind_ovmf_firmware_guest, box_guest_firmware,
    dispatch_guest_fw_rest, guest_fw_bytes, load_guest_firmware, load_ovmf_from_esp,
    ovmf_private_vmcs_is_armed, prepare_ovmf_firmware_launch, probe_ovmf_firmware, reset_guest_fw,
...
use super::m7_e5_live_exec_gate::run_m7_e5_live_exec_gate;
use super::VmTable;

#[cfg(test)]
use super::guest_fw::{
    arm_ovmf_esp_launch, arm_ovmf_firmware_alias, arm_ovmf_real_launch, arm_ovmf_reset_vector,
    install_ovmf_alias_ept, map_live_esp_ovmf, program_ovmf_alias_ept, qualify_real_esp_ovmf,
    require_ovmf_live_esp, stage_edk2_ovmf_firmware, try_vmlaunch_ovmf_firmware,
...
/// Host / CI marker when the E5 Stage 21 private-VMCS gate passes.
pub const M7_E5_PRIV_VMCS_OK_MARKER: &str = "RAYNU-V-M7-E5-PRIV-VMCS-OK";

/// Honest residual: private VMCS selected; VMLAUNCH insn not issued.
pub const E5_PRIV_VMCS_RESIDUAL_NOTE: &str =
    "residual: private guest-UEFI VMCS is selected (not E4 SHELL); 4 MiB fixture is not a shipped OVMF.fd; live EPT is not written; VMLAUNCH insn not issued; attach_cdrom_uefi stays UnsupportedOnFirmware; no guest UEFI VMLAUNCH";

fn stage_through_live_exec() -> bool {
    reset_guest_fw();
...
/// Live-ESP require then private-VMCS arm. VMLAUNCH stays PrivateVmcsNotLaunched.
pub fn prop_priv_vmcs_after_require() -> bool {
    #[cfg(not(test))]
    {
        return false;
    }
    #[cfg(test)]
    {
        if !stage_through_live_exec() {
            return false;
        }
        if try_vmlaunch_ovmf_firmware() != Err(GuestFwError::LiveEspRequired) {
            return false;
        }
        let mut live = vec![0u8; MIN_LIVE_ESP_OVMF_BYTES];
        if write_live_esp_ovmf_fv(&mut live).is_err() || write_reset_vector_stub(&mut live).is_err()
        ...
        if arm_ovmf_private_vmcs(&live) != Err(GuestFwError::TooSmall) {
            return false;
        }
        let mut alias = vec![0u8; MIN_FIRMWARE_ALIAS_BYTES];
        if write_firmware_alias_fv(&mut alias).is_err() {
        ...
        match arm_ovmf_private_vmcs(&alias) {
            Ok(a) => {
                a.bytes_len == MIN_FIRMWARE_ALIAS_BYTES as u64
                    && a.gpa == 0xFFC0_0000
                    && ovmf_private_vmcs_is_armed()
                    && try_vmlaunch_ovmf_firmware() == Err(GuestFwError::PrivateVmcsNotLaunched)
            }
            Err(_) => false,
        }
    }
...
/// Private-VMCS arm without require, reject. attach_cdrom_uefi stays closed.
pub fn prop_priv_vmcs_rejects() -> bool {
    reset_guest_fw();
    if arm_ovmf_private_vmcs(&[]) != Err(GuestFwError::LaunchNotWired) {
        return false;
    }
    attach_cdrom_uefi(1) == Err(IsoError::UnsupportedOnFirmware)
}

/// Bearer REST: require then arm. Without require → 409.
/// `POST /fw/vmlaunch` → 409 (PrivateVmcsNotLaunched). `iso=0` SHELL stays 201.
pub fn prop_rest_priv_vmcs() -> bool {
    reset_guest_fw();
    ...
    if armed.status != 201 || !ovmf_private_vmcs_is_armed() {
        return false;
    }
...
/// SPA + ADR-014 Stage 21 phrases. Private VMCS selected; VMLAUNCH insn not issued.
pub fn priv_vmcs_surface_present() -> bool {
    ...
    spa.contains("Arm private VMCS")
        ...
        && adr.contains("Stage 21")
        && adr.contains("arm_ovmf_private_vmcs")
        && adr.contains("PrivateVmcsNotLaunched")
        && src.contains("fn arm_ovmf_private_vmcs")
        && src.contains("ovmf_private_vmcs_is_armed")
        && launch.contains("fn try_vmlaunch_guest_uefi_ovmf")
        && launch.contains("fn arm_guest_uefi_private_vmcs")
        && launch.contains("GUEST_UEFI_PRIVATE_VMCS_ID")
        && launch.contains("PrivateVmcsNotLaunched")
        && launch.contains("LiveEspRequired")
...
/// Full E5 Stage 21 package. Host gate only — not iron, not Everest E5.
pub fn run_m7_e5_priv_vmcs_gate() -> bool {
    let _ = (M7_E5_PRIV_VMCS_OK_MARKER, E5_PRIV_VMCS_RESIDUAL_NOTE);
    reset_guest_fw();
    let ok = prop_priv_vmcs_after_require()
        && prop_priv_vmcs_rejects()
        && prop_rest_priv_vmcs()
        && priv_vmcs_surface_present()
        && run_m7_e5_live_exec_gate()
        && E5_PRIV_VMCS_RESIDUAL_NOTE.contains("UnsupportedOnFirmware")
        && E5_PRIV_VMCS_RESIDUAL_NOTE.contains("no guest UEFI VMLAUNCH")
        && E5_PRIV_VMCS_RESIDUAL_NOTE.contains("not a shipped OVMF.fd")
        && E5_PRIV_VMCS_RESIDUAL_NOTE.contains("VMLAUNCH insn not issued")
        && E5_PRIV_VMCS_RESIDUAL_NOTE.contains("live EPT is not written")
        && M7_E5_PRIV_VMCS_OK_MARKER == "RAYNU-V-M7-E5-PRIV-VMCS-OK";
    reset_guest_fw();
    ok
}

#[cfg(test)]
#[path = "m7_e5_priv_vmcs_gate_test.rs"]
mod m7_e5_priv_vmcs_gate_test;
