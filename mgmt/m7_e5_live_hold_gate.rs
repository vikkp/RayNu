//! E5 Stage 35 — live-ESP hold-attempt (outside Proven Core).
//!
//! Pillar: [Z] [A]
//! Proven Core: **outside** mgmt bookkeeping; launch.rs entry is L0 (ADR-014)
//! VERIFICATION: N/A
//!
//! Host/CI: `hold_ovmf_live_esp` records that real ESP
//! `\\EFI\\RayNu\\OVMF.fd` bytes were hold-attempted for a private guest-UEFI
...
//! are still absent (`LiveEspHoldAbsent`). A 2 MiB fixture is
...
use super::guest_fw::{
    ...
    guest_fw_bytes, hold_ovmf_live_esp, load_guest_firmware, load_ovmf_from_esp,
    ovmf_live_esp_is_held, prepare_ovmf_firmware_launch, probe_ovmf_firmware, reset_guest_fw,
...
use super::m7_e5_live_lock_gate::run_m7_e5_live_lock_gate;
use super::VmTable;

#[cfg(test)]
use super::guest_fw::{
    admit_ovmf_live_esp, apply_ovmf_live_esp, arm_ovmf_esp_launch, arm_ovmf_firmware_alias,
    arm_ovmf_live_issue, arm_ovmf_private_vmcs, arm_ovmf_real_launch, arm_ovmf_reset_vector,
    commit_ovmf_live_esp, copy_ovmf_live_esp, install_ovmf_alias_ept, latch_ovmf_live_esp,
    lock_ovmf_live_esp, map_live_esp_ovmf, place_ovmf_live_esp, present_ovmf_live_esp,
    probe_ovmf_live_bytes, program_ovmf_alias_ept, qualify_real_esp_ovmf, read_ovmf_live_esp,
    require_ovmf_live_esp, require_ovmf_live_fd, seal_ovmf_live_esp, stage_edk2_ovmf_firmware,
...
/// Host / CI marker when the E5 Stage 35 live-hold gate passes.
pub const M7_E5_LIVE_HOLD_OK_MARKER: &str = "RAYNU-V-M7-E5-LIVE-HOLD-OK";

/// Honest residual: live ESP hold-attempted; still absent; VMLAUNCH insn not issued.
pub const E5_LIVE_HOLD_RESIDUAL_NOTE: &str =
    "residual: real ESP OVMF.fd bytes were hold-attempted; live ESP bytes are not present; 4 MiB fixture is not a shipped OVMF.fd; live EPT is not written; VMLAUNCH insn not issued; attach_cdrom_uefi stays UnsupportedOnFirmware; no guest UEFI VMLAUNCH";

fn stage_through_live_lock() -> bool {
    reset_guest_fw();
    ...
    #[cfg(test)]
    {
        ...
        return arm_ovmf_firmware_alias(&alias).is_ok()
            ...
            && present_ovmf_live_esp(&alias).is_ok()
            && admit_ovmf_live_esp(&alias).is_ok()
            && read_ovmf_live_esp(&alias).is_ok()
            && copy_ovmf_live_esp(&alias).is_ok()
            && place_ovmf_live_esp(&alias).is_ok()
            && apply_ovmf_live_esp(&alias).is_ok()
            && commit_ovmf_live_esp(&alias).is_ok()
            && latch_ovmf_live_esp(&alias).is_ok()
            && seal_ovmf_live_esp(&alias).is_ok()
            && lock_ovmf_live_esp(&alias).is_ok();
    }
    #[cfg(not(test))]
    {
        false
...
/// Live-lock then live-ESP hold. VMLAUNCH stays LiveEspHoldAbsent.
pub fn prop_live_hold_after_live_lock() -> bool {
    #[cfg(not(test))]
    {
        return false;
    }
    #[cfg(test)]
    {
        if !stage_through_live_lock() {
            return false;
        }
        if try_vmlaunch_ovmf_firmware() != Err(GuestFwError::LiveEspLockAbsent) {
            return false;
        }
        let mut live = vec![0u8; MIN_LIVE_ESP_OVMF_BYTES];
        if write_live_esp_ovmf_fv(&mut live).is_err() || write_reset_vector_stub(&mut live).is_err()
        ...
        if hold_ovmf_live_esp(&live) != Err(GuestFwError::TooSmall) {
            return false;
        }
        let mut alias = vec![0u8; MIN_FIRMWARE_ALIAS_BYTES];
        if write_firmware_alias_fv(&mut alias).is_err() {
        ...
        match hold_ovmf_live_esp(&alias) {
            Ok(a) => {
                a.bytes_len == MIN_FIRMWARE_ALIAS_BYTES as u64
                    && a.gpa == 0xFFC0_0000
                    && ovmf_live_esp_is_held()
                    && try_vmlaunch_ovmf_firmware() == Err(GuestFwError::LiveEspHoldAbsent)
            }
            Err(_) => false,
        }
    }
...
/// Live-ESP hold without lock-attempt, reject. attach_cdrom_uefi stays closed.
pub fn prop_live_hold_rejects() -> bool {
    reset_guest_fw();
    if hold_ovmf_live_esp(&[]) != Err(GuestFwError::LaunchNotWired) {
        return false;
    }
    attach_cdrom_uefi(1) == Err(IsoError::UnsupportedOnFirmware)
}

/// Bearer REST: live-lock then live-hold. Without lock → 409.
/// `POST /fw/vmlaunch` → 409 (LiveEspHoldAbsent). `iso=0` SHELL stays 201.
pub fn prop_rest_live_hold() -> bool {
    reset_guest_fw();
    ...
    let hold = dispatch_guest_fw_rest(RestRequest {
        method: RestMethod::Post,
        path: "/fw/live-hold",
        auth_token: tok,
    });
    if hold.status != 201 || !ovmf_live_esp_is_held() {
        return false;
    }
...
/// SPA + ADR-014 Stage 35 phrases. Live ESP hold-attempted; VMLAUNCH insn not issued.
pub fn live_hold_surface_present() -> bool {
    ...
    spa.contains("Hold live ESP")
        && spa.contains("Lock live ESP")
        && spa.contains("Seal live ESP")
        && spa.contains("Latch live ESP")
        && spa.contains("Commit live ESP")
        ...
        && adr.contains("Stage 35")
        && adr.contains("hold_ovmf_live_esp")
        && adr.contains("LiveEspHoldAbsent")
        && src.contains("fn hold_ovmf_live_esp")
        && src.contains("ovmf_live_esp_is_held")
        && launch.contains("fn try_vmlaunch_guest_uefi_ovmf")
        && launch.contains("fn hold_guest_uefi_live_esp")
        && launch.contains("GUEST_UEFI_PRIVATE_VMCS_ID")
        && launch.contains("LiveEspHoldAbsent")
        && launch.contains("LiveEspLockAbsent")
        && !launch.contains("hold_ovmf_live_esp")
        && !launch.contains("lock_ovmf_live_esp")
        && !launch.contains("seal_ovmf_live_esp")
        && !launch.contains("latch_ovmf_live_esp")
        && !launch.contains("commit_ovmf_live_esp")
        && !launch.contains("apply_ovmf_live_esp")
        && !launch.contains("place_ovmf_live_esp")
        && !launch.contains("copy_ovmf_live_esp")
        && !launch.contains("read_ovmf_live_esp")
        && !launch.contains("admit_ovmf_live_esp")
...
/// Full E5 Stage 35 package. Host gate only — not iron, not Everest E5.
pub fn run_m7_e5_live_hold_gate() -> bool {
    let _ = (M7_E5_LIVE_HOLD_OK_MARKER, E5_LIVE_HOLD_RESIDUAL_NOTE);
    reset_guest_fw();
    let ok = prop_live_hold_after_live_lock()
        && prop_live_hold_rejects()
        && prop_rest_live_hold()
        && live_hold_surface_present()
        && run_m7_e5_live_lock_gate()
        && E5_LIVE_HOLD_RESIDUAL_NOTE.contains("UnsupportedOnFirmware")
        && E5_LIVE_HOLD_RESIDUAL_NOTE.contains("no guest UEFI VMLAUNCH")
        && E5_LIVE_HOLD_RESIDUAL_NOTE.contains("not a shipped OVMF.fd")
        && E5_LIVE_HOLD_RESIDUAL_NOTE.contains("VMLAUNCH insn not issued")
        && E5_LIVE_HOLD_RESIDUAL_NOTE.contains("live EPT is not written")
        && M7_E5_LIVE_HOLD_OK_MARKER == "RAYNU-V-M7-E5-LIVE-HOLD-OK";
    reset_guest_fw();
    ok
}

#[cfg(test)]
#[path = "m7_e5_live_hold_gate_test.rs"]
mod m7_e5_live_hold_gate_test;
