//! E5 Stage 28 — live-ESP copy-attempt (outside Proven Core).
//!
//! Pillar: [Z] [A]
//! Proven Core: **outside** mgmt bookkeeping; launch.rs entry is L0 (ADR-014)
//! VERIFICATION: N/A
//!
//! Host/CI: `copy_ovmf_live_esp` records that real ESP
//! `\\EFI\\RayNu\\OVMF.fd` bytes were copy-attempted for a private guest-UEFI
...
//! are still absent (`LiveEspCopyAbsent`). A 2 MiB fixture is
...
use super::guest_fw::{
    arm_ovmf_firmware_slot, bind_ovmf_firmware_guest, box_guest_firmware, copy_ovmf_live_esp,
    dispatch_guest_fw_rest, guest_fw_bytes, load_guest_firmware, load_ovmf_from_esp,
    ovmf_live_esp_is_copied, prepare_ovmf_firmware_launch, probe_ovmf_firmware, reset_guest_fw,
...
use super::m7_e5_live_read_gate::run_m7_e5_live_read_gate;
use super::VmTable;

#[cfg(test)]
use super::guest_fw::{
    admit_ovmf_live_esp, arm_ovmf_esp_launch, arm_ovmf_firmware_alias, arm_ovmf_live_issue,
    arm_ovmf_private_vmcs, arm_ovmf_real_launch, arm_ovmf_reset_vector, install_ovmf_alias_ept,
    map_live_esp_ovmf, present_ovmf_live_esp, probe_ovmf_live_bytes, program_ovmf_alias_ept,
...
/// Host / CI marker when the E5 Stage 28 live-copy gate passes.
pub const M7_E5_LIVE_COPY_OK_MARKER: &str = "RAYNU-V-M7-E5-LIVE-COPY-OK";

/// Honest residual: live ESP copy-attempted; still absent; VMLAUNCH insn not issued.
pub const E5_LIVE_COPY_RESIDUAL_NOTE: &str =
    "residual: real ESP OVMF.fd bytes were copy-attempted; live ESP bytes are not present; 4 MiB fixture is not a shipped OVMF.fd; live EPT is not written; VMLAUNCH insn not issued; attach_cdrom_uefi stays UnsupportedOnFirmware; no guest UEFI VMLAUNCH";

fn stage_through_live_read() -> bool {
    reset_guest_fw();
...
/// Live-read then live-ESP copy. VMLAUNCH stays LiveEspCopyAbsent.
pub fn prop_live_copy_after_live_read() -> bool {
    #[cfg(not(test))]
    {
        return false;
    }
    #[cfg(test)]
    {
        if !stage_through_live_read() {
            return false;
        }
        if try_vmlaunch_ovmf_firmware() != Err(GuestFwError::LiveEspReadAbsent) {
            return false;
        }
        let mut live = vec![0u8; MIN_LIVE_ESP_OVMF_BYTES];
        if write_live_esp_ovmf_fv(&mut live).is_err() || write_reset_vector_stub(&mut live).is_err()
        ...
        if copy_ovmf_live_esp(&live) != Err(GuestFwError::TooSmall) {
            return false;
        }
        let mut alias = vec![0u8; MIN_FIRMWARE_ALIAS_BYTES];
        if write_firmware_alias_fv(&mut alias).is_err() {
        ...
        match copy_ovmf_live_esp(&alias) {
            Ok(a) => {
                a.bytes_len == MIN_FIRMWARE_ALIAS_BYTES as u64
                    && a.gpa == 0xFFC0_0000
                    && ovmf_live_esp_is_copied()
                    && try_vmlaunch_ovmf_firmware() == Err(GuestFwError::LiveEspCopyAbsent)
            }
            Err(_) => false,
        }
    }
...
/// Live-ESP copy without read-attempt, reject. attach_cdrom_uefi stays closed.
pub fn prop_live_copy_rejects() -> bool {
    reset_guest_fw();
    if copy_ovmf_live_esp(&[]) != Err(GuestFwError::LaunchNotWired) {
        return false;
    }
    attach_cdrom_uefi(1) == Err(IsoError::UnsupportedOnFirmware)
}

/// Bearer REST: live-read then live-copy. Without read → 409.
/// `POST /fw/vmlaunch` → 409 (LiveEspCopyAbsent). `iso=0` SHELL stays 201.
pub fn prop_rest_live_copy() -> bool {
    reset_guest_fw();
    ...
    let copy = dispatch_guest_fw_rest(RestRequest {
        method: RestMethod::Post,
        path: "/fw/live-copy",
        auth_token: tok,
    });
    if copy.status != 201 || !ovmf_live_esp_is_copied() {
        return false;
    }
...
/// SPA + ADR-014 Stage 28 phrases. Live ESP copy-attempted; VMLAUNCH insn not issued.
pub fn live_copy_surface_present() -> bool {
    ...
    spa.contains("Copy live ESP")
        ...
        && adr.contains("Stage 28")
        && adr.contains("copy_ovmf_live_esp")
        && adr.contains("LiveEspCopyAbsent")
        && src.contains("fn copy_ovmf_live_esp")
        && src.contains("ovmf_live_esp_is_copied")
        && launch.contains("fn try_vmlaunch_guest_uefi_ovmf")
        && launch.contains("fn copy_guest_uefi_live_esp")
        && launch.contains("GUEST_UEFI_PRIVATE_VMCS_ID")
        && launch.contains("LiveEspCopyAbsent")
        && launch.contains("LiveEspReadAbsent")
...
/// Full E5 Stage 28 package. Host gate only — not iron, not Everest E5.
pub fn run_m7_e5_live_copy_gate() -> bool {
    let _ = (M7_E5_LIVE_COPY_OK_MARKER, E5_LIVE_COPY_RESIDUAL_NOTE);
    reset_guest_fw();
    let ok = prop_live_copy_after_live_read()
        && prop_live_copy_rejects()
        && prop_rest_live_copy()
        && live_copy_surface_present()
        && run_m7_e5_live_read_gate()
        && E5_LIVE_COPY_RESIDUAL_NOTE.contains("UnsupportedOnFirmware")
        && E5_LIVE_COPY_RESIDUAL_NOTE.contains("no guest UEFI VMLAUNCH")
        && E5_LIVE_COPY_RESIDUAL_NOTE.contains("not a shipped OVMF.fd")
        && E5_LIVE_COPY_RESIDUAL_NOTE.contains("VMLAUNCH insn not issued")
        && E5_LIVE_COPY_RESIDUAL_NOTE.contains("live EPT is not written")
        && M7_E5_LIVE_COPY_OK_MARKER == "RAYNU-V-M7-E5-LIVE-COPY-OK";
    reset_guest_fw();
    ok
}

#[cfg(test)]
#[path = "m7_e5_live_copy_gate_test.rs"]
mod m7_e5_live_copy_gate_test;
