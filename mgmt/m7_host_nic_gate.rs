...
pub fn host_nic_surface_present() -> bool {
    ...
    mmio.contains("All PCI config")
        ...
        && listen.contains("HTTPS window before RayNu-F")
        && listen.contains("before RayNu-F; SNP is dead")
        && nic.contains("PRE_RAYNUF_HTTPS_MS")
        ...
        && listen.contains("fn dhcp_bcm5720(")
        && listen.contains("HOST-NIC BCM5720 DHCP discover")
        && listen.contains("HOST-NIC BCM5720 DHCP failed")
        && listen.contains("prefer_mac()")
        && nic.contains("HOST_NIC_DHCP_MS")
        ...
        && listen.contains("HOST_NIC_HTTP_HS_IDLE_MS")
        && listen.contains("http_accept_idle_limit_ms")
        ...
        && listen.contains("HTTP keep-alive")
        && listen.contains("write_line_nowait(\"boot: HOST-NIC HTTP keep-alive")
        && listen.contains("write_line_nowait(\"boot: HOST-NIC HTTP exchange ok")
        && listen.contains("COEXIST_KEEPALIVE_IDLE_MS")
        && listen.contains("clear_http")
