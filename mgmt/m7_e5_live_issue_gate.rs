//! E5 Stage 22 — live-ESP VMLAUNCH issue path (outside Proven Core).
//!
//! Pillar: [Z] [A]
//! Proven Core: **outside** mgmt bookkeeping; launch.rs entry is L0 (ADR-014)
//! VERIFICATION: N/A
//!
//! Host/CI: `arm_ovmf_live_issue` records that the VMLAUNCH instruction
//! would issue against real ESP `\\EFI\\RayNu\\OVMF.fd` in a private
//! guest-UEFI VMCS (not the E4 SHELL). `try_vmlaunch_ovmf_firmware`
//! then calls `try_vmlaunch_guest_uefi_ovmf`, which refuses because
//! live ESP bytes are absent (`LiveEspBytesNotPresent`). A 2 MiB
//! fixture is `TooSmall`. The E4 SHELL EPT is not written.
...
use super::guest_fw::{
    arm_ovmf_firmware_slot, arm_ovmf_live_issue, bind_ovmf_firmware_guest, box_guest_firmware,
    dispatch_guest_fw_rest, guest_fw_bytes, load_guest_firmware, load_ovmf_from_esp,
    ovmf_live_issue_is_armed, prepare_ovmf_firmware_launch, probe_ovmf_firmware, reset_guest_fw,
...
use super::m7_e5_priv_vmcs_gate::run_m7_e5_priv_vmcs_gate;
use super::VmTable;

#[cfg(test)]
use super::guest_fw::{
    arm_ovmf_esp_launch, arm_ovmf_firmware_alias, arm_ovmf_private_vmcs, arm_ovmf_real_launch,
    arm_ovmf_reset_vector, install_ovmf_alias_ept, map_live_esp_ovmf, program_ovmf_alias_ept,
    qualify_real_esp_ovmf, require_ovmf_live_esp, stage_edk2_ovmf_firmware,
...
/// Host / CI marker when the E5 Stage 22 live-issue gate passes.
pub const M7_E5_LIVE_ISSUE_OK_MARKER: &str = "RAYNU-V-M7-E5-LIVE-ISSUE-OK";

/// Honest residual: issue path armed; VMLAUNCH insn not issued.
pub const E5_LIVE_ISSUE_RESIDUAL_NOTE: &str =
    "residual: live-ESP VMLAUNCH issue path is armed; live ESP bytes are not present; 4 MiB fixture is not a shipped OVMF.fd; live EPT is not written; VMLAUNCH insn not issued; attach_cdrom_uefi stays UnsupportedOnFirmware; no guest UEFI VMLAUNCH";

fn stage_through_priv_vmcs() -> bool {
    reset_guest_fw();
...
/// Private-VMCS arm then live-issue arm. VMLAUNCH stays LiveEspBytesNotPresent.
pub fn prop_live_issue_after_priv_vmcs() -> bool {
    #[cfg(not(test))]
    {
        return false;
    }
    #[cfg(test)]
    {
        if !stage_through_priv_vmcs() {
            return false;
        }
        if try_vmlaunch_ovmf_firmware() != Err(GuestFwError::PrivateVmcsNotLaunched) {
            return false;
        }
        let mut live = vec![0u8; MIN_LIVE_ESP_OVMF_BYTES];
        if write_live_esp_ovmf_fv(&mut live).is_err() || write_reset_vector_stub(&mut live).is_err()
        ...
        if arm_ovmf_live_issue(&live) != Err(GuestFwError::TooSmall) {
            return false;
        }
        let mut alias = vec![0u8; MIN_FIRMWARE_ALIAS_BYTES];
        if write_firmware_alias_fv(&mut alias).is_err() {
        ...
        match arm_ovmf_live_issue(&alias) {
            Ok(a) => {
                a.bytes_len == MIN_FIRMWARE_ALIAS_BYTES as u64
                    && a.gpa == 0xFFC0_0000
                    && ovmf_live_issue_is_armed()
                    && try_vmlaunch_ovmf_firmware() == Err(GuestFwError::LiveEspBytesNotPresent)
            }
            Err(_) => false,
        }
    }
...
/// Live-issue arm without private VMCS, reject. attach_cdrom_uefi stays closed.
pub fn prop_live_issue_rejects() -> bool {
    reset_guest_fw();
    if arm_ovmf_live_issue(&[]) != Err(GuestFwError::LaunchNotWired) {
        return false;
    }
    attach_cdrom_uefi(1) == Err(IsoError::UnsupportedOnFirmware)
}

/// Bearer REST: private VMCS then live-issue. Without private VMCS → 409.
/// `POST /fw/vmlaunch` → 409 (LiveEspBytesNotPresent). `iso=0` SHELL stays 201.
pub fn prop_rest_live_issue() -> bool {
    reset_guest_fw();
    ...
    if armed.status != 201 || !ovmf_live_issue_is_armed() {
        return false;
    }
...
/// SPA + ADR-014 Stage 22 phrases. Issue path armed; VMLAUNCH insn not issued.
pub fn live_issue_surface_present() -> bool {
    ...
    spa.contains("Arm live issue")
        ...
        && adr.contains("Stage 22")
        && adr.contains("arm_ovmf_live_issue")
        && adr.contains("LiveEspBytesNotPresent")
        && src.contains("fn arm_ovmf_live_issue")
        && src.contains("ovmf_live_issue_is_armed")
        && launch.contains("fn try_vmlaunch_guest_uefi_ovmf")
        && launch.contains("fn arm_guest_uefi_live_issue")
        && launch.contains("GUEST_UEFI_PRIVATE_VMCS_ID")
        && launch.contains("LiveEspBytesNotPresent")
        && launch.contains("PrivateVmcsNotLaunched")
...
/// Full E5 Stage 22 package. Host gate only — not iron, not Everest E5.
pub fn run_m7_e5_live_issue_gate() -> bool {
    let _ = (M7_E5_LIVE_ISSUE_OK_MARKER, E5_LIVE_ISSUE_RESIDUAL_NOTE);
    reset_guest_fw();
    let ok = prop_live_issue_after_priv_vmcs()
        && prop_live_issue_rejects()
        && prop_rest_live_issue()
        && live_issue_surface_present()
        && run_m7_e5_priv_vmcs_gate()
        && E5_LIVE_ISSUE_RESIDUAL_NOTE.contains("UnsupportedOnFirmware")
        && E5_LIVE_ISSUE_RESIDUAL_NOTE.contains("no guest UEFI VMLAUNCH")
        && E5_LIVE_ISSUE_RESIDUAL_NOTE.contains("not a shipped OVMF.fd")
        && E5_LIVE_ISSUE_RESIDUAL_NOTE.contains("VMLAUNCH insn not issued")
        && E5_LIVE_ISSUE_RESIDUAL_NOTE.contains("live EPT is not written")
        && M7_E5_LIVE_ISSUE_OK_MARKER == "RAYNU-V-M7-E5-LIVE-ISSUE-OK";
    reset_guest_fw();
    ok
}

#[cfg(test)]
#[path = "m7_e5_live_issue_gate_test.rs"]
mod m7_e5_live_issue_gate_test;
