...
//! VERIFICATION: N/A
//!
//! Guest-UEFI PCI virtio 1.0 block at `00:00.1` plus fw_cfg `bootorder`
//! (CD then disk). Marker after past-SEC and virtio PCI enum.
//! Not a completed firmware CD boot. Not installer. No new `*Absent` enum. No TLS.
...
use crate::devices::guest_platform::{
    boot_order_cd_then_disk, fwcfg_bootorder_served, io, reset as reset_plat, BOOTORDER,
...
pub fn prop_virtio_pci_and_bootorder() -> bool {
    ...
    reset_plat();
    ide_cdrom::reset();
    virtio_ok && boot_served && first == *b"/pci@i0c" && BOOTORDER.starts_with(b"/pci@i0cf8/ide@0")
}
...
pub fn run_m7_e5_ovmf_virtio_gate() -> bool {
    ...
    let ok = ovmf_virtio_surface_present()
        ...
        && E5_OVMF_VMLAUNCH_RESIDUAL_NOTE.contains("empty virtio-blk at 00:00.1")
