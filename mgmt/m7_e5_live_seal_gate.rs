//! E5 Stage 33 — live-ESP seal-attempt (outside Proven Core).
//!
//! Pillar: [Z] [A]
//! Proven Core: **outside** mgmt bookkeeping; launch.rs entry is L0 (ADR-014)
//! VERIFICATION: N/A
//!
//! Host/CI: `seal_ovmf_live_esp` records that real ESP
//! `\\EFI\\RayNu\\OVMF.fd` bytes were seal-attempted for a private guest-UEFI
...
//! are still absent (`LiveEspSealAbsent`). A 2 MiB fixture is
...
use super::guest_fw::{
    ...
    guest_fw_bytes, load_guest_firmware, load_ovmf_from_esp, ovmf_live_esp_is_sealed,
    prepare_ovmf_firmware_launch, probe_ovmf_firmware, reset_guest_fw, seal_ovmf_live_esp,
...
use super::m7_e5_live_latch_gate::run_m7_e5_live_latch_gate;
use super::VmTable;
...
/// Host / CI marker when the E5 Stage 33 live-seal gate passes.
pub const M7_E5_LIVE_SEAL_OK_MARKER: &str = "RAYNU-V-M7-E5-LIVE-SEAL-OK";

/// Honest residual: live ESP seal-attempted; still absent; VMLAUNCH insn not issued.
pub const E5_LIVE_SEAL_RESIDUAL_NOTE: &str =
    "residual: real ESP OVMF.fd bytes were seal-attempted; live ESP bytes are not present; 4 MiB fixture is not a shipped OVMF.fd; live EPT is not written; VMLAUNCH insn not issued; attach_cdrom_uefi stays UnsupportedOnFirmware; no guest UEFI VMLAUNCH";

fn stage_through_live_latch() -> bool {
    reset_guest_fw();
...
/// Live-latch then live-ESP seal. VMLAUNCH stays LiveEspSealAbsent.
pub fn prop_live_seal_after_live_latch() -> bool {
    #[cfg(not(test))]
    {
        return false;
    }
    #[cfg(test)]
    {
        if !stage_through_live_latch() {
            return false;
        }
        if try_vmlaunch_ovmf_firmware() != Err(GuestFwError::LiveEspLatchAbsent) {
            return false;
        }
        let mut live = vec![0u8; MIN_LIVE_ESP_OVMF_BYTES];
        if write_live_esp_ovmf_fv(&mut live).is_err() || write_reset_vector_stub(&mut live).is_err()
        ...
        if seal_ovmf_live_esp(&live) != Err(GuestFwError::TooSmall) {
            return false;
        }
        let mut alias = vec![0u8; MIN_FIRMWARE_ALIAS_BYTES];
        if write_firmware_alias_fv(&mut alias).is_err() {
        ...
        match seal_ovmf_live_esp(&alias) {
            Ok(a) => {
                a.bytes_len == MIN_FIRMWARE_ALIAS_BYTES as u64
                    && a.gpa == 0xFFC0_0000
                    && ovmf_live_esp_is_sealed()
                    && try_vmlaunch_ovmf_firmware() == Err(GuestFwError::LiveEspSealAbsent)
            }
            Err(_) => false,
        }
    }
...
/// Live-ESP seal without latch-attempt, reject. attach_cdrom_uefi stays closed.
pub fn prop_live_seal_rejects() -> bool {
    reset_guest_fw();
    if seal_ovmf_live_esp(&[]) != Err(GuestFwError::LaunchNotWired) {
        return false;
    }
    attach_cdrom_uefi(1) == Err(IsoError::UnsupportedOnFirmware)
}

/// Bearer REST: live-latch then live-seal. Without latch → 409.
/// `POST /fw/vmlaunch` → 409 (LiveEspSealAbsent). `iso=0` SHELL stays 201.
pub fn prop_rest_live_seal() -> bool {
    reset_guest_fw();
    ...
    if latch.status != 201 || !ovmf_live_esp_is_sealed() {
        return false;
    }
...
/// SPA + ADR-014 Stage 33 phrases. Live ESP seal-attempted; VMLAUNCH insn not issued.
pub fn live_seal_surface_present() -> bool {
    ...
    spa.contains("Seal live ESP")
        ...
        && adr.contains("Stage 33")
        && adr.contains("seal_ovmf_live_esp")
        && adr.contains("LiveEspSealAbsent")
        && src.contains("fn seal_ovmf_live_esp")
        && src.contains("ovmf_live_esp_is_sealed")
        && launch.contains("fn try_vmlaunch_guest_uefi_ovmf")
        && launch.contains("fn seal_guest_uefi_live_esp")
        && launch.contains("GUEST_UEFI_PRIVATE_VMCS_ID")
        && launch.contains("LiveEspSealAbsent")
        && launch.contains("LiveEspLatchAbsent")
...
/// Full E5 Stage 33 package. Host gate only — not iron, not Everest E5.
pub fn run_m7_e5_live_seal_gate() -> bool {
    let _ = (M7_E5_LIVE_SEAL_OK_MARKER, E5_LIVE_SEAL_RESIDUAL_NOTE);
    reset_guest_fw();
    let ok = prop_live_seal_after_live_latch()
        && prop_live_seal_rejects()
        && prop_rest_live_seal()
        && live_seal_surface_present()
        && run_m7_e5_live_latch_gate()
        && E5_LIVE_SEAL_RESIDUAL_NOTE.contains("UnsupportedOnFirmware")
        && E5_LIVE_SEAL_RESIDUAL_NOTE.contains("no guest UEFI VMLAUNCH")
        && E5_LIVE_SEAL_RESIDUAL_NOTE.contains("not a shipped OVMF.fd")
        && E5_LIVE_SEAL_RESIDUAL_NOTE.contains("VMLAUNCH insn not issued")
        && E5_LIVE_SEAL_RESIDUAL_NOTE.contains("live EPT is not written")
        && M7_E5_LIVE_SEAL_OK_MARKER == "RAYNU-V-M7-E5-LIVE-SEAL-OK";
    reset_guest_fw();
    ok
}

#[cfg(test)]
#[path = "m7_e5_live_seal_gate_test.rs"]
mod m7_e5_live_seal_gate_test;
