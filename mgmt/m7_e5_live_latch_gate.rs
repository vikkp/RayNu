//! E5 Stage 32 — live-ESP latch-attempt (outside Proven Core).
//!
//! Pillar: [Z] [A]
//! Proven Core: **outside** mgmt bookkeeping; launch.rs entry is L0 (ADR-014)
//! VERIFICATION: N/A
//!
//! Host/CI: `latch_ovmf_live_esp` records that real ESP
//! `\\EFI\\RayNu\\OVMF.fd` bytes were latch-attempted for a private guest-UEFI
...
//! are still absent (`LiveEspLatchAbsent`). A 2 MiB fixture is
...
use super::guest_fw::{
    ...
    guest_fw_bytes, latch_ovmf_live_esp, load_guest_firmware, load_ovmf_from_esp,
    ovmf_live_esp_is_latched, prepare_ovmf_firmware_launch, probe_ovmf_firmware, reset_guest_fw,
...
use super::m7_e5_live_commit_gate::run_m7_e5_live_commit_gate;
use super::VmTable;
...
use super::guest_fw::{
    ...
    commit_ovmf_live_esp, copy_ovmf_live_esp, install_ovmf_alias_ept, map_live_esp_ovmf,
    place_ovmf_live_esp, present_ovmf_live_esp, probe_ovmf_live_bytes, program_ovmf_alias_ept,
...
/// Host / CI marker when the E5 Stage 32 live-latch gate passes.
pub const M7_E5_LIVE_LATCH_OK_MARKER: &str = "RAYNU-V-M7-E5-LIVE-LATCH-OK";

/// Honest residual: live ESP latch-attempted; still absent; VMLAUNCH insn not issued.
pub const E5_LIVE_LATCH_RESIDUAL_NOTE: &str =
    "residual: real ESP OVMF.fd bytes were latch-attempted; live ESP bytes are not present; 4 MiB fixture is not a shipped OVMF.fd; live EPT is not written; VMLAUNCH insn not issued; attach_cdrom_uefi stays UnsupportedOnFirmware; no guest UEFI VMLAUNCH";

fn stage_through_live_commit() -> bool {
    reset_guest_fw();
...
/// Live-commit then live-ESP latch. VMLAUNCH stays LiveEspLatchAbsent.
pub fn prop_live_latch_after_live_commit() -> bool {
    #[cfg(not(test))]
    {
        return false;
    }
    #[cfg(test)]
    {
        if !stage_through_live_commit() {
            return false;
        }
        if try_vmlaunch_ovmf_firmware() != Err(GuestFwError::LiveEspCommitAbsent) {
            return false;
        }
        let mut live = vec![0u8; MIN_LIVE_ESP_OVMF_BYTES];
        if write_live_esp_ovmf_fv(&mut live).is_err() || write_reset_vector_stub(&mut live).is_err()
        ...
        if latch_ovmf_live_esp(&live) != Err(GuestFwError::TooSmall) {
            return false;
        }
        let mut alias = vec![0u8; MIN_FIRMWARE_ALIAS_BYTES];
        if write_firmware_alias_fv(&mut alias).is_err() {
        ...
        match latch_ovmf_live_esp(&alias) {
            Ok(a) => {
                a.bytes_len == MIN_FIRMWARE_ALIAS_BYTES as u64
                    && a.gpa == 0xFFC0_0000
                    && ovmf_live_esp_is_latched()
                    && try_vmlaunch_ovmf_firmware() == Err(GuestFwError::LiveEspLatchAbsent)
            }
            Err(_) => false,
        }
    }
...
/// Live-ESP latch without commit-attempt, reject. attach_cdrom_uefi stays closed.
pub fn prop_live_latch_rejects() -> bool {
    reset_guest_fw();
    if latch_ovmf_live_esp(&[]) != Err(GuestFwError::LaunchNotWired) {
        return false;
    }
    attach_cdrom_uefi(1) == Err(IsoError::UnsupportedOnFirmware)
}

/// Bearer REST: live-commit then live-latch. Without commit → 409.
/// `POST /fw/vmlaunch` → 409 (LiveEspLatchAbsent). `iso=0` SHELL stays 201.
pub fn prop_rest_live_latch() -> bool {
    reset_guest_fw();
    ...
    if latch.status != 201 || !ovmf_live_esp_is_latched() {
        return false;
    }
...
/// SPA + ADR-014 Stage 32 phrases. Live ESP latch-attempted; VMLAUNCH insn not issued.
pub fn live_latch_surface_present() -> bool {
    ...
    spa.contains("Latch live ESP")
        ...
        && adr.contains("Stage 32")
        && adr.contains("latch_ovmf_live_esp")
        && adr.contains("LiveEspLatchAbsent")
        && src.contains("fn latch_ovmf_live_esp")
        && src.contains("ovmf_live_esp_is_latched")
        && launch.contains("fn try_vmlaunch_guest_uefi_ovmf")
        && launch.contains("fn latch_guest_uefi_live_esp")
        && launch.contains("GUEST_UEFI_PRIVATE_VMCS_ID")
        && launch.contains("LiveEspLatchAbsent")
        && launch.contains("LiveEspCommitAbsent")
...
/// Full E5 Stage 32 package. Host gate only — not iron, not Everest E5.
pub fn run_m7_e5_live_latch_gate() -> bool {
    let _ = (M7_E5_LIVE_LATCH_OK_MARKER, E5_LIVE_LATCH_RESIDUAL_NOTE);
    reset_guest_fw();
    let ok = prop_live_latch_after_live_commit()
        && prop_live_latch_rejects()
        && prop_rest_live_latch()
        && live_latch_surface_present()
        && run_m7_e5_live_commit_gate()
        && E5_LIVE_LATCH_RESIDUAL_NOTE.contains("UnsupportedOnFirmware")
        && E5_LIVE_LATCH_RESIDUAL_NOTE.contains("no guest UEFI VMLAUNCH")
        && E5_LIVE_LATCH_RESIDUAL_NOTE.contains("not a shipped OVMF.fd")
        && E5_LIVE_LATCH_RESIDUAL_NOTE.contains("VMLAUNCH insn not issued")
        && E5_LIVE_LATCH_RESIDUAL_NOTE.contains("live EPT is not written")
        && M7_E5_LIVE_LATCH_OK_MARKER == "RAYNU-V-M7-E5-LIVE-LATCH-OK";
    reset_guest_fw();
    ok
}

#[cfg(test)]
#[path = "m7_e5_live_latch_gate_test.rs"]
mod m7_e5_live_latch_gate_test;
