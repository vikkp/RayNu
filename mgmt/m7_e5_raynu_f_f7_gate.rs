...
pub const E5_RAYNU_F_F7_RESIDUAL_NOTE: &str =
    "residual: nested fe4785a reboot-to-disk (DISK-BOOT-OK + second Linux root=UUID=) is proven on raynuvsrv1; iron 59ac070 installed to vda and printed the install marker but F7 relaunch failed VMCLEAR/VMPTRLD (host-stack overflow into the VMCS; fixed by RayNu-F F7 template reset + guest-UEFI host stack guard); iron 975f8fc relaunched and reached the installed GRUB menu, then the 1 M exit-cap fired inside GRUB's 2 s menu timeout (fixed by the RayNu-F wall cap: time, not exits, bounds the loader phase); iron 56a3ffd (run 34480107961) closed reboot-to-disk on the real R640: countdown 2s/1s/0s, DISK-BOOT-OK, EBS-OK, second Linux root=UUID= from vda2, login; E5 Phase A is closed on iron; Phase B host wire (RAYNU-V-M7-PHASE-B-SPA-WIRE-OK) queues RayNu-F from SPA/REST product-ISO start — iron Phase B is not claimed (no flash; iron SPA is still the SHELL stub until COM2 shows SPA start launching RayNu-F) and the ISO-INSTALL-OK marker is never printed from host/CI";
