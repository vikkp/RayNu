//! E5 Stage 19 — guest-UEFI VMLAUNCH insn-path arm (outside Proven Core).
//!
//! Pillar: [Z] [A]
//! Proven Core: **outside** mgmt bookkeeping; launch.rs entry is L0 (ADR-014)
//! VERIFICATION: N/A
//!
//! Host/CI: `arm_ovmf_real_launch` records that the VMLAUNCH instruction
//! is selected for real ESP `\\EFI\\RayNu\\OVMF.fd` only.
//! `try_vmlaunch_ovmf_firmware` then calls `try_vmlaunch_guest_uefi_ovmf`,
//! which refuses to issue VMLAUNCH (`RealLaunchNotIssued`). A 2 MiB
...
use super::guest_fw::{
    arm_ovmf_firmware_slot, arm_ovmf_real_launch, bind_ovmf_firmware_guest, box_guest_firmware,
    dispatch_guest_fw_rest, guest_fw_bytes, load_guest_firmware, load_ovmf_from_esp,
    ovmf_real_launch_is_armed, prepare_ovmf_firmware_launch, probe_ovmf_firmware, reset_guest_fw,
...
use super::m7_e5_real_esp_gate::run_m7_e5_real_esp_gate;
use super::VmTable;

#[cfg(test)]
use super::guest_fw::{
    arm_ovmf_esp_launch, arm_ovmf_firmware_alias, arm_ovmf_reset_vector, install_ovmf_alias_ept,
    map_live_esp_ovmf, program_ovmf_alias_ept, qualify_real_esp_ovmf, stage_edk2_ovmf_firmware,
...
/// Host / CI marker when the E5 Stage 19 real-launch arm gate passes.
pub const M7_E5_REAL_LAUNCH_OK_MARKER: &str = "RAYNU-V-M7-E5-REAL-LAUNCH-OK";

/// Honest residual: insn path armed; VMLAUNCH insn not issued.
pub const E5_REAL_LAUNCH_RESIDUAL_NOTE: &str =
    "residual: guest-UEFI VMLAUNCH insn path is armed; 4 MiB fixture is not a shipped OVMF.fd; live EPT is not written; VMLAUNCH insn not issued; attach_cdrom_uefi stays UnsupportedOnFirmware; no guest UEFI VMLAUNCH";

fn stage_through_real_esp() -> bool {
    reset_guest_fw();
...
/// Real-ESP qualify then insn-path arm. VMLAUNCH stays RealLaunchNotIssued.
pub fn prop_real_launch_after_qualify() -> bool {
    #[cfg(not(test))]
    {
        return false;
    }
    #[cfg(test)]
    {
        if !stage_through_real_esp() {
            return false;
        }
        if try_vmlaunch_ovmf_firmware() != Err(GuestFwError::RealEspNotLaunched) {
            return false;
        }
        let mut live = vec![0u8; MIN_LIVE_ESP_OVMF_BYTES];
        if write_live_esp_ovmf_fv(&mut live).is_err() || write_reset_vector_stub(&mut live).is_err()
        ...
        if arm_ovmf_real_launch(&live) != Err(GuestFwError::TooSmall) {
            return false;
        }
        let mut alias = vec![0u8; MIN_FIRMWARE_ALIAS_BYTES];
        if write_firmware_alias_fv(&mut alias).is_err() {
        ...
        match arm_ovmf_real_launch(&alias) {
            Ok(a) => {
                a.bytes_len == MIN_FIRMWARE_ALIAS_BYTES as u64
                    && a.gpa == 0xFFC0_0000
                    && ovmf_real_launch_is_armed()
                    && try_vmlaunch_ovmf_firmware() == Err(GuestFwError::RealLaunchNotIssued)
            }
            Err(_) => false,
        }
    }
...
/// Insn-path arm without qualify, reject. attach_cdrom_uefi stays closed.
pub fn prop_real_launch_rejects() -> bool {
    reset_guest_fw();
    if arm_ovmf_real_launch(&[]) != Err(GuestFwError::LaunchNotWired) {
        return false;
    }
    attach_cdrom_uefi(1) == Err(IsoError::UnsupportedOnFirmware)
}

/// Bearer REST: qualify then arm. Without qualify → 409.
/// `POST /fw/vmlaunch` → 409 (RealLaunchNotIssued). `iso=0` SHELL stays 201.
pub fn prop_rest_real_launch() -> bool {
    reset_guest_fw();
    ...
    if armed.status != 201 || !ovmf_real_launch_is_armed() {
        return false;
    }
...
/// SPA + ADR-014 Stage 19 phrases. Insn path armed; VMLAUNCH insn not issued.
pub fn real_launch_surface_present() -> bool {
    ...
    spa.contains("Arm real launch")
        ...
        && adr.contains("Stage 19")
        && adr.contains("arm_ovmf_real_launch")
        && adr.contains("RealLaunchNotIssued")
        && src.contains("fn arm_ovmf_real_launch")
        && src.contains("ovmf_real_launch_is_armed")
        && launch.contains("fn try_vmlaunch_guest_uefi_ovmf")
        && launch.contains("fn arm_guest_uefi_real_launch")
        && launch.contains("GUEST_UEFI_VMLAUNCH_OPCODE")
...
/// Full E5 Stage 19 package. Host gate only — not iron, not Everest E5.
pub fn run_m7_e5_real_launch_gate() -> bool {
    let _ = (M7_E5_REAL_LAUNCH_OK_MARKER, E5_REAL_LAUNCH_RESIDUAL_NOTE);
    reset_guest_fw();
    let ok = prop_real_launch_after_qualify()
        && prop_real_launch_rejects()
        && prop_rest_real_launch()
        && real_launch_surface_present()
        && run_m7_e5_real_esp_gate()
        && E5_REAL_LAUNCH_RESIDUAL_NOTE.contains("UnsupportedOnFirmware")
        && E5_REAL_LAUNCH_RESIDUAL_NOTE.contains("no guest UEFI VMLAUNCH")
        && E5_REAL_LAUNCH_RESIDUAL_NOTE.contains("not a shipped OVMF.fd")
        && E5_REAL_LAUNCH_RESIDUAL_NOTE.contains("VMLAUNCH insn not issued")
        && E5_REAL_LAUNCH_RESIDUAL_NOTE.contains("live EPT is not written")
        && M7_E5_REAL_LAUNCH_OK_MARKER == "RAYNU-V-M7-E5-REAL-LAUNCH-OK";
    reset_guest_fw();
    ok
}

#[cfg(test)]
#[path = "m7_e5_real_launch_gate_test.rs"]
mod m7_e5_real_launch_gate_test;
