//! E5 Stage 29 — live-ESP place-attempt (outside Proven Core).
//!
//! Pillar: [Z] [A]
//! Proven Core: **outside** mgmt bookkeeping; launch.rs entry is L0 (ADR-014)
//! VERIFICATION: N/A
//!
//! Host/CI: `place_ovmf_live_esp` records that real ESP
//! `\\EFI\\RayNu\\OVMF.fd` bytes were place-attempted for a private guest-UEFI
...
//! are still absent (`LiveEspPlaceAbsent`). A 2 MiB fixture is
...
use super::guest_fw::{
    ...
    guest_fw_bytes, load_guest_firmware, load_ovmf_from_esp, ovmf_live_esp_is_placed,
    place_ovmf_live_esp, prepare_ovmf_firmware_launch, probe_ovmf_firmware, reset_guest_fw,
...
use super::m7_e5_live_copy_gate::run_m7_e5_live_copy_gate;
use super::VmTable;

#[cfg(test)]
use super::guest_fw::{
    admit_ovmf_live_esp, arm_ovmf_esp_launch, arm_ovmf_firmware_alias, arm_ovmf_live_issue,
    arm_ovmf_private_vmcs, arm_ovmf_real_launch, arm_ovmf_reset_vector, copy_ovmf_live_esp,
    install_ovmf_alias_ept, map_live_esp_ovmf, present_ovmf_live_esp, probe_ovmf_live_bytes,
...
/// Host / CI marker when the E5 Stage 29 live-place gate passes.
pub const M7_E5_LIVE_PLACE_OK_MARKER: &str = "RAYNU-V-M7-E5-LIVE-PLACE-OK";

/// Honest residual: live ESP place-attempted; still absent; VMLAUNCH insn not issued.
pub const E5_LIVE_PLACE_RESIDUAL_NOTE: &str =
    "residual: real ESP OVMF.fd bytes were place-attempted; live ESP bytes are not present; 4 MiB fixture is not a shipped OVMF.fd; live EPT is not written; VMLAUNCH insn not issued; attach_cdrom_uefi stays UnsupportedOnFirmware; no guest UEFI VMLAUNCH";

fn stage_through_live_copy() -> bool {
    reset_guest_fw();
...
/// Live-copy then live-ESP place. VMLAUNCH stays LiveEspPlaceAbsent.
pub fn prop_live_place_after_live_copy() -> bool {
    #[cfg(not(test))]
    {
        return false;
    }
    #[cfg(test)]
    {
        if !stage_through_live_copy() {
            return false;
        }
        if try_vmlaunch_ovmf_firmware() != Err(GuestFwError::LiveEspCopyAbsent) {
            return false;
        }
        let mut live = vec![0u8; MIN_LIVE_ESP_OVMF_BYTES];
        if write_live_esp_ovmf_fv(&mut live).is_err() || write_reset_vector_stub(&mut live).is_err()
        ...
        if place_ovmf_live_esp(&live) != Err(GuestFwError::TooSmall) {
            return false;
        }
        let mut alias = vec![0u8; MIN_FIRMWARE_ALIAS_BYTES];
        if write_firmware_alias_fv(&mut alias).is_err() {
        ...
        match place_ovmf_live_esp(&alias) {
            Ok(a) => {
                a.bytes_len == MIN_FIRMWARE_ALIAS_BYTES as u64
                    && a.gpa == 0xFFC0_0000
                    && ovmf_live_esp_is_placed()
                    && try_vmlaunch_ovmf_firmware() == Err(GuestFwError::LiveEspPlaceAbsent)
            }
            Err(_) => false,
        }
    }
...
/// Live-ESP place without copy-attempt, reject. attach_cdrom_uefi stays closed.
pub fn prop_live_place_rejects() -> bool {
    reset_guest_fw();
    if place_ovmf_live_esp(&[]) != Err(GuestFwError::LaunchNotWired) {
        return false;
    }
    attach_cdrom_uefi(1) == Err(IsoError::UnsupportedOnFirmware)
}

/// Bearer REST: live-copy then live-place. Without copy → 409.
/// `POST /fw/vmlaunch` → 409 (LiveEspPlaceAbsent). `iso=0` SHELL stays 201.
pub fn prop_rest_live_place() -> bool {
    reset_guest_fw();
    ...
    let place = dispatch_guest_fw_rest(RestRequest {
        method: RestMethod::Post,
        path: "/fw/live-place",
        auth_token: tok,
    });
    if place.status != 201 || !ovmf_live_esp_is_placed() {
        return false;
    }
...
/// SPA + ADR-014 Stage 29 phrases. Live ESP place-attempted; VMLAUNCH insn not issued.
pub fn live_place_surface_present() -> bool {
    ...
    spa.contains("Place live ESP")
        ...
        && adr.contains("Stage 29")
        && adr.contains("place_ovmf_live_esp")
        && adr.contains("LiveEspPlaceAbsent")
        && src.contains("fn place_ovmf_live_esp")
        && src.contains("ovmf_live_esp_is_placed")
        && launch.contains("fn try_vmlaunch_guest_uefi_ovmf")
        && launch.contains("fn place_guest_uefi_live_esp")
        && launch.contains("GUEST_UEFI_PRIVATE_VMCS_ID")
        && launch.contains("LiveEspPlaceAbsent")
        && launch.contains("LiveEspCopyAbsent")
...
/// Full E5 Stage 29 package. Host gate only — not iron, not Everest E5.
pub fn run_m7_e5_live_place_gate() -> bool {
    let _ = (M7_E5_LIVE_PLACE_OK_MARKER, E5_LIVE_PLACE_RESIDUAL_NOTE);
    reset_guest_fw();
    let ok = prop_live_place_after_live_copy()
        && prop_live_place_rejects()
        && prop_rest_live_place()
        && live_place_surface_present()
        && run_m7_e5_live_copy_gate()
        && E5_LIVE_PLACE_RESIDUAL_NOTE.contains("UnsupportedOnFirmware")
        && E5_LIVE_PLACE_RESIDUAL_NOTE.contains("no guest UEFI VMLAUNCH")
        && E5_LIVE_PLACE_RESIDUAL_NOTE.contains("not a shipped OVMF.fd")
        && E5_LIVE_PLACE_RESIDUAL_NOTE.contains("VMLAUNCH insn not issued")
        && E5_LIVE_PLACE_RESIDUAL_NOTE.contains("live EPT is not written")
        && M7_E5_LIVE_PLACE_OK_MARKER == "RAYNU-V-M7-E5-LIVE-PLACE-OK";
    reset_guest_fw();
    ok
}

#[cfg(test)]
#[path = "m7_e5_live_place_gate_test.rs"]
mod m7_e5_live_place_gate_test;
