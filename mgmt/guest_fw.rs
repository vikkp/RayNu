...
pub enum GuestFwError {
    ...
    LiveEspLockAbsent,
    /// Real ESP `\EFI\RayNu\OVMF.fd` bytes were hold-attempted; they are still absent.
    LiveEspHoldAbsent,
}
...
/// Live-ESP seal bookkeeping. Not a shipped `OVMF.fd` and not VMLAUNCH.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct OvmfLiveSeal {
    pub bytes_len: u64,
    pub gpa: u64,
}
...
/// Live-ESP hold bookkeeping. Not a shipped `OVMF.fd` and not VMLAUNCH.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct OvmfLiveHold {
    pub bytes_len: u64,
    pub gpa: u64,
}
...
static GUEST_FW_OVMF_LIVE_ESP_HELD: AtomicBool = AtomicBool::new(false);

/// Reset the process-local boxed / loaded / probed / ESP / slot / bind / prep / floor / EDK2 / ESP-launch / live-map / reset-vector / alias / alias-EPT / EPT-install / real-ESP / live-exec / private-VMCS / live-issue / live-bytes / live-fd / live-present / live-admit / live-read / live-copy / live-place / live-apply / live-commit / live-latch / live-seal / live-lock / live-hold flags.
...
/// True after a successful [`hold_ovmf_live_esp`].
pub fn ovmf_live_esp_is_held() -> bool {
    GUEST_FW_OVMF_LIVE_ESP_HELD.load(Ordering::Acquire)
}
...
///   [`GuestFwError::LiveEspLockAbsent`]; live ESP hold-attempted →
///   [`GuestFwError::LiveEspHoldAbsent`])
...
///
/// INVARIANTS:
/// - Requires a prior successful [`latch_ovmf_live_esp`]
/// - `bytes.len()` and `_FVH` `FvLength` must be `>= MIN_FIRMWARE_ALIAS_BYTES`
/// - Reset vector GPA must sit inside the alias window
/// - Does not flip [`crate::vmx::launch::guest_uefi_live_esp_bytes_present`]
/// - Does not write the E4 SHELL EPT and does not issue VMLAUNCH
/// - Does not flip [`crate::mgmt::iso::attach_cdrom_uefi`]
/// - A heap fixture is not a shipped EDK2 `OVMF.fd`
pub fn seal_ovmf_live_esp(bytes: &[u8]) -> Result<OvmfLiveSeal, GuestFwError> {
    if !ovmf_live_esp_is_latched() {
        return Err(GuestFwError::LaunchNotWired);
    }
    if bytes.len() < MIN_FIRMWARE_ALIAS_BYTES {
        return Err(GuestFwError::TooSmall);
    ...
    crate::vmx::launch::seal_guest_uefi_live_esp(bytes.len() as u64)
        .map_err(|_| GuestFwError::TooSmall)?;
    let gpa = crate::vmx::launch::firmware_alias_gpa(bytes.len() as u64).unwrap_or(0);
    audit_log!(AuditEvent::OvmfLiveEspSealed {
        bytes_len: bytes.len() as u64,
        gpa,
    });
    GUEST_FW_OVMF_LIVE_ESP_SEALED.store(true, Ordering::Release);
    Ok(OvmfLiveSeal {
        bytes_len: bytes.len() as u64,
        gpa,
    })
...
///
/// INVARIANTS:
/// - Requires a prior successful [`lock_ovmf_live_esp`]
/// - `bytes.len()` and `_FVH` `FvLength` must be `>= MIN_FIRMWARE_ALIAS_BYTES`
/// - Reset vector GPA must sit inside the alias window
/// - Does not flip [`crate::vmx::launch::guest_uefi_live_esp_bytes_present`]
/// - Does not write the E4 SHELL EPT and does not issue VMLAUNCH
/// - Does not flip [`crate::mgmt::iso::attach_cdrom_uefi`]
/// - A heap fixture is not a shipped EDK2 `OVMF.fd`
pub fn hold_ovmf_live_esp(bytes: &[u8]) -> Result<OvmfLiveHold, GuestFwError> {
    if !ovmf_live_esp_is_locked() {
        return Err(GuestFwError::LaunchNotWired);
    }
    if bytes.len() < MIN_FIRMWARE_ALIAS_BYTES {
        return Err(GuestFwError::TooSmall);
    ...
    crate::vmx::launch::hold_guest_uefi_live_esp(bytes.len() as u64)
        .map_err(|_| GuestFwError::TooSmall)?;
    let gpa = crate::vmx::launch::firmware_alias_gpa(bytes.len() as u64).unwrap_or(0);
    audit_log!(AuditEvent::OvmfLiveEspHeld {
        bytes_len: bytes.len() as u64,
        gpa,
    });
    GUEST_FW_OVMF_LIVE_ESP_HELD.store(true, Ordering::Release);
    Ok(OvmfLiveHold {
        bytes_len: bytes.len() as u64,
        gpa,
    })
...
fn route_guest_fw(method: RestMethod, path: &str) -> Result<GuestFwOp, ()> {
    ...
    match (method, path) {
        ...
        (RestMethod::Get, "/fw/live-hold") => Ok(GuestFwOp::OvmfLiveHoldStatus),
        (RestMethod::Post, "/fw/live-hold") => Ok(GuestFwOp::OvmfLiveHold),
...
fn guest_fw_err_status(e: GuestFwError) -> u16 {
    match e {
        ...
        | GuestFwError::LiveEspHoldAbsent => 409,
    }
}
...
/// `POST /fw/live-hold` records a real-ESP hold-attempt after
/// live-lock (host test heap fixture only).
...
/// 409 `LiveEspLockAbsent`; live ESP hold-attempted →
/// 409 `LiveEspHoldAbsent`).
...
pub fn dispatch_guest_fw_rest(req: RestRequest<'_>) -> RestResponse {
    ...
    match route_guest_fw(req.method, req.path) {
        ...
        Ok(GuestFwOp::OvmfLiveSealStatus) => RestResponse {
            status: 200,
            reply: Some(ApiReply::Listed {
                count: if ovmf_live_esp_is_sealed() { 1 } else { 0 },
            }),
        },
        ...
        Ok(GuestFwOp::OvmfLiveHoldStatus) => RestResponse {
            status: 200,
            reply: Some(ApiReply::Listed {
                count: if ovmf_live_esp_is_held() { 1 } else { 0 },
            }),
        },
        Ok(GuestFwOp::OvmfLiveHold) => live_hold_rest(),
...
/// Production UEFI: 409 `MissingEsp` — no embedded 4 MiB (ADR-003).
#[cfg(test)]
fn live_hold_rest() -> RestResponse {
    let mut fv = vec![0u8; MIN_FIRMWARE_ALIAS_BYTES];
    if write_firmware_alias_fv(&mut fv).is_err() {
        return RestResponse {
            status: 500,
...
/// Production: no embedded 4 MiB live-hold image (ADR-003 split-mode / ESP only).
#[cfg(not(test))]
fn live_hold_rest() -> RestResponse {
    RestResponse {
        status: guest_fw_err_status(GuestFwError::MissingEsp),
