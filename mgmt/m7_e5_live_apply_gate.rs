//! E5 Stage 30 — live-ESP apply-attempt (outside Proven Core).
//!
//! Pillar: [Z] [A]
//! Proven Core: **outside** mgmt bookkeeping; launch.rs entry is L0 (ADR-014)
//! VERIFICATION: N/A
//!
//! Host/CI: `apply_ovmf_live_esp` records that real ESP
//! `\\EFI\\RayNu\\OVMF.fd` bytes were apply-attempted for a private guest-UEFI
...
//! are still absent (`LiveEspApplyAbsent`). A 2 MiB fixture is
...
use super::guest_fw::{
    apply_ovmf_live_esp, arm_ovmf_firmware_slot, bind_ovmf_firmware_guest, box_guest_firmware,
    dispatch_guest_fw_rest, guest_fw_bytes, load_guest_firmware, load_ovmf_from_esp,
    ovmf_live_esp_is_applied, prepare_ovmf_firmware_launch, probe_ovmf_firmware, reset_guest_fw,
...
use super::m7_e5_live_place_gate::run_m7_e5_live_place_gate;
use super::VmTable;
...
use super::guest_fw::{
    ...
    install_ovmf_alias_ept, map_live_esp_ovmf, place_ovmf_live_esp, present_ovmf_live_esp,
    probe_ovmf_live_bytes, program_ovmf_alias_ept, qualify_real_esp_ovmf, read_ovmf_live_esp,
    require_ovmf_live_esp, require_ovmf_live_fd, stage_edk2_ovmf_firmware,
...
/// Host / CI marker when the E5 Stage 30 live-apply gate passes.
pub const M7_E5_LIVE_APPLY_OK_MARKER: &str = "RAYNU-V-M7-E5-LIVE-APPLY-OK";

/// Honest residual: live ESP apply-attempted; still absent; VMLAUNCH insn not issued.
pub const E5_LIVE_APPLY_RESIDUAL_NOTE: &str =
    "residual: real ESP OVMF.fd bytes were apply-attempted; live ESP bytes are not present; 4 MiB fixture is not a shipped OVMF.fd; live EPT is not written; VMLAUNCH insn not issued; attach_cdrom_uefi stays UnsupportedOnFirmware; no guest UEFI VMLAUNCH";

fn stage_through_live_place() -> bool {
    reset_guest_fw();
...
/// Live-place then live-ESP apply. VMLAUNCH stays LiveEspApplyAbsent.
pub fn prop_live_apply_after_live_place() -> bool {
    #[cfg(not(test))]
    {
        return false;
    }
    #[cfg(test)]
    {
        if !stage_through_live_place() {
            return false;
        }
        if try_vmlaunch_ovmf_firmware() != Err(GuestFwError::LiveEspPlaceAbsent) {
            return false;
        }
        let mut live = vec![0u8; MIN_LIVE_ESP_OVMF_BYTES];
        if write_live_esp_ovmf_fv(&mut live).is_err() || write_reset_vector_stub(&mut live).is_err()
        ...
        if apply_ovmf_live_esp(&live) != Err(GuestFwError::TooSmall) {
            return false;
        }
        let mut alias = vec![0u8; MIN_FIRMWARE_ALIAS_BYTES];
        if write_firmware_alias_fv(&mut alias).is_err() {
        ...
        match apply_ovmf_live_esp(&alias) {
            Ok(a) => {
                a.bytes_len == MIN_FIRMWARE_ALIAS_BYTES as u64
                    && a.gpa == 0xFFC0_0000
                    && ovmf_live_esp_is_applied()
                    && try_vmlaunch_ovmf_firmware() == Err(GuestFwError::LiveEspApplyAbsent)
            }
            Err(_) => false,
        }
    }
...
/// Live-ESP apply without place-attempt, reject. attach_cdrom_uefi stays closed.
pub fn prop_live_apply_rejects() -> bool {
    reset_guest_fw();
    if apply_ovmf_live_esp(&[]) != Err(GuestFwError::LaunchNotWired) {
        return false;
    }
    attach_cdrom_uefi(1) == Err(IsoError::UnsupportedOnFirmware)
}

/// Bearer REST: live-place then live-apply. Without place → 409.
/// `POST /fw/vmlaunch` → 409 (LiveEspApplyAbsent). `iso=0` SHELL stays 201.
pub fn prop_rest_live_apply() -> bool {
    reset_guest_fw();
    ...
    let apply = dispatch_guest_fw_rest(RestRequest {
        method: RestMethod::Post,
        path: "/fw/live-apply",
        auth_token: tok,
    });
    if apply.status != 201 || !ovmf_live_esp_is_applied() {
        return false;
    }
...
pub fn live_apply_surface_present() -> bool {
    ...
    spa.contains("Apply live ESP")
        ...
        && src.contains("fn apply_ovmf_live_esp")
        && src.contains("ovmf_live_esp_is_applied")
        && launch.contains("fn try_vmlaunch_guest_uefi_ovmf")
        && launch.contains("fn apply_guest_uefi_live_esp")
...
/// Full E5 Stage 30 package. Host gate only — not iron, not Everest E5.
pub fn run_m7_e5_live_apply_gate() -> bool {
    let _ = (M7_E5_LIVE_APPLY_OK_MARKER, E5_LIVE_APPLY_RESIDUAL_NOTE);
    reset_guest_fw();
    let ok = prop_live_apply_after_live_place()
        && prop_live_apply_rejects()
        && prop_rest_live_apply()
        && live_apply_surface_present()
        && run_m7_e5_live_place_gate()
        && E5_LIVE_APPLY_RESIDUAL_NOTE.contains("UnsupportedOnFirmware")
        && E5_LIVE_APPLY_RESIDUAL_NOTE.contains("no guest UEFI VMLAUNCH")
        && E5_LIVE_APPLY_RESIDUAL_NOTE.contains("not a shipped OVMF.fd")
        && E5_LIVE_APPLY_RESIDUAL_NOTE.contains("VMLAUNCH insn not issued")
        && E5_LIVE_APPLY_RESIDUAL_NOTE.contains("live EPT is not written")
        && M7_E5_LIVE_APPLY_OK_MARKER == "RAYNU-V-M7-E5-LIVE-APPLY-OK";
    reset_guest_fw();
    ok
}

#[cfg(test)]
#[path = "m7_e5_live_apply_gate_test.rs"]
mod m7_e5_live_apply_gate_test;
