//! E5 Stage 24 — live-ESP FD require (outside Proven Core).
//!
//! Pillar: [Z] [A]
//! Proven Core: **outside** mgmt bookkeeping; launch.rs entry is L0 (ADR-014)
//! VERIFICATION: N/A
//!
//! Host/CI: `require_ovmf_live_fd` records that a real ESP
//! `\\EFI\\RayNu\\OVMF.fd` is required for a private guest-UEFI
//! VMCS (not the E4 SHELL). `try_vmlaunch_ovmf_firmware` then calls
//! `try_vmlaunch_guest_uefi_ovmf`, which refuses because that file
//! is still absent (`LiveEspFdAbsent`). A 2 MiB fixture is
...
use super::guest_fw::{
    ...
    guest_fw_bytes, load_guest_firmware, load_ovmf_from_esp, ovmf_live_fd_is_required,
    prepare_ovmf_firmware_launch, probe_ovmf_firmware, require_ovmf_live_fd, reset_guest_fw,
...
use super::m7_e5_live_bytes_gate::run_m7_e5_live_bytes_gate;
use super::VmTable;
...
/// Host / CI marker when the E5 Stage 24 live-FD gate passes.
pub const M7_E5_LIVE_FD_OK_MARKER: &str = "RAYNU-V-M7-E5-LIVE-FD-OK";

/// Honest residual: live FD required; still absent; VMLAUNCH insn not issued.
pub const E5_LIVE_FD_RESIDUAL_NOTE: &str =
    "residual: a real ESP OVMF.fd is required; live ESP bytes are not present; 4 MiB fixture is not a shipped OVMF.fd; live EPT is not written; VMLAUNCH insn not issued; attach_cdrom_uefi stays UnsupportedOnFirmware; no guest UEFI VMLAUNCH";

fn stage_through_live_bytes() -> bool {
    reset_guest_fw();
...
/// Live-bytes probe then live-FD require. VMLAUNCH stays LiveEspFdAbsent.
pub fn prop_live_fd_after_live_bytes() -> bool {
    #[cfg(not(test))]
    {
        return false;
    }
    #[cfg(test)]
    {
        if !stage_through_live_bytes() {
            return false;
        }
        if try_vmlaunch_ovmf_firmware() != Err(GuestFwError::LiveEspBytesAbsent) {
            return false;
        }
        let mut live = vec![0u8; MIN_LIVE_ESP_OVMF_BYTES];
        if write_live_esp_ovmf_fv(&mut live).is_err() || write_reset_vector_stub(&mut live).is_err()
        ...
        if require_ovmf_live_fd(&live) != Err(GuestFwError::TooSmall) {
            return false;
        }
        let mut alias = vec![0u8; MIN_FIRMWARE_ALIAS_BYTES];
        if write_firmware_alias_fv(&mut alias).is_err() {
        ...
        match require_ovmf_live_fd(&alias) {
            Ok(a) => {
                a.bytes_len == MIN_FIRMWARE_ALIAS_BYTES as u64
                    && a.gpa == 0xFFC0_0000
                    && ovmf_live_fd_is_required()
                    && try_vmlaunch_ovmf_firmware() == Err(GuestFwError::LiveEspFdAbsent)
            }
            Err(_) => false,
        }
    }
...
/// Live-FD require without live-bytes probe, reject. attach_cdrom_uefi stays closed.
pub fn prop_live_fd_rejects() -> bool {
    reset_guest_fw();
    if require_ovmf_live_fd(&[]) != Err(GuestFwError::LaunchNotWired) {
        return false;
    }
    attach_cdrom_uefi(1) == Err(IsoError::UnsupportedOnFirmware)
}

/// Bearer REST: live-bytes then live-FD. Without live-bytes → 409.
/// `POST /fw/vmlaunch` → 409 (LiveEspFdAbsent). `iso=0` SHELL stays 201.
pub fn prop_rest_live_fd() -> bool {
    reset_guest_fw();
    ...
    if required.status != 201 || !ovmf_live_fd_is_required() {
        return false;
    }
...
/// SPA + ADR-014 Stage 24 phrases. Live FD required; VMLAUNCH insn not issued.
pub fn live_fd_surface_present() -> bool {
    ...
    spa.contains("Require live FD")
        ...
        && adr.contains("Stage 24")
        && adr.contains("require_ovmf_live_fd")
        && adr.contains("LiveEspFdAbsent")
        && src.contains("fn require_ovmf_live_fd")
        && src.contains("ovmf_live_fd_is_required")
        && launch.contains("fn try_vmlaunch_guest_uefi_ovmf")
        && launch.contains("fn require_guest_uefi_live_fd")
...
/// Full E5 Stage 24 package. Host gate only — not iron, not Everest E5.
pub fn run_m7_e5_live_fd_gate() -> bool {
    let _ = (M7_E5_LIVE_FD_OK_MARKER, E5_LIVE_FD_RESIDUAL_NOTE);
    reset_guest_fw();
    let ok = prop_live_fd_after_live_bytes()
        && prop_live_fd_rejects()
        && prop_rest_live_fd()
        && live_fd_surface_present()
        && run_m7_e5_live_bytes_gate()
        && E5_LIVE_FD_RESIDUAL_NOTE.contains("UnsupportedOnFirmware")
        && E5_LIVE_FD_RESIDUAL_NOTE.contains("no guest UEFI VMLAUNCH")
        && E5_LIVE_FD_RESIDUAL_NOTE.contains("not a shipped OVMF.fd")
        && E5_LIVE_FD_RESIDUAL_NOTE.contains("VMLAUNCH insn not issued")
        && E5_LIVE_FD_RESIDUAL_NOTE.contains("live EPT is not written")
        && M7_E5_LIVE_FD_OK_MARKER == "RAYNU-V-M7-E5-LIVE-FD-OK";
    reset_guest_fw();
    ok
}

#[cfg(test)]
#[path = "m7_e5_live_fd_gate_test.rs"]
mod m7_e5_live_fd_gate_test;
