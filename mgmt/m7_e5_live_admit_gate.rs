//! E5 Stage 26 — live-ESP admit-attempt (outside Proven Core).
//!
//! Pillar: [Z] [A]
//! Proven Core: **outside** mgmt bookkeeping; launch.rs entry is L0 (ADR-014)
//! VERIFICATION: N/A
//!
//! Host/CI: `admit_ovmf_live_esp` records that real ESP
//! `\\EFI\\RayNu\\OVMF.fd` bytes were admitted for a private guest-UEFI
...
//! are still absent (`LiveEspAdmitAbsent`). A 2 MiB fixture is
...
use super::guest_fw::{
    admit_ovmf_live_esp, arm_ovmf_firmware_slot, bind_ovmf_firmware_guest, box_guest_firmware,
    dispatch_guest_fw_rest, guest_fw_bytes, load_guest_firmware, load_ovmf_from_esp,
    ovmf_live_esp_is_admitted, prepare_ovmf_firmware_launch, probe_ovmf_firmware, reset_guest_fw,
...
use super::m7_e5_live_present_gate::run_m7_e5_live_present_gate;
use super::VmTable;
...
/// Host / CI marker when the E5 Stage 26 live-admit gate passes.
pub const M7_E5_LIVE_ADMIT_OK_MARKER: &str = "RAYNU-V-M7-E5-LIVE-ADMIT-OK";

/// Honest residual: live ESP admitted; still absent; VMLAUNCH insn not issued.
pub const E5_LIVE_ADMIT_RESIDUAL_NOTE: &str =
    "residual: real ESP OVMF.fd bytes were admitted; live ESP bytes are not present; 4 MiB fixture is not a shipped OVMF.fd; live EPT is not written; VMLAUNCH insn not issued; attach_cdrom_uefi stays UnsupportedOnFirmware; no guest UEFI VMLAUNCH";

fn stage_through_live_present() -> bool {
    reset_guest_fw();
...
/// Live-present then live-ESP admit. VMLAUNCH stays LiveEspAdmitAbsent.
pub fn prop_live_admit_after_live_present() -> bool {
    #[cfg(not(test))]
    {
        return false;
    }
    #[cfg(test)]
    {
        if !stage_through_live_present() {
            return false;
        }
        if try_vmlaunch_ovmf_firmware() != Err(GuestFwError::LiveEspPresentAbsent) {
            return false;
        }
        let mut live = vec![0u8; MIN_LIVE_ESP_OVMF_BYTES];
        if write_live_esp_ovmf_fv(&mut live).is_err() || write_reset_vector_stub(&mut live).is_err()
        ...
        if admit_ovmf_live_esp(&live) != Err(GuestFwError::TooSmall) {
            return false;
        }
        let mut alias = vec![0u8; MIN_FIRMWARE_ALIAS_BYTES];
        if write_firmware_alias_fv(&mut alias).is_err() {
        ...
        match admit_ovmf_live_esp(&alias) {
            Ok(a) => {
                a.bytes_len == MIN_FIRMWARE_ALIAS_BYTES as u64
                    && a.gpa == 0xFFC0_0000
                    && ovmf_live_esp_is_admitted()
                    && try_vmlaunch_ovmf_firmware() == Err(GuestFwError::LiveEspAdmitAbsent)
            }
            Err(_) => false,
        }
    }
...
/// Live-ESP admit without present-attempt, reject. attach_cdrom_uefi stays closed.
pub fn prop_live_admit_rejects() -> bool {
    reset_guest_fw();
    if admit_ovmf_live_esp(&[]) != Err(GuestFwError::LaunchNotWired) {
        return false;
    }
    attach_cdrom_uefi(1) == Err(IsoError::UnsupportedOnFirmware)
}

/// Bearer REST: live-present then live-admit. Without present → 409.
/// `POST /fw/vmlaunch` → 409 (LiveEspAdmitAbsent). `iso=0` SHELL stays 201.
pub fn prop_rest_live_admit() -> bool {
    reset_guest_fw();
    ...
    let admitted = dispatch_guest_fw_rest(RestRequest {
        method: RestMethod::Post,
        path: "/fw/live-admit",
        auth_token: tok,
    });
    if admitted.status != 201 || !ovmf_live_esp_is_admitted() {
        return false;
    }
...
/// SPA + ADR-014 Stage 26 phrases. Live ESP admitted; VMLAUNCH insn not issued.
pub fn live_admit_surface_present() -> bool {
    ...
    spa.contains("Admit live ESP")
        ...
        && adr.contains("Stage 26")
        && adr.contains("admit_ovmf_live_esp")
        && adr.contains("LiveEspAdmitAbsent")
        && src.contains("fn admit_ovmf_live_esp")
        && src.contains("ovmf_live_esp_is_admitted")
        && launch.contains("fn try_vmlaunch_guest_uefi_ovmf")
        && launch.contains("fn admit_guest_uefi_live_esp")
...
/// Full E5 Stage 26 package. Host gate only — not iron, not Everest E5.
pub fn run_m7_e5_live_admit_gate() -> bool {
    let _ = (M7_E5_LIVE_ADMIT_OK_MARKER, E5_LIVE_ADMIT_RESIDUAL_NOTE);
    reset_guest_fw();
    let ok = prop_live_admit_after_live_present()
        && prop_live_admit_rejects()
        && prop_rest_live_admit()
        && live_admit_surface_present()
        && run_m7_e5_live_present_gate()
        && E5_LIVE_ADMIT_RESIDUAL_NOTE.contains("UnsupportedOnFirmware")
        && E5_LIVE_ADMIT_RESIDUAL_NOTE.contains("no guest UEFI VMLAUNCH")
        && E5_LIVE_ADMIT_RESIDUAL_NOTE.contains("not a shipped OVMF.fd")
        && E5_LIVE_ADMIT_RESIDUAL_NOTE.contains("VMLAUNCH insn not issued")
        && E5_LIVE_ADMIT_RESIDUAL_NOTE.contains("live EPT is not written")
        && M7_E5_LIVE_ADMIT_OK_MARKER == "RAYNU-V-M7-E5-LIVE-ADMIT-OK";
    reset_guest_fw();
    ok
}

#[cfg(test)]
#[path = "m7_e5_live_admit_gate_test.rs"]
mod m7_e5_live_admit_gate_test;
