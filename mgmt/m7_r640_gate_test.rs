use super::{
    prop_r640_scaffold_package, r640_honesty_holds, r640_scripts_present,
    run_m7_r640_scaffold_gate, ship_kit_names_r640_efi, M7_R640_OK_MARKER, M7_R640_SCAFFOLD_MARKER,
    R640_GAP_NOTE, R640_HOST_LIMIT_NOTE,
};
...
fn m7_5_r640_scaffold_passes() {
    ...
    assert!(
        ship_kit_names_r640_efi(),
        "ship kit must name r640-hypervisor.efi"
    );
    assert!(
        r640_scripts_present(),
        "runbook + closed evidence + smoke must be present"
    );
    assert!(
        r640_honesty_holds(),
        "GAP must be CLOSED M7.5; host limit note required"
    );
