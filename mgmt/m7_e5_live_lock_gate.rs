//! E5 Stage 34 — live-ESP lock-attempt (outside Proven Core).
//!
//! Pillar: [Z] [A]
//! Proven Core: **outside** mgmt bookkeeping; launch.rs entry is L0 (ADR-014)
//! VERIFICATION: N/A
//!
//! Host/CI: `lock_ovmf_live_esp` records that real ESP
//! `\\EFI\\RayNu\\OVMF.fd` bytes were lock-attempted for a private guest-UEFI
...
//! are still absent (`LiveEspLockAbsent`). A 2 MiB fixture is
...
use super::guest_fw::{
    ...
    guest_fw_bytes, load_guest_firmware, load_ovmf_from_esp, lock_ovmf_live_esp,
    ovmf_live_esp_is_locked, prepare_ovmf_firmware_launch, probe_ovmf_firmware, reset_guest_fw,
...
use super::m7_e5_live_seal_gate::run_m7_e5_live_seal_gate;
use super::VmTable;
...
use super::guest_fw::{
    ...
    map_live_esp_ovmf, place_ovmf_live_esp, present_ovmf_live_esp, probe_ovmf_live_bytes,
    program_ovmf_alias_ept, qualify_real_esp_ovmf, read_ovmf_live_esp, require_ovmf_live_esp,
    require_ovmf_live_fd, seal_ovmf_live_esp, stage_edk2_ovmf_firmware, try_vmlaunch_ovmf_firmware,
...
/// Host / CI marker when the E5 Stage 34 live-lock gate passes.
pub const M7_E5_LIVE_LOCK_OK_MARKER: &str = "RAYNU-V-M7-E5-LIVE-LOCK-OK";

/// Honest residual: live ESP lock-attempted; still absent; VMLAUNCH insn not issued.
pub const E5_LIVE_LOCK_RESIDUAL_NOTE: &str =
    "residual: real ESP OVMF.fd bytes were lock-attempted; live ESP bytes are not present; 4 MiB fixture is not a shipped OVMF.fd; live EPT is not written; VMLAUNCH insn not issued; attach_cdrom_uefi stays UnsupportedOnFirmware; no guest UEFI VMLAUNCH";

fn stage_through_live_seal() -> bool {
    reset_guest_fw();
...
/// Live-seal then live-ESP lock. VMLAUNCH stays LiveEspLockAbsent.
pub fn prop_live_lock_after_live_seal() -> bool {
    #[cfg(not(test))]
    {
        return false;
    }
    #[cfg(test)]
    {
        if !stage_through_live_seal() {
            return false;
        }
        if try_vmlaunch_ovmf_firmware() != Err(GuestFwError::LiveEspSealAbsent) {
            return false;
        }
        let mut live = vec![0u8; MIN_LIVE_ESP_OVMF_BYTES];
        if write_live_esp_ovmf_fv(&mut live).is_err() || write_reset_vector_stub(&mut live).is_err()
        ...
        if lock_ovmf_live_esp(&live) != Err(GuestFwError::TooSmall) {
            return false;
        }
        let mut alias = vec![0u8; MIN_FIRMWARE_ALIAS_BYTES];
        if write_firmware_alias_fv(&mut alias).is_err() {
        ...
        match lock_ovmf_live_esp(&alias) {
            Ok(a) => {
                a.bytes_len == MIN_FIRMWARE_ALIAS_BYTES as u64
                    && a.gpa == 0xFFC0_0000
                    && ovmf_live_esp_is_locked()
                    && try_vmlaunch_ovmf_firmware() == Err(GuestFwError::LiveEspLockAbsent)
            }
            Err(_) => false,
        }
    }
...
/// Live-ESP lock without seal-attempt, reject. attach_cdrom_uefi stays closed.
pub fn prop_live_lock_rejects() -> bool {
    reset_guest_fw();
    if lock_ovmf_live_esp(&[]) != Err(GuestFwError::LaunchNotWired) {
        return false;
    }
    attach_cdrom_uefi(1) == Err(IsoError::UnsupportedOnFirmware)
}

/// Bearer REST: live-seal then live-lock. Without seal → 409.
/// `POST /fw/vmlaunch` → 409 (LiveEspLockAbsent). `iso=0` SHELL stays 201.
pub fn prop_rest_live_lock() -> bool {
    reset_guest_fw();
    ...
    let latch = dispatch_guest_fw_rest(RestRequest {
        method: RestMethod::Post,
        path: "/fw/live-lock",
        auth_token: tok,
    });
    if latch.status != 201 || !ovmf_live_esp_is_locked() {
        return false;
    }
...
/// SPA + ADR-014 Stage 34 phrases. Live ESP lock-attempted; VMLAUNCH insn not issued.
pub fn live_lock_surface_present() -> bool {
    ...
    spa.contains("Lock live ESP")
        ...
        && adr.contains("Stage 34")
        && adr.contains("lock_ovmf_live_esp")
        && adr.contains("LiveEspLockAbsent")
        && src.contains("fn lock_ovmf_live_esp")
        && src.contains("ovmf_live_esp_is_locked")
        && launch.contains("fn try_vmlaunch_guest_uefi_ovmf")
        && launch.contains("fn lock_guest_uefi_live_esp")
        && launch.contains("GUEST_UEFI_PRIVATE_VMCS_ID")
        && launch.contains("LiveEspLockAbsent")
        && launch.contains("LiveEspSealAbsent")
...
/// Full E5 Stage 34 package. Host gate only — not iron, not Everest E5.
pub fn run_m7_e5_live_lock_gate() -> bool {
    let _ = (M7_E5_LIVE_LOCK_OK_MARKER, E5_LIVE_LOCK_RESIDUAL_NOTE);
    reset_guest_fw();
    let ok = prop_live_lock_after_live_seal()
        && prop_live_lock_rejects()
        && prop_rest_live_lock()
        && live_lock_surface_present()
        && run_m7_e5_live_seal_gate()
        && E5_LIVE_LOCK_RESIDUAL_NOTE.contains("UnsupportedOnFirmware")
        && E5_LIVE_LOCK_RESIDUAL_NOTE.contains("no guest UEFI VMLAUNCH")
        && E5_LIVE_LOCK_RESIDUAL_NOTE.contains("not a shipped OVMF.fd")
        && E5_LIVE_LOCK_RESIDUAL_NOTE.contains("VMLAUNCH insn not issued")
        && E5_LIVE_LOCK_RESIDUAL_NOTE.contains("live EPT is not written")
        && M7_E5_LIVE_LOCK_OK_MARKER == "RAYNU-V-M7-E5-LIVE-LOCK-OK";
    reset_guest_fw();
    ok
}

#[cfg(test)]
#[path = "m7_e5_live_lock_gate_test.rs"]
mod m7_e5_live_lock_gate_test;
