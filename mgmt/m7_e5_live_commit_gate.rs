//! E5 Stage 31 — live-ESP commit-attempt (outside Proven Core).
//!
//! Pillar: [Z] [A]
//! Proven Core: **outside** mgmt bookkeeping; launch.rs entry is L0 (ADR-014)
//! VERIFICATION: N/A
//!
//! Host/CI: `commit_ovmf_live_esp` records that real ESP
//! `\\EFI\\RayNu\\OVMF.fd` bytes were commit-attempted for a private guest-UEFI
...
//! are still absent (`LiveEspCommitAbsent`). A 2 MiB fixture is
...
use super::guest_fw::{
    arm_ovmf_firmware_slot, bind_ovmf_firmware_guest, box_guest_firmware, commit_ovmf_live_esp,
    dispatch_guest_fw_rest, guest_fw_bytes, load_guest_firmware, load_ovmf_from_esp,
    ovmf_live_esp_is_committed, prepare_ovmf_firmware_launch, probe_ovmf_firmware, reset_guest_fw,
...
use super::m7_e5_live_apply_gate::run_m7_e5_live_apply_gate;
use super::VmTable;
...
use super::guest_fw::{
    ...
    copy_ovmf_live_esp, install_ovmf_alias_ept, map_live_esp_ovmf, place_ovmf_live_esp,
    present_ovmf_live_esp, probe_ovmf_live_bytes, program_ovmf_alias_ept, qualify_real_esp_ovmf,
    read_ovmf_live_esp, require_ovmf_live_esp, require_ovmf_live_fd, stage_edk2_ovmf_firmware,
...
/// Host / CI marker when the E5 Stage 31 live-commit gate passes.
pub const M7_E5_LIVE_COMMIT_OK_MARKER: &str = "RAYNU-V-M7-E5-LIVE-COMMIT-OK";

/// Honest residual: live ESP commit-attempted; still absent; VMLAUNCH insn not issued.
pub const E5_LIVE_COMMIT_RESIDUAL_NOTE: &str =
    "residual: real ESP OVMF.fd bytes were commit-attempted; live ESP bytes are not present; 4 MiB fixture is not a shipped OVMF.fd; live EPT is not written; VMLAUNCH insn not issued; attach_cdrom_uefi stays UnsupportedOnFirmware; no guest UEFI VMLAUNCH";

fn stage_through_live_apply() -> bool {
    reset_guest_fw();
...
/// Live-apply then live-ESP commit. VMLAUNCH stays LiveEspCommitAbsent.
pub fn prop_live_commit_after_live_apply() -> bool {
    #[cfg(not(test))]
    {
        return false;
    }
    #[cfg(test)]
    {
        if !stage_through_live_apply() {
            return false;
        }
        if try_vmlaunch_ovmf_firmware() != Err(GuestFwError::LiveEspApplyAbsent) {
            return false;
        }
        let mut live = vec![0u8; MIN_LIVE_ESP_OVMF_BYTES];
        if write_live_esp_ovmf_fv(&mut live).is_err() || write_reset_vector_stub(&mut live).is_err()
        ...
        if commit_ovmf_live_esp(&live) != Err(GuestFwError::TooSmall) {
            return false;
        }
        let mut alias = vec![0u8; MIN_FIRMWARE_ALIAS_BYTES];
        if write_firmware_alias_fv(&mut alias).is_err() {
        ...
        match commit_ovmf_live_esp(&alias) {
            Ok(a) => {
                a.bytes_len == MIN_FIRMWARE_ALIAS_BYTES as u64
                    && a.gpa == 0xFFC0_0000
                    && ovmf_live_esp_is_committed()
                    && try_vmlaunch_ovmf_firmware() == Err(GuestFwError::LiveEspCommitAbsent)
            }
            Err(_) => false,
        }
    }
...
/// Live-ESP commit without apply-attempt, reject. attach_cdrom_uefi stays closed.
pub fn prop_live_commit_rejects() -> bool {
    reset_guest_fw();
    if commit_ovmf_live_esp(&[]) != Err(GuestFwError::LaunchNotWired) {
        return false;
    }
    attach_cdrom_uefi(1) == Err(IsoError::UnsupportedOnFirmware)
}

/// Bearer REST: live-apply then live-commit. Without apply → 409.
/// `POST /fw/vmlaunch` → 409 (LiveEspCommitAbsent). `iso=0` SHELL stays 201.
pub fn prop_rest_live_commit() -> bool {
    reset_guest_fw();
    ...
    let commit = dispatch_guest_fw_rest(RestRequest {
        method: RestMethod::Post,
        path: "/fw/live-commit",
        auth_token: tok,
    });
    if commit.status != 201 || !ovmf_live_esp_is_committed() {
        return false;
    }
...
/// SPA + ADR-014 Stage 31 phrases. Live ESP commit-attempted; VMLAUNCH insn not issued.
pub fn live_commit_surface_present() -> bool {
    ...
    spa.contains("Commit live ESP")
        ...
        && adr.contains("Stage 31")
        && adr.contains("commit_ovmf_live_esp")
        && adr.contains("LiveEspCommitAbsent")
        && src.contains("fn commit_ovmf_live_esp")
        && src.contains("ovmf_live_esp_is_committed")
        && launch.contains("fn try_vmlaunch_guest_uefi_ovmf")
        && launch.contains("fn commit_guest_uefi_live_esp")
        && launch.contains("GUEST_UEFI_PRIVATE_VMCS_ID")
        && launch.contains("LiveEspCommitAbsent")
        && launch.contains("LiveEspApplyAbsent")
...
/// Full E5 Stage 31 package. Host gate only — not iron, not Everest E5.
pub fn run_m7_e5_live_commit_gate() -> bool {
    let _ = (M7_E5_LIVE_COMMIT_OK_MARKER, E5_LIVE_COMMIT_RESIDUAL_NOTE);
    reset_guest_fw();
    let ok = prop_live_commit_after_live_apply()
        && prop_live_commit_rejects()
        && prop_rest_live_commit()
        && live_commit_surface_present()
        && run_m7_e5_live_apply_gate()
        && E5_LIVE_COMMIT_RESIDUAL_NOTE.contains("UnsupportedOnFirmware")
        && E5_LIVE_COMMIT_RESIDUAL_NOTE.contains("no guest UEFI VMLAUNCH")
        && E5_LIVE_COMMIT_RESIDUAL_NOTE.contains("not a shipped OVMF.fd")
        && E5_LIVE_COMMIT_RESIDUAL_NOTE.contains("VMLAUNCH insn not issued")
        && E5_LIVE_COMMIT_RESIDUAL_NOTE.contains("live EPT is not written")
        && M7_E5_LIVE_COMMIT_OK_MARKER == "RAYNU-V-M7-E5-LIVE-COMMIT-OK";
    reset_guest_fw();
    ok
}

#[cfg(test)]
#[path = "m7_e5_live_commit_gate_test.rs"]
mod m7_e5_live_commit_gate_test;
