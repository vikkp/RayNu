...
//!
//! Proves coexist TCP feeds [`crate::mgmt::tls_coexist::PlaintextListen`],
//! rustls is still not in `uefi-bin` (ring C needs libc), and CURL NOW stays
//! `http://`. Does **not** print `RAYNU-V-M8-TLS-OK`. Nested QEMU is not this
//! gate. Iron `curl --cacert` after `BOOT-OK` is not this gate.

use crate::mgmt::tls::{firmware_listen_is_plaintext, host_never_prints_iron_tls_ok, M8_TLS_OK_MARKER};
...
/// True when wrap wiring, plaintext firmware, and ring/UEFI residual hold.
