//! E5 Stage 23 — live-ESP bytes probe (outside Proven Core).
//!
//! Pillar: [Z] [A]
//! Proven Core: **outside** mgmt bookkeeping; launch.rs entry is L0 (ADR-014)
//! VERIFICATION: N/A
//!
//! Host/CI: `probe_ovmf_live_bytes` records that live ESP
//! `\\EFI\\RayNu\\OVMF.fd` bytes were probed for a private guest-UEFI
...
//! are still absent (`LiveEspBytesAbsent`). A 2 MiB fixture is
...
use super::guest_fw::{
    ...
    guest_fw_bytes, load_guest_firmware, load_ovmf_from_esp, ovmf_live_bytes_is_probed,
    prepare_ovmf_firmware_launch, probe_ovmf_firmware, probe_ovmf_live_bytes, reset_guest_fw,
...
use super::m7_e5_live_issue_gate::run_m7_e5_live_issue_gate;
use super::VmTable;
...
use super::guest_fw::{
    ...
    program_ovmf_alias_ept, qualify_real_esp_ovmf, require_ovmf_live_esp, stage_edk2_ovmf_firmware,
...
/// Host / CI marker when the E5 Stage 23 live-bytes gate passes.
pub const M7_E5_LIVE_BYTES_OK_MARKER: &str = "RAYNU-V-M7-E5-LIVE-BYTES-OK";

/// Honest residual: live bytes probed; still absent; VMLAUNCH insn not issued.
pub const E5_LIVE_BYTES_RESIDUAL_NOTE: &str =
    "residual: live ESP bytes were probed; live ESP bytes are not present; 4 MiB fixture is not a shipped OVMF.fd; live EPT is not written; VMLAUNCH insn not issued; attach_cdrom_uefi stays UnsupportedOnFirmware; no guest UEFI VMLAUNCH";

fn stage_through_live_issue() -> bool {
    reset_guest_fw();
...
/// Live-issue arm then live-bytes probe. VMLAUNCH stays LiveEspBytesAbsent.
pub fn prop_live_bytes_after_live_issue() -> bool {
    #[cfg(not(test))]
    {
        return false;
    }
    #[cfg(test)]
    {
        if !stage_through_live_issue() {
            return false;
        }
        if try_vmlaunch_ovmf_firmware() != Err(GuestFwError::LiveEspBytesNotPresent) {
            return false;
        }
        let mut live = vec![0u8; MIN_LIVE_ESP_OVMF_BYTES];
        if write_live_esp_ovmf_fv(&mut live).is_err() || write_reset_vector_stub(&mut live).is_err()
        ...
        if probe_ovmf_live_bytes(&live) != Err(GuestFwError::TooSmall) {
            return false;
        }
        let mut alias = vec![0u8; MIN_FIRMWARE_ALIAS_BYTES];
        if write_firmware_alias_fv(&mut alias).is_err() {
        ...
        match probe_ovmf_live_bytes(&alias) {
            Ok(a) => {
                a.bytes_len == MIN_FIRMWARE_ALIAS_BYTES as u64
                    && a.gpa == 0xFFC0_0000
                    && ovmf_live_bytes_is_probed()
                    && try_vmlaunch_ovmf_firmware() == Err(GuestFwError::LiveEspBytesAbsent)
            }
            Err(_) => false,
        }
    }
...
/// Live-bytes probe without live-issue, reject. attach_cdrom_uefi stays closed.
pub fn prop_live_bytes_rejects() -> bool {
    reset_guest_fw();
    if probe_ovmf_live_bytes(&[]) != Err(GuestFwError::LaunchNotWired) {
        return false;
    }
    attach_cdrom_uefi(1) == Err(IsoError::UnsupportedOnFirmware)
}

/// Bearer REST: live-issue then live-bytes. Without live-issue → 409.
/// `POST /fw/vmlaunch` → 409 (LiveEspBytesAbsent). `iso=0` SHELL stays 201.
pub fn prop_rest_live_bytes() -> bool {
    reset_guest_fw();
    ...
    if probed.status != 201 || !ovmf_live_bytes_is_probed() {
        return false;
    }
...
/// SPA + ADR-014 Stage 23 phrases. Live bytes probed; VMLAUNCH insn not issued.
pub fn live_bytes_surface_present() -> bool {
    ...
    spa.contains("Probe live bytes")
        ...
        && adr.contains("Stage 23")
        && adr.contains("probe_ovmf_live_bytes")
        && adr.contains("LiveEspBytesAbsent")
        && src.contains("fn probe_ovmf_live_bytes")
        && src.contains("ovmf_live_bytes_is_probed")
        && launch.contains("fn try_vmlaunch_guest_uefi_ovmf")
        && launch.contains("fn probe_guest_uefi_live_bytes")
        && launch.contains("GUEST_UEFI_PRIVATE_VMCS_ID")
        && launch.contains("LiveEspBytesAbsent")
...
/// Full E5 Stage 23 package. Host gate only — not iron, not Everest E5.
pub fn run_m7_e5_live_bytes_gate() -> bool {
    let _ = (M7_E5_LIVE_BYTES_OK_MARKER, E5_LIVE_BYTES_RESIDUAL_NOTE);
    reset_guest_fw();
    let ok = prop_live_bytes_after_live_issue()
        && prop_live_bytes_rejects()
        && prop_rest_live_bytes()
        && live_bytes_surface_present()
        && run_m7_e5_live_issue_gate()
        && E5_LIVE_BYTES_RESIDUAL_NOTE.contains("UnsupportedOnFirmware")
        && E5_LIVE_BYTES_RESIDUAL_NOTE.contains("no guest UEFI VMLAUNCH")
        && E5_LIVE_BYTES_RESIDUAL_NOTE.contains("not a shipped OVMF.fd")
        && E5_LIVE_BYTES_RESIDUAL_NOTE.contains("VMLAUNCH insn not issued")
        && E5_LIVE_BYTES_RESIDUAL_NOTE.contains("live EPT is not written")
        && M7_E5_LIVE_BYTES_OK_MARKER == "RAYNU-V-M7-E5-LIVE-BYTES-OK";
    reset_guest_fw();
    ok
}

#[cfg(test)]
#[path = "m7_e5_live_bytes_gate_test.rs"]
mod m7_e5_live_bytes_gate_test;
