...
if grep -q 'RAYNU-V-M4-NGUEST-VERIFY-OK' "$MODEL_SRC"; then
  echo "error: M4.6 must not embed NGUEST-VERIFY-OK (that is M4.7)" >&2
  exit 1
fi
...
if ! grep -q 'GAP: N-guest L3 discharge' "$ROOT/memory/ept_proof.rs"; then
  echo "error: ept_proof must leave N-guest L3 discharge open for M4.7" >&2
  exit 1
fi
