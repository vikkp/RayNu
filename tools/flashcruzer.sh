...
# Pin this SHA after CI (firmware skip PIT inject). skip-without-inject plus
# El Torito ide@ first plus skip HLT after PACKET plus ATA 14 inject (not PIT).
...
refuse_2d6b109_dest_skip() {
  ...
  if [[ "$PIN_RUN" == "33333506987" ]]; then
    ...
    echo "       firmware skip PIT inject; wait for this SHA CI." >&2
