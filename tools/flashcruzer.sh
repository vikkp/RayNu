...
refuse_24c5fa6_pit_livelock() {
  ...
  if [[ "$PIN_RUN" == "33699177232" ]]; then
    ...
    echo "       firmware ConIn CR fired; DxeCore Wait still ataio=0." >&2
    echo "       do not F11 6c4bfde / --run 33699177232 again." >&2
    echo "       do not F11 2d4ab51 / --run 33697154185." >&2
    echo "       wait for this SHA CI (print HLT callsite)." >&2
    exit 1
  fi
  if [[ "$PIN_RUN" == "33697154185" ]]; then
    echo "error: run 33697154185 is 2d4ab51 HLT ret=0x7ff0e055 hang" >&2
    echo "       DxeCore-range CpuSleep caller; still ataio=0." >&2
    echo "       do not F11 2d4ab51 / --run 33697154185 again." >&2
    echo "       do not F11 6c4bfde / --run 33699177232." >&2
    echo "       do not F11 27eda8c / --run 33695570769." >&2
    exit 1
  fi
  ...
  if [[ "$PIN_RUN" == "33569757025" ]]; then
    ...
    echo "       wr=0x0000 n=0..5; seq=0,0,0,0,0,0; EnableAttributes never wrote 0x5." >&2
    echo "       do not F11 060c504 / --run 33569757025 again." >&2
    echo "       do not F11 abba969 / --run 33567464001." >&2
    echo "       wait for this SHA CI (EnableAttributes 0x0005 after write-0)." >&2
    exit 1
  fi
  if [[ "$PIN_RUN" == "33567464001" ]]; then
    echo "error: run 33567464001 is abba969 honor COMMAND hang" >&2
    echo "       cmdwr=6 wr=0x0 pcicmd=0x0 (honor stuck; EnableAttributes did not re-enable)." >&2
    echo "       do not F11 abba969 / --run 33567464001 again." >&2
    echo "       do not F11 184ee61 / --run 33562028442." >&2
    echo "       wait for this SHA CI (IDE pci cmdwr seq of the six writes)." >&2
    exit 1
  fi
  ...
  if [[ "$PIN_RUN" == "33559849096" ]]; then
    ...
    echo "       HLT skip after PIT one-shot printed; IRET to rip=0x7f0680d0 ataio=0." >&2
    echo "       do not F11 21dc562 / --run 33559849096 again." >&2
    echo "       do not F11 e3cbfa5 / --run 33558261624 (one-shot hang)." >&2
    echo "       wait for this SHA CI (IDE pci cmdwr on HLT/stop)." >&2
    exit 1
  fi
