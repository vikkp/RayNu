...
/// Process-local stub sink used by `audit_log!` before a locked global exists.
///
/// JUSTIFICATION (global state): firmware needs a single boot-time sink; M1 will
/// replace this with a spinlock-protected global per CLAUDE.md.
struct BootRing(UnsafeCell<AuditRing>);

// SAFETY: M0 single-threaded boot/tests only; no concurrent accessors.
// KANI-TARGET: replace with spinlock + Sync proof when SMP lands.
unsafe impl Sync for BootRing {}

static BOOT_RING: BootRing = BootRing(UnsafeCell::new(AuditRing::new()));

/// Record an event into the boot stub ring.
///
/// Not concurrency-safe. Single-threaded UEFI entry / host tests only.
pub fn record_event(event: AuditEvent) {
    // SAFETY: M0 single-threaded; exclusive access assumed.
    // KANI-TARGET: concurrency model when spinlock global lands.
    unsafe {
        let _ = (*BOOT_RING.0.get()).append(event);
    }
}
