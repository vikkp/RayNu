...
| M8   | Operator product hardening      | after M7 | Persist → TLS → auth → console → ISO upload → catalog → Windows. **M8.7** PERC mailbox on RAYNU-SPARE; fwstate loads `outbound_msg_0`, no doorbell (ADR-018) |
