...
| 017   | Guest reset under RayNu-F (F7)        | Disk-before-ISO boot order; preserve virtio-blk disk across reset (`reset_keep_disk`); CF9/KBC/TF reset; cap=1 (`reset-cap`); HANDLE_DISK whole-disk Vendor path (not HD node — GRUB `hd0`); do not claim ISO-INSTALL-OK |
