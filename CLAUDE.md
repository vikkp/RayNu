...
| M8   | Operator product hardening      | after M7 | Persist disk across HV reboot → TLS → real auth → console UI → ISO upload → UEFI catalog → Windows later (ADR-018) |
...
**Lab disks (2026-09-26):** Ubuntu 26.04 is the standing OS on PERC VD **UBUNTU0** (~400 GB). **RAYNU-SPARE** (~2.9 TB) is empty. Do not format either. Do not open the H840. Map: [`docs/runbooks/r640_perc_lab.md`](docs/runbooks/r640_perc_lab.md). RayNu is not booted. A5 is parked. perc stays 15 until MegaRAID I/O.
